...
function M.handle_log_new(flag, ignore_immut)
  ...
  local flag_map = {
    after = {
      ...
      ok = is_multiple and "Successfully created merge change after: `%s`" or "Successfully created change after: `%s`",
    },
    default = {
      opt = "",
      ...
      if choice == "[Create new]" then
        ...
        vim.ui.input({ prompt = "Enter new bookmark name: ", default = prefix }, function(input)
          if input and input ~= "" then
            ...
            runner.execute_command_async(cmd, function()
              utils.notify(string.format("Created bookmark `%s` at `%s`", input, revset), vim.log.levels.INFO)
              M.log({})
