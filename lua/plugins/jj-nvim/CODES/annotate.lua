...
local function build_annotate_cmd(path, template, rev)
  ...
  return string.format("jj file annotate %s%s -T %s", rev_flag, vim.fn.shellescape(path), vim.fn.shellescape(template))
end
...
local function align_annotations(lines)
  ...
  for i, p in ipairs(parsed) do
    if p.rev then
      result[i] =
        string.format("%-" .. max_id .. "s | %-" .. max_name .. "s | %-" .. max_date .. "s", p.rev, p.name, p.date)
    else
      result[i] = p.raw
    end
  end
