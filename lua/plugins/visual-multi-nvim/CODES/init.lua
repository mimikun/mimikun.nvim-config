...
local function define_global_mappings()
  ...
  global_map("n", maps.select_left, function()
    M.select_horizontal(-1)
  end, "select left")
  global_map("n", maps.select_right, function()
    M.select_horizontal(1)
  end, "select right")
  global_map("n", maps.add_cursor_down, function()
    M.add_cursor_vertical(1)
  end, "add cursor down")
  global_map("n", maps.add_cursor_up, function()
    M.add_cursor_vertical(-1)
  end, "add cursor up")
  global_map("n", maps.add_cursor, M.add_cursor, "add cursor")
  global_map("n", maps.add_cursor_word, M.add_cursor_word, "add word")
  global_map("x", maps.find_next, function()
    M.from_visual(false)
  end, "find next from selection")
  global_map("x", maps.select_all, function()
    M.from_visual(true)
  end, "select all from selection")
end
