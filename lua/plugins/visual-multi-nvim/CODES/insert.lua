...
local function insertion_position(session, region, kind)
  ...
  if session.mode == "extend" then
    ...
    local pos = kind == "i" and { row = start_pos.row, col = start_pos.col }
...
local function mirror(session, event, generation)
  ...
  for index = #targets, 1, -1 do
    ...
    pcall(vim.api.nvim_buf_set_text, session.buf, start_pos.row, start_pos.col, end_pos.row, end_pos.col, event.text)
  end
  session.syncing = false
