...
function M.offset_to_position(buf, offset)
  local count = vim.api.nvim_buf_line_count(buf)
  local last = count - 1
  local eof = vim.api.nvim_buf_get_offset(buf, last) + #(vim.api.nvim_buf_get_lines(buf, last, last + 1, true)[1] or "")
...
function M.get_text(buf, start_pos, end_pos)
  return table.concat(vim.api.nvim_buf_get_text(buf, start_pos.row, start_pos.col, end_pos.row, end_pos.col, {}), "\n")
end
