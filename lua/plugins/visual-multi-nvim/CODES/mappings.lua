...
function M.activate(session)
  ...
  local actions = {
    ...
    add_cursor = {
      function()
        session:add_cursor_at_current()
      end,
      "add cursor",
    },
    add_cursor_word = {
      function()
        session:add_word_at_current()
      end,
      "add word",
    },
    move_left = {
    ...
    insert = {
      function()
        session:begin_insert("i")
      end,
      "insert",
    },
    append = {
      function()
        session:begin_insert("a")
      end,
      "append",
    },
    insert_bol = {
  ...
  map(session, "n", "<Esc>", function()
    session:escape()
  end, "leave mode or clear")
  map(session, "n", "~", function()
    session:toggle_char_case()
  end, "toggle character case")
  ...
  map(session, "n", "v", function()
    session:expand_selection()
  end, "expand selection")
  map(session, "n", "V", function()
    session:shrink_selection()
  end, "shrink selection")
  map(session, "n", "D", function()
    session:delete_to_eol()
  end, "delete to end of line")
  map(session, "n", "o", function()
    session:begin_insert("o")
  end, "open line below")
  map(session, "n", "O", function()
    session:begin_insert("O")
  end, "open line above")
