...
function Session:render()
  ...
  for index, region in ipairs(self.regions) do
    ...
    if start_pos then
      ...
      local cursor_hl = self.mode == "insert" and (active and highlights.insert_active or highlights.insert)
...
function Session:_region_index_at_cursor()
  ...
  if candidate then
    ...
    if (same_pos(start_pos, end_pos) and same_pos(pos, start_pos)) or (after_start and before_end) then
      return candidate
    end
  end
end
