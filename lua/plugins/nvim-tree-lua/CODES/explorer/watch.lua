...
local function is_folder_ignored(path)
  ...
  if type(config.g.filesystem_watchers.ignore_dirs) == "table" then
    if
      matches_dirs(path, config.g.filesystem_watchers.ignore_dirs --[[@as string[] ]])
    then
      return true
    end
  ...
  if
    type(config.g.filesystem_watchers.whitelist_dirs) == "table" and #config.g.filesystem_watchers.whitelist_dirs > 0
  then
    if
      not matches_dirs(path, config.g.filesystem_watchers.whitelist_dirs --[[@as string[] ]])
    then
      return true
    end
...
function M.create_watcher(node)
  ...
  local function callback(watcher)
    ...
    if
      config.g.filesystem_watchers.max_events > 0
      and watcher.data.outstanding_events > config.g.filesystem_watchers.max_events
    then
      notify.error(
        string.format(
