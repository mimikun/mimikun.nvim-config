...
local function onWin(name, winid, bufnr, topRow, botRow)
  ...
  if not fb or fb.foldedLineCount == 0 and not vim.wo[winid].foldenable or render.rendering(bufnr, winid) then
  ...
  for winid, rows in pairs(collection or {}) do
    if #rows > 0 then
      ...
      if not needRedraw then
        ...
        if
          lastCurFoldStart ~= curFoldStart
          and lastCurFoldStart < lastCurLnum
          and lastCurLnum <= lastCurFoldEnd
          and lastCurFoldStart < curLnum
          and curLnum <= lastCurFoldEnd
        then
...
function Decorator:highlightOpenFold(fb, winid, lnum, shared)
  if self.openFoldHlEnabled and winid == self.lastWinid and api.nvim_get_mode().mode ~= "c" then
    ...
    render.highlightLinesWithTimeout(
      shared and winid or fb.bufnr,
      "UfoFoldedBg",
      self.hlNs,
      lnum,
      endLnum,
    ...
    if not res then
      ...
      if self.enableGetFoldVirtText then
        getFoldVirtText = function(l)
          ...
          assert(lnum <= l and l <= endLnum, ("expected lnum range from %d to %d, got %d"):format(lnum, endLnum, l))
