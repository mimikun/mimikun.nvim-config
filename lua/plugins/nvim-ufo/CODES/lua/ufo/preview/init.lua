...
function Preview:toggleCursor()
  ...
  if self.bufnr == bufnr and self.lnum - self.foldedLnum > 0 then
    self.cursorSignId = fn.sign_place(
      self.cursorSignId or 0,
      "UfoPreview",
      self.cursorSignName,
      floatBufnr,
...
function Preview:attach(bufnr, winid, foldedLnum, foldedEndLnum, isAbove)
  ...
  table.insert(
    disposables,
    keymap:attach(bufnr, floatBufnr, self.ns, self.keyMessages, {
...
function Preview:peekFoldedLinesUnderCursor(enter, nextLineIncluded)
  ...
  local function doHighlight(s, e)
    render.mapHighlightLimitByRange(
      bufnr,
      floatBufnr,
      lnum - 1,
      { s - 1, 0 },
...
function Preview:afterKey()
  ...
  if winid == self.winid then
    local view = utils.saveView(winid)
    if self.lnum ~= view.lnum or self.col ~= view.col then
      self.close()
