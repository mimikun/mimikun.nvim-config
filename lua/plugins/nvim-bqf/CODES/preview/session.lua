...
function PreviewSession:showCountLabel(text, hlGroup)
  ...
  self.labelId = extmark.setVirtText(
    self:floatBufnr(),
    self.ns,
    lnum - 1,
    -1,
...
function PreviewSession:display(pwinid, pbufnr, idx, size, handler)
  ...
  if res then
    ...
    if self.enableScrollBar then
      ...
      if
        hasShowedScrollBar == false
        and floatwin.showScrollBar == true
        and utils.has10()
        and api.nvim_get_mode().mode == "t"
      then
        -- ^L = 0x0c
...
function PreviewSession:initialize(o)
  ...
  self.scrollThrottled = self.enableScrollBar
