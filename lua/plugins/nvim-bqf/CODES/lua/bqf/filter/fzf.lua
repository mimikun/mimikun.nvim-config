...
local function sourceCmd(qwinid, signs, delim)
  ...
  local bqfRtp
  local qfFiles =
    vim.list_extend(api.nvim_get_runtime_file("syntax/qf.vim", true), api.nvim_get_runtime_file("syntax/qf.lua", true))
  ...
  table.insert(
    script,
    ([[require('bqf.filter.fzf').headlessRun(%s, %d, %q)]]):format(
      vim.inspect(ansiTbl, { newline = "" }),
      utils.textoff(qwinid) - 4,
      delim
    )
  )
...
function M.run()
  ...
  local baseOpt = { "--color", "gutter:-1", "--no-separator", "--scroll-off", utils.scrolloff(qwinid) }
  ...
  cmd(
    ("au BqfFilterFzf FileType fzf ++once %s"):format(
...
local function init()
  ...
  assert(
    vim.g.loaded_fzf or fn.exists("*fzf#run") == 1,
