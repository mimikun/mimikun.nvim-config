...
local function registerWinEnter(qwinid)
  ...
  cmd(
    ("au BqfMagicWin WinEnter * %s"):format(([[lua require('bqf.magicwin.handler').resetWinView(%d)]]):format(qbufnr))
  )
    ...
    for _, winid in ipairs(wpos.findAdjacentWins(qwinid, pwinid)) do
      if utils.isWinValid(winid) then
        ...
        if scrollOff ~= 0 then
          ...
          cmd(
            ("au BqfMagicWin WinLeave * ++once %s"):format(
              ([[lua vim.schedule(function() if vim.api.nvim_win_is_valid(%d) then vim.wo[%d].scrolloff = %d end end)]]):format(
                winid,
                winid,
                scrollOff
              )
...
local function revertClosingWins(qwinid, pwinid, qfPos, layoutCallback)
  ...
  for _, winid in ipairs(wpos.findAdjacentWins(qwinid, pwinid)) do
    ...
    if aws and aws.winView then
      ...
      if qfPos[1] == POS.ABOVE or qfPos[2] == POS.TOP then
        topline, topfill = mcore.tuneTop(winid, topline, api.nvim_win_get_height(qwinid) + 1 + topfill)
      end
