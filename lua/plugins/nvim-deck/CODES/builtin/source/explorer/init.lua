...
local function file_kind(path)
  return vim.fn.isdirectory(path) == 1 and LSP.FileOperationPatternKind.folder or LSP.FileOperationPatternKind.file
end
      ...
      execute = function(ctx)
        if option.narrow.enabled and ctx.get_query() ~= "" then
          ...
          Node.narrow(option.cwd, option.narrow.ignore_globs or {}, ctx.on_abort, ctx.aborted, function(path)
            ctx.queue(function()
              local score =
                ctx.get_config().matcher.match(ctx.get_query(), assert(vim.fs.relpath(option.cwd, path, {})))
              if score == 0 then
                return
              end
              local parents = {}
              ...
              actions = {
                {
                  ...
                  execute = function(ctx)
                    explorer_ctx.focus()
                    explorer_ctx
                      .do_action("explorer.get_api")
                      .set_cwd(ctx.get_cursor_item().data.filename, state:get_root().path)
                    ctx.hide()
                  end,
                },
              },
          ...
          execute = function(ctx)
            return Async.run(function()
              ...
              if item then
                ...
                local kind = path:sub(-1, -1) == "/" and LSP.FileOperationPatternKind.folder
        ...
        },
        {
          ...
          execute = function(ctx)
            return Async.run(function()
              ...
              if
                not x.confirm(("Delete below items?\n%s"):format(vim
                  .iter(items)
                  :map(function(item)
                ...
                if node then
                  ...
                  local renames = vim.iter(clipboard.paths):fold({}, function(renames, path)
                    ...
                    if path == path_new then
                      local index = 1
                      while true do
                        path_new = IO.join(paste_target.path, ("%s - copy%s"):format(vim.fs.basename(path), index))
