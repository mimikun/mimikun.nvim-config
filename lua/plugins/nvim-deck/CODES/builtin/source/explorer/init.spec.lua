...
local function setup()
  return Async.run(function()
    ...
    local fixture_dir = IO.join(
      IO.normalize(debug.getinfo(1, "S").source:sub(2):gsub("\\", "/"):match("(.*/)")),
      "../../../../../fixtures/fs"
    )
...
describe("deck.builtin.source.explorer", function()
  ...
  it("expand and collapse", function()
    ...
    assert.are.same({ "deck-fixture-fs", "dir1", "file1", "dir2" }, item_basenames(ctx))
    do_action_with_path(ctx, "explorer.collapse", "file1")
    assert.are.same({ "deck-fixture-fs", "dir1", "dir2" }, item_basenames(ctx))
  end)
  ...
  it("create file", function()
    ...
    assert.are.same({ "deck-fixture-fs", "dir1", "file1", "new_file.txt", "dir2" }, item_basenames(ctx))
  end)
  ...
  it("create directory", function()
    ...
    assert.are.same({ "deck-fixture-fs", "dir1", "new_dir", "file1", "dir2" }, item_basenames(ctx))
  end)
  ...
  it("rename", function()
    ...
    vim.wait(300)
    ctx.sync()
    assert.are.same({ "deck-fixture-fs", "dir1", "file_renamed", "dir2" }, item_basenames(ctx))
  end)
  ...
  it("rename multiple", function()
    ...
    vim.wait(300)
    ctx.sync()
    assert.are.same({ "deck-fixture-fs", "dir1", "file1_renamed", "dir2", "file2_renamed" }, item_basenames(ctx))
  end)
  ...
  it("rename cancel aborts sequence", function()
    ...
    vim.wait(300)
    ctx.sync()
    assert.are.same({ "deck-fixture-fs", "dir1", "file1", "dir2", "file2" }, item_basenames(ctx))
  end)
  ...
  it("clipboard move", function()
    ...
    assert.are.same({ "deck-fixture-fs", "dir1", "dir2", "file1", "file2" }, item_basenames(ctx))
  end)
  ...
  it("clipboard copy and paste", function()
    ...
    do_action_with_path(ctx, "explorer.expand", "dir2")
    do_action_with_path(ctx, "explorer.clipboard.paste", "dir2")
    assert.are.same({ "deck-fixture-fs", "dir1", "file1", "dir2", "file1", "file2" }, item_basenames(ctx))
  end)
  ...
  it("yank", function()
    ...
    assert.are.same(fixture_target_dir .. "/dir1/file1", vim.fn.getreg('"'):gsub("\n$", ""))
  end)
end)
