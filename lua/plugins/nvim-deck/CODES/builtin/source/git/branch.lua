    ...
    actions = {
        ...
        execute = function(ctx)
          Async.run(function()
            ...
            if item.data.remote then
              git:exec_print({ "git", "fetch", item.data.remotename, item.data.name }):await()
              git
                :exec_print({ "git", "merge", "--ff-only", ("%s/%s"):format(item.data.remotename, item.data.name) })
                :await()
            else
        ...
        execute = function(ctx)
          Async.run(function()
            ...
            if item.data.remote then
              git:exec_print({ "git", "fetch", item.data.remotename, item.data.name }):await()
              git
                :exec_print({ "git", "merge", "--no-ff", ("%s/%s"):format(item.data.remotename, item.data.name) })
                :await()
            else
        ...
        execute = function(ctx)
          Async.run(function()
            ...
            if item.data.remote then
              git:exec_print({ "git", "fetch", item.data.remotename, item.data.name }):await()
              git
                :exec_print({ "git", "merge", "--squash", ("%s/%s"):format(item.data.remotename, item.data.name) })
                :await()
            else
      ...
      },
      {
        ...
        execute = function(rebase_ctx)
          ...
          require("deck").start(require("deck.builtin.source.git.branch")({ cwd = option.cwd }), {
            get_view = function()
              return require("deck.builtin.view.float_picker")({
                title = (" rebase --onto %s | old base: "):format(new_base),
              })
            end,
            actions = {
              {
