    ...
    :next(function(out)
      ...
      for _, text in ipairs(out.stdout) do
        if text ~= "" then
          ...
          local branch = reflog_subject:match("^WIP on ([^:]+):") or reflog_subject:match("^On ([^:]+):") or ""
          table.insert(items, {
            text = text,
            index = index,
            selector = selector,
  ...
  return self
    :exec(
      kit.concat({
        ...
        params.offset and ("--skip=%s"):format(params.offset),
        params.rev_range,
        "--pretty=format:%H%x00%P%x00%an%x00%ae%x00%ai%x00%s%x00%b%x00%B%x00%gD%x00%gd%x00%gs"
          .. ("%x00"):rep(sep_count),
        ...
        callback = function()
          Async.run(function()
            if x.confirm("Commit?") then
              ...
              IO.cp(
                vim.fs.joinpath(self:get_git_dir(), "COMMIT_EDITMSG"),
