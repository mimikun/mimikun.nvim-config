...
local function get_operation_filters(client, capability_name, method)
  ...
  local server_capabilities_options = kit.get(client.client.server_capabilities, {
    "workspace",
    "fileOperations",
    capability_name,
  })
...
local function filter_files(files, filters)
  return vim
    ...
    :filter(function(file)
      ...
      local kind = vim.fn.isdirectory(path) == 1 and LSP.FileOperationPatternKind.folder
...
local function start_operations(will_capability_name, did_capbility_name, will_method_name, did_method_name, files)
