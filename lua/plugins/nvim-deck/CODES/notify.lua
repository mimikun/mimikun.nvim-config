...
local function show()
  ...
  for _, lane in ipairs(active_lanes) do
    ...
    if win_state and vim.api.nvim_win_is_valid(win_state.id) then
      vim.api.nvim_win_set_config(win_state.id, {
  ...
  for _, lane in
    ipairs(vim.iter(active_lanes):rev():totable() --[=[@as deck.notify.Lane[]]=])
  do
