...
function E.setup(opts)
  ...
  if type(var) == "table" or var:match("^cmd:(.*)") then
    return
  end
...
M = setmetatable(M, {
  ...
  __index = function(t, k)
    if Config.providers[k] == nil then
      error("Failed to find provider: " .. k, 2)
    end
    ...
    if provider_config.__inherited_from ~= nil then
      ...
      if not ok then
        error("Failed to load provider: " .. provider_config.__inherited_from, 2)
      end
    ...
    if rawget(t[k], "parse_api_key") == nil then
    ...
    if rawget(t[k], "is_env_set") == nil then
      t[k].is_env_set = function()
        if not E.require_api_key(t[k]) then
          return true
        end
        if type(t[k].api_key_name) == "string" and t[k].api_key_name:match("^cmd:") then
          return true
        end
...
function M.parse_config(opts)
  ...
  for key, value in pairs(opts) do
    if key ~= "extra_request_body" then
