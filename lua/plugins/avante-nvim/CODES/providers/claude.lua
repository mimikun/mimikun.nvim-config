...
local function start_manager_check_timer()
  ...
  M._manager_check_timer:start(
    ...
    vim.schedule_wrap(function()
      if not M._refresh_timer and try_acquire_claude_timer_lock() then
...
function M:get_rate_limit_sleep_time(headers)
  ...
  if remaining_tokens > 10000 then
    return
  end
  local reset_dt_str = headers["anthropic-ratelimit-tokens-reset"]
  if remaining_tokens ~= 0 then
    reset_dt_str = reset_dt_str or headers["anthropic-ratelimit-requests-reset"]
  end
...
function M.is_temperature_unsupported(model)
  ...
  if model:match("claude%-opus%-[4-9]") then
    return true
  end
  if model:match("claude%-sonnet%-[4-9]") then
    return true
  end
  if model:match("claude%-haiku%-[4-9]") then
    return true
  end
  return false
end
...
function M:parse_messages(opts)
  ...
  table.sort(messages_with_length, function(a, b)
...
function M:parse_response(ctx, data_stream, event_state, opts)
  ...
  if ctx.content_blocks == nil then
    ctx.content_blocks = {}
  end
  ...
  elseif event_state == "content_block_delta" then
    ...
    if jsn.delta.type == "input_json_delta" then
      if not content_block.input_json then
    ...
    elseif jsn.delta.type == "signature_delta" then
      if ctx.content_blocks[jsn.index + 1].signature == nil then
        ctx.content_blocks[jsn.index + 1].signature = ""
      end
  ...
  elseif event_state == "message_delta" then
    ...
    if jsn.usage and ctx.usage then
      ctx.usage.output_tokens = ctx.usage.output_tokens + jsn.usage.output_tokens
    end
...
function M:parse_curl_args(prompt_opts)
  ...
  if not disable_tools and prompt_opts.tools then
    for _, tool in ipairs(prompt_opts.tools) do
      if Config.mode == "agentic" then
        if tool.name == "create_file" then
          -- goto continue
        end
        if tool.name == "view" then
          -- goto continue
        end
        if tool.name == "str_replace" then
          -- goto continue
        end
        if tool.name == "create" then
          -- goto continue
        end
        if tool.name == "insert" then
          -- goto continue
        end
        if tool.name == "undo_edit" then
          -- goto continue
        end
  ...
  if M.is_temperature_unsupported(provider_conf.model) then
...
function M.authenticate()
  ...
  if not state then
    vim.schedule(function()
      vim.notify("Failed to generate PKCE state: " .. (state_err or "Unknown error"), vim.log.levels.ERROR)
    end)
    return
  end
...
function M.refresh_token(async, force)
  if not M.state or not M.state.claude_token then
    return false
  end -- Exit early if no state
