...
function M:parse_messages(opts)
  ...
  vim.iter(opts.messages):each(function(msg)
    table.insert(messages, { content = msg.content, role = M.role_map[msg.role] })
  end)
  return messages
end
...
local function parse_response_wo_stream(self, data, _, opts)
  if Utils.debug then
    Utils.debug("WCA parse_response_without_stream called with opts: " .. vim.inspect(opts))
  end

  local json = vim.json.decode(data)
  if Utils.debug then
    Utils.debug("WCA Response: " .. vim.inspect(json))
  end
  ...
  if json.response and json.response.message and json.response.message.content then
    ...
    if Utils.debug then
      Utils.debug("WCA Original Content: " .. tostring(content))
    end
    ...
    if Utils.debug then
      Utils.debug("WCA Cleaned Content: " .. tostring(content))
    end
  ...
  elseif json.error and json.error ~= vim.NIL then
    ...
    if Utils.debug then
      Utils.debug("WCA: No content found in response, treating as empty response")
    end
    vim.schedule(function()
      if opts.on_stop then
        opts.on_stop({ reason = "complete" })
