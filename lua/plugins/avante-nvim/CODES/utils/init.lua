...
function M.shell_run_async(input_cmd, shell_cmd, on_complete, cwd, timeout)
  ...
  local job_id = fn.jobstart(cmd, {
    ...
    on_exit = function(_, exit_code)
      complete_once(table.concat(output, "\n"), exit_code)
    end,
    cwd = cwd,
  })
...
function M.safe_keymap_set(mode, lhs, rhs, opts)
  ...
  modes = vim.tbl_filter(function(m)
    return not (Keys and Keys.have and Keys:have(lhs, m))
  end, modes)
...
function M.is_valid_container(container, check_winid)
  ...
  if container.bufnr == nil or not api.nvim_buf_is_valid(container.bufnr) then
    return false
  end

  -- Check window validity if requested
  if check_winid then
    if container.winid == nil or not api.nvim_win_is_valid(container.winid) then
      return false
    end
  end
...
function M.fuzzy_match(original_lines, target_lines)
  local start_line, end_line
  ---exact match
  start_line, end_line = M.try_find_match(original_lines, target_lines, function(line_a, line_b)
  ...
  ---fuzzy match
  start_line, end_line = M.try_find_match(original_lines, target_lines, function(line_a, line_b)
    return M.trim(line_a, { suffix = " \t" }) == M.trim(line_b, { suffix = " \t" })
  end)
  if start_line ~= nil and end_line ~= nil then
    return start_line, end_line
  end
  ---trim_space match
  start_line, end_line = M.try_find_match(original_lines, target_lines, function(line_a, line_b)
    return M.trim_space(line_a) == M.trim_space(line_b)
  end)
  if start_line ~= nil and end_line ~= nil then
    return start_line, end_line
  end
  ---trim slashes match
  start_line, end_line = M.try_find_match(original_lines, target_lines, function(line_a, line_b)
    return line_a == M.trim_escapes(line_b)
  end)
  if start_line ~= nil and end_line ~= nil then
    return start_line, end_line
  end
  ...
  start_line, end_line = M.try_find_match(original_lines, target_lines, function(line_a, line_b)
    return M.trim_space(line_a) == M.trim_space(M.trim_escapes(line_b))
  end)
  return start_line, end_line
end
...
function M.prepend_line_numbers(lines, start_line)
  ...
  return vim
    ...
    :enumerate()
    :map(function(i, line)
      return string.format("L%d: %s", i + start_line, line)
    end)
    :totable()
end
...
function M.is_same_file_ext(target_ext, filepath)
  ...
  if (target_ext == "tsx" and ext == "ts") or (target_ext == "ts" and ext == "tsx") then
    return true
  end
  if (target_ext == "jsx" and ext == "js") or (target_ext == "js" and ext == "jsx") then
    return true
  end
  ...
  for _, file in ipairs(oldfiles) do
    if vim.startswith(file, project_root) then
      ...
      if same_file_ext then
        if not M.is_same_file_ext(current_ext, file) then
          -- goto continue
        end
      end
      ...
      if #recent_files >= (limit or 10) then
        break
      end
    end
    -- ::continue::
...
function M.is_ignored(file, ignore_patterns, negate_patterns)
  ...
  for _, pattern in ipairs(ignore_patterns) do
    if file:match(pattern .. "/") or file:match(pattern .. "$") then
      return true
    end
  end
  return false
  ...
  files = vim
    ...
    :map(function(file)
      if not M.is_absolute_path(file) then
        return vim.fs.joinpath(options.directory, file)
      end
      return file
    end)
    :totable()
  ...
  if options.max_depth ~= nil and not cmd_supports_max_depth then
    files = vim
      ...
      :filter(function(file)
        ...
        if base_dir:sub(-2) == "/." or base_dir:sub(-2) == "\\." then
...
function M.get_parent_path(filepath)
  ...
  if filepath:sub(-1) == M.path_sep then
...
function M.make_relative_path(filepath, base_dir)
  return M.path.relative(base_dir, filepath, false)
end

function M.is_absolute_path(path)
  return M.path.is_absolute(path)
end

function M.abspath(path)
  return M.path.abspath(path)
end
...
function M.to_absolute_path(path)
  ...
  if M.is_absolute_path(path) or path:sub(1, 7) == "term://" then
    return path
  end
...
function M.path_exists(path)
  return M.path.is_exist(path)
end

function M.is_first_letter_uppercase(str)
...
function M.open_buffer(path, set_current_buf)
  if set_current_buf == nil then
    set_current_buf = true
  end
...
function M.update_buffer_lines(ns_id, bufnr, old_lines, new_lines, skip_line_count)
  ...
  for i, line in ipairs(new_lines) do
    ...
    if type(line.set_highlights) == "function" then
      line:set_highlights(ns_id, bufnr, line_pos_0b)
    end
...
function M.uniform_path(path)
  ...
  if not M.file.is_in_project(path) then
    return path
  end
  local project_root = M.get_project_root()
...
function M.is_same_file(filepath_a, filepath_b)
  return M.uniform_path(filepath_a) == M.uniform_path(filepath_b)
end

---Removes <think> tags, returning only text between them
---@param content string
---@return string
function M.trim_think_content(content)
  return (content:gsub("^<think>.-</think>", "", 1))
end
...
function M.get_filetype(filepath)
  ...
  if cached_filetype then
    return cached_filetype
  end
...
function M.read_file_from_buf_or_disk(filepath)
  ...
  if not stat then
    return {}, stat_err, stat_errname
  end
  if stat.type == "directory" then
    return {}, "Cannot read a directory as file" .. filepath, nil
  end
...
function M.icons_enabled()
  return M.has("nvim-web-devicons") or M.has("mini.icons") or M.has("mini.nvim")
end
...
function M.is_left_adjacent(win_a, win_b)
  if not vim.api.nvim_win_is_valid(win_a) or not vim.api.nvim_win_is_valid(win_b) then
    return false
  end
...
function M.llm_tool_param_fields_to_json_schema(fields)
  ...
  for _, field in ipairs(fields) do
    if field.type == "object" and field.fields then
      ...
      if vim.tbl_isempty(properties_) then
        properties_ = vim.empty_dict()
      end
...
function M.parse_args(fargs, options)
  ...
  if options.collect_remaining and #remaining_parts > 0 then
    return parsed_args, table.concat(remaining_parts, " ")
  end
...
function M.tbl_override(value, override)
  ...
  if type(override) == "function" then
    return override(value) or value
  end
...
function M.fix_diff(diff)
  ...
  for _, line in ipairs(fixed_diff_lines) do
    if should_delete_following_lines then
      -- goto continue
    end
    if line:match("^-------%s*SEARCH") then
      has_split_line = false
    end
    if line:match("^=======") then
  ...
  if not replace_block_closed then
    table.insert(the_final_diff_lines, "+++++++ REPLACE")
  end
