...
local function has_scheme(path)
  return path:find("^(?!term://)%w+://") ~= nil
end

function FileSelector:process_directory(absolute_path)
  if absolute_path:sub(-1) == Utils.path_sep then
...
local function get_project_filepaths(excluded_paths_set)
  ...
  return vim
    ...
    :filter(function(path)
      return not excluded_paths_set[path]
    end)
    :map(function(path)
...
function FileSelector:add_selected_file(filepath)
  if not filepath or filepath == "" or has_scheme(filepath) then
    return
  end
    ...
    if Config.file_selector.provider ~= nil then
      ...
      if type(Config.file_selector.provider) == "function" then
        ...
        local selector = Selector:new({
          provider = provider,
          title = PROMPT_TITLE,
          items = vim.tbl_map(function(filepath)
            return { id = filepath, title = filepath }
          end, self:get_filepaths()),
          ...
          on_select = function(item_ids)
            self:handle_path_selection(item_ids)
          end,
...
function FileSelector:remove_selected_file(rel_path)
  ...
  if idx then
    self:remove_selected_filepaths_with_index(idx)
  end
end
...
function FileSelector:add_quickfix_files()
  local quickfix_files = vim
    ...
    :map(function(item)
      return Utils.to_absolute_path(vim.api.nvim_buf_get_name(item.bufnr))
    end)
    :totable()
