  ...
  local items = vim
    ...
    :map(function(s)
      ...
      for i = s.start_row, s.start_row + #lines - 1 do
        if current_content[i] ~= lines[i - s.start_row + 1] then
          break
        end
        new_start_row = i + 1
      end
...
local function build_suggestion_list(full_response, bufnr)
  ...
  return vim
    ...
    :map(function(suggestions)
      return build_suggestion_set(suggestions, current_lines)
    end)
    :totable()
end
...
[
  ...
  local history_messages = vim
    .iter(llm_messages)
    :map(function(msg)
      return HistoryMessage:new(msg.role, msg.content)
    end)
    :totable()
...
function Suggestion:show()
  ...
  for _, suggestion in ipairs(suggestions) do
    ...
    for i = start_row, end_row do
      if i == start_row and start_row == cursor_row and virt_text_win_col > 0 then
        -- goto continue
      end
...
function Suggestion:hide()
  api.nvim_buf_clear_namespace(0, SUGGESTION_NS, 0, -1)
end
...
function Suggestion:next()
  local ctx = self:ctx()
  if #ctx.suggestions_list == 0 then
    return
  end
...
function Suggestion:get_current_suggestion()
  ...
  for _, suggestion in ipairs(suggestions) do
    if suggestion.original_start_row - 1 <= cursor_row and suggestion.end_row >= cursor_row then
      return suggestion
    end
  end
end
...
function Suggestion:get_next_suggestion()
  ...
  table.sort(new_suggestions, function(a, b)
    return math.abs(a.start_row - cursor_row) < math.abs(b.start_row - cursor_row)
  end)
...
function Suggestion:accept()
  ...
  if not suggestion then
    ...
    if suggestion then
      ...
      if first_line_row > 1 then
        first_line_row = first_line_row - 1
      end
  ...
  ctx.suggestions_list[ctx.current_suggestions_idx] = vim
    .iter(suggestions)
    :filter(function(s)
      return s.start_row ~= suggestion.start_row
    end)
    :map(function(s)
...
function Suggestion:setup_autocmds()
  ...
  local check_for_suggestion = Utils.debounce(function()
    ...
    if last_cursor_pos[1] == current_cursor_pos[1] and last_cursor_pos[2] == current_cursor_pos[2] then
      ...
      end, Config.suggestion.throttle)
      self:suggest()
    end
