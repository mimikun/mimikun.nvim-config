      ...
      if self.config.reconnect and self.reconnect_count < (self.config.max_reconnect_attempts or 3) then
        ...
        vim.defer_fn(function()
          if self.state == "disconnected" then
            self:connect(function(_err) end)
          end
          ...
          if line ~= "" then
            ...
            if ok then
              ...
              vim.schedule(function()
                vim.notify("Failed to parse JSON-RPC message: " .. line, vim.log.levels.WARN)
              end)
            end
          end
        end
...
function ACPClient:_create_websocket_transport()
  error("WebSocket transport not implemented yet")
end

---Create TCP transport layer (placeholder)
function ACPClient:_create_tcp_transport()
  error("TCP transport not implemented yet")
end
