        ...
        if incompleteContent:match("^<[%w_]") then
          ...
          if tagName then
            ...
            elseif self.current then
              ...
              if not self.current[tagName] then
                self.current[tagName] = {}
              end
...
function StreamParser:getAllElements()
  ...
  -- 如果有当前正在处理的元素，也添加进去
  if self.root then
    table.insert(all_elements, self.root)
  end
...
-- 获取当前未完成的元素（保留向后兼容性）
function StreamParser:getCurrentElement()
  return self.root
end
...
function StreamParser:finalize()
  ...
  if self.current then
    ...
    local function markIncompleteElements(element)
      if element._state and element._state:match("incomplete") then
        element._state = "incomplete_unclosed"
      end
      -- 处理 children 数组中的子元素
      if element.children and type(element.children) == "table" then
        for _, child in ipairs(element.children) do
          if type(child) == "table" and child._name then
            markIncompleteElements(child)
          end
        end
      end
    end
    ...
    for i = #self.stack, 1, -1 do
      ...
      if ancestor._state and ancestor._state:match("incomplete") then
        ancestor._state = "incomplete_unclosed"
      end
    end
  end
...
-- 创建流式解析器实例
function XmlParser.createStreamParser()
