...
function StreamParser:getCurrentChar()
  if self.position <= #self.buffer then
    return self.buffer:sub(self.position, self.position)
  end
  return nil
end

-- 前进一个字符位置
function StreamParser:advance()
  self.position = self.position + 1
end
...
function StreamParser:parseBuffer()
  ...
  if self.state == PARSE_STATE.INCOMPLETE then
    ...
    elseif self.incomplete_literal ~= "" then
      ...
      if value ~= nil or self.incomplete_literal == "null" then
        if not self:addValue(value) then
          return
        end
      elseif self.state == PARSE_STATE.ERROR then
        return
  ...
  while self.position <= #self.buffer and self.state == PARSE_STATE.PARSING do
    ...
    elseif char == "}" then
      ...
      if not self:addValue(parent.value) then
        return
      end
      self:advance()
    elseif char == "[" then
    ...
    elseif isNumberStart(char) then
      ...
      if num ~= nil and not self:addValue(num) then
        return
      end
...
-- 获取已完成的对象（保留向后兼容性）
function StreamParser:getCompletedObjects()
  return self.results
end
...
function StreamParser:finalize()
  ...
  -- 只添加根对象到结果
  if root_object then
    table.insert(self.results, root_object)
  end
...
-- 获取当前解析深度
function StreamParser:getCurrentDepth()
  return self.depth
end

-- 检查是否有错误
function StreamParser:hasError()
  return self.state == PARSE_STATE.ERROR
end

-- 获取错误信息
function StreamParser:getError()
  return self.last_error
end

-- 创建流式解析器实例
function JsonParser.createStreamParser()
