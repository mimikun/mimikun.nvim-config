...
local function random_bytes_urandom(n)
  ...
  if not fd then
    return nil, ("uv.fs_open(%s) failed: %s"):format(path, tostring(open_err))
  end
  ...
  if not chunk then
    return nil, ("uv.fs_read(%s) failed: %s"):format(path, tostring(read_err))
  end
  if #chunk ~= n then
    return nil, ("short read from %s: wanted %d got %d"):format(path, n, #chunk)
  end
  return chunk, nil
end
