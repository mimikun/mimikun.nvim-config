...
function M:open()
  ...
  local buttons_line = Line:new({
    {
      "  [Y]es ",
      function()
        return focus_index == 1 and BUTTON_FOCUS or BUTTON_NORMAL
      end,
    },
    { "   " },
    {
      "  [A]ll yes ",
      function()
        return focus_index == 2 and BUTTON_FOCUS or BUTTON_NORMAL
      end,
    },
    { "    " },
    {
      "  [N]o ",
      function()
        return focus_index == 3 and BUTTON_FOCUS or BUTTON_NORMAL
      end,
    },
  })
  ...
  vim.api.nvim_create_autocmd("CursorMoved", {
    ...
    callback = function()
      ...
      if row ~= button_row then
        vim.api.nvim_win_set_cursor(self._popup.winid, { button_row, buttons_start_col })
      end
      if col >= yes_button_pos[1] and col <= yes_button_pos[2] then
        focus_index = 1
        render_content()
  ...
  vim.api.nvim_create_autocmd("WinClosed", {
    ...
    callback = function()
      ...
      if not vim.list_contains(winids, self._container_winid) then
        self:close()
      end
    end,
  })
...
function M:bind_window_focus_keymaps()
  vim.keymap.set({ "n", "i" }, Config.mappings.confirm.focus_window, function()
    self:window_focus_handler()
  end)
end

function M:unbind_window_focus_keymaps()
  pcall(vim.keymap.del, { "n", "i" }, Config.mappings.confirm.focus_window)
end
