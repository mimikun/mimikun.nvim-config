...
local function ensure_dispatch(bufnr)
  ...
  if not entry.mapped then
    ...
    vim.keymap.set("n", "<Tab>", function()
      ...
      if group._focus_index > #group._button_options then
        group._focus_index = 1
      end
    ...
    vim.keymap.set("n", "<S-Tab>", function()
      ...
      group._focus_index = group._focus_index - 1
      if group._focus_index < 1 then
        group._focus_index = #group._button_options
      end
      group:_refresh_highlights()
      group:_move_cursor_to_focus()
    end, { buffer = bufnr, nowait = true })
...
function ButtonGroupLine:new(button_options, opts)
  ...
  local sections = {}
  if o._group_label and #o._group_label > 0 then
    table.insert(sections, { o._group_label .. " " })
  end
  local btn_sep = "   "
  for i, opt in ipairs(o._button_options) do
    ...
    table.insert(sections, {
      label,
      function()
        return (o._focus_index == i) and focus_hl or BUTTON_NORMAL
      end,
    })
    if i < #o._button_options then
      table.insert(sections, { btn_sep })
    end
  end
...
function ButtonGroupLine:__tostring()
  return string.rep(" ", self._start_col) .. tostring(self._line)
end
...
function ButtonGroupLine:_refresh_highlights()
  if not (self._ns_id and self._bufnr and self._line_1b) then
    return
  end
  --- refresh content
...
function ButtonGroupLine:_move_cursor_to_focus()
  ...
  if winid and vim.api.nvim_win_is_valid(winid) then
    vim.api.nvim_win_set_cursor(winid, { self._line_1b, pos[1] })
  end
end
