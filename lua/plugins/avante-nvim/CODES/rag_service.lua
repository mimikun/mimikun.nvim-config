...
function M.get_rag_service_url()
  return string.format("http://localhost:%d", M.get_rag_service_port())
end
...
function M.launch_rag_service(cb)
  ...
  if M.get_rag_service_runner() == "docker" then
    ...
    if result.code ~= 0 then
      Utils.debug(string.format("cmd: %s execution error", table.concat(cmd, " ")))
    end
...
function M.stop_rag_service()
  if M.get_rag_service_runner() == "docker" then
    ...
    if result ~= "" then
      vim.system({ "docker", "rm", "-fv", container_name }):wait()
    end
  else
