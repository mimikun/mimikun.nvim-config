...
local function has_set_colors(hl_group)
  return next(Utils.get_hl(hl_group)) ~= nil
end
...
function M.setup()
  if Config.behaviour.auto_set_highlight_group then
    vim
      ...
      :each(function(_, hl)
        if first_setup and has_set_colors(hl.name) then
          already_set_highlights[hl.name] = true
        end
        if not already_set_highlights[hl.name] then
          local bg = hl.bg
          local fg = hl.fg
          if hl.bg_link ~= nil then
            bg = Utils.get_hl(hl.bg_link).bg
          end
          if hl.fg_link ~= nil then
            fg = Utils.get_hl(hl.fg_link).fg
          end
          if hl.bg_link_fg ~= nil then
            bg = Utils.get_hl(hl.bg_link_fg).fg
          end
          if hl.fg_link_bg ~= nil then
            fg = Utils.get_hl(hl.fg_link_bg).bg
          end
          api.nvim_set_hl(
            0,
            hl.name,
  ...
  if first_setup then
    vim.iter(Highlights.conflict):each(function(_, hl)
      if hl.name and has_set_colors(hl.name) then
        already_set_highlights[hl.name] = true
      end
    end)
  end
...
function M.setup_conflict_highlights()
  ...
  local function get_bg(hl_name)
    return Utils.get_hl(hl_name).bg
  end

  local function get_bold(hl_name)
    return Utils.get_hl(hl_name).bold
  end

  vim.iter(Highlights.conflict):each(function(key, hl)
    --- set none shade linked highlights first
    if hl.shade_link ~= nil and hl.shade ~= nil then
      return
    end

    if already_set_highlights[hl.name] then
      return
    end
