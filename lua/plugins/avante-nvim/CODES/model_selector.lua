...
local function create_model_entries(provider_name, provider_cfg)
  ...
  if provider_cfg.list_models then
    ...
    if type(provider_cfg.list_models) == "function" then
      if M.list_models_invoked[cache_key] then
        return {}
      end
      ...
      if M.list_models_returned[provider_cfg.list_models] then
        return {}
      end
  ...
  if provider_cfg.model then
    local seen = vim.iter(res):find(function(item)
      return item.model == provider_cfg.model
    end)
    if not seen then
      table.insert(res, {
...
function M.open()
  ...
  for provider_name, _ in pairs(Config.providers) do
    ...
    if provider_cfg.hide_in_model_selector then
      -- goto continue
    end
    if not provider_cfg.is_env_set() then
      -- goto continue
    end
  ...
  local default_item = vim.iter(models):find(function(item)
    return item.model == current_model and item.provider_name == Config.provider
  end)
  ...
  local function on_select(item_ids)
    ...
    local choice = vim.iter(models):find(function(item)
      return item.name == item_ids[1]
    end)
    if not choice then
      return
    end
    ...
    if choice.provider_name ~= Config.provider then
      require("avante.providers").refresh(choice.provider_name)
    end
  ...
  local selector = Selector:new({
    ...
    get_preview_content = function(item_id)
      local model = vim.iter(models):find(function(item)
