...
function M.build(opts)
  ...
  if not ok then
    Utils.error("Failed to build the command: " .. cmd .. "\n" .. job_or_err, { once = true })
  end
...
function M.focus(opts)
  ...
  if sidebar:is_open() then
    if curbuf == sidebar.containers.input.bufnr then
      if sidebar.code.winid and sidebar.code.winid ~= curwin then
    ...
    if sidebar.code.winid then
      vim.api.nvim_set_current_win(sidebar.code.winid)
    end
    ---@cast opts SidebarOpenOptions
    sidebar:open(opts)
    if sidebar.containers.input.winid then
      vim.api.nvim_set_current_win(sidebar.containers.input.winid)
    end
  end
end
...
function M.select_acp_model()
  require("avante.acp_config_selector").open_model()
end

function M.select_acp_mode()
  require("avante.acp_config_selector").open_mode()
end
...
function M.stop()
  require("avante.llm").cancel_inflight_request()
end
