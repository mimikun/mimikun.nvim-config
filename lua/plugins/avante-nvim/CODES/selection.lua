    ...
    for _, line in ipairs(response_lines_) do
      if line:match("^<code>") then
        ...
        if line_processed ~= "" then
          table.insert(response_lines, line_processed)
        end
...
function Selection:on_entering_visual_mode(bufnr)
  if Config.selection.hint_display == "none" then
    return
  end
  if vim.bo[bufnr].buftype == "terminal" or Utils.is_sidebar_buffer(bufnr) then
    return
  end
  ...
  api.nvim_create_autocmd({ "ModeChanged" }, {
    ...
    callback = function(ev)
      ...
      if ev.match:match("[vV]:[^vV]") then
        self:on_exiting_visual_mode()
      end
    end,
  })
...
function Selection:setup_autocmds()
  ...
  api.nvim_create_autocmd("User", {
    ...
    callback = function(ev)
      self:submit_input(ev.data.request)
    end,
  })
  ...
  api.nvim_create_autocmd({ "ModeChanged" }, {
    ...
    callback = function(ev)
      self:on_entering_visual_mode(ev.buf)
    end,
  })
end
