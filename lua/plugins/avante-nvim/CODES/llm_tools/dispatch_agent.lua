...
function M.on_render(input, opts)
  ...
  for _, msg in ipairs(messages) do
    ...
    if tool_use then
      ...
      if tool_result then
        if tool_use.name == "ls" then
          ...
          if tool_result.is_error then
            ...
            if ok then
              summary = string.format("Ls %s: %d paths", path, #filepaths)
            end
          end
        elseif tool_use.name == "grep" then
          ...
          if tool_result.is_error then
            ...
            local ok, filepaths = pcall(vim.json.decode, tool_result.content)
            if ok then
              summary = string.format("Grep %s in %s: %d paths", query, path, #filepaths)
            end
          end
        elseif tool_use.name == "glob" then
          ...
          if tool_result.is_error then
            ...
            if ok then
              ...
              if matches then
                summary = string.format("Glob %s in %s: %d matches", pattern, path, #matches)
              end
            end
          end
      ...
      if summary then
        summary = "  " .. Utils.icon("🛠️ ") .. summary
      end
    else
      summary = History.Helpers.get_text_data(msg)
    end
    if summary then
      table.insert(tool_use_summary, summary)
    end
  end
  local state = "running"
  ...
  local agent_loop_options = {
    ...
    on_messages_add = function(msgs)
      ...
      if opts.set_store then
        opts.set_store("messages", history_messages)
      end
      for _, msg in ipairs(msgs) do
        local tool_use = History.Helpers.get_tool_use_data(msg)
        if tool_use then
