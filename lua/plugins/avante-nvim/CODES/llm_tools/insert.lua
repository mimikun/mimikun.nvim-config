...
function M.func(input, opts)
  ...
  if not Path:new(abs_path):is_file() then
    return false, "Path is not a file: " .. abs_path
  end
  if input.insert_line == nil then
    return false, "insert_line not provided"
  end
  if input.new_str == nil then
    return false, "new_str not provided"
  end
  ...
  if input.insert_line > line_count - 1 then
    input.insert_line = line_count - 1
  end
