...
M.func = vim.schedule_wrap(function(input, opts)
  ...
  if not input.instructions then
    input.instructions = ""
  end
  if not input.code_edit then
    return false, "code_edit not provided"
  end
  ...
  if lines and #lines > 0 then
    if lines[#lines] == "" then
      lines = vim.list_slice(lines, 0, #lines - 1)
    end
  end
  ...
  vim.system(
    ...
    vim.schedule_wrap(function(result)
      ...
      if result.code ~= 0 then
        ...
        if not error_msg or error_msg == "" then
          error_msg = "No detailed error message available"
        end
