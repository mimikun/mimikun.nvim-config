...
function M.read_file_toplevel_symbols(input, opts)
  ...
  if on_log then
    on_log("path: " .. abs_path)
  end
  if not Path:new(abs_path):exists() then
    return "", "File does not exists: " .. abs_path
  end
...
function M.str_replace_editor(input, opts)
  if input.command == "undo_edit" then
    return require("avante.llm_tools.undo_edit").func(input, opts)
  end
  ---@cast input any
...
function M.str_replace_based_edit_tool(input, opts)
  ...
  if not input.command then
    return false, "command not provided"
  end
  if on_log then
    on_log("command: " .. input.command)
  end
  ...
  if input.command == "create" then
    return require("avante.llm_tools.create").func(input, opts)
  end
  if input.command == "insert" then
    return require("avante.llm_tools.insert").func(input, opts)
  end
...
function M.read_global_file(input, opts)
  ...
  if Helpers.is_ignored(abs_path) then
    return "", "This file is ignored: " .. abs_path
  end
  if on_log then
    on_log("path: " .. abs_path)
  end
...
function M.write_global_file(input, opts)
  ...
  local abs_path = Helpers.get_abs_path(input.abs_path)
  if Helpers.is_ignored(abs_path) then
    return false, "This file is ignored: " .. abs_path
  end
  if on_log then
    on_log("path: " .. abs_path)
  end
  if on_log then
    on_log("content: " .. input.content)
  end
  if not on_complete then
    return false, "on_complete not provided"
  end
...
function M.copy_path(input, opts)
  ...
  Helpers.confirm(
    ...
    function(ok, reason)
      ...
      if on_log then
        on_log("Copying path: " .. abs_path .. " to " .. new_abs_path)
      end
...
function M.create_dir(input, opts)
  ...
  if Path:new(abs_path):exists() then
    return false, "Directory already exists: " .. abs_path
  end
  if not on_complete then
    return false, "on_complete not provided"
  end
  Helpers.confirm("Are you sure you want to create the directory: " .. abs_path, function(ok, reason)
    ...
    if on_log then
      on_log("Creating directory: " .. abs_path)
    end
...
function M.web_search(input, opts)
  ...
  if provider_type == nil then
    return nil, "Search engine provider is not set"
  end
  if on_log then
    on_log("provider: " .. provider_type)
  end
  if on_log then
    on_log("query: " .. input.query)
  end
  local search_engine = Config.web_search_engine.providers[provider_type]
  if search_engine == nil then
    return nil, "No search engine found: " .. provider_type
  end
  if provider_type ~= "searxng" and search_engine.api_key_name == "" then
...
function M.git_diff(input, opts)
  ...
  if git_cmd == "" then
    return nil, "Git command not found"
  end
  ...
  if diff == "" then
    ...
    cmd = { "git", "diff", scope }
    if on_log then
      on_log("No staged changes. Running command: " .. table.concat(cmd, " "))
    end
...
function M.git_commit(input, opts)
  ...
  if on_log then
    on_log(string.format("GPG signing %s", has_gpg and "enabled" or "disabled"))
  end
  ...
  Helpers.confirm("Are you sure you want to commit with message:\n" .. full_commit_msg, function(ok, reason)
    ...
    if input.scope then
      ...
      if on_log then
        on_log("Staging files: " .. table.concat(stage_cmd, " "))
      end
...
function M.rag_search(input, opts)
  ...
  local on_complete = opts.on_complete
  if not on_complete then
    return nil, "on_complete not provided"
  end
  if not Config.rag_service.enabled then
    return nil, "Rag service is not enabled"
  end
...
function M.python(input, opts)
  ...
  local abs_path = Helpers.get_abs_path(input.path)
  if not Helpers.has_permission_to_access(abs_path) then
    return nil, "No permission to access path: " .. abs_path
  end
  if not Path:new(abs_path):exists() then
    return nil, "Path not found: " .. abs_path
  end
  if on_log then
    on_log("cwd: " .. abs_path)
  end
  ...
  Helpers.confirm(
    ...
    function(ok, reason)
      ...
      local function handle_result(result) ---@param result vim.SystemCompleted
        if result.code ~= 0 then
          return nil, "Error: " .. (result.stderr or "Unknown error")
        end
...
function M.get_tools(user_input, history_messages)
  ...
  return vim
    ...
    :filter(function(tool) ---@param tool AvanteLLMTool
      ...
      if vim.tbl_contains(Config.disabled_tools, tool.name) then
        return false
      end
...
M._tools = {
  ...
  require("avante.llm_tools.glob"),
  {
    name = "rag_search",
    enabled = function()
      return Config.rag_service.enabled and require("avante.rag_service").is_ready()
    end,
...
Then you can use the "read_definitions" tool to retrieve the source code definitions of these symbols.
  ...
  require("avante.llm_tools.undo_edit"),
  {
    ...
    enabled = function(opts)
      if opts.user_input:match("@read_global_file") then
        return true
      end
      for _, message in ipairs(opts.history_messages) do
        if message.role == "user" then
          local content = message.content
          if type(content) == "string" and content:match("@read_global_file") then
            return true
          end
          if type(content) == "table" then
            for _, item in ipairs(content) do
              if type(item) == "string" and item:match("@read_global_file") then
                return true
              end
            end
          end
  ...
  },
  {
    ...
    enabled = function(opts)
      if opts.user_input:match("@write_global_file") then
        return true
      end
      for _, message in ipairs(opts.history_messages) do
        if message.role == "user" then
          local content = message.content
          if type(content) == "string" and content:match("@write_global_file") then
            return true
          end
          if type(content) == "table" then
            for _, item in ipairs(content) do
              if type(item) == "string" and item:match("@write_global_file") then
                return true
              end
            end
          end
...
You can delete the first file by providing a path of "directory1/a/something.txt"
  ...
  },
  {
    ...
    func = function(input, opts)
      ...
      if on_log then
        on_log("symbol_name: " .. vim.inspect(symbol_name))
      end
      if on_log then
        on_log("show_line_numbers: " .. vim.inspect(show_line_numbers))
      end
      if not symbol_name then
        return nil, "No symbol name provided"
      end
      local sidebar = require("avante").get()
      if not sidebar then
        return nil, "No sidebar"
      end
      ...
      if not vim.api.nvim_buf_is_valid(bufnr) then
        return nil, "Invalid bufnr"
      end
      if on_log then
        on_log("bufnr: " .. vim.inspect(bufnr))
      end
...
function M.process_tool_use(tools, tool_use, opts)
  ...
  elseif tool_use.name == "str_replace_based_edit_tool" then
    ...
    ---@type AvanteLLMTool?
    local tool = vim.iter(tools):find(function(tool)
      return tool.name == tool_use.name
  ...
  if not func then
    return nil, "Tool not found: " .. tool_use.name
  end
  if on_log then
    on_log(tool_use.id, tool_use.name, "running tool", "running")
  end
  ...
  local function handle_result(result, err)
    ...
    if Helpers.is_cancelled then
      if on_log then
        on_log(tool_use.id, tool_use.name, "cancelled during result handling", "failed")
      end
      return nil, Helpers.CANCEL_TOKEN
    end
    ...
    if err ~= nil then
      if on_log then
        on_log(tool_use.id, tool_use.name, "Error: " .. err, "failed")
      end
    else
      if on_log then
        on_log(tool_use.id, tool_use.name, "tool finished", "succeeded")
      end
    end
  ...
  local result, err = func(input_json, {
    ...
    on_log = function(log)
      ...
      if on_log then
        on_log(tool_use.id, tool_use.name, log, "running")
      end
    end,
    set_store = function(key, value)
      if opts.set_tool_use_store then
        opts.set_tool_use_store(tool_use.id, key, value)
      end
    end,
