...
local function filepath_to_filename(filepath)
  return tostring(filepath):sub(tostring(filepath:parent()):len() + 2)
end
...
function History.list(bufnr)
  ...
  table.sort(res, function(a, b)
    ...
    if a.filename == latest_filename then
      return true
    end
    if b.filename == latest_filename then
      return false
    end
...
function History.save_latest_filename(bufnr, filename)
  ...
  if metadata.project_root == nil then
    metadata.project_root = Utils.root.get({
      buf = bufnr,
    })
  end
  ...
  if filepath:exists() then
    ...
    if content ~= nil then
      ...
      if decode_ok and type(history) == "table" then
        if not history.title or type(history.title) ~= "string" then
          history.title = "untitled"
        end
        if not history.timestamp or history.timestamp ~= "string" then
          history.timestamp = Utils.get_timestamp()
        end
        -- TODO: sanitize individual entries of the lists below as well.
        if not vim.islist(history.entries) then
          history.entries = {}
        end
        if not vim.islist(history.messages) then
          history.messages = {}
        end
        if not vim.islist(history.todos) then
          history.todos = {}
        end
...
function P.list_projects()
  ...
  for _, dir_path in ipairs(dirs) do
    ...
    if metadata_file:exists() then
      ...
      if content then
        ...
        if metadata and metadata.project_root then
  ...
  table.sort(projects, function(a, b)
    return a.history_count > b.history_count
  end)
...
---@return string
function Prompt.get_custom_prompts_filepath(mode)
  return string.format("custom.%s.avanterules", mode)
end

function Prompt.get_builtin_prompts_filepath(mode)
...
function Prompt.get_templates_dir(project_root)
  ...
  if vim.fn.isdirectory(cache_dir_str) == 0 then
    vim.fn.mkdir(cache_dir_str, "p")
  end
  ...
  local function find_rules(dir)
    ...
    for _, entry in ipairs(scanner) do
      ...
      if file:is_file() then
        ...
        if not mode or not Prompt.custom_modes[mode] then
          -- goto continue
        end
  ...
  if Config.rules.project_dir then
    ...
    if not project_rules_path:is_absolute() then
      project_rules_path = directory:joinpath(project_rules_path)
    end
  ...
  local source_dir = Path:new(debug.getinfo(1).source:match("@?(.*/)"):gsub("/lua/avante/path.lua$", "") .. "templates")
...
function Prompt.get_filepath(mode)
  if Prompt.custom_prompts_contents[mode] ~= nil then
    return Prompt.get_custom_prompts_filepath(mode)
  end
...
function RepoMap.get(project_root, ext)
  return Path:new(P.data_path):joinpath(RepoMap.filename(project_root, ext))
end
...
function P.setup()
  ...
  if vim.uv.fs_stat(history_path) == nil then
  ...
  if vim.uv.fs_stat(data_path) == nil then
...
function P.available()
  return P._init_templates_lib() ~= nil
end
...
function P.clear()
  ...
  if vim.uv.fs_stat(P.cache_path) == nil then
    vim.fn.mkdir(P.cache_path, "p")
  end
  if vim.uv.fs_stat(P.history_path) == nil then
    vim.fn.mkdir(P.history_path, "p")
  end
end
