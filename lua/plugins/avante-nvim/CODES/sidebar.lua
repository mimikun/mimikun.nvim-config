...
local function get_acp_config_options(acp_client, provider)
  ...
  if not acp_provider or not acp_client or not acp_client.config_options then
    return {}
  end
  if not acp_client.config or acp_client.config.command ~= acp_provider.command then
    return {}
  end
  if not vim.deep_equal(acp_client.config.args, acp_provider.args) then
    return {}
  end
...
function Sidebar:open(opts)
  ...
  if not self:is_open() then
    ...
    if opts.selection then
      self.code.selection = opts.selection
    end
    self:render(opts)
    self:focus()
  else
  ...
  vim.api.nvim_create_autocmd("WinNew", {
    ...
    callback = function(env)
      ...
      for _, winid in ipairs(vim.api.nvim_tabpage_list_wins(self.id)) do
        if not vim.api.nvim_win_is_valid(winid) or self:is_sidebar_winid(winid) then
          -- goto continue
        end
...
function Sidebar:set_code_winhl()
  if not self.code.winid or not api.nvim_win_is_valid(self.code.winid) then
    return
  end
  if not Utils.is_valid_container(self.containers.result, true) then
    return
  end
  ...
  if Utils.should_hidden_border(self.code.winid, self.containers.result.winid) then
    ...
    for _, piece in ipairs(pieces) do
      if not piece:find("WinSeparator:") and piece ~= "" then
...
function Sidebar:is_open()
  return Utils.is_valid_container(self.containers.result, true)
end
  ...
  while true do
    ...
    if line_content:match("^%s*<[Ss][Ee][Aa][Rr][Cc][Hh]>") then
      ...
      if not line_content:match("^%s*<[Ss][Ee][Aa][Rr][Cc][Hh]>%s*$") then
        ...
        if search_start_line and search_start_line ~= "" then
          table.insert(result_lines, i + 1, search_start_line)
        end
      end
      line_content = "<SEARCH>"
      ...
      if next_line and next_line:match("^%s*```%w+$") then
        i = i + 1
      end
      search_start = i + 1
    ...
    elseif line_content:match("</[Rr][Ee][Pp][Ll][Aa][Cc][Ee]>%s*$") then
      ...
      is_replacing = false
      local prev_line = result_lines[i - 1]
      if not (prev_line and prev_line:match("^%s*```$")) then
...
local function generate_display_content(replacement)
  ...
  if replacement.last_think_tag_start_line > 0 then
    ...
    if last_think_tag_end_line == 0 then
      last_think_tag_end_line = #lines + 1
    end
    ...
    local formatted_thinking_content_lines = vim
      ...
      :map(function(line)
        if Utils.trim_spaces(line) == "" then
          return line
        end
  ...
  for _, node in ipairs(tree_sitter_markdown_parse_code_blocks(response_content)) do
    ...
    then
      ...
      if start_line ~= nil and end_line ~= nil then
        ...
        if filepath:match("^[Ff]ilepath:") then
          filepath = filepath:match("^[Ff]ilepath:%s*(.+)")
        end
        local content =
...
local function insert_conflict_contents(bufnr, snippets)
  ...
  for _, snippet in ipairs(snippets) do
    ...
    if first_line_content then
      old_first_line_indentation = Utils.get_indentation(first_line_content)
    end
...
function Sidebar:get_current_user_request_block(position)
  ...
  if current_resp_content == nil then
    return nil
  end
  if current_resp_content == "" then
    return nil
  end
...
local function parse_codeblocks(buf)
  ...
  for _, node in ipairs(tree_sitter_markdown_parse_code_blocks(buf)) do
    ...
    elseif node:type() == "fenced_code_block_delimiter" and start_line ~= nil and node:start() >= start_line then
      ...
      valid = lines[start_line - 1]:match("^%s*(%d*)[%.%)%s]*[Aa]?n?d?%s*[Rr]eplace%s+[Ll]ines:?%s*(%d+)%-(%d+)") ~= nil
      if valid then
...
function Sidebar:minimize_snippets(filepath, snippets)
  ...
  if original_lines_ then
    original_lines = original_lines_
  end
  ...
  vim.defer_fn(function()
    ...
    for filepath, snippets in pairs(selected_snippets_map) do
      if Config.behaviour.minimize_diff then
        snippets = self:minimize_snippets(filepath, snippets)
      end
    ...
    if Config.acp_providers[Config.provider] then
      ...
      for _, opt in ipairs(get_acp_config_options(self.acp_client, Config.provider)) do
        if opt.category == "model" then
          table.insert(parts, opt.currentValue)
        end
...
function Sidebar:render_result()
  if not Utils.is_valid_container(self.containers.result) then
    return
  end
...
function Sidebar:render_input(ask)
  ...
  if not Utils.is_valid_container(self.containers.input) then
    return
  end
...
function Sidebar:render_selected_code()
  if not self.code.selection then
    return
  end
  if not Utils.is_valid_container(self.containers.selected_code) then
    return
  end
  ...
  if Config.windows.sidebar_header.enabled then
    max_shown = max_shown - 1
  end

  local header_text = Utils.icon(" ") .. "Selected Code"
  if max_shown < count then
    header_text = string.format("%s (%d/%d lines)", header_text, max_shown, count)
  end
...
function Sidebar:bind_apply_key()
  if self.containers.result then
    vim.keymap.set("n", Config.mappings.sidebar.apply_cursor, function()
      self:apply(true)
    end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
  end
end
...
function Sidebar:bind_retry_user_request_key()
  if self.containers.result then
    vim.keymap.set("n", Config.mappings.sidebar.retry_user_request, function()
      self:retry_user_request()
    end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
  end
end
...
function Sidebar:bind_expand_tool_use_key(message_uuid)
  if self.containers.result then
    ...
    vim.keymap.set("n", Config.mappings.sidebar.expand_tool_use, function()
      self:handle_expand_message(message_uuid, not expanded)
    end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
  end
end
...
function Sidebar:bind_edit_user_request_key()
  if self.containers.result then
    vim.keymap.set("n", Config.mappings.sidebar.edit_user_request, function()
      self:edit_user_request()
    end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
  end
end
...
function Sidebar:bind_sidebar_keys(codeblocks)
  ...
  vim.keymap.set("n", Config.mappings.sidebar.apply_all, function()
    self:apply(false)
  end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
  ...
  vim.keymap.set("n", Config.mappings.sidebar.next_prompt, function()
    jump_to_prompt("next")
  end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
  vim.keymap.set("n", Config.mappings.sidebar.prev_prompt, function()
    jump_to_prompt("prev")
  end, { buffer = self.containers.result.bufnr, noremap = true, silent = true })
end
...
function Sidebar:on_mount(opts)
  ...
  local function show_apply_button(block)
    ...
    current_apply_extmark_id =
      api.nvim_buf_set_extmark(self.containers.result.bufnr, CODEBLOCK_KEYBINDING_NAMESPACE, block.start_line - 1, -1, {
        virt_text = {
          {
            string.format(
      ...
      if self.code.bufnr and api.nvim_buf_is_valid(self.code.bufnr) then
        ...
        if ts_ok and ts_highlighter.active[self.code.bufnr] then
          ...
          if lang and lang ~= "" then
            vim.treesitter.start(selected_code_buf, lang)
          end
        end
...
function Sidebar:switch_window_focus(direction)
  ...
  for _, name in ipairs(SIDEBAR_CONTAINERS) do
    ...
    if container and container.winid then
      ...
      if container.winid == current_winid then
        current_index = #ordered_winids
      end
    end
  end
...
function Sidebar:setup_window_navigation(container)
  ...
  vim.keymap.set({ "n", "i" }, "<Plug>(AvanteSidebarSwitchWindow)", function()
    self:switch_window_focus("next")
  end, { buffer = buf, noremap = true, silent = true, nowait = true })
  vim.keymap.set({ "n", "i" }, "<Plug>(AvanteSidebarReverseSwitchWindow)", function()
    self:switch_window_focus("previous")
  end, { buffer = buf, noremap = true, silent = true, nowait = true })
...
function Sidebar:toggle_code_window()
  ...
  local winids = vim
    ...
    :filter(function(winid)
      return not self:is_sidebar_winid(winid)
    end)
    :totable()
...
function Sidebar:initialize()
  ...
  if vim.list_contains(Config.selector.exclude_auto_select, buf_ft) then
    return self
  end
...
function Sidebar:is_sidebar_winid(winid)
  return self:get_sidebar_window(winid) ~= nil
end

---@return boolean
function Sidebar:should_auto_scroll()
  if not self.containers.result or not self.containers.result.winid then
    return false
  end
  if not api.nvim_win_is_valid(self.containers.result.winid) then
    return false
  end
...
function Sidebar:clear_history(args, cb)
  ...
  if next(self.chat_history) ~= nil then
    ...
    self:update_content("Chat history cleared", {
      focus = false,
      scroll = false,
      callback = function()
        self:focus_input()
    ...
    self:update_content("Chat history is already empty", {
      focus = false,
      scroll = false,
      callback = function()
        self:focus_input()
...
function Sidebar:render_state()
  ...
  if self.current_state == "thinking" then
    spinner_chars = self.thinking_spinner_chars
  end
  local hl = "AvanteStateSpinnerGenerating"
  if self.current_state == "tool calling" then
    hl = "AvanteStateSpinnerToolCalling"
  end
  if self.current_state == "failed" then
    hl = "AvanteStateSpinnerFailed"
  end
  if self.current_state == "succeeded" then
    hl = "AvanteStateSpinnerSucceeded"
  end
  if self.current_state == "searching" then
    hl = "AvanteStateSpinnerSearching"
  end
  if self.current_state == "thinking" then
    hl = "AvanteStateSpinnerThinking"
  end
  if self.current_state == "compacting" then
    hl = "AvanteStateSpinnerCompacting"
  end
  local spinner_char = spinner_chars[self.state_spinner_idx]
  if not spinner_char then
    spinner_char = spinner_chars[1]
  end
  ...
  self.state_timer = vim.defer_fn(function()
    self:render_state()
  end, 160)
end
...
function Sidebar:compact_history_messages(args, cb)
  ...
  self:render_state()
  self:update_content("compacting history messsages", {
    focus = false,
    scroll = true,
    callback = function()
      self:focus_input()
...
function Sidebar:new_chat(args, cb)
  ...
  vim.schedule(function()
    vim.api.nvim_win_call(self.containers.result.winid, function()
      vim.cmd("normal! ggG")
    end)
  end)
  if cb then
...
local debounced_save_history = Utils.debounce(function(self)
  Path.history.save(self.code.bufnr, self.chat_history)
end, 1000)

function Sidebar:save_history()
  debounced_save_history(self)
end
...
function Sidebar:delete_history_messages(uuids)
  ...
  for _, msg in ipairs(history_messages) do
    if vim.list_contains(uuids, msg.uuid) then
      msg.is_deleted = true
    end
  end
  Path.history.save(self.code.bufnr, self.chat_history)
...
function Sidebar:get_history_messages_for_api(opts)
  ...
  messages = vim
    ...
    :filter(function(message)
      if message.just_for_display or message.is_compacted then
        return false
      end
      if not opts.all then
        if message.state == "generating" then
          return false
        end
        if last_message and message.uuid == last_message then
      ...
      messages = vim
        ...
        :filter(function(message)
          if not last_message_seen then
            if message.uuid == last_message then
...
function Sidebar:get_generate_prompts_options(request, cb)
  ...
  if buf_name and buf_name ~= "" then
    file_ext = fn.fnamemodify(buf_name, ":e")
  end
  ...
  if self.chat_history.memory then
    prompts_opts.memory = self.chat_history.memory.content
  end

  if Config.behaviour.enable_token_counting then
    self.token_count = Llm.calculate_tokens(prompts_opts)
  end
...
function Sidebar:handle_submit(request)
    ...
    if cmd then
      if cmd.callback then
        ...
        elseif command == "commit" then
          cmd.callback(self, args, function(question)
            request = question
          end)
        else
  ...
  local function on_messages_add(messages)
    self:add_history_messages(messages)
  end
  ...
  local function on_stop(stop_opts)
    ...
    if stop_opts.error ~= nil and stop_opts.error ~= vim.NIL then
      ...
      if type(msg_content) ~= "string" then
        msg_content = vim.inspect(msg_content)
      end
  ...
  self:get_generate_prompts_options(request, function(generate_prompts_options)
    ...
    local stream_options = vim.tbl_deep_extend("force", generate_prompts_options, {
      ...
      get_history_messages = function(opts)
        return self:get_history_messages_for_api(opts)
      end,
      get_todos = function()
        local history = Path.history.load(self.code.bufnr)
        return history.todos
      end,
      update_todos = function(todos)
      ...
      get_tokens_usage = function()
        return self.chat_history.tokens_usage
      end,
    })
...
function Sidebar:initialize_token_count()
  if Config.behaviour.enable_token_counting then
    self:get_generate_prompts_options("")
  end
end
...
function Sidebar:create_input_container()
  ...
  if Config.mappings.sidebar.close_from_input ~= nil then
    if Config.mappings.sidebar.close_from_input.normal ~= nil then
      self.containers.input:map("n", Config.mappings.sidebar.close_from_input.normal, function()
        self:shutdown()
      end)
    end
    if Config.mappings.sidebar.close_from_input.insert ~= nil then
      self.containers.input:map("i", Config.mappings.sidebar.close_from_input.insert, function()
        self:shutdown()
      end)
    end
  end
  ...
  if Config.mappings.sidebar.toggle_code_window_from_input ~= nil then
    if Config.mappings.sidebar.toggle_code_window_from_input.normal ~= nil then
      self.containers.input:map("n", Config.mappings.sidebar.toggle_code_window_from_input.normal, function()
        self:toggle_code_window()
      end)
    end
    if Config.mappings.sidebar.toggle_code_window_from_input.insert ~= nil then
      self.containers.input:map("i", Config.mappings.sidebar.toggle_code_window_from_input.insert, function()
        self:toggle_code_window()
      end)
    end
  end
...
function Sidebar:set_input_value(value)
  if not self.containers.input then
    return
  end
  if not value then
    return
...
function Sidebar:get_result_container_height()
  ...
  if self:get_layout() == "horizontal" then
    return math.floor(Config.windows.height / 100 * vim.o.lines)
  end
...
function Sidebar:get_result_container_width()
  if self:get_layout() == "vertical" then
    return math.floor(Config.windows.width / 100 * vim.o.columns)
  end
...
function Sidebar:render(opts)
  ...
  if opts.sidebar_pre_render then
    opts.sidebar_pre_render(self)
  end
  ...
  self.containers.result:on(event.BufWinEnter, function()
    xpcall(function()
      api.nvim_buf_set_name(self.containers.result.bufnr, RESULT_BUF_NAME)
    end, function(_) end)
  end)

  self.containers.result:map("n", Config.mappings.sidebar.close, function()
    self:shutdown()
  end)
  self.containers.result:map("n", Config.mappings.sidebar.toggle_code_window, function()
    self:toggle_code_window()
  end)
  ...
  api.nvim_create_autocmd("User", {
    ...
    callback = function(ev)
      if ev.data and ev.data.request then
        self:handle_submit(ev.data.request)
      end
    end,
  })
...
function Sidebar:adjust_selected_code_container_layout()
  if not Utils.is_valid_container(self.containers.selected_code, true) then
    return
  end
...
function Sidebar:create_selected_files_container()
  ...
  local function remove_file(line_number)
    self.file_selector:remove_selected_filepaths_with_index(line_number)
  end
  ...
  self.containers.selected_files:map("n", Config.mappings.sidebar.add_file, function()
    self.file_selector:open()
  end, { noremap = true, silent = true })

  -- Set up autocmd to show hint on cursor move
  self.containers.selected_files:on({ event.CursorMoved }, function()
    self:show_selected_files_hint()
  end, {})

  -- Clear hint when leaving the window
  self.containers.selected_files:on(event.BufLeave, function()
    self:close_selected_files_hint()
  end, {})
...
function Sidebar:create_todos_container()
  ...
  if focused_idx > #todos_content_lines then
    focused_idx = #todos_content_lines
  end
  ...
  if not ok then
    Utils.debug("Failed to adjust layout after todos creation:", err)
  end
end
