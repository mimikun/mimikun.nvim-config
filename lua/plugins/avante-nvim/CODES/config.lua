...
local function get_config_dir_path()
  return vim.fs.joinpath(vim.fn.expand("~"), ".config", "avante.nvim")
end
local function get_config_file_path()
  return vim.fs.joinpath(get_config_dir_path(), "config.json")
end
...
function M.setup(opts)
  ...
  if last_model then
    if provider_configured then
      if last_provider == nil or last_provider == merged.provider then
        apply_model_selection(merged, last_model)
      end
    else
