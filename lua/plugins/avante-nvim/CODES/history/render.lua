...
local function text_to_lines(text, decoration, filter)
  ...
  for idx, text_line in ipairs(text_lines) do
    if filter and not filter(text_line, idx, #text_lines) then
      -- goto continue
    end
    if decoration then
      ...
      if content_item.type == "content" then
        if content_item.content.type == "text" then
          local lines_ = text_to_lines(content_item.content.text, decoration, function(text_line, idx, len)
            if idx == 1 and text_line:match("^%s*```%s*$") then
              return false
            end
            if idx == len and text_line:match("^%s*```%s*$") then
              return false
            end
            return true
          end)
...
function M.get_tool_display_name(message)
  ...
  if type(content) ~= "table" then
    return "", "expected message content to be a table"
  end

  ---@cast content AvanteLLMMessageContentItem[]

  if not islist(content) then
    return "", "expected message content to be a list"
  end
  ...
  if message.acp_tool_call and message.acp_tool_call.title then
    native_tool_name = message.acp_tool_call.title
  end
  ...
  if message.displayed_tool_name then
    ...
    if item.input and type(item.input) == "table" then
      local path
      if type(item.input.path) == "string" then
        path = item.input.path
      end
      if type(item.input.rel_path) == "string" then
        path = item.input.rel_path
      end
      if type(item.input.filepath) == "string" then
        path = item.input.filepath
      end
      if type(item.input.file_path) == "string" then
        path = item.input.file_path
      end
      if type(item.input.query) == "string" then
        param = item.input.query
      end
      if type(item.input.pattern) == "string" then
      ...
      if native_tool_name == "execute" and not param then
        if message.acp_tool_call and message.acp_tool_call.title then
          param = message.acp_tool_call.title
        end
      end
    ...
    if param then
      tool_name = native_tool_name .. "(" .. vim.inspect(param) .. ")"
    end
  end
...
local function message_content_item_to_lines(item, message, messages, expanded)
  ...
  elseif type(item) == "table" then
    ...
    elseif item.type == "tool_use" and item.name then
      ...
      if message.tool_use_log_lines then
        lines = vim.list_extend(lines, message.tool_use_log_lines)
      end
      return lines
    end
  end
...
function M.message_to_lines(message, messages, expanded)
  if message.displayed_content then
    return text_to_lines(message.displayed_content)
  end
  local content = message.message.content
  if type(content) == "string" then
...
function M.message_to_text(message, messages)
  ...
  if islist(content) then
    return vim
      .iter(content)
      :map(function(item)
        return message_content_item_to_text(item, message, messages)
      end)
