  ...
  if max_tool_use then
    local n_edits = vim.iter(files):fold(
      ...
      function(count, file_info)
        if file_info.edit_tool_id then
          count = count + 1
        end
        return count
      end
...
function M.get_pending_tools(messages)
  ...
  if #messages > 0 then
    last_turn_id = messages[#messages].turn_id
  end
  ...
  for idx = #messages, 1, -1 do
    ...
    if last_turn_id and message.turn_id ~= last_turn_id then
      break
    end
    ...
    local result = Helpers.get_tool_result_data(message)
    if result then
      tool_result_seen[result.tool_use_id] = true
    end
