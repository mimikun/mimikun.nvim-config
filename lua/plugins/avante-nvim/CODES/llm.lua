...
function M.summarize_memory(prev_memory, history_messages, cb)
  ...
  local conversation_items = vim
    ...
    :map(function(msg)
      return msg.message.role .. ": " .. HistoryRender.message_to_text(msg, history_messages)
    end)
    :totable()
  ...
  local function on_memory_summarize(pending_compaction_history_messages)
    ...
    M.summarize_memory(memory_content, pending_compaction_history_messages or {}, function(memory)
      ...
      for _, msg in ipairs(history_messages) do
        if not compaction_history_message_uuids[msg.uuid] then
...
function M.generate_prompts(opts)
  ...
  selected_files = vim
    ...
    :filter(function(file)
      return viewed_files[file.path] == nil
    end)
    :totable()

  local is_acp_provider = false
  if not opts.provider then
    is_acp_provider = Config.acp_providers[Config.provider] ~= nil
  end
  ...
  if context_window and context_window > 0 then
    ...
    if opts.get_tokens_usage then
      ...
      if tokens_usage and tokens_usage.prompt_tokens ~= nil and tokens_usage.completion_tokens ~= nil then
        ...
        if tokens_count > target_tokens then
          pending_compaction_history_messages = opts.history_messages
        end
      end
    end
  end
  ...
  messages = vim
    .iter(messages)
    :filter(function(msg)
      return type(msg.content) ~= "string" or msg.content ~= ""
    end)
    :totable()
  ...
  local tools = {}
  if opts.tools then
    tools = vim.list_extend(tools, opts.tools)
  end
  if opts.prompt_opts and opts.prompt_opts.tools then
    tools = vim.list_extend(tools, opts.prompt_opts.tools)
  end
    ...
    vim.tbl_extend("force", curl_options, {
      ...
      callback = function(result)
        ...
        if result.status >= 400 then
          ...
          if headers_map["retry-after"] then
            retry_after = tonumber(headers_map["retry-after"]) or 10
          end
  ...
  api.nvim_create_autocmd("User", {
    ...
    callback = function()
      ...
      if active_job then
        ...
        -- 检查 active_job 的状态
        local job_is_alive = pcall(function()
          return active_job:is_closing() == false
        end)
...
local function truncate_history_for_recovery(history_messages)
  if not history_messages or #history_messages == 0 then
    return {}
  end
...
function M._stream_acp(opts)
  ...
  local get_history_messages = function()
    if opts.get_history_messages then
      return opts.get_history_messages()
    end
    return history_messages
  end
  ...
  if not acp_client then
      ...
      handlers = {
        on_session_update = function(update)
          ...
          local function try_follow_agent_location(upd)
            ...
            if not sidebar or sidebar.is_in_full_view then
              return
            end
          ...
          if update.sessionUpdate == "tool_call_update" then
            ...
            local merged = tool_call_messages[update.toolCallId] and tool_call_messages[update.toolCallId].acp_tool_call
            if merged then
          ...
          if update.sessionUpdate == "available_commands_update" then
            ...
            if has_cmp then
              ...
              if slash_commands_id ~= nil then
                cmp.unregister_source(slash_commands_id)
              end
        ...
        on_request_permission = function(tool_call, options, callback)
          ...
          if not message then
            ...
            if message.acp_tool_call then
              if tool_call.content and next(tool_call.content) == nil then
                tool_call.content = nil
              end
        ...
        on_read_file = function(path, line, limit, callback, error_callback)
          ...
          if line ~= nil and limit ~= nil then
            file_lines = vim.list_slice(file_lines, line, line + limit)
          end
    ...
    acp_client:connect(function(conn_err)
      ...
      if opts.on_save_acp_client then
...
function M._create_acp_session_and_continue(opts, acp_client)
  ...
  acp_client:create_session(project_root, mcp_servers, function(session_id_, err)
    ...
    if opts.on_save_acp_session_id then
      opts.on_save_acp_session_id(session_id_)
    end
      ...
      if recovery_enabled and is_session_not_found and not rawget(opts, "_session_recovery_attempted") then
        ...
        if should_truncate and #original_history > 20 then -- Only truncate if history is long enough (20条)
          ...
          if ok then
            ...
            Utils.info(
              "History truncated from " .. #original_history .. " to " .. #truncated_history .. " messages for recovery"
            )
          else
...
function M._stream(opts)
  ...
  if LLMToolHelpers then
    LLMToolHelpers.is_cancelled = false
  end
  ...
  if not opts.session_ctx.on_messages_add then
    opts.session_ctx.on_messages_add = opts.on_messages_add
  end
  if not opts.session_ctx.on_state_change then
    opts.session_ctx.on_state_change = opts.on_state_change
  end
  if not opts.session_ctx.on_start then
    opts.session_ctx.on_start = opts.on_start
  end
  if not opts.session_ctx.on_chunk then
    opts.session_ctx.on_chunk = opts.on_chunk
  end
  if not opts.session_ctx.on_stop then
    opts.session_ctx.on_stop = opts.on_stop
  end
  if not opts.session_ctx.on_tool_log then
    opts.session_ctx.on_tool_log = opts.on_tool_log
  end
  ...
  local handler_opts = {
    ...
    on_stop = function(stop_opts)
      if stop_opts.usage and opts.update_tokens_usage then
        opts.update_tokens_usage(stop_opts.usage)
      end
      ...
      local function handle_next_tool_use(
        ...
        local function handle_tool_result(result, error)
          ...
          if opts.on_messages_add then
            opts.on_messages_add({ partial_tool_use_message })
          end
        ...
        local llm_tool = vim.iter(prompt_opts.tools):find(function(tool)
          return tool.name == partial_tool_use.name
        end)
        if llm_tool then
          support_streaming = llm_tool.support_streaming == true
        end
        ...
        if partial_tool_use.state == "generating" then
          if not is_edit_tool_use and not support_streaming then
            return
          end
        ...
        if result ~= nil or error ~= nil then
          return handle_tool_result(result, error)
        end
      end
      ...
      if stop_opts.reason == "complete" and Config.mode == "agentic" then
        ...
        if opts.get_todos then
          ...
          unfinished_todos = vim.tbl_filter(function(todo)
            return todo.status ~= "done" and todo.status ~= "cancelled"
          end, todos)
        end
      ...
      if stop_opts.reason == "rate_limit" then
        ...
        local function countdown()
          ...
          if opts.on_chunk then
            ...
            local prefix = ""
            if retry_count < stop_opts.retry_after then
              prefix = [[\033[1A\033[K]]
            end
  ...
  return M.curl({
    ...
    on_response_headers = function(headers)
...
local function _merge_response(first_response, second_response, opts)
  ...
  prompt = prompt
    :gsub("{{[%s]*provider1_output[%s]*}}", function()
      return first_response
    end)
    :gsub("{{[%s]*provider2_output[%s]*}}", function()
      return second_response
    end)

  prompt = prompt .. "\n"

  if opts.instructions == nil then
    opts.instructions = ""
  end
...
function M._dual_boost_stream(opts, Provider1, Provider2)
  ...
  if not success then
    Utils.error("Failed to start dual_boost streams: " .. tostring(err))
  end
end
...
function M.stream(opts)
  ...
  if opts.on_tool_log ~= nil then
    ...
    opts.on_tool_log = vim.schedule_wrap(function(...)
      if not original_on_tool_log then
        return
      end
  ...
  if opts.set_tool_use_store ~= nil then
    ...
    opts.set_tool_use_store = vim.schedule_wrap(function(...)
      if not original_set_tool_use_store then
        return
      end
...
function M.cancel_inflight_request()
  if LLMToolHelpers.is_cancelled ~= nil then
