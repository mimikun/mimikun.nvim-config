...
if vim.g.avante_loaded ~= nil then
  return
end
...
if Config.support_paste_image() then
  vim.paste = (function(overridden)
    ...
    return function(lines, phase)
      ...
      if not is_image_candidate then
        return overridden(lines, phase)
      end
...
local function ask_complete(prefix, _, _)
  ...
  vim.list_extend(
    ...
    ---@param x string
    vim.tbl_map(function(x)
      return "project_root=" .. x.root
    end, P.list_projects())
  )
  return vim.tbl_filter(function(candidate)
    return vim.startswith(candidate, prefix)
  end, candidates)
end
...
cmd("Ask", function(opts)
  ...
  if parsed_args.position then
    args.win.position = parsed_args.position
  end
...
cmd("Edit", function(opts)
  require("avante.api").edit(vim.trim(opts.args), opts.line1, opts.line2)
end, { desc = "avante: edit selected block", nargs = "*", range = 2 })
cmd("Refresh", function()
  require("avante.api").refresh()
end, { desc = "avante: refresh windows" })
cmd("Focus", function()
  require("avante.api").focus()
end, { desc = "avante: switch focus windows" })
...
cmd("SwitchSelectorProvider", function(opts)
  require("avante.api").switch_selector_provider(vim.trim(opts.args or ""))
end, {
  nargs = 1,
  desc = "avante: switch selector provider",
})
cmd("SwitchInputProvider", function(opts)
  require("avante.api").switch_input_provider(vim.trim(opts.args or ""))
end, {
  nargs = 1,
