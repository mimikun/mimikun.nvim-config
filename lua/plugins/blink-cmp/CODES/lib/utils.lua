...
if vim.fn.has("nvim-0.13") == 1 then
  ...
  --- @return vim.Pos
  function utils.get_vim_pos(buf, row, col)
    return vim.pos(buf, row, col)
  end
else
  --- @param buf integer
...
function utils.to_string_or_empty(v)
  return (lib.is_not_nil(v) and type(v) == "string") and v or ""
end
...
function utils.defer_neovide_redraw(fn)
  ...
  if not _G.neovide or not neovide.enable_redraw or not neovide.disable_redraw then
    return fn()
  end
