...
function cmdline_events:listen(opts)
  ...
  vim.on_key(function(raw_key, escaped_key)
    ...
    if key:sub(1, 1) == "<" and key:sub(#key, #key) == ">" and raw_key ~= " " then
      return
    end
    if key == "" then
      return
  ...
  nvim.create_autocmd("CursorMovedC", {
    callback = function()
      ...
      if is_change_queued then
        return
      end

      if not is_burst_move() then
...
function cmdline_events:suppress_events_for_callback(cb)
  ...
  if not nvim.get_mode().mode == "c" then
    return
  end
