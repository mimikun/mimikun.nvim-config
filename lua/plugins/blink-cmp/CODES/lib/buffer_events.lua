...
local function make_char_added(self, on_char_added)
  return function()
    if not require("blink.cmp").is_enabled() or self:hide_in_snippet() then
      return
    end
...
local function make_cursor_moved(self, on_cursor_moved)
  ...
  vim.on_key(function(key)
    did_backspace = self.backspace_keycodes[key]
  end)
  ...
  return function(ev)
    ...
    if ev.event == "CursorMoved" and (nvim.get_mode().mode ~= "v" or not in_snippet_context) then
      return
    end
...
function buffer_events:resubscribe(opts)
  if not opts.on_char_added or self.textchangedi_id == -1 then
    return
  end
...
function buffer_events:hide_in_snippet()
  return not self.show_in_snippet and not self.has_context() and snippet.active()
end
