...
function cursor_line:update(win)
  if not nvim.win_is_valid(win) then
    return
  end
  ...
  nvim.set_decoration_provider(self.ns, {
    on_win = function(_, maybe_win)
      if win ~= maybe_win then
        return false
      end
      if not nvim.win_is_valid(win) then
        return false
      end
      if not nvim.get_option_value("cursorline", { win = win }) then
        return false
      end
    ...
    on_line = function(_, _, bufnr, line_number)
      if pos and line_number ~= pos.row then
        return
      end
