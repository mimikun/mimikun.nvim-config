...
function win:get_win()
  if self.id ~= nil and not nvim.win_is_valid(self.id) then
    self.id = nil
  end
  return self.id
end
...
function win:is_open()
  return self.id ~= nil and nvim.win_is_valid(self.id)
end

function win:open()
  -- window already exists
  if self.id ~= nil and nvim.win_is_valid(self.id) then
    return
  end
  ...
  if self.scrollbar then
    self.scrollbar:update(self.id)
  end
  self:redraw_if_needed()
end

function win:set_option_value(option, value)
  if not self.id or not nvim.win_is_valid(self.id) then
    return
  end
...
function win:get_content_width()
  if not self:is_open() or not self.buf then
    return 0
  end
  local max_width = 0
...
function win:get_vertical_direction_and_height(direction_priority, max_height)
  if type(direction_priority) == "function" then
  ...
  if height <= border_size.vertical then
    return
  end
...
function win:get_direction_with_window_constraints(anchor_win, direction_priority, desired_min_size)
  ...
  if width <= border_size.horizontal then
    return
  end
...
function win:redraw_if_needed()
  if self.redraw_queued or nvim.get_mode().mode ~= "c" or not self:get_win() then
    return
  end
