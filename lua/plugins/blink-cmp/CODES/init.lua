if vim.fn.has("nvim-0.12") == 0 then
  error("blink.cmp v2 requires nvim 0.12+, consider pinning to v1")
end

local success, mod = pcall(require, "blink.lib")
if not success then
  error('blink.cmp v2 requires "saghen/blink.lib" installed via your package manager: ' .. mod)
end
---@module "blink.lib"
local lib = mod
...
function cmp.is_enabled()
  ...
  if mode == "c" or vim.fn.getcmdwintype() ~= "" then
    return config.cmdline.enabled
  end
  if mode == "t" then
    return config.term.enabled
  end
  ...
  if user_enabled == "force" then
    return true
  end

  -- Buffer explicitly set completion to true, always enable
  if user_enabled and vim.b.completion == true then
    return true
  end
...
function cmp.get_library_name()
  return native.library_name
end
...
function cmp.download(opts)
  return native:download(function(git_tag, platform)
...
function cmp.is_active()
  return require("blink.cmp.completion.list").context ~= nil
end

--- Checks if the completion menu or ghost text is visible
--- @return boolean
function cmp.is_visible()
  return cmp.is_menu_visible() or cmp.is_ghost_text_visible()
end

--- Checks if the completion menu is visible
--- @return boolean
function cmp.is_menu_visible()
  return require("blink.cmp.completion.windows.menu").win:is_open()
end

--- Checks if the ghost text is visible
--- @return boolean
function cmp.is_ghost_text_visible()
  return require("blink.cmp.completion.windows.ghost_text").is_open()
end

--- Checks if the documentation window is visible
--- @return boolean
function cmp.is_documentation_visible()
  return require("blink.cmp.completion.windows.documentation").win:is_open()
end
...
function cmp.show(opts)
  ...
  if require("blink.cmp.completion.windows.menu").win:is_open() then
    ...
    if ctx and vim.deep_equal(ctx.providers, opts.providers) then
      return false
    end
  end
  ...
  if opts.callback then
    vim.api.nvim_create_autocmd("User", {
      ...
      callback = function(event)
        if context ~= nil and event.data.context.id == context.id then
          opts.callback()
        end
      end,
      once = true,
...
function cmp.accept(opts)
  opts = opts or {}
  if not cmp.is_visible() and not opts.force then
    return false
  end
...
function cmp.get_context()
  return require("blink.cmp.completion.list").context
end

--- Gets the currently selected completion item
--- @return blink.cmp.CompletionItem?
function cmp.get_selected_item()
  return require("blink.cmp.completion.list").get_selected_item()
end

--- Gets the currently selected completion item index
--- @return integer?
function cmp.get_selected_item_idx()
  return require("blink.cmp.completion.list").selected_item_idx
end

--- Gets the sorted list of completion items
--- @return blink.cmp.CompletionItem[]
function cmp.get_items()
...
function cmp.hide_documentation()
  local documentation = require("blink.cmp.completion.windows.documentation")
  if not documentation.win:is_open() then
    return false
  end
...
--- @return boolean
function cmp.is_signature_visible()
  return require("blink.cmp.signature.window").win:is_open()
end

--- Show the signature help window
--- @return boolean
function cmp.show_signature()
  if not config.signature.enabled or cmp.is_signature_visible() then
    return false
  end
...
function cmp.hide_signature()
  if not config.signature.enabled or not cmp.is_signature_visible() then
    return false
  end
...
--- @return boolean
function cmp.snippet_active(filter)
  return config.snippets.active(filter)
end
...
function cmp.resubscribe()
  require("blink.cmp.completion.trigger").resubscribe()
end

--- Tells the sources to reload a specific provider or all providers (when nil)
--- @param provider? string
function cmp.reload(provider)
  require("blink.cmp.sources.lib").reload(provider)
end
