...
function fuzzy.set_provider_items(provider_id, items)
  fuzzy.provider_items[provider_id] = items
end
...
function fuzzy.fuzzy_matched_indices(line, cursor_col, haystack, match_suffix)
  ...
  return vim.tbl_map(function(text)
    return match_indices(keyword, text)
  end, haystack)
end

function fuzzy.get_keyword_range(line, col, match_suffix)
  return get_keyword_range(line, col, match_suffix)
end
