...
function fuzzy.access(item)
  if fuzzy.implementation_type ~= "rust" or not config.fuzzy.frecency.enabled then
    return
  end
...
---@param lines string
function fuzzy.get_words(lines)
  return fuzzy.implementation.get_words(lines)
end
...
function fuzzy.fuzzy(line, cursor_col, haystacks_by_provider, range)
  if config.fuzzy.frecency.enabled then
    fuzzy.init_db()
  end
