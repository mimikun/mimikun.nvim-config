...
function apply.has_insert_command(commands)
  for _, command in ipairs(commands) do
    if not vim.tbl_contains(snippet_commands, command) and command ~= "fallback" then
      return true
    end
  end
  return false
...
function apply.has_snippet_commands(commands)
  for _, command in ipairs(commands) do
    if vim.tbl_contains(snippet_commands, command) or type(command) == "function" then
      return true
    end
  end
  return false
...
--- @param keys_to_commands blink.cmp.KeymapList
function apply.keymaps(mode, keys_to_commands)
  keymaps_per_mode[mode](keys_to_commands)
end
