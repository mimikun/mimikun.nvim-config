...
local function single_key(key, mode)
  return { { key = key, mode = mode or "n" } }
end
...
function fallback.wrap(mode, key)
  ...
  return function(mappings_only)
    ...
    if normalized_raw == "\27" then
      ...
      if #pending > 0 then
        return single_key(normalized_raw .. table.concat(pending))
      end
    end

    local mapping = buffer_index[normalized_key] or global_index[normalized_key]
    if mapping ~= nil then
      return fallback.run_non_blink_keymap(mapping, key)
    end
    if not mappings_only then
      return single_key(key)
    end
...
function fallback.run_non_blink_keymap(mapping, default_key)
  ...
  -- Empty RHS: fallback
  if not mapping.rhs or mapping.rhs == "" then
    return single_key(default_key)
  end
  ...
  if mapping.expr == 1 then
    ...
    if ok and type(expr_key) == "string" then
      return single_key(expr_key, mapping.noremap ~= 0 and "n" or "m")
    end
    return single_key(default_key)
  end
  ...
  if mapping.noremap == 0 then
    return single_key(mapping.rhs, "m")
  end
