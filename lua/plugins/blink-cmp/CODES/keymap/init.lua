...
local function repair_mappings(ctx, expected_mappings)
  ...
  for _, map in ipairs(nvim.buf_get_keymap(ctx.bufnr, ctx.vim_mode)) do
    if utils.is_blink_keymap(map) then
      existing_mappings[utils.normalize_lhs(assert(map.lhs))] = true
    end
  end

  local missing_mappings = {}
  for lhs, commands in pairs(expected_mappings) do
    if not existing_mappings[utils.normalize_lhs(lhs)] then
      missing_mappings[lhs] = commands
    end
  end

  if next(missing_mappings) then
    apply.keymaps(ctx.blink_mode, missing_mappings)
  end
end
...
function keymap.ensure_mappings()
  ...
  if expected_mappings ~= nil then
    return repair_mappings(ctx, expected_mappings)
  end
...
function keymap.get_mappings(keymap_config, mode)
  ...
  for key, commands in pairs(keymaps) do
    if commands == false or #commands == 0 then
      keymaps[key] = nil
    end
  end
