...
function sources.get_enabled_provider_ids(mode)
  ...
  if mode ~= "default" then
    ...
    if type(providers) == "function" then
      providers = providers()
    end
  ...
  if type(default_providers) == "function" then
  ...
  for _, filetype in pairs(vim.split(vim.bo.filetype, ".", { plain = true, trimempty = true })) do
    ...
    if config.sources.per_filetype[filetype] ~= nil then
      ...
      if type(filetype_providers) == "function" then
        filetype_providers = filetype_providers()
      end

      vim.list_extend(providers, filetype_providers)
      if filetype_providers.inherit_defaults then
  ...
  if not user_defined_providers then
    vim.list_extend(providers, default_providers)
  end
...
function sources.add_filetype_provider_id(filetype, provider_id)
  if sources.per_filetype_provider_ids[filetype] == nil then
    sources.per_filetype_provider_ids[filetype] = {}
  end
...
function sources.emit_completions(context, _items_by_provider)
  ...
  for id, items in pairs(_items_by_provider) do
    if sources.providers[id]:should_show_items(context, items) then
      items_by_provider[id] = items
    end
  end
...
function sources.request_completions(context)
  ...
  -- send cached completions if they exist to immediately trigger updates
    ...
    if cached_completions then
      sources.emit_completions(context, cached_completions)
    end
  end
...
function sources.apply_max_items_for_completions(context, items)
  ...
  if vim.tbl_count(max_items_for_sources) == 0 then
    return items
  end
...
function sources.resolve(context, item)
  ...
  if item_source == nil then
    return task.new(function(resolve)
      resolve(item)
    end) --[[@as blink.lib.Task<blink.cmp.CompletionItem>]]
  end

  return item_source:resolve(context, item):catch(function(err)
...
function sources.execute(context, item, default_implementation)
  ...
  return item_source:execute(context, item, default_implementation):catch(function(err)
    vim.print("failed to execute item with error: " .. err)
  end) --[[@as blink.lib.Task<blink.cmp.CompletionItem>]]
end
...
function sources.get_signature_help(context)
  ...
  sources.current_signature_help = task.all(tasks):map(function(signature_helps)
    return vim.tbl_filter(function(signature_help)
      return signature_help ~= nil
    end, signature_helps)
...
function sources.reload(provider)
  ...
  if provider then
    ...
    if sources.providers[provider] ~= nil then
      sources.providers[provider]:reload()
    end
    return
  end
