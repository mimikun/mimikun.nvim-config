...
function source.new(id, config)
  ...
  if config.name == nil then
    config.name = id:sub(1, 1):upper() .. id:sub(2)
  end
...
function source:enabled()
  ...
  -- source defined
  if self.module.enabled == nil then
    return true
  end
...
function source:get_trigger_characters()
  if self.module.get_trigger_characters == nil then
    return {}
  end
...
function source:should_show_items(context, items)
  ...
  if provider_min_keyword_length > 0 then
    min_keyword_length = provider_min_keyword_length
  end

  local current_keyword_length = context.bounds.length
  if current_keyword_length < min_keyword_length then
    return false
  end

  -- check if the source wants to show items
  if self.module.should_show_items ~= nil and not self.module:should_show_items(context, items) then
    return false
  end

  -- check if the user wants to show items
  if self.config.should_show_items ~= nil and not self.config.should_show_items(context, items) then
    return false
  end
...
function source:transform_items(context, items)
  if self.config.transform_items ~= nil then
    items = self.config.transform_items(context, items)
  end
...
function source:resolve(context, item)
  ...
  if cached_task == nil or cached_task.status == task.STATUS.CANCELLED then
    self.resolve_cache[item] = task.new(function(resolve)
      if self.module.resolve == nil then
        return resolve(item)
      end
...
function source:execute(context, item, default_implementation)
  ...
  return task.new(function(resolve)
    return self.module:execute(context, item, resolve, default_implementation)
...
function source:reload()
  if self.module.reload == nil then
    return
  end
  self.module:reload()
end
