...
function list.new(provider, context, on_items, opts)
  ...
  if self.provider.module.get_completions == nil then
    ...
    self.cancel_completions = self.provider.module:get_completions(self.context, function(response)
      self:append(response or default_response)
    end)
  end

  -- if async, immediately send the default response/initial items
  local is_async = self.provider.config.async(self.context)
  if is_async and not self.has_completed then
    self:emit()
  end
...
function list:append(response)
  if self.has_completed and #response.items == 0 then
    return
  end
...
function list:destroy()
  if self.cancel_completions ~= nil then
...
function list:is_valid_for_context(new_context)
  if self.context.id ~= new_context.id then
    return false
  end
