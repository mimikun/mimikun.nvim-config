...
function tree.new(context)
  ...
  for _, provider_id in ipairs(context.providers) do
    ...
    if provider:enabled() then
      table.insert(sources, provider)
    end
  end
  local source_ids = vim.tbl_map(function(source)
    return source.id
  end, sources)

  -- create a node for each source
  local nodes = vim.tbl_map(function(source)
    return { id = source.id, source = source, dependencies = {}, dependents = {} }
  end, sources)
...
function tree:get_completions(context, on_items_by_provider)
  ...
  local function get_completions_for_node(node)
    ...
    for _, dependency in ipairs(node.dependencies) do
      if not nodes_falling_back[dependency.id] then
        return task.resolve()
      end
    end
    ...
    return task.new(function(resolve, reject)
      return node.source:get_completions(context, function(items, is_cached)
        ...
        local tasks = vim.tbl_map(function(dependent)
          return get_completions_for_node(dependent)
        end, node.dependents)
  ...
  local tasks = vim.tbl_map(function(node)
    return get_completions_for_node(node)
  end, self:get_top_level_nodes())
  return task
    .all(tasks)
    :map(function()
...
function tree:get_top_level_nodes()
  ...
  for _, node in ipairs(self.nodes) do
    if #node.dependencies == 0 then
      table.insert(top_level_nodes, node)
    end
  end
...
function tree.detect_cycle(node, visited, path)
  ...
  for _, dependent in ipairs(node.dependents) do
    if tree.detect_cycle(dependent, visited, path) then
      return true
    end
  end
