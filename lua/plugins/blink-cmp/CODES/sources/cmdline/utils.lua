...
function utils.in_ex_search_commands()
  if not utils.is_command_line({ ":" }) then
    return false
  end
...
function utils.get_completion_type(context)
  ...
  if completion_type == "" then
    ...
    if cmd then
      ...
      if find_cmds[cmd] and vim.opt.findfunc ~= "" then
        return "findfunc"
      end
    end
  end
...
function utils.is_path_completion(completion_type, line)
  if vim.tbl_contains(constants.completion_types.path, completion_type) then
    return true
  end
...
--- @param line string
--- @return boolean
function utils.contains_wildcard(line)
...
function utils.smart_split(line, is_path_completion)
  ...
  if is_path_completion then
    ...
    for i = 2, #tokens do
      ...
      if arg and not arg:match("^['\"]") and not arg:find("\\ ") and arg:find(" ") then
        arg = fnameescape(arg)
      end
...
function utils.get_range_prefix(str)
  ...
  for _, pat in ipairs(range_patterns) do
    ...
    if m and (not best or #m > #best) then
      best = m
    end
  end
  return best
...
function utils.get_completions(pattern, type, completion_type)
  ...
  if completion_type == "shellcmd" then
    ...
    if vim.fn.has("win32") == 1 then
      ...
      filter_fn = function(part)
        return not part:lower():match("^[a-z]:[/\\]windows[/\\]system32[/\\]?$")
      end
    elseif vim.fn.has("wsl") == 1 then
      separator = ":"
      -- Remove all Windows filesystem mounts on WSL
      filter_fn = function(part)
        return not part:lower():match("^/mnt/[a-z]/")
      end
    end
