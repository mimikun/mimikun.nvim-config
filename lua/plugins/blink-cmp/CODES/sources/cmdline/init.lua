...
function cmdline.new()
  return setmetatable({}, { __index = cmdline })
end

function cmdline:enabled()
  return vim.bo.ft == "vim" or utils.is_command_line({ ":", "@" })
end

function cmdline:get_trigger_characters()
  return { " ", ".", "#", "&", "-", "=", "/", ":", "!", "%", "~" }
end
    ...
    :map(function(completions)
      ...
      for _, completion in ipairs(completions) do
        ...
        elseif completion_type == "option" then
          ...
          if option_info then
            label_details = { description = option_info.shortname }
          end
        ...
        elseif completion_type == "expression" then
          if not vim.startswith(completion, current_arg_prefix) then
            new_text = current_arg_prefix .. completion
          end
