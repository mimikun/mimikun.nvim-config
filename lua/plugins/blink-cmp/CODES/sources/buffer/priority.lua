...
function priority.focused()
  return function(bufnr)
    return bufnr == nvim.win_get_buf(0) and 0 or 1
  end
end
...
function priority.visible()
  ...
  return function(bufnr)
    return visible[bufnr] and 0 or 1
  end
end
...
function priority.recency()
  ...
  return function(bufnr)
    return time - recency.accessed_at(bufnr)
  end
end

function priority.largest(buf_sizes)
  return function(bufnr)
    return -buf_sizes[bufnr]
  end
end
...
function priority.comparator(order, buf_sizes)
  ...
  for _, key in ipairs(order) do
    if priority[key] ~= nil then
      table.insert(value_fns, priority[key](buf_sizes))
    end
  end
