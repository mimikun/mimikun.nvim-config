...
function cache.get(context, client)
  ...
  if context.id ~= entry.context.id then
    return
  end
  if entry.response.is_incomplete_forward and entry.context.pos.col ~= context.pos.col then
    return
  end
  if not entry.response.is_incomplete_forward and entry.context.pos.col > context.pos.col then
    return
  end
