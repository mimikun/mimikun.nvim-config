    ...
    if node.type == types.choiceNode then
      node.node_callbacks = {
        [events.enter] = function(n)
          ...
          local index = lib.list.find_idx(n.choices, function(choice)
            return choice == n.active_choice
          end)
...
function source:get_completions(ctx, callback)
  ...
  if self.opts.use_show_condition then
    ...
    items = vim.tbl_filter(function(item)
      return item.data.show_condition(line_to_cursor)
    end, items)
  end

  -- Compute insertText respecting indent
  -- TODO: Ideally, handle this in resolve()
  local indent = ctx.line:match("^%s*") or ""
  local prefix = "\n" .. indent
...
function source:execute(ctx, item)
  ...
  if snip.regTrig then
    ...
    if docTrig ~= nil then
      add_luasnip_callback(snip, events.pre_expand, function(s)
        regex_callback(s, docTrig)
      end)
    end
  else
    add_luasnip_callback(snip, events.pre_expand, function(s)
      choice_callback(s, events)
    end)
  end
