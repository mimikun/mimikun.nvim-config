...
function registry.new(config)
  ...
  self.config.search_paths = vim.tbl_map(function(path)
    return vim.fs.normalize(path)
  end, self.config.search_paths)

  if self.config.friendly_snippets then
    for _, path in ipairs(vim.api.nvim_list_runtime_paths()) do
      if string.match(path, "friendly.snippets") then
        table.insert(self.config.search_paths, path)
      end
    end
  end
  ...
  if self.config.filter_snippets then
    ...
    for ft, files in pairs(self.registry) do
      ...
      for _, file in ipairs(files) do
        if self.config.filter_snippets(ft, file) then
          table.insert(filtered_registry[ft], file)
        end
      end
    end
