...
function list.get_selected_item()
  return list.selected_item_idx and list.items[list.selected_item_idx]
end
...
function list.get_item_idx_in_list(item)
  ...
  return lib.list.find_idx(list.items, function(i)
    return i.label == item.label and item.source_id == i.source_id
  end)
end

function list.select(idx, opts)
  if idx ~= nil and idx < 0 then
    idx = math.max(#list.items + idx + 1, 1)
  end
  opts = opts or {}
  ...
  if auto_insert == nil then
    auto_insert = list.get_selection_mode(list.context).auto_insert
  end

  require("blink.cmp.completion.trigger").suppress_events_for_callback(function()
    list.undo_preview()
    if auto_insert and item ~= nil then
...
function list.select_next(opts)
  ...
  if not list.can_select(opts) then
    return false
  end
  ...
  if opts and opts.jump_by and list.jump_by(1, opts) then
    return true
  end
...
function list.can_select(opts)
  ...
  if not cmp.is_menu_visible() and (not on_ghost_text or not cmp.is_ghost_text_visible()) then
    return false
  end

  if not opts or not opts.jump_by then
    return true
  end

  if list.selected_item_idx then
    local current = list.items[list.selected_item_idx][opts.jump_by]
    if not vim.tbl_contains({ "string", "number", "boolean" }, type(current)) then
      return false
    end
  end
...
function list.jump_by(dir, opts)
  ...
  if not list.items or #list.items == 0 or not list.selected_item_idx then
    return false
  end
  ...
  if dir == 1 then
    if try_jump(list.selected_item_idx + 1, #list.items, 1) then
      return true
    end
  ...
  elseif dir == -1 then
    if try_jump(list.selected_item_idx - 1, 1, -1) then
      return true
    end
