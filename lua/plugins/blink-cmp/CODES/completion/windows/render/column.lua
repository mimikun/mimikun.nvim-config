...
function column:render(context, ctxs)
  ...
  for component_idx, component in ipairs(self.components) do
    if component.width and component.width.fill then
      table.insert(fill_idxs, component_idx)
    end
  end
  if #fill_idxs == 0 then
    fill_idxs = { #self.components }
  end
  ...
  for _, line in ipairs(lines) do
    ...
    for _, component_text in ipairs(line) do
      if #component_text > 0 then
        line_width = line_width + vim.api.nvim_strwidth(component_text) + self.gap
      end
    end
...
function column:get_line_text(line_idx)
  ...
  for _, component in ipairs(line) do
    if #component > 0 then
      text = text .. component .. string.rep(" ", self.gap)
    end
  end
