...
function renderer.new(draw)
  ...
  nvim.set_decoration_provider(ns, {
    on_win = function(_, _, win_bufnr)
      return self.bufnr == win_bufnr
    end,
...
function renderer:get_columns(context, draw)
  ...
  return vim.tbl_map(function(col)
    return render_column.new(col.component_names, col.components, col.gap, col.overlap_components)
  end, columns_definitions)
end
...
function renderer:draw(context, bufnr, items)
  ...
  for idx, _ in ipairs(draw_contexts) do
    local line = ""
    if self.padding[1] > 0 then
      line = string.rep(" ", self.padding[1])
    end

    for _, column in ipairs(columns) do
      local text = column:get_line_text(idx)
      if #text > 0 then
        line = line .. text .. string.rep(" ", self.gap)
      end
    end
    line = line:sub(1, -self.gap - 1)

    if self.padding[2] > 0 then
      line = line .. string.rep(" ", self.padding[2])
    end
...
function renderer:get_component_column_location(columns, component_name)
  for column_idx, column in ipairs(columns) do
    for component_idx, other_component_name in ipairs(column.component_names) do
      if other_component_name == component_name then
        return { column_idx, component_idx }
      end
    end
  end
...
function renderer:get_alignment_start_col()
  ...
  if component_name == nil or component_name == "none" or component_name == "cursor" then
    return 0
  end
