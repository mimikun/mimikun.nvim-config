...
function text_lib.apply_component_width(context, text, component)
  ...
  if width.fixed ~= nil then
    return text_lib.set_width(text, width.fixed, component)
  end
  if width.min ~= nil then
    text = text_lib.pad(text, width.min)
  end
  local max_width = width.max
  if type(width.max) == "function" then
    max_width = width.max(context)
  end
  if max_width ~= nil then
    text = text_lib.truncate(text, max_width, component.ellipsis)
  end
  return text
end
...
function text_lib.truncate(text, target_width, ellipsis)
  ...
  if ellipsis ~= false and config.appearance.nerd_font_variant == "normal" then
