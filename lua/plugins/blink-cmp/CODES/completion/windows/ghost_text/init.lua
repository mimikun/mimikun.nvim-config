...
function ghost_text.is_open()
  return ghost_text.extmark_id ~= nil
end
...
function ghost_text.show_preview(context, items, selection_idx)
  ...
  if not ghost_text_buf or not nvim.buf_is_valid(ghost_text_buf) then
    return
  end
...
function ghost_text.draw_preview()
  ...
  if not ghost_text.selected_item or not ghost_text.context then
    return
  end
  local buf = utils.get_buf()
  if not buf or not nvim.buf_is_valid(buf) then
    return
  end
