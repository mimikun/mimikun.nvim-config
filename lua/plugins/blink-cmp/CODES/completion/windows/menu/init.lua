...
local menu = {
  ...
  auto_show = {
    enabled = type(config.auto_show) == "function" and config.auto_show or function()
      return config.auto_show
    end,
    delay_ms = type(config.auto_show_delay_ms) == "function" and config.auto_show_delay_ms or function()
      return config.auto_show_delay_ms
    end,
    timer = lib.timer.new(),
    timer_key = "",
  },
...
function menu.open_with_items(context, items)
  ...
  if not menu.renderer then
    menu.renderer = require("blink.cmp.completion.windows.render").new(config.draw)
  end
...
function menu.open()
  ...
  if menu.selected_item_idx ~= nil then
    nvim.win_set_cursor(menu_winnr, { menu.selected_item_idx, 0 })
  end
...
function menu.set_selected_item_idx(idx)
  ...
  if type(config.direction_priority) == "function" then
    menu.update_position()
  end
end
...
function menu.queue_auto_show(context, items)
  ...
  if not is_auto_show_enabled then
    return
  end
  ...
  if menu.auto_show.timer:is_active() and menu.auto_show.timer_key == timer_key then
    return
  end
...
function menu.force_auto_show()
  menu.auto_show.enabled = function()
    return true
  end
  menu.auto_show.delay_ms = function()
    return 0
  end
end
...
function menu.update_position()
  ...
  -- otherwise, we use the cursor position
    ...
    if config.draw.align_to == "cursor" then
      col = 0
    end
    if off ~= 0 then
      col = col + off
