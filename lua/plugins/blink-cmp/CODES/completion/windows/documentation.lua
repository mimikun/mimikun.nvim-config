...
menu.position_update_emitter:on(function()
...
function docs.update_position()
  ...
  if signature.win and signature.win:is_open() then
    direction_priority = vim.tbl_filter(function(dir)
      return dir ~= (menu_win_is_up and "s" or "n")
    end, direction_priority)
  end
  ...
  local function clamped_to_menu()
    ...
    if overflow_menu then
      return menu_win_height - menu_border_size.top - height
    end
