...
function utils.get_for_filetype(filetype, item)
  ...
  if type(per_filetype) == "function" then
    return per_filetype(item) or default
  end
...
function utils.should_run_resolution(ctx, filetype, resolution_method)
  ...
  if not config[resolution_method .. "_resolution"].enabled then
    return false
  end
  ---@type string[]
  local resolution_blocked_filetypes = config[resolution_method .. "_resolution"].blocked_filetypes
  if vim.tbl_contains(resolution_blocked_filetypes, filetype) then
    return false
  end
  ...
  if exceptions.by_filetype[filetype] ~= nil then
    if not exceptions.by_filetype[filetype](ctx) then
      return false
    end
  end
  ...
  if vim.tbl_contains(config.force_allow_filetypes, filetype) then
    return true
  end
