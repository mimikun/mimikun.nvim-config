...
function context:within_query_bounds(pos, include_start_bound)
  ...
  if pos.row + 1 ~= bounds.line_number then
    return false
  end
...
function context.get_pos()
  ...
  if context.get_mode() == "cmdline" then
    return utils.get_vim_pos(bufnr, 0, vim.fn.getcmdpos() - 1)
  end
  return utils.get_vim_pos_cursor(bufnr)
end

function context.get_cursor()
  return utils.vim_pos_to_cursor(context.get_pos())
end
