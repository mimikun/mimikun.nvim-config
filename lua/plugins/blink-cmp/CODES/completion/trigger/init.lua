...
function trigger.show(opts)
  if vim.fn.pumvisible() == 1 or not require("blink.cmp").is_enabled() then
    return trigger.hide()
  end
  ...
  if not opts.force and ctx ~= nil and ctx.mode == mode and ctx.pos == pos then
    return
  end

  -- update the context id to indicate a new context, and not an update to an existing context
  if not ctx or opts.providers ~= nil then
  ...
  if initial_trigger_kind == "prefetch" then
    initial_trigger_kind = opts.trigger_kind
  end
  ...
  if ctx ~= nil and ctx.id ~= trigger.current_context_id then
