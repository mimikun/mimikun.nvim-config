...
local function apply_item(ctx, item)
  ...
  all_text_edits = vim.tbl_map(function(text_edit)
    return text_edits_lib.to_utf_8(text_edit, text_edits_lib.offset_encoding_from_item(item))
  end, all_text_edits)
...
local function accept(ctx, item, callback)
  ...
  sources
    ...
    :map(function(resolved_item)
      ...
      return sources.execute(ctx, resolved_item, function(alternate_ctx, alternate_item)
        apply_item(alternate_ctx or ctx, alternate_item or resolved_item)
      end)
    end)
    :map(function()
      require("blink.cmp.completion.trigger").show_if_on_trigger_character({ is_accept = true })
    ...
    :catch(function(err)
      logger:notify(vim.log.levels.ERROR, tostring(err))
    end)
end
