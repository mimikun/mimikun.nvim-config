...
string.gsub("'\"=$()[]<>{} \t\n\r", ".", function(char)
  PAIRS_AND_INVALID_CHARS[string.byte(char)] = true
end)
...
local ALPHANUMERIC = {}
string.gsub("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", ".", function(char)
...
local function get_prefix_before_brackets_and_quotes(text)
  ...
  for i = 1, #text do
    ...
    elseif not has_alphanumeric or #closing_pairs_stack ~= 0 then
      ...
      if has_alphanumeric and #closing_pairs_stack == 0 then
        break
      end
    else
      break
