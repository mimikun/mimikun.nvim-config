...
function completion.setup()
  ...
  sources.completions_emitter:on(function(event)
    ...
    vim.schedule(function()
      ...
      if trigger.context == nil or event.context.id ~= trigger.context.id then
        return
      end
      -- don't show the list if prefetching results
      if trigger.context.trigger.kind == "prefetch" then
        return
      end
      ...
      if event.context.trigger.character ~= nil then
        ...
        if not triggering_source_returned_items then
          return list.hide()
        end
      end
  ...
  if config.completion.menu.enabled then
    local menu = function()
      return require("blink.cmp.completion.windows.menu")
    end
    ...
    trigger.show_emitter:on(function(event)
      ...
      loading_timer:start(
        ...
        vim.schedule_wrap(function()
          menu().open_loading(event.context)
        end)
      )
    end)
  ...
  -- setup ghost text
  local ghost_text = function()
    return require("blink.cmp.completion.windows.ghost_text")
  end
  ...
  list.show_emitter:on(function(event)
    ghost_text().show_preview(event.context, event.items, 1)
  end)
  list.select_emitter:on(function(event)
    ghost_text().show_preview(event.context, event.items, event.idx)
  end)
