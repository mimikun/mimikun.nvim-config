...
function M.update_results_sync()
  ...
  if S.next_search_force_combo_boost then
    min_combo_override = 0
  end
  ...
  if S.suggestion_items and #S.suggestion_items > 0 then
    S.filtered_items = S.suggestion_items
  end
...
function M.load_next_page()
  ...
  if S.mode == "grep" then
    if S.pagination.grep_next_file_offset == 0 then
      return false
    end
    return M.load_page_at_index(current_page + 1, function()
      S.cursor = 1
    end)
  end
  ...
  local max_page_index = math.max(0, math.ceil(total / page_size) - 1)
  if current_page >= max_page_index then
    return false
  end
...
function M.load_previous_page()
  ...
  return M.load_page_at_index(S.pagination.page_index - 1, function(result_count)
    S.cursor = result_count
  end)
end
...
function M.on_input_change()
  ...
  if #lines > 1 then
    ...
    if full_line:sub(1, prompt_len) == S.config.prompt then
...
function M.cycle_grep_modes()
  if not P.state.active or S.mode ~= "grep" then
    return
  end
...
function M.recall_query_from_history()
  ...
  if S.mode ~= "grep" then
    S.next_search_force_combo_boost = true
  end
