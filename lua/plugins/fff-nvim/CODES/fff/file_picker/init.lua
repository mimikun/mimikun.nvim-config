...
function M.search_files_paginated(query, current_file, max_threads, min_combo_count_override, page_index, page_size)
  ...
  if min_combo_count == nil then
    min_combo_count = config.history and config.history.min_combo_count or 3
  end
...
function M.get_search_metadata()
  if not M.state.last_search_result then
    return { total_matched = 0, total_files = 0 }
  end
  return {
...
function M.get_file_score(index)
  if not M.state.last_search_result or not M.state.last_search_result.scores then
    return nil
  end
...
function M.track_access(file_path)
  ...
  if not ok then
    vim.notify("Failed to record file access: " .. tostring(result), vim.log.levels.WARN)
  end
end
...
function M.get_scan_progress()
  if not M.state.initialized then
    return { total_files = 0, scanned_files_count = 0, is_scanning = false }
  end
