...
local function show_preview_unavailable(bufnr)
  set_buffer_lines(bufnr, { "No preview available" })
end
...
local function cleanup_file_operation()
  if M.state.file_operation then
    if M.state.file_operation.fd then
      pcall(vim.uv.fs_close, M.state.file_operation.fd)
    end
...
local function load_forward_chunk_async(target_size, callback)
  ...
  vim.uv.fs_read(M.state.file_operation.fd, chunk_size, M.state.file_operation.position, function(err, data)
    vim.schedule(function()
      ...
      if M.state.file_operation then
        M.state.file_operation.position = M.state.file_operation.position + #data
      end
...
ensure_content_loaded_async = function(target_line)
  ...
  if not M.state.has_more_content or M.state.is_loading then
    return
  end
...
function M.is_big_file(file_path)
  ...
  if stat and stat.size > M.config.max_size then
    return true
  end
...
function M.preview_file(file_path, bufnr)
  ...
  if existing_bufnr then
    ...
    if success then
      ...
      vim.schedule(function()
        if M.state.preview_generation == gen then
          M.apply_location_highlighting(bufnr)
        end
      end)
...
local function ensure_hex_ns()
  if not hex_ns then
    hex_ns = vim.api.nvim_create_namespace("fff_hex_preview")
  end
  return hex_ns
end
...
local function apply_hex_highlights(bufnr, highlights, line_offset)
  if not highlights or not vim.api.nvim_buf_is_valid(bufnr) then
    return
  end
...
local function load_more_hex_content()
  ...
  if not M.state.has_more_content or not M.state.current_file then
    return
  end
...
function M.preview_binary_file(file_path, bufnr)
  ...
  if vim.fn.executable("file") == 1 then
    ...
    if vim.v.shell_error == 0 and output then
      ...
      if info and info.size_formatted then
        table.insert(lines, "Size: " .. info.size_formatted)
      end
      table.insert(lines, "")
    end
  end
...
function M.get_file_config(file_path)
  if not M.config or not M.config.filetypes then
    return {}
  end
...
function M.set_preview_window(winid)
  M.state.winid = winid
end
...
local function resolve_absolute_path(rel)
  ...
  if base and base ~= "" then
    return (base:sub(-1) == "/") and (base .. rel) or (base .. "/" .. rel)
  end
...
local function build_file_info_data(file, file_metadata, file_index)
  ...
  if is_grep then
    sections = vim.tbl_extend("force", sections, { score_breakdown = false })
  end
  ...
  local score = nil
  if not is_grep and file_index then
    score = file_picker.get_file_score(file_index)
  end
...
local function resize_file_info(file_info_win, target_h, preview_win)
  if not (file_info_win and vim.api.nvim_win_is_valid(file_info_win)) then
    return
  end
...
function M.update_file_info_buffer(file, bufnr, file_index, preview_win)
  ...
  if result.height > 0 then
    resize_file_info(target_win, result.height, preview_win)
  end
...
function M.clear_preview_visual_state(bufnr)
  ...
  if M.state.location_namespace then
