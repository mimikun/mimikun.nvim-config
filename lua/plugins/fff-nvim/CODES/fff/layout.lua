...
local function get_border_chars(config)
  ...
  if type(border) == "table" then
    if #border == 2 and type(border[1]) == "table" and type(border[2]) == "table" then
      return border[1], border[2]
    end
  end
  if BORDER_PRESETS[border] then
    return BORDER_PRESETS[border], T_JUNCTION_PRESETS[border]
  end
...
local function resolve_corner(corner, chars, j, which, n)
  ...
  if up and down and (left or right) then
    return left and j[2] or j[1]
  end
  if left and right and (up or down) then
    return up and j[4] or j[3]
  end
  return corner
end
...
local function resolve_prompt_position(config)
  if config and config.layout and config.layout.prompt_position then
    return utils.resolve_config_value(config.layout.prompt_position, vim.o.columns, vim.o.lines, function(value)
      return utils.is_one_of(value, { "top", "bottom" })
    end, "bottom", "layout.prompt_position")
  end
  return "bottom"
end
...
local function resolve_preview_position(config)
  if config and config.layout and config.layout.preview_position then
    ...
    local position = utils.resolve_config_value(
      ...
      vim.o.lines,
      function(value)
        return utils.is_one_of(value, { "left", "right", "top", "bottom" })
      end,
      "right",
...
local function build_window_configs(layout, config, prompt_position, preview_position)
  ...
  local function tl_extends_up()
    ...
    if input_neighbour_preview_top then
      return true
    end -- preview stack above
    return false
  end
  local function tr_extends_up()
    return tl_extends_up()
  end
  local function bl_extends_down()
    if prompt_position == "top" then
      return true
    end -- list below
    if input_neighbour_preview_bottom then
      return true
    end -- preview stack below
    return false
  end
  local function br_extends_down()
    return bl_extends_down()
  end
...
function M.compute(config, preview_user_enabled)
  ...
  local anchor = utils.resolve_config_value(config.layout.anchor, terminal_width, terminal_height, function(v)
