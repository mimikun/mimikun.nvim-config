...
if not fuzzy then
  error("Failed to load fff.fuzzy module. Ensure the Rust backend is compiled and available.")
end
...
local function setup_global_autocmds(config)
  ...
  if config.frecency.enabled then
    vim.api.nvim_create_autocmd({ "BufEnter" }, {
      ...
      callback = function(args)
        ...
        if not (file_path and file_path ~= "" and not vim.startswith(file_path, "term://")) then
          return
        end
        ...
        vim.uv.fs_stat(file_path, function(err, stat)
          ...
          vim.uv.fs_realpath(file_path, function(rp_err, real_path)
            if rp_err or not real_path then
              return
            end
  ...
  vim.api.nvim_create_autocmd("DirChanged", {
    ...
    callback = function()
      ...
      if not new_cwd or new_cwd == "" then
        return
      end
      ...
      if new_canonical == base_canonical then
        return
      end
...
--- picker (see #772).
M.mark_file_picker_uninitialized = function()
  state.file_picker_initialized = false
end
...
M.ensure_initialized = function()
  ...
  if not state.initialized then
    ...
    if not ok then
      vim.notify("Failed to databases: " .. tostring(result), vim.log.levels.WARN)
    end
