...
function M.resolve(repo_root)
  ...
  if not short_sha then
    return nil, "Failed to determine git SHA"
  end

  local base_version = M.read_base_version(repo_root)
  if not base_version then
    return nil, "Could not read base version from crates/fff-core/Cargo.toml"
  end

  local next_version = bump_patch(base_version)
  if not next_version then
    return nil, "Could not parse base version: " .. base_version
  end
