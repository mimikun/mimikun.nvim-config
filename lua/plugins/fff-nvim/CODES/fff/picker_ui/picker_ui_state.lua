...
function M.get_selected_items()
  ...
  if M.state.mode == "grep" then
    ...
    for relative_path, _ in pairs(M.state.selected_files) do
      ...
      if abs_path then
        table.insert(selected, { relative_path = relative_path })
      end
    end
  end
...
function M.toggle_selection()
  ...
  if not item or not item.relative_path then
    return
  end
  ...
  if M.state.mode == "grep" then
    ...
    if was_selected then
      ...
      M.state.selected_file_order = vim.tbl_filter(function(path)
        return path ~= item.relative_path
      end, M.state.selected_file_order)
    else
...
function M.get_selected_file_entries()
  if not next(M.state.selected_files) then
    return {}
  end
  ...
  local function add(relative_path)
    if not relative_path or seen[relative_path] or not M.state.selected_files[relative_path] then
      return
    end
