...
local function maybe_hide_combo_separator()
  if not (S.combo_initial_cursor and S.combo_visible) then
    return
  end
  local distance_past = S.cursor - S.combo_initial_cursor
  if distance_past == 0 then
    return
  end
  local half_page = math.floor(S.pagination.page_size * 0.5)
  if math.abs(distance_past) <= half_page then
    return
  end
...
function M.wrap_to_first()
  ...
  if S.mode ~= "grep" then
    return P.load_page_at_index(0, function()
      S.cursor = 1
    end)
  end
...
function M.wrap_to_last()
  ...
  if S.mode ~= "grep" then
    ...
    return P.load_page_at_index(max_page_index, function(result_count)
...
function M.move_up()
  ...
  if not P.render_after_cursor_move(old_cursor) then
    return
  end
  P.update_status()
...
function M.scroll_preview_up()
  if not P.state.active or not S.preview_win then
    return
  end
