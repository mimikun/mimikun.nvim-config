...
local function format_file_display(item, max_width)
  ...
  if type(filename) ~= "string" then
    filename = filename and tostring(filename) or ""
  end
  local dir_path = item.directory or ""
  if type(dir_path) ~= "string" then
    dir_path = dir_path and tostring(dir_path) or ""
  end
  ...
  if dir_path == "" or path_max_width == 0 then
    return filename, ""
  end
...
function M.render_list()
  ...
  if ctx.prompt_position == "bottom" then
    M.scroll_to_bottom()
  end
...
local function rerender_cursor_rows(old_cursor, new_cursor)
  if S.suggestion_source then
    return false
  end

  local ctx = S.last_render_ctx
  if not ctx or ctx.items ~= S.filtered_items then
    return false
  end
  if ctx.renderer and not ctx.renderer.supports_cursor_rerender then
    return false
  end
