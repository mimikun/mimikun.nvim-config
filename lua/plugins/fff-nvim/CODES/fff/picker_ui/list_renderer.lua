...
local function generate_item_lines(ctx)
  ...
  if not renderer then
    renderer = require("fff.picker_ui.file_renderer")
  end
...
local function apply_bottom_padding(lines, item_to_lines, ctx, separator_line)
  ...
  if empty_lines_needed > 0 then
    ...
    if separator_line then
      separator_line = separator_line + empty_lines_needed
    end
  end
...
function M.render(ctx, list_buf, list_win, ns_id)
  ...
  if #ctx.items > 0 then
    apply_all_highlights(lines, item_to_lines, ctx, list_buf, ns_id)
  end
