...
function M.update_preview_debounced()
  ...
  timer:start(
    ...
    vim.schedule_wrap(function()
      ...
      if is_current and P.state.active then
        M.update_preview()
      end
    end)
  )
...
function M.update_preview_title(item, location)
  ...
  if is_grep_item and location and location.line then
    suffix = ":" .. tostring(location.line)
  end
...
function M.update_preview()
  ...
  if is_grep_item and item.line_number and item.line_number > 0 then
    ...
    if item.col and item.col > 0 then
      effective_location.col = item.col + 1
    end
    effective_location.grep_query = S.query
    if S.grep_mode == "fuzzy" and item.match_ranges then
  ...
  if S.last_preview_file == item.relative_path and not location_changed then
    return
  end
