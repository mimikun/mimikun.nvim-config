...
local function restore_from_state(state, source_label)
  ...
  if state.base_path then
    require("fff.core").change_indexing_directory(state.base_path)
  end
...
function M.resume()
  ...
  if resume_state.grep then
    return restore_from_state(resume_state.grep, "grep resume")
  end
  if resume_state.files then
    return restore_from_state(resume_state.files, "files resume")
  end
...
function M.resume_find_files(opts)
  ...
  if not resume_state.files then
    return M.open(opts)
  end
...
function M.update_status(progress)
  ...
  if M.state.mode == "grep" then
    ...
    if M.state.grep_regex_fallback_error then
      fallback_label = "invalid regex, using literal"
    end
    ...
    if cache_key == M.state.last_status_info then
      return
    end
  ...
  if status_info == M.state.last_status_info then
    return
  end
...
function M.select(action)
  ...
  if action == "edit" and not is_grep_item then
    selected_file_entries = picker_ui_state.get_selected_file_entries()
  end
  ...
  if #selected_file_entries > 0 and selected_file_entries[1].relative_path ~= item.relative_path then
    location = nil
  end
  ...
  vim.schedule(function()
    if type(on_submit) == "function" then
      ...
      if not ok then
        vim.notify("FFF: on_submit error: " .. tostring(err), vim.log.levels.ERROR)
      end
    else
      ...
      if location then
        location_utils.jump_to_location(location)
      end
    end
...
function M.open(opts)
  ...
  if base_path then
    require("fff.core").change_indexing_directory(base_path)
  end
...
function M.monitor_scan_progress(iteration)
  ...
  if progress.is_scanning then
    ...
    vim.defer_fn(function()
      M.monitor_scan_progress(iteration + 1)
    end, timeout)
  else
    M.update_results()
  end
