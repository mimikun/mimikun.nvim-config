...
local function resolve_winhl(kind)
  ...
  if type(winhl) == "table" then
    return winhl[kind] or default_winhl
  end
  return default_winhl
end
...
local function handle_mouse_click_or_fallback(action, fallback)
  ...
  if P.state.active and pos.winid == S.list_win then
    ...
    vim.schedule(function()
      if not P.state.active then
        return
      end
      if not S.filtered_items[item_idx] then
        return
      end
...
function M.setup_buffers()
  ...
  vim.api.nvim_create_autocmd("Syntax", {
    buffer = S.input_buf,
    callback = function()
      vim.api.nvim_set_option_value("syntax", "", { buf = S.input_buf })
    end,
  })
...
function M.setup_windows()
  ...
  local function is_picker_window(win)
    ...
    if S.preview_win then
      table.insert(picker_windows, S.preview_win)
    end
    if S.file_info_win then
      table.insert(picker_windows, S.file_info_win)
    end

    for _, picker_win in ipairs(picker_windows) do
      if picker_win and vim.api.nvim_win_is_valid(picker_win) and win == picker_win then
        return true
      end
    end
  ...
  vim.api.nvim_create_autocmd("WinLeave", {
    ...
    callback = function()
      ...
      if not is_picker_window(leaving_win) then
        return
      end
      ...
      vim.schedule(function()
        ...
        if not is_picker_window(new_win) then
          P.close()
        end
      end)
    end,
...
function M.setup_keymaps()
  ...
  if keymaps.grep_jump_to_next_file then
    set_keymap({ "i", "n" }, keymaps.grep_jump_to_next_file, function()
      P.grep_jump_to_next_file()
    end, input_opts)
  end
  if keymaps.grep_jump_to_prev_file then
    set_keymap({ "i", "n" }, keymaps.grep_jump_to_prev_file, function()
      P.grep_jump_to_prev_file()
    end, input_opts)
  end
  ...
  set_keymap({ "i", "n" }, keymaps.select_split, function()
    P.select("split")
  end, input_opts)
  set_keymap({ "i", "n" }, keymaps.select_vsplit, function()
    P.select("vsplit")
  end, input_opts)
  set_keymap({ "i", "n" }, keymaps.select_tab, function()
    P.select("tab")
  end, input_opts)
  ...
  set_keymap("n", keymaps.select_split, function()
    P.select("split")
  end, list_opts)
  set_keymap("n", keymaps.select_vsplit, function()
    P.select("vsplit")
  end, list_opts)
  set_keymap("n", keymaps.select_tab, function()
    P.select("tab")
  end, list_opts)
...
function M.focus_input_win()
  ...
  if not S.input_win or not vim.api.nvim_win_is_valid(S.input_win) then
    return
  end
