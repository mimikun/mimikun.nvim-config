...
local function ensure_git_cache()
  if git_text_highlights_cache then
    return
  end
...
function M.get_git_border_char(git_status)
  return M.git_border_chars[git_status] or ""
end
...
function M.get_git_sign_highlight(git_status, is_cursor, cursor_hl)
  if not is_cursor then
    return M.get_git_border_highlight(git_status)
  end
...
function M.setup()
  ...
  local function resolve_hl_attr(name, attr)
    ...
    if linked.link then
      return resolve_hl_attr(linked.link, attr)
    end
    return nil
  end
