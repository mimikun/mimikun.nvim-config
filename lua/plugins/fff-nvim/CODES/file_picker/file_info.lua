...
function Builder.new()
  return setmetatable({ lines = {}, extmarks = {} }, Builder)
end

function Builder:add_line(text)
...
local function pad(text, width)
  ...
  if width <= 1 then
    return text:sub(1, math.max(0, width))
  end
...
local function add_grid_row(b, indent_w, label1_w, value1_w, label2_w, cells, hls_by_cell, value2_max)
  ...
  if cells[1] and cells[1] ~= "" then
    b:add_hl(row, pos, pos + #cells[1], hls_by_cell[1])
  end
  pos = pos + #l1
  if cells[2] and cells[2] ~= "" then
    b:add_hl(row, pos, pos + #cells[2], hls_by_cell[2])
  end
  pos = pos + #v1
  if cells[3] and cells[3] ~= "" then
    b:add_hl(row, pos, pos + #cells[3], hls_by_cell[3])
  end
  pos = pos + #l2
  if v2 ~= "" then
...
local function format_opened(count)
  ...
  if count == 1 then
    return "1 time last 30 days"
  end
  if count >= 128 then
    return "128+ times last 30 days"
  end
...
local function render_score_section(b, score, file, hls, width)
  ...
  local function bonus_hl(n)
    return (n or 0) > 0 and pos_hl or val_hl
  end
  local function mod_hl(n)
    return (n or 0) >= 0 and pos_hl or neg_hl
  end
  ...
  if vim.fn.strdisplaywidth(line_text) > width then
    line_text = line_text:sub(1, width - 1) .. "…"
  end
  b:add_line(line_text)
  for _, r in ipairs(hl_ranges) do
    if r[1] < #line_text then
      b:add_hl(row2, r[1], math.min(r[2], #line_text), r[3])
    end
  end
end
...
local function render_timings_section(b, file, hls, width, timings_opt)
  ...
  if not show_modified and not show_accessed then
    return
  end
...
local function render_path_section(b, file, hls, width)
  ...
  if #path <= content_width then
    ...
    if #rest > content_width then
      rest = "…" .. rest:sub(-content_width + 1)
    end
    local r1 = #b.lines
...
function M.build(input)
  ...
  if sections.file_info ~= false then
    render_file_overview_section(b, file, hls, width)
  end
  if sections.score_breakdown ~= false then
    render_score(b, input.score, file, hls, width)
  end
  if sections.timings ~= false then
    render_timings_section(b, file, hls, width, sections.timings)
  end
  if sections.full_path ~= false then
    render_path_section(b, file, hls, width)
  end
...
function M.calculate_required_height(sections, panel_width)
  ...
  local total = 0
  if s.file_info ~= false then
    total = total + 2
  end
  if s.score_breakdown ~= false then
    total = total + 3
  end
  ...
  if s.full_path ~= false and s.full_path ~= nil then
    total = total + 2
  end
