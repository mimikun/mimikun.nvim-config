...
function M.jump_to_location(location)
  ...
  elseif location.start and location["end"] then
    ...
    if end_line > start_line then
      vim.cmd("normal! " .. (end_line - start_line) .. "k")
    end
    vim.cmd("normal! zz")
  end
end
...
function M.highlight_location(bufnr, location, namespace)
  if not location or not vim.api.nvim_buf_is_valid(bufnr) then
    return nil
  end
  ...
  if location.line then
    ...
    if location.col then
      ...
      if ok then
        table.insert(extmarks, { id = mark_id, line = target_line - 1 })
      end
    else
  ...
  elseif location.start and location["end"] then
    ...
    if location.start.col and location["end"].col then
      if start_line == end_line then
        ...
        if ok then
          table.insert(extmarks, { id = mark_id, line = start_line - 1 })
        end
      else
        ...
        for line = start_line, end_line do
          ...
          if ok then
            table.insert(extmarks, { id = mark_id, line = line - 1 })
          end
        end
      end
    else
...
function M.highlight_grep_matches(bufnr, location, namespace)
  ...
  for idx, line in ipairs(lines) do
    ...
    while true do
      ...
      if ok then
        table.insert(extmarks, { id = mark_id, line = i - 1 })
      end
      start_pos = e + 1
    end
  end
