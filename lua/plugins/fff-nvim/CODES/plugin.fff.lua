...
local init = vim.schedule_wrap(function()
  ...
  if lazy_sync == nil or not lazy_sync then
...
vim.api.nvim_create_user_command("FFFScan", function()
  require("fff").scan_files()
end, {
  desc = "Scan files for FFF",
})

vim.api.nvim_create_user_command("FFFRefreshGit", function()
  require("fff").refresh_git_status()
end, {
  desc = "Manually refresh git status for all files",
})

vim.api.nvim_create_user_command("FFFClearCache", function(opts)
  require("fff").clear_cache(opts.args)
end, {
  nargs = "?",
  complete = function()
    return { "all", "frecency", "files" }
  end,
  desc = "Clear FFF caches (all|frecency|files)",
})

vim.api.nvim_create_user_command("FFFHealth", function()
  vim.cmd("checkhealth fff")
end, {
