...
function M.clear_cache(scope)
  ...
  if scope == "all" or scope == "frecency" then
    ...
    if not ok then
      table.insert(errors, "destroy frecency db: " .. tostring(err))
    end

    ok, err = pcall(fuzzy.destroy_query_db)
    if not ok then
      table.insert(errors, "destroy query db: " .. tostring(err))
    end
  end
...
function M.search(query, max_results)
  ...
  if ok and search_result.items then
...
local function ensure_indexed(cwd, wait_for_index_ms)
  ...
  local function canon(p)
    ...
    if realpath_ok and realpath then
      abs = realpath
    end
  ...
  if cwd and cwd ~= "" then
    ...
    if vim.fn.isdirectory(expanded) ~= 1 then
      return false, "cwd does not exist: " .. expanded
    end
  ...
  if wait_ms == nil then
    wait_ms = cwd_triggered_reindex and 10000 or 0
  end
  if wait_ms <= 0 then
    return true
  end
  ...
  if cwd_triggered_reindex then
    ...
    if not matched then
      return false, "timeout waiting for re-index swap"
    end
    ...
    if remaining == 0 then
      return false, "timeout waiting for index scan"
    end
    wait_ms = remaining
  end
...
function M.change_indexing_directory(new_path)
  return require("fff.core").change_indexing_directory(new_path)
end
...
local function split_location_suffix(s)
  ...
  if p and p ~= "" then
    return p, { line = tonumber(l), col = tonumber(c) }
  end
  local p2, l2 = s:match("^(.-):(%d+)$")
  if p2 and p2 ~= "" then
    return p2, { line = tonumber(l2) }
  end
  return s, nil
end
...
local function open_resolved_file(abs_path, relative_path, location, open_cb)
  ...
  if not opened_via_split then
    vim.cmd("edit " .. vim.fn.fnameescape(cwd_relative))
  end
