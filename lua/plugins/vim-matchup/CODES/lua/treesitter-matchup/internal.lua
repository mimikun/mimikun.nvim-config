...
function M.containing_scope(info, key, matches)
  ...
  if not info or not scopes or not scopes[key] then
    return
  end
...
function M.get_delim(bufnr, opts)
  ...
  if opts.direction == "current" then
    ...
    for _, side in ipairs(side_table[opts.side]) do
      if not (side == "mid" and vim.g.matchup_delim_nomids > 0) then
        for _, info in
          ipairs(active_matches[side] --[=[@as matchup.treesitter.Match[]]=])
        do
    ...
    if result_info then
      return M.do_match_result(result_info.info, bufnr, opts, result_info.side, result_info.key, matches)
    end
  ...
  for _, side in ipairs(side_table[opts.side]) do
    ...
    do
      ...
      if opts.direction == "next" and pos >= cur_pos or opts.direction == "prev" and pos <= cur_pos then
  ...
  return M.do_match_result(closest_match, bufnr, opts, result_info.side, result_info.key, matches)
end
  ...
  for _, side in ipairs(sides) do
    ...
    do
      ...
      if
        ...
        and symbols[id] == cached_info.key
        and (is_down and (row > cached_info.row or row == cached_info.row and col > cached_info.col) or not is_down and (row < cached_info.row or row == cached_info.row and col < cached_info.col))
        and (row >= cached_info.search_range[1] and row <= cached_info.search_range[3])
      then
