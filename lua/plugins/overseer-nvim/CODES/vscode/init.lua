...
local function get_task_builder(defn, precalculated_vars)
  local task_provider = M.get_provider(defn.type)
  ...
  return function(params)
    ...
    -- (if present)
    local task_opts = task_provider.get_task_opts(defn, params[M.LAUNCH_CONFIG_KEY])
...
---@param defn table
---@param precalculated_vars? table
M.convert_vscode_task = function(defn, precalculated_vars)
  ...
  elseif task_builder then
    ...
    log.warn('VSCode task \'%s\' is missing type. Try setting "type": "shell"', defn.label or defn.name or defn.command)
    return nil
  end
