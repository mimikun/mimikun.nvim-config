...
local function file_converter(file_loc, precalculated_vars)
  ...
  local rel_path = type(file_loc) == "table" and variables.replace_vars(file_loc[2], {}, precalculated_vars)
    or vim.fn.getcwd()
...
M.get_parser_from_problem_matcher = function(problem_matcher, precalculated_vars)
  ...
  local convert = problem_matcher.fileLocation and file_converter(problem_matcher.fileLocation, precalculated_vars)
  if type(pattern) == "string" then
