...
M.format_standard = function(task)
  ...
  table.insert(ret, M.join(M.duration(task), M.time_since_completed(task, { hl_group = "Comment" })))
...
M.output_lines = function(task, opts)
  ...
  opts = vim.tbl_extend("keep", opts or {}, { num_lines = 1, prefix = "> ", prefix_hl_group = "Comment" })
