...
M.get_last_output_lines = function(bufnr, num_lines)
  ...
  while end_line > 0 and #lines < num_lines do
    ...
    lines =
      vim.list_extend(vim.api.nvim_buf_get_lines(bufnr, math.max(0, end_line - need_lines), end_line, false), lines)
    while not vim.tbl_isempty(lines) and (lines[#lines]:match("^%s*$") or lines[#lines]:match("^%[Process exited")) do
      table.remove(lines)
    end
