  ...
  constructor = function(opts)
    ...
    return {
      ...
      _start_timer = function(self, task)
        self:_stop_timer()
        log.debug("task(%s)[on_complete_dispose] starting dispose timer for %ds", task.id, opts.timeout)
      ...
      on_complete = function(self, task, status)
        if not vim.tbl_contains(opts.statuses, task.status) then
          log.debug("task(%s)[on_complete_dispose] complete, not auto-disposing task of status %s", task.id, status)
          return
        end
        local bufnr = task:get_bufnr()
        if not bufnr or is_buffer_visible(bufnr) or not vim.tbl_contains(opts.require_view, status) then
          self:_start_timer(task)
        else
          log.debug("task(%s)[on_complete_dispose] complete, waiting for output view", task.id, status)
