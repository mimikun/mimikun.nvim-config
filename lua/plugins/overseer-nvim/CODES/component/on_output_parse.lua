...
return {
  ...
  constructor = function(params)
    ...
    elseif num_parse_opts > 1 then
      log.error("on_output_parse: only one of 'parser', 'problem_matcher', 'errorformat' is allowed")
      return {}
    end

    local parser
    if params.problem_matcher then
      parser = problem_matcher.get_parser_from_problem_matcher(params.problem_matcher, params.precalculated_vars)
    ...
    if not parser then
      log.error("Could not create output parser from %s", params.problem_matcher or params.parser or params.errorformat)
      return {}
    end
