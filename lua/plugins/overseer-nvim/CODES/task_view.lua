...
function TaskView.new(winid, opts)
  ...
  local self = {
    ...
    list_task_opts = opts.list_task_opts or { include_ephemeral = true, sort = config.task_list.sort },
    autocmd_ids = {},
  }
  ...
  if not TaskView.focus_autocmd_id then
    TaskView.focus_autocmd_id = vim.api.nvim_create_autocmd("WinEnter", {
      ...
      nested = true,
      callback = function()
        vim.api.nvim_exec_autocmds("User", { pattern = "OverseerListFocusChanged", modeline = false })
      end,
    })
  end
