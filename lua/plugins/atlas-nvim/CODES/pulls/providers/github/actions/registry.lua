    ...
    run = function(ctx, done)
      ...
      cli.gh({
        ...
        if own_active_id ~= nil then
          local loading_msg = own_active_state == "APPROVED" and "Unapproving PR..." or "Dismissing changes request..."
          local success_msg = own_active_state == "APPROVED" and "PR unapproved" or "Changes request dismissed"
          footer.notify("loading", loading_msg)
          cli.gh({
            "api",
            "-X",
            "PUT",
            string.format("repos/%s/pulls/%s/reviews/%s/dismissals", slug, tostring(pr.id), tostring(own_active_id)),
            "-f",
        ...
        pullrequests.get_reviewers(pr, nil, function(reviewers, r_err)
          ...
          multi_select.open({
            ...
            on_done = function(selected)
              ...
              footer.notify("loading", string.format("Updating reviewers on PR #%s...", tostring(pr.id or "")))
              cli.gh(args, function(_, edit_err)
                if edit_err then
                  footer.notify("error", string.format("Update reviewers failed: %s", tostring(edit_err)))
      ...
      issues_api.list_assignees(slug, function(items, err)
        ...
        multi_select.open({
          ...
          on_done = function(selected)
            ...
            footer.notify("loading", string.format("Updating assignees on PR #%s...", tostring(pr.id or "")))
            cli.gh(args, function(_, edit_err)
              if edit_err then
      ...
      table.insert(out, {
        ...
        run = function(action_ctx, done)
          ...
          if not ok then
            ...
            logger.logerror(string.format("Custom action '%s' execution error: %s", item.label, tostring(run_err)))
          end
        end,
      })
    end
