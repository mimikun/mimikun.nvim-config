...
local function render_reviewers(_pr, width, lines, spans)
  ...
  for _, d in ipairs(decisions) do
    ...
    local name = (d.name and d.name ~= "") and d.name or (d.nickname and d.nickname ~= "") and d.nickname or "Unknown"
...
local function render_description(pr, width, lines, spans)
  ...
  if truncated then
    ...
    table.insert(spans, { line = line_idx, start_col = prefix_start, end_col = key_start, hl_group = "AtlasTextMuted" })
    table.insert(spans, { line = line_idx, start_col = key_start, end_col = suffix_start, hl_group = "Normal" })
    table.insert(spans, { line = line_idx, start_col = suffix_start, end_col = hint_end, hl_group = "AtlasTextMuted" })
  end
