  ...
  visibleFieldNames = {
    {
      ...
      operators = { "in", "was in", "is", "is not", "was not in", "changed", "!=", "=", "was not", "not in", "was" },
  ...
  if not ok or type(cached) ~= "table" then
    ...
    if #clause_tokens == 0 then
      if after_connector then
        return cap(
          merge_suggestions(connector_tokens(prefix, static_reserved), filter_by_prefix(static_fields, prefix))
        )
      end
