  ...
  },
  {
    ...
    run = function(ctx, done)
      ...
      async_picker.open({
        ...
        fetch = function(fetch_ctx, fetch_done)
          users_api.get_assignable_users({ issue_key = issue_key, project = nil }, fetch_ctx.query, function(users, err)
            if err then
              fetch_done(nil, err)
              return
            end
                ...
                if was_retry and type(raw_desc) == "string" then
                  ...
                  issues_api.update_issue(result.key, update, function(ok)
                    if ok then
                      ...
                      submit_done(true, nil)
                      done({ changed_issue_key = result.key, message = "Created " .. result.key }, nil)
                    else
                      notify_create("Issue created but failed to set description", vim.log.levels.WARN, 3000)
      ...
      async_picker.open({
        ...
        format_item = function(item)
          ...
          if category_name ~= "" then
            return string.format("%s %s - %s (%s)", provider_icon, item.label, project.name, category_name), provider_hl
          end
              ...
              vim.ui.input({ prompt = "Template name: " }, function(name_input)
                ...
                if existed then
                  vim.ui.input({
                    ...
                    finish({
                      changed_issue_key = nil,
                      message = string.format("Updated template %s", tostring(final_name or normalized_name or name)),
                    }, nil)
                  end)
                  return
                end
  ...
  },
  {
    ...
    run = function(ctx, done)
      ...
      local function finish(subscribed, err)
        ...
        done({ changed_issue_key = issue.key, message = issue.is_subscribed and "Subscribed" or "Unsubscribed" }, nil)
      end
