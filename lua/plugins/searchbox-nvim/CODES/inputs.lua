...
M.default_mappings = function(input, search_opts, state)
  ...
  if vim.fn.has("nvim-0.7") == 0 then
    ...
    bind({ "i" }, "<BS>", function()
      M.prompt_backspace(prompt_length)
    end, true)
  end
  ...
  bind({ "", "i" }, "<Plug>(searchbox-scroll-up)", function()
    win_exe("\\<C-y>")
  end, true)
  bind({ "", "i" }, "<Plug>(searchbox-scroll-down)", function()
    win_exe("\\<C-e>")
  end, true)

  bind({ "", "i" }, "<Plug>(searchbox-scroll-page-up)", function()
    win_exe("\\<C-b>")
  end, true)
  bind({ "", "i" }, "<Plug>(searchbox-scroll-page-down)", function()
    win_exe("\\<C-f>")
  end, true)

  bind({ "", "i" }, "<Plug>(searchbox-prev-match)", function()
    move("bw")
  end, true)
  bind({ "", "i" }, "<Plug>(searchbox-next-match)", function()
    move("w")
  end, true)
...
s.update_matches = vim.schedule_wrap(function(input, search_opts, state)
  ...
  local str = search_opts.show_matches:gsub("{total}", total):gsub("{match}", index)
