...
commands.linewiseToggle = function()
  ...
  local linewise_hybrid_mode = require("markview.spec").get(
...
commands.complete = function(_, cmd, cursorpos)
  ...
  elseif is_subcommand(parts[2]) == true then
    ...
    if vim.list_contains({ "enable", "disable", "toggle", "detach" }, parts[2]) then
      return num_to_items(state.get_attached_buffers(), #parts > 2 and parts[#parts] or "")
    elseif vim.list_contains({ "attach", "clear", "render", "splitOpen" }, parts[2]) then
      return num_to_items(vim.api.nvim_list_bufs(), #parts > 2 and parts[#parts] or "")
    elseif
      vim.list_contains(
        { "hybridEnable", "hybridDisable", "hybridToggle", "linewiseEnable", "linewiseDisable", "linewiseToggle" },
        parts[2]
      )
    then
      return num_to_items(state.get_enabled_buffers(), #parts > 2 and parts[#parts] or "")
    end
  end
end
