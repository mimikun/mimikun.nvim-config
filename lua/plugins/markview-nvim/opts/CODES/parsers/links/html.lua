...
html.link_attribute = function(buffer, TSNode, _, _)
  ...
  elseif value:type() == "quoted_attribute_value" then
    ...
    require("markview.links").new(buffer, vim.treesitter.get_node_text(inner, buffer, {}), {
      row_start = range[1],
      row_end = range[3],
  ...
  elseif value:type() == "attribute_value" then
    ...
    require("markview.links").new(buffer, vim.treesitter.get_node_text(value, buffer, {}), {
      row_start = range[1],
      row_end = range[3],
...
html.parse = function(buffer, TSTree, from, to)
  ...
  for capture_id, capture_node, _, _ in scanned_queries:iter_captures(TSTree:root(), buffer, from, to) do
    ...
    ---@type boolean, string
    local success, err = pcall(html[capture_name:gsub("^html%.", "")], buffer, capture_node, lines, {
      row_start = r_start,
      col_start = c_start,
