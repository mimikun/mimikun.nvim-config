...
latex.command = function(buffer, TSNode, text, range)
  ...
  if within_text_mode(TSNode) then
    return
  elseif command_name:len() == 2 or command_name:match("^\\math") then
    return
  end
...
latex.styled_command = function(_, TSNode, text, range, style)
  ...
  if within_text_mode(TSNode) then
    return
  elseif name:len() == 2 or name:match("^\\math") then
    return
  end
...
latex.parse = function(buffer, TSTree, from, to)
  ...
  for capture_id, capture_node, _, _ in scanned_queries:iter_captures(TSTree:root(), buffer, from, to) do
    ...
    ---@type boolean, string
    local success, err = pcall(latex[capture_name:gsub("^latex%.", "")], buffer, capture_node, lines, {
      row_start = r_start,
      col_start = c_start,
