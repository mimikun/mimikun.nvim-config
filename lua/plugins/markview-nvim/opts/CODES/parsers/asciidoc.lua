...
asciidoc.block_quote = function(buffer, _, text, range)
  ...
  if range.row_start > 0 then
    ...
    if string.match(before, "^quote") then
      ...
      if #parts > 2 then
        context = table.concat(vim.list_slice(parts, 3), ",")
...
asciidoc.doc_attr = function(buffer, TSNode, text, range)
  ...
  elseif name == "toclevels" and _value then
    asciidoc.data.toc_max_depth =
      math.max(math.min(tonumber(vim.treesitter.get_node_text(_value, buffer, {})) or 0, 5), 0)
  end
...
asciidoc.parse = function(buffer, TSTree, from, to)
  ...
  local can_scan, scanned_queries = pcall(
  ...
  local function iter(queries)
    ...
    for capture_id, capture_node, _, _ in queries:iter_captures(TSTree:root(), buffer, from, to) do
      ...
      ---@type boolean, string
      local success, error = pcall(asciidoc[capture_name:gsub("^asciidoc%.", "")], buffer, capture_node, lines, {
        row_start = r_start,
        col_start = c_start,
  ...
  local can_scan_tquery, scanned_tqueries = pcall(
    vim.treesitter.query.parse,
    "asciidoc",
    [[
		(document_attr
