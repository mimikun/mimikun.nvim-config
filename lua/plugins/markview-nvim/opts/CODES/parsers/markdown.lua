...
markdown.code_block = function(buffer, TSNode, _, range)
  ...
  if info_node and info_node:type() == "info_string" then
    ...
    info_string = string.gsub(vim.treesitter.get_node_text(info_node, buffer), "^^%s+", "")
...
markdown.list_item = function(buffer, TSNode, _, range)
  ...
  if before:match("%>$") then
    local tmp = text[1]:gsub("^" .. vim.pesc(before), "")
...
local cont = lpeg.C((esc_pipe + not_pipe) ^ 1)
local empty_cont = lpeg.C((esc_pipe + not_pipe) ^ 1)
...
markdown.parse = function(buffer, TSTree, from, to)
  ...
  for capture_id, capture_node, _, _ in scanned_queries:iter_captures(TSTree:root(), buffer, from, to) do
    ...
    ---@type boolean, string
    local success, err = pcall(markdown[capture_name:gsub("^markdown%.", "")], buffer, capture_node, lines, {
      row_start = r_start,
      col_start = c_start,
