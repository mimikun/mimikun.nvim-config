...
typst.code = function(buffer, TSNode, text, range)
  ...
  local from =
    vim.api.nvim_buf_get_lines(buffer, range.row_start, range.row_start + 1, false)[1]:sub(0, range.col_start)
...
typst.list_item = function(buffer, TSNode, text, range)
  ...
  if marker == "+" then
    ...
    if not prev_item or (prev_item:type() == "item" and item_text:match("^(%+)")) then
...
typst.math = function(buffer, _, text, range)
  local from, to =
    vim.api.nvim_buf_get_lines(buffer, range.row_start, range.row_start + 1, false)[1]:sub(0, range.col_start),
    vim.api.nvim_buf_get_lines(buffer, range.row_end, range.row_end + 1, true)[1]:sub(0, range.col_end)
  local inline = false
...
typst.term = function(buffer, TSNode, text, range)
  ...
  typst.insert({
    class = "typst_term",
    label = vim.treesitter.get_node_text(TSNode:field("term")[1], buffer),
...
typst.idet = function(_, TSNode, text, range)
  ...
  if not symbols.typst_entries[text[1]] then
    return
  end
...
typst.parse = function(buffer, TSTree, from, to)
  ...
  for capture_id, capture_node, _, _ in scanned_queries:iter_captures(TSTree:root(), buffer, from, to) do
    ...
    local success, err = pcall(typst[capture_name:gsub("^typst%.", "")], buffer, capture_node, lines, {
      row_start = r_start,
      col_start = c_start,
