...
local function as_wspace(m)
  return { kind = "space", value = m }
end
local function as_comma(m)
  return { kind = "comma", value = m }
end

local function as_issue(m)
  return { kind = "issue", value = m }
end
local function as_mention(m)
  return { kind = "mention", value = m }
end
local function as_word(m)
  return { kind = "word", value = m }
end
...
comment.parse = function(buffer, TSTree, from, to)
  ...
  if pcall(vim.treesitter.query.parse, "comment", "(source) @test") then
    scanned_queries = vim.treesitter.query.parse(
      "comment",
      [[
  ...
  for capture_id, capture_node, _, _ in scanned_queries:iter_captures(TSTree:root(), buffer, from, to) do
    ...
    ---@type boolean, string
    local success, error = pcall(comment[capture_name:gsub("^comment%.", "")], buffer, capture_node, lines, {
      row_start = r_start,
      col_start = c_start,
