...
inline.parse = function(buffer, TSTree, from, to)
  ...
  local pre_queries = vim.treesitter.query.parse(
    "markdown_inline",
    [[
		(
  ...
  for capture_id, capture_node, _, _ in scanned_queries:iter_captures(TSTree:root(), buffer, from, to) do
    ...
    ---@type boolean, string
    local success, err = pcall(inline[capture_name:gsub("^markdown_inline%.", "")], buffer, capture_node, lines, {
      row_start = r_start,
      col_start = c_start,
