...
markdown.atx_heading = function(buffer, item)
  ...
  local got_icon, icon = pcall(string.format, config.icon or "", unpack(item.levels or {}))
...
markdown.block_quote = function(buffer, item)
  ...
  if not main_config or not main_config.default then
    return
  end
  ...
  if item.callout then
    config = spec.get({ string.lower(item.callout) }, { source = main_config, eval_args = { buffer, item } })
      or spec.get({ string.upper(item.callout) }, { source = main_config, eval_args = { buffer, item } })
      or spec.get({ item.callout }, { source = main_config, eval_args = { buffer, item } })
      or spec.get({ "default" }, { source = main_config, eval_args = { buffer, item } })
...
markdown.code_block = function(buffer, item)
  ...
  local function render_simple()
    ...
    vim.api.nvim_buf_set_extmark(
      ...
      markdown.ns,
      range.row_end - 1,
      (range.col_start + #item.text[#item.text]) - #delims[2],
      {
        undo_restore = false,
        invalidate = true,
  ...
  local function render_block()
    ...
    local pad_width = vim.fn.strdisplaywidth(string.rep(pad_char, left_padding))
...
markdown.indented_code_block = function(buffer, item)
  ...
  local function render_block()
    ...
    local pad_width = vim.fn.strdisplaywidth(string.rep(pad_char, pad_amount))
  ...
  for _, part in ipairs(config.parts) do
    if part.type == "text" then
      table.insert(virt_text, {
        part.text,
        utils.set_hl(part.hl --[[ @as string ]]),
      })
...
markdown.link_ref_definition = function(buffer, item)
  ...
  local config = utils.match(main_config, item.description or "", {
...
markdown.list_item = function(buffer, item)
  ...
  if config.add_padding == true then
    for _, l in ipairs(item.candidates) do
      local from, to =
        math.min(range.col_start, item.text[l + 1]:len()),
        math.min(range.col_start + item.indent, item.text[l + 1]:len())
  ...
  local function get_state(state)
    ...
    return utils.match(
      checkboxes,
      "^" .. _state .. "$",
      { default = false, ignore_keys = { "checked", "unchecked", "enable" }, eval_args = chk_args }
    )
  end
    ...
    for l, line in ipairs(item.text) do
      if l == 1 then
        vim.api.nvim_buf_set_extmark(
          ...
          markdown.ns,
          range.row_start,
          range.col_start + (item.indent + #item.marker + 5),
          {
            undo_restore = false,
            invalidate = true,
            end_col = #item.text[1],
...
markdown.metadata_minus = function(buffer, item)
  ...
  vim.api.nvim_buf_set_extmark(buffer, markdown.ns, range.row_start, 0, {
    ...
    virt_text = config.border_top and {
      {
        string.rep(config.border_top, vim.api.nvim_win_get_width(utils.buf_getwin(buffer))),
  ...
  vim.api.nvim_buf_set_extmark(buffer, markdown.ns, range.row_end - 1, 0, {
    ...
    virt_text = config.border_bottom and {
      {
        string.rep(config.border_bottom, vim.api.nvim_win_get_width(utils.buf_getwin(buffer))),
  ...
  elseif config.style == "decorated" then
    ...
    if config.icon then
      for l = 1, (range.row_end - range.row_start) - 1 do
        ...
        if
          math.floor((range.row_end - range.row_start) / 2) == 0
          or l == math.floor((range.row_end - range.row_start) / 2)
        then
          vim.api.nvim_buf_set_extmark(
            buffer,
            markdown.ns,
            range.row_start + (l - 1),
            math.min(#line, range.col_start),
            {
              undo_restore = false,
              invalidate = true,
...
markdown.table = function(buffer, item)
  ...
  local function bottom_part(index)
    if config.block_decorator == true and config.use_virt_lines == false and item.border_overlap == true then
  ...
  for p, part in ipairs(item.header) do
    if part.class == "separator" then
      ...
      if p == #item.header and config.block_decorator == true then
        local prev_line = range.row_start == 0 and 0
          or #vim.api.nvim_buf_get_lines(buffer, range.row_start - 1, range.row_start, false)[1]
        ...
        if strict then
          ...
          if #before > 1 then
            vim.api.nvim_buf_set_extmark(
              ...
              markdown.ns,
              range.row_start + (r + 1),
              range.col_start + part.col_start + 1,
              {
                undo_restore = false,
                invalidate = true,
                end_col = range.col_start + part.col_start + #before,
          ...
          if #after > 1 then
            vim.api.nvim_buf_set_extmark(
              ...
              markdown.ns,
              range.row_start + (r + 1),
              range.col_start + part.col_end - #after,
              {
                undo_restore = false,
                invalidate = true,
                end_col = range.col_start + part.col_end - 1,
        ...
        if visible_width < column_width then
          ...
          elseif item.alignments[c] == "right" then
            vim.api.nvim_buf_set_extmark(
              ...
              markdown.ns,
              range.row_start + (r + 1),
              range.col_start + part.col_start,
              {
                undo_restore = false,
                invalidate = true,
                virt_text_pos = "inline",
  ...
  for p, part in ipairs(item.rows[#item.rows] or {}) do
    if part.class == "separator" then
      ...
      if p == #item.header and config.block_decorator == true then
        local next_line = range.row_end == vim.api.nvim_buf_line_count(buffer) and 0
          or #vim.api.nvim_buf_get_lines(buffer, range.row_end, range.row_end + 1, false)[1]
...
markdown.__block_quote = function(buffer, item)
  ...
  elseif
    item.callout
    and not vim.list_contains(keys, string.lower(item.callout))
    and not vim.list_contains(keys, string.upper(item.callout))
    and not vim.list_contains(keys, item.callout)
  then
    return
  end
  ...
  if item.callout then
    config = vim.tbl_extend(
      "force",
      config,
      spec.get({ string.lower(item.callout) }, { source = main_config, eval_args = { buffer, item } })
        or spec.get({ string.upper(item.callout) }, { source = main_config, eval_args = { buffer, item } })
        or spec.get({ item.callout }, { source = main_config, eval_args = { buffer, item } })
    )
  end
...
markdown.__section = function(buffer, item)
  ...
  for l = range.row_start, (range.org_end or range.row_end) - 1, 1 do
    require("markview.wrap").wrap_indent(buffer, {
      ...
      {
        string.rep(" " or shift_char, math.max(0, shift_width * (item.level - 1))),
      },
    })
  end
end
  ...
  for _, ext in ipairs(exts) do
    ...
    if detail then
      ...
      if
        col == 0
        and virt_text
        and (not conceal and pos == "inline") -- Check if it's a space/border type extmark.
        and #virt_text == 1
        and string.match(virt_text[1][1], "^%s+$") -- Check if it's an org indent border.
      then
        goto ignore
      end
    end
