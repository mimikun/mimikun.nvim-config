...
health.print = function(msg)
  ...
  if msg.kind == "hl" then
    ...
    local text = vim.split(vim.inspect(msg.value) or "", "\n", { trimempty = true })
    local lines = { { "\n" } }
  ...
  elseif msg.show then
    vim.api.nvim_echo(type(msg.message) == "string" and { { msg.message, "@comment" } } or msg.message, true, {})
  end
end
...
health.export = function()
  ...
  for i, item in ipairs(health.history) do
    ...
    table.insert(lines, string.upper(item.kind) .. " | " .. center(context[i], WC) .. " | " .. text)
  end
...
health.check = function()
  ...
  if vim.fn.has("nvim-0.10.1") == 1 then
    vim.health.ok(string.format("`v%d.%d.%d`", ver.major, ver.minor, ver.patch))
  elseif ver.major == 0 and ver.minor == 10 and ver.patch == 0 then
    vim.health.warn("Version(may experience bugs): " .. string.format("`%d.%d.%d`", ver.major, ver.minor, ver.patch))
  else
    vim.health.error(
  ...
  vim.health.info(
    "`Subscript`         "
      .. symbols.tostring("subscripts", "ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 0123456789 + () =")
  )
  vim.health.info(
    "`Superscript`       "
      .. symbols.tostring("superscripts", "ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 0123456789 + () =")
  )
  ...
  for font, _ in pairs(symbols.fonts) do
    vim.health.info(
      string.format("%-20s", "`" .. font .. "`")
        .. symbols.tostring(font, "ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 0123456789")
    )
  end
end
