...
function source:get_completions(ctx, callback)
  ...
  local function comelete_block_quote()
    ...
    local offset_a = vim.fn.strchars(string.match(after, "%w*%]") or string.match(after, "[A-Z]*"))
    ...
    for _, key in pairs(keys) do
      ...
      table.insert(output, {
        label = string.format("[!%s] -> %s", key, value.preview or items.default.preview),
  ...
  local function comelete_checkbox()
    ...
    ---@type integer Characters after the `cursor` that will be edited.
    local offset_a = vim.fn.strchars(string.match(after, "%w%]") or string.match(after, "%S?"))
    ...
    for _, key in pairs(keys) do
      ...
      table.insert(output, {
        label = string.format("[%s] -> %s %s", text, value.text or items.default.text, key),
        kind = require("blink.cmp.types").CompletionItemKind.Text,
