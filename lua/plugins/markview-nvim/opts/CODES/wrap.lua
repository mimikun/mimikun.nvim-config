...
wrap.huristic_wrap = function(buffer, win, row, ns, indent)
  ...
  for d = 1, divisions, 1 do
    ...
    vim.api.nvim_buf_set_extmark(buffer, ns, row, char - 1, indent_opts)
  end
...
wrap.fine_wrap = function(buffer, win, row, ns, indent)
  ...
  local reallen = vim.fn.strdisplaywidth(vim.api.nvim_buf_get_lines(buffer, row, row + 1, false)[1] or "")
  ...
  for w = 1, wraps, 1 do
    ...
    vim.api.nvim_buf_set_extmark(buffer, ns, row, wrapcol - 1, indent_opts)
  end
