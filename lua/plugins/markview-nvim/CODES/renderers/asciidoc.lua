...
asciidoc.block_quote = function(buffer, item)
  ...
  if item.from then
    config = spec.get({ item.from, item.context }, { source = main_config, eval_args = { buffer, item } })
...
asciidoc.image = function(buffer, item)
  ...
  local config = utils.match(main_config, item.destination, {
    eval_args = { buffer, item },
  })
  ...
  if config.text then
    utils.set_extmark(buffer, asciidoc.ns, range.destination[1], range.destination[2], {
      end_col = range.destination[4],
      end_row = range.destination[3],
...
asciidoc.keycode = function(buffer, item)
  ...
  local config = utils.match(main_config, string.upper(item.content or ""), {
    eval_args = { buffer, item },
  })
  ...
  utils.set_extmark(buffer, asciidoc.ns, range.content[1], range.content[2], {
    end_col = range.content[4],
    end_row = range.content[3],
...
asciidoc.list_item = function(buffer, item)
  ...
  elseif item.checkbox then
    ...
    checkbox_config = utils.match(
      checkboxes,
      "^" .. _state .. "$",
      { default = false, ignore_keys = { "checked", "unchecked", "enable" }, eval_args = { buffer, item } }
    )
  end
...
asciidoc.toc = function(buffer, item)
  ...
  for _, entry in ipairs(item.entries or {}) do
    ...
    if config then
      local text = require("markview.renderers.asciidoc.tostring").tostring(
        buffer,
        entry.text,
        utils.set_hl(config.hl) --[[@as string]]
      )
