...
inline.checkbox = function(buffer, item)
  ...
  if
    (state == "X" or state == "x") and spec.get({ "checked" }, { source = main_config, eval_args = { buffer, item } })
  then
...
inline.link_hyperlink = function(buffer, item)
  ...
  ---@type markview.config.__inline
  local config = utils.match(main_config, item.description, {
    eval_args = { buffer, item },
  })
...
inline.link_internal = function(buffer, item)
  ...
  vim.api.nvim_buf_set_extmark(
    ...
    inline.ns,
    range.row_start,
    vim.islist(range.alias) and range.alias[4] or (range.col_end - 2),
    {
      undo_restore = false,
      invalidate = true,
      end_col = range.col_end,
