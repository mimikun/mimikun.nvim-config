...
adoc_str.monospace = function(match)
  ...
  if not config then
    ...
    if config.corner_left then
      table.insert(output, { config.corner_left, config.corner_left_hl or config.hl })
    end
    if config.padding_left then
      table.insert(output, { config.padding_left, config.padding_left_hl or config.hl })
    end
    if config.icon then
      table.insert(output, { config.icon, config.icon_hl or config.hl })
    end

    table.insert(output, { removed, config.hl })

    if config.padding_right then
      table.insert(output, { config.padding_right, config.padding_right_hl or config.hl })
    end
    if config.corner_right then
      table.insert(output, { config.corner_right, config.corner_right_hl or config.hl })
    end
...
adoc_str.highlight = function(match)
  ...
  local config = utils.match(main_config, table.concat(item.text, "\n"), {
...
local chl = lpeg.C(lpeg.P("[") * role_content ^ 1 * lpeg.P("]") * lpeg.P("##") * hl_content ^ 1 * lpeg.P("##"))
  / adoc_str.highlight
