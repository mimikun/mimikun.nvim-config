  ...
  elseif spec.tmp_config then
    if vim.list_contains(spec.__external_config, keys[1]) then
      ...
      if
        ...
        and #vim.tbl_keys(loaded) ~= #vim.tbl_keys(spec.tmp_config[keys[1]] or {})
      then
        spec.tmp_config[keys[1]] = vim.tbl_deep_extend("keep", spec.tmp_config[keys[1]] or {}, loaded)
      end
    end
  ...
  elseif spec.config then
    if vim.list_contains(spec.__external_config, keys[1]) then
      ...
      if
        could_load
        and type(loaded) == "table"
        and #vim.tbl_keys(loaded) ~= #vim.tbl_keys(spec.config[keys[1]] or {})
      then
        spec.config[keys[1]] = vim.tbl_deep_extend("keep", spec.config[keys[1]] or {}, loaded)
      end
    end
