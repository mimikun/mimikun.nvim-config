...
highlights.create_pallete = function(n, src, light, dark)
  ...
  local fg = highlights.srgb_to_oklab(highlights.get_property("fg", src, light or "#1E1E2E", dark or "#EFF1F5"))

  ---@type number
  local alpha = vim.g.markview_alpha or (bg.L >= 0.5 and 0.15 or 0.25)

  local _mix = highlights.mix(bg, fg, (1 - alpha), alpha) --[[ @as markview.hl.Lab ]]
...
highlights.groups = {
  ...
  ["0"] = function()
    return highlights.create_pallete(0, { "Comment" }, "#9CA0B0", "#6C7086")
  end,
  ["1"] = function()
  ...
  ["7"] = function()
    return highlights.create_pallete(7, { "@conditional", "@keyword.conditional", "Conditional" }, "#8839EF", "#CBA6F7")
  end,
    ...
    return {
      {
        ...
        value = {
          bg = highlights.rgb_to_hex(highlights.oklab_to_srgb(mix)),
        },
      },
    }
  end,
  ...
  ["CheckboxStriked"] = function()
    return highlights.inherit("MarkviewPalette0Fg", {
      strikethrough = true,
    })
  end,

  ["CodeInfo"] = function()
    local fg = highlights.get_property("fg", { "Comment" }, "#9CA0B0", "#6C7086")
  ...
  ["CodeFg"] = function()
    local fg = highlights.get_property("bg", { "MarkviewCode" }, "#ccced2", "#2d2d42")
  ...
  ["Gradient"] = function()
    local from = highlights.srgb_to_oklab(highlights.get_property("bg", { "Normal" }, "#CDD6F4", "#1E1E2E"))
    local to = highlights.srgb_to_oklab(highlights.get_property("fg", { "Title" }, "#1e66f5", "#89b4fa"))

    local output = {}

    for i = 0, 9, 1 do
      local step = highlights.mix(from, to, 1 - (i / 9), i / 9)

      table.insert(output, {
        group_name = string.format("MarkviewGradient%d", i),
        value = {
          fg = highlights.rgb_to_hex(highlights.oklab_to_srgb(step)),
        },
      })
    end
