...
toggler.__add_checkbox = function(buffer, lnum, line)
  ...
  elseif line:match("^[%s%>]*[%-%+%*]%s+%[ %]") or line:match("^[%s%>]*%d+[%.%)]%s+%[ %]") then
    ...
    return before .. string.format(" [%s]", cached_state or checkboxes.config.default or "") .. after,
      { before, state, after }
  end
end
...
toggler.__has_markers = function(buffer, from, to, lines)
  ...
  local function should_add_checkbox(line)
    if
      checkboxes.config.remove_style == "disable" and line:match("^[%s%>]*[%-%+%*]%s+%[%]")
      or line:match("^[%s%>]*%d+[%.%)]%s+%[%]")
    then
      return true
    elseif
...
interactive.__close = function()
  ...
  for _, keymap in ipairs(interactive.keymaps) do
    vim.api.nvim_buf_set_keymap(interactive.source_buffer, keymap.mode, keymap.lhs or "", keymap.rhs or "", {
