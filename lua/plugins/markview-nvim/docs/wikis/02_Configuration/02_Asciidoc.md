<!--markdoc
    {
        "generic": {
            "filename": "../doc/markview.nvim-asciidoc.txt",
            "force_write": true,
            "header": {
                "desc": "🧩 Asciidoc options for `markview.nvim`",
                "tag": "markview.nvim-asciidoc"
            }
        },
        "markdown": {
            "heading_ratio": [ 26, 54 ],
            ...
            "tags": {
                ...
                "block_quotes": [ "markview.nvim-asciidoc.block_quotes" ],
                "code_blocks$": [ "markview.nvim-asciidoc.code_blocks" ],
                "delimited_blocks": [ "markview.nvim-asciidoc.delimited_blocks" ],
                "checkboxes": [ "markview.nvim-asciidoc.checkboxes" ],
                "document_attributes": [ "markview.nvim-asciidoc.document_attributes" ],
                "document_titles": [ "markview.nvim-asciidoc.document_titles" ],
                "horizontal_rules": [ "markview.nvim-asciidoc.horizontal_rules" ],
...
```lua $config: ../lua/markview/types/renderers/asciidoc.lua, from: $config class: markview.config.asciidoc
...
```lua from: $config class: markview.config.asciidoc.admonitions
...
```lua $src: ../lua/markview/config/asciidoc.lua, from: $src field: admonitions
admonitions = {
    enable = true,
...
>[!IMPORTANT]
> The key names are **case-insensitive**. So, `foo` & `FOO` refer to the same type.
> However, modifying `FOO` instead of `foo` can lead to undefined behavior. Use lower case for type names!

```lua from: $config class: markview.config.asciidoc.admonitions.opts
...
```lua from: $config class: markview.config.asciidoc.block_quotes
...
```lua from: $src field: block_quotes
block_quotes = {
    enable = true,
...
```lua from: $config class: markview.config.asciidoc.block_quotes.opts
...
```lua from: $config, alias: markview.config.asciidoc.code_blocks
...
```lua from: $config class: markview.config.asciidoc.code_blocks.block
...
```lua from: $config class: markview.config.asciidoc.code_blocks.simple
...
```lua from: $config class: markview.config.asciidoc.code_blocks.opts
...
```lua from: $config class: markview.config.asciidoc.delimited_blocks
...
>[!NOTE]
> `Admonition block`s are configured via [asciidoc.admonitions](#admonitions) as they are a variation of delimited blocks. Same with [preview.raw_previews](https://github.com/OXY2DEV/markview.nvim/wiki/Preview#raw_previews) where disabling `admonitions` won't disable rendering of the block(only the admonition label will disappear).

```lua $src: ../lua/markview/config/asciidoc.lua, from: $src field: delimited_blocks
delimited_blocks = {
    enable = true,
...
```lua from: $config class: markview.config.asciidoc.document_attributes
...
```lua from: $src field: document_attributes
document_attributes = {
    enable = true,
},
```
...
```lua from: $config class: markview.config.asciidoc.hrs
...
```lua from: $src field: horizontal_rules
horizontal_rules = {
    enable = true,
...
```lua from: $config, class: markview.config.asciidoc.hrs.text
...
```lua from: $config class: markview.config.asciidoc.hrs.repeating
...
```lua from: $config class: markview.config.asciidoc.list_items
...
```lua from: $config class: markview.config.asciidoc.list_items.opts
...
```lua from: $config class: markview.config.asciidoc.section_titles
...
```lua from: $src field: section_titles
section_titles = {
    enable = true,
...
```lua from: $config class: markview.config.asciidoc.section_titles.opts
...
```lua from: $config class: markview.config.asciidoc.tocs
...
```lua from: $config class: markview.config.asciidoc.tocs.opts
