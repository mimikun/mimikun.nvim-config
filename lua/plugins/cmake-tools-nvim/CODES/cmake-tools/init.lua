...
function cmake.generate(opt, callback)
  ...
  if presets_exists then
    ...
    if config.configure_preset then
      local configure_preset = presets:get_configure_preset(config.configure_preset, { include_hidden = true })
    ...
    if (not config.configure_preset) or (find_preset == false) then
      ...
      if config.build_preset then
        ...
        if build_preset then
          local preset = presets:get_configure_preset(build_preset.configurePreset, { include_hidden = true })
          if preset then
  ...
  local kit_option =
    kits.build_env_and_args(config.kit, config.executor.name == "terminal", config.cwd, const.cmake_kits_path)
  ...
  return utils.execute(cmd, config.env_script, env, args, config.cwd, config.executor, function(result)
    if result:is_ok() then
      cmake.configure_compile_commands()
      cmake.create_regenerate_on_save_autocmd()
    end
...
function cmake.build(opt, callback)
  ...
  if not (config:has_build_directory()) or not (ct.code == Types.SUCCESS) then
    ...
    return cmake.generate({ bang = false, fargs = {} }, function(result)
      if result:is_ok() then
        if config:has_build_directory() and config:get_codemodel_targets().code == Types.SUCCESS then
...
function cmake.install(opt, callback)
  ...
  local cmd, args =
    utils.apply_wrap_call(opt.wrap_call, const.cmake_command, { "--install", config:build_directory_path() })
  vim.list_extend(args, fargs)
...
function cmake.get_launch_path(target)
  ...
  if config.base_settings.working_dir and type(config.base_settings.working_dir) == "string" then
    launch_path = cmake.substitute_path(config.base_settings.working_dir, cmake.get_target_vars(target))
  end
...
function cmake.run(opt, callback)
  ...
  if opt.target then
    ...
    return cmake.build({ target = opt.target }, function(build_result)
      ...
      local _args = opt.args and opt.args or utils.get_nested(config, "target_settings", opt.target, "args") or {}
    ...
    if result_code == Types.NOT_CONFIGURED or result_code == Types.CANNOT_FIND_CODEMODEL_FILE then
      -- Configure it
      return cmake.generate({ bang = false, fargs = utils.deepcopy(opt.fargs) }, function(generate_result)
        if result:is_ok() then
    ...
    then
      ...
      return cmake.build({ target = config.launch_target, fargs = utils.deepcopy(opt.fargs) }, function(build_result)
        if not build_result:is_ok() then
          callback(build_result)
          return
        end
        ...
        local cmd, args = utils.apply_wrap_call(opt.wrap_call, target_path, cmake:get_launch_args())
...
function cmake.quick_run(opt, callback)
  ...
  if opt.fargs[1] == nil then
    ...
    vim.ui.select(
      ...
      vim.schedule_wrap(function(_, idx)
        if not idx then
          callback(Result:new_error(Types.NOT_SELECT_LAUNCH_TARGET, "No target selected: cancelled"))
          return
        end
...
function cmake.select_configure_preset(callback)
  ...
  if Presets.exists(config.cwd) then
    ...
    callback(Result:new_error(Types.CANNOT_FIND_PRESETS_FILE, "Cannot find CMake[User]Presets file"))
      ...
      vim.schedule_wrap(function(choice)
        ...
        if config.build_preset ~= choice then
          ...
          if build_preset and build_preset:is_valid() then
            ...
            local associated_configure_preset_name = associated_configure_preset and associated_configure_preset.name
              or nil

            if associated_configure_preset_name and config.configure_preset ~= associated_configure_preset_name then
  ...
  ctest.list_all_tests(build_dir, preset_name, function(all_tests)
    ...
    if config:ctest_show_labels() then
      ...
      for _, entry in ipairs(labels) do
        ...
        table.insert(display, table.concat({ "[label]", entry.label, "(" .. entry.count, "tests)" }, " "))
      end
    end
...
function cmake.get_launch_args()
  ...
  if config.target_settings[cmake.get_launch_target()] and config.target_settings[cmake.get_launch_target()].args then
...
function cmake.get_run_environment(target)
  return environment.get_run_environment_table(config, target and target or cmake.get_launch_target())
end
    ...
    if const.cmake_virtual_text_support then
      vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI" }, {
        ...
        callback = function(ev)
          ...
          local file = ev.file
          local all_targets = config:build_targets_with_sources()
          if all_targets and all_targets.data and type(all_targets.data) == "table" and all_targets.data["sources"] then
            for _, target in ipairs(all_targets.data["sources"]) do
              if target.path:lower() == file:lower() then
...
function cmake.register_dap_function()
  ...
  if has_nvim_dap then
    ...
    function cmake.debug(opt, callback)
      ...
      local env = environment.get_run_environment_table(config, opt.target and opt.target or config.launch_target)
      ...
      if opt.target then
        ...
        return cmake.build({ target = opt.target }, function(build_result)
          ...
          local dap_config = {
            ...
            args = opt.args and opt.args or utils.get_nested(config, "target_settings", opt.target, "args") or {},
            env = env,
