...
local function significant_text_list(node, buf, acc)
  ...
  if node:child_count() == 0 then
    table.insert(acc, ts.get_node_text(node, buf))
  end
  if node:child_count() > 0 then
...
function M.extract_var(range_type, config)
  ...
  local task = async.run(function()
    ...
    if not get_variable_declaration then
      return code_gen_error("variable_declaration", lang)
    end
    ...
    for _, tree in ipairs(nested_lang_tree:trees()) do
      for _, node in encompasing_query:iter_captures(tree:root(), buf) do
        ...
        if node_significant_text == selected_significant_text then
    ...
    iter(matching_nodes):each(
      ---@param n TSNode
      function(n)
        local node_range = range(buf, n:range())
    ...
    local nested_scope_ranges = iter(scopes)
      :filter(
        ---@param si refactor.Scope
        function(si)
          if si == smallest_common_scope then
            return false
          end
    ...
    local output_statement = iter(output_statements)
      :filter(
        ...
        function(os)
          ...
          local is_before_matching_nodes = iter(matching_nodes):all(
            ---@param n TSNode
            function(n)
              local node_start_pos = pos(buf, n:start())
      ...
      :fold(
        ...
        function(acc, os)
          ...
          local os_start_pos = pos(buf, os.output_statement:start())
          if os_start_pos > acc_start_pos then
            return os
          end
          return acc
        end
