...
local function get_output_node(in_buf, out_buf, lang, selected_range)
  ...
  if not nested_lang_tree then
    return
  end

  local output_function_query = ts.query.get(lang, "refactor_output_function")
  if not output_function_query then
    return query_error("refactor_output_function", lang)
  end

  local output_fns = get_output_functions(out_buf, nested_lang_tree, output_function_query)

  if in_buf ~= out_buf and output_fns[#output_fns] then
    return choose_output(output_fns[#output_fns])
  end
  if in_buf ~= out_buf then
    return
  end
...
local function get_input_fn(buf, nested_lang_tree, selected_range)
  ...
  if not output_function_query then
    return query_error("refactor_input_function", lang)
  end
  ...
  local surrounding_input = iter(input_fns)
    ...
    :fold(
      ...
      function(acc, i)
        ...
        if i.fn:byte_length() < acc.fn:byte_length() then
          return i
        end
        return acc
      end
  ...
  if surrounding_input then
    return surrounding_input
  end
...
local function extract_func(opts)
  ...
  if not reference_query then
    return query_error("refactor_reference", lang)
  end
  local scope_query = ts.query.get(lang, "refactor_scope")
  if not scope_query then
    return query_error("refactor_scope", lang)
  end
  ...
  if not get_return_statement then
    return code_gen_error("return_statement", lang)
  end
  local get_function_declaration = code_generation.function_declaration[lang]
  if not get_function_declaration then
    return code_gen_error("function_declaration", lang)
  end
  local get_function_call = code_generation.function_call[lang]
  if not get_function_call then
    return code_gen_error("function_call", lang)
  end
  ...
  if not expandtab then
    function_declaration:gsub("^(%s+)", function(spaces)
      return ("\t"):rep(#spaces)
    end)
  end
...
M.extract_func_to_file = function(range_type, config)
  ...
  local task = async.run(function()
    ...
    if not vim.bo[out_buf].buflisted then
