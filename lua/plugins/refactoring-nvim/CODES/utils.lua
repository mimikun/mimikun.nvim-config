...
M.get_lsp_definitions = async.wrap(2, function(is_preview, cb)
  if cached_definitions then
    return cb(cached_definitions)
  end

  lsp.buf.definition({
    on_list = function(args)
      if is_preview then
        cached_definitions = args.items
      end
...
M.get_lsp_references = async.wrap(2, function(is_preview, cb)
  if cached_references then
    return cb(cached_references)
  end
  ...
  local declaration_scope = iter(scopes)
    ...
    :fold(
      ...
      function(acc, si, s)
        ...
        if s:byte_length() < acc.s:byte_length() then
          return { si = si, s = s }
        end
        return acc
      end
    )
  if not declaration_scope then
    return
  end
...
function M.get_declaration_scope(declarations_by_scope, all_scopes, reference, buf)
  ...
  return iter(scopes_for_reference):find(
    ...
    function(si)
      ...
      if not scope_declarations then
        return
      end
      local identifier_declarations = scope_declarations[identifier]
      if not identifier_declarations then
        return
      end
...
function M.is_first_closer(first, second, position)
  ...
  if second_row_distance < first_row_distance then
    return false
  end

  local first_col_distance = math.abs(first.col - position.col)
  local second_col_distance = math.abs(second.col - position.col)
  if second_row_distance == first_row_distance and second_col_distance < first_col_distance then
    return false
  end
  return true
end
...
function M.get_output_statements(buf, nested_lang_tree, query)
  ...
  for _, tree in ipairs(nested_lang_tree:trees()) do
    for _, match, metadata in query:iter_matches(tree:root(), buf) do
      ...
      if output_statement then
        table.insert(output_statements, output_statement)
      end
    end
  end
...
function M.get_output_functions(buf, nested_lang_tree, query)
  ...
  for _, tree in ipairs(nested_lang_tree:trees()) do
    for _, match in query:iter_matches(tree:root(), buf) do
      ...
      if output_function then
        table.insert(output_functions, output_function)
      end
    end
  end
    ...
    for _, match, metadata in query:iter_matches(tree:root(), buf) do
      ...
      for capture_id, nodes in pairs(match) do
        ...
        if name == "input_function" then
          ...
          if struct_name then
            input_function.struct_name = ts.get_node_text(match[struct_name][1], buf)
          end
          local struct_var_name = metadata.struct_var_name
          if struct_var_name then
            input_function.struct_var_name = ts.get_node_text(match[struct_var_name][1], buf)
          end
        end
      end
      if input_function then
        table.insert(input_functions, input_function)
      end
    end
  end
...
function M.get_debug_path_for_range(buf, lang_tree, output_range)
  ...
  if not query then
    return M.query_error("refactor_debug_path", lang)
  end
