...
local function get_has_indent_before(output_range, buf)
  ...
  while s and e do
    ...
    if e and e > output_range.start_col then
      break
    end
...
function M.get_statement_output_range(buf, output_statements, output_location, reference_range, reference_pos)
  ...
  local statement_for_range = iter(output_statements)
    ...
    :fold(
      ...
      function(acc, os)
        ...
        if os.output_statement:byte_length() < acc.output_statement:byte_length() then
          return os
        end
        return acc
      end
  ...
  if output_location == "above" and has_indent_before then
    output_range.start_col = space_start - 1
  end
...
function M.get_is_in_midline(output_range, buf, output_location)
  ...
  if output_location == "below" then
    is_in_mid_line = output_line:find("[^%s]+", output_range.end_col + 1) ~= nil
  end
  if output_location == "above" then
