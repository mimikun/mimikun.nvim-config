...
M[c.InternalCommands.CallApiFunction] = function(params)
  ...
  if api_function then
    ...
    vim.notify("Unknown 'typescript-tools.api." .. api_function .. "' function!", vim.log.levels.WARN)
  end
end
...
M[c.InternalCommands.InteractiveCodeAction] = function(params)
  ...
  a.void(function()
    ...
    local err, result = async.buf_request_isomorphic(
      ...
      vim.tbl_deep_extend("force", request, { data = { interactiveRefactorArguments = { targetFile = target_file } } })
    )
