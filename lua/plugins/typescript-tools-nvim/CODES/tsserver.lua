...
local function dispatch_update_event(method, data)
  if not (method == c.LspMethods.DidOpen or method == c.LspMethods.DidChange or method == c.LspMethods.DidClose) then
    return
  end
...
function Tsserver:handle_response(response)
  ...
  if coroutine.status(handler) ~= "dead" then
    local _, err = coroutine.resume(handler, response.body or response, response.command or response.event)

    if err then
      local _ = log.error() and log.error("tsserver", "Unexpected error while handling response: ", metadata.method)
...
function Tsserver:handle_request(method, params, callback, notify_reply_callback)
  ...
  function handler_context.request(request)
    ...
    if handler_module.cancel_on_change and params then
      self.requests_to_cancel_on_change[handler_context.seq] = params.textDocument or (params.data or {}).textDocument
    end
  ...
  local _, err = coroutine.resume(handler, handler_context.request, handler_context.response, params, handler_context)

  local seq = get_seq()

  if err then
    local _ = log.error() and log.error("tsserver", "Unexpected error while handling request: ", handler_context.method)
