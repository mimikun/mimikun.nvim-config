...
local function tsserver_reason_to_lsp_kind(context)
  ...
  elseif kind == c.SignatureHelpTriggerKind.ContentChange then
    return context.isRetrigger and c.SignatureHelpTriggerReason.Retrigger or c.SignatureHelpTriggerReason.CharacterTyped
...
local function make_signatures(items)
  return vim.tbl_map(function(item)
    return {
      label = make_signature_label(item.prefixDisplayParts, item.parameters, item.suffixDisplayParts),
      ...
      parameters = vim.tbl_map(function(param)
        return {
          ...
          documentation = utils.make_markup_content(utils.tsserver_docs_to_plain_text(param.documentation, "")),
        }
      end, item.parameters),
    }
  end, items)
