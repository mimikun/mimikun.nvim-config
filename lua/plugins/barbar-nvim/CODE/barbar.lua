...
function barbar.setup(options)
  ...
  create_user_command("BufferNext", function(tbl)
    api.goto_buffer_relative(max(1, tbl.count))
  ...
  create_user_command("BufferGoto", function(tbl)
    ...
    if not index then
      return notify("Invalid argument to " .. markdown_inline_code(":BufferGoto"), vim.log.levels.ERROR)
    end
  ...
  create_user_command("BufferGotoPinned", function(tbl)
    local index = tonumber(tbl.args)
    if not index then
      return notify("Invalid argument to " .. markdown_inline_code(":BufferGotoPinned"), vim.log.levels.ERROR)
    end
  ...
  create_user_command("BufferGotoUnpinned", function(tbl)
    local index = tonumber(tbl.args)
    if not index then
      return notify("Invalid argument to " .. markdown_inline_code(":BufferGotoUnpinned"), vim.log.levels.ERROR)
    end
  ...
  create_user_command("BufferMove", vim.api.nvim_cmd and function(tbl)
    vim.cmd.BufferMovePrevious({ count = tbl.count })
  end or function(tbl)
    command("BufferMovePrevious " .. tbl.count)
  end, { count = true, desc = "Synonym for " .. markdown_inline_code(":BufferMovePrevious") })

  create_user_command("BufferMoveNext", function(tbl)
    api.move_current_buffer(max(1, tbl.count))
  ...
  create_user_command("BufferMoveStart", function()
    api.move_current_buffer_to(1)
  end, { desc = "Move current buffer to the front" })

  create_user_command("BufferPick", api.pick_buffer, { desc = "Pick a buffer" })

  create_user_command("BufferPickDelete", function(cmd)
  ...
  end, { bang = true, count = true, desc = "Pick buffers to delete" })

  create_user_command("BufferPin", function(tbl)
    api.toggle_pin(tbl.fargs[1] and bufnr(tbl.fargs[1]))
  end, { complete = "buffer", desc = "Un/pin a buffer", nargs = "?" })
  ...
  create_user_command("BufferOrderByName", api.order_by_name, { desc = "Order the bufferline by name" })
  ...
  for cmd, desc in pairs({
    ...
    create_user_command("Buffer" .. cmd, function(tbl)
      bbye.bdelete(tbl.bang, tbl.args, tbl.smods or tbl.mods)
    end, { bang = true, complete = "buffer", desc = desc, nargs = "?" })
  end

  create_user_command("BufferWipeout", function(tbl)
    bbye.bwipeout(tbl.bang, tbl.args, tbl.smods or tbl.mods)
  ...
  create_user_command("BufferRestore", api.restore_buffer, { desc = "Restore the last recently closed buffer" })
