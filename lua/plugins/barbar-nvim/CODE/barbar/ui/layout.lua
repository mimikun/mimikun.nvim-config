  ...
  for i, buffer_number in ipairs(state.buffers_visible) do
    if state.is_pinned(buffer_number) then
      ...
      pinned_position = pinned_position
        + layout.calculate_width(data.buffers.base_widths[i], config.options.minimum_padding)
    else
      positions[buffer_number] = unpinned_position
      unpinned_position = unpinned_position + layout.calculate_width(data.buffers.base_widths[i], data.buffers.padding)
    end
  end
