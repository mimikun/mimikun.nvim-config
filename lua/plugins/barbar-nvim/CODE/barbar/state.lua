...
function state.get_buffer_list()
  ...
  for _, bufnr in ipairs(list_bufs()) do
    if buf_get_option(bufnr, "buflisted") and not tbl_contains(exclude_ft, buf_get_option(bufnr, "filetype")) then
...
function state.close_buffer_animated(bufnr)
  ...
  animate.start(ANIMATION.CLOSE_DURATION, current_width, 0, vim.v.t_number, function(new_width, animation)
...
local function open_buffer_start_animation(data, bufnr)
  ...
  buffer_data.computed_width = layout.calculate_width(
    data.buffers.base_widths[index] or layout.calculate_buffer_width(state, bufnr, #state.buffers_visible + 1),
  ...
  defer_fn(function()
    animate.start(ANIMATION.OPEN_DURATION, 1, target_width, vim.v.t_number, function(new_width, animation)
...
local function open_buffers(new_buffers)
  ...
  for _, new_buffer in ipairs(new_buffers) do
    if list.index_of(state.buffers, new_buffer) == nil then
      ...
      local should_insert_at_end = config.options.insert_at_end
  ...
  if initial_buffers <= 1 and #new_buffers > 1 or initial_buffers == 0 and #new_buffers == 1 then
    return
  end
...
function state.pop_recently_closed()
  local open_filepaths = tbl_map(function(bufnr)
    return fnamemodify(bufname(bufnr), ":p")
  end, state.buffers)
...
function state.update_names()
  ...
  for name, indexes in pairs(buffer_index_by_name) do
    if #indexes == 1 then
      ...
      local buffer_numbers = tbl_map(function(i)
