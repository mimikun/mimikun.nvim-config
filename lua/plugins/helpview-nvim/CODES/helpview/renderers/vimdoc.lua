...
vimdoc.code_block = function(buffer, item)
  ...
  local label = {
    string.format(" %s%s ", decorations.icon, decorations.name),
    utils.set_hl(config.label_hl or decorations.icon_hl),
  }
...
vimdoc.heading = function(buffer, item)
  ...
  vim.api.nvim_buf_set_extmark(buffer, vimdoc.ns, range.row_start, range.col_start, {
    ...
    virt_text = {
      {
        string.rep(config.marker or " ", vim.bo[buffer].tw - vim.fn.strdisplaywidth(label[1] .. sign .. label[2])),
        utils.set_hl(config.marker_hl or config.hl),
      },
...
vimdoc.heading_no_delim = function(buffer, item)
  ...
  vim.api.nvim_buf_set_extmark(
    ...
    vimdoc.ns,
    range.row_start,
    range.col_start + #item.text[1] - (item.level == 4 and 2 or 0),
    {
      undo_restore = false,
      invalidate = true,
      ...
      virt_text = {
        {
          string.rep(config.marker or " ", vim.bo[buffer].tw - used_width),
          utils.set_hl(config.marker_hl or config.hl),
        },
...
vimdoc.modeline = function(buffer, item)
  ...
  for _, opt in ipairs(item.options) do
    ...
    option_config =
      vim.tbl_extend("keep", option_config, utils.match(config, opt.option, { ignore_keys = { "data_types" } }))
