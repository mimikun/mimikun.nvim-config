...
utils.within_range = function(range, pos)
  ...
  elseif pos.row_end > range.row_end then
    return false
  elseif
    (pos.row_start == range.row_start and pos.row_end == range.row_end)
    and (pos.col_start < range.col_start or pos.col_end > range.col_end)
  then
    return false
  end
...
utils.tostatic = function(tbl, opts)
  ...
  for k, v in pairs(tbl or {}) do
    ---@diagnostic disable
    if pcall(v, unpack(opts.args or {})) then
...
utils.match = function(config, name, opts)
  ...
  for _, entry in ipairs(sorted) do
    if is_valid(name, entry.key) == true then
      match = spec.get({ entry.key }, vim.tbl_extend("force", opts, { source = config }))
      break
    end
  end
