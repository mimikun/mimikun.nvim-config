...
function SC.handle_complete(subcmds)
  return function(_, line, cursorpos)
    ...
    if #chunks < 1 then
      ...
      require("juu.logger").warn("could not tokenize line: `", line, "'")
      return
    end

    if (#chunks == 1 and trailing_whitespace) or (#chunks == 2 and not trailing_whitespace) then -- e.g., `:Cmd |` or `:Cmd cl|`
...
--[[
================
Everything below this is Juu-specific (and everything above this isn't)
================
--]]

local COMMAND_NAME = "Juu"
local COMMAND_DESC = "Juu ex-mode command interface"

local GroupKey = SC.Any:override({
  suggestions = function()
    return vim.tbl_filter(function(key)
      return type(key) == "string"
    end, require("juu.notify").group_keys())
  end,
})
...
SC.subcommands = {
  clear = {
    ...
    func = function(args)
      require("juu.notify").clear(args[1])
    end,
    args = {
  ...
  history = {
    ...
    args = {
      ...
      before = {
        name = "seconds",
        type = SC.Number,
        desc = "filter history for items updated at least this long ago",
      },
      since = {
        name = "seconds",
        type = SC.Number,
        desc = "filter history for items updated at most this long ago",
      },
      include_removed = {
        name = "true|false",
        type = SC.Boolean,
  ...
  clear_history = {
    ...
    func = function(args)
      args.group_key = args[1] or args.group_key
      require("juu.notify").clear_history(args)
    end,
    args = {
  ...
  suppress = {
    ...
    func = function(args)
      require("juu.notify").suppress(args[1])
    end,
    args = {
      {
        name = "suppress",
        type = SC.Boolean,
        desc = "whether to suppress (omitting this argument toggles suppression)",
      },
    },
  },
  -- lsp_suppress = {
  ...
  --   func = function(args)
  --     require("juu.progress").suppress(args[1])
  --   end,
  --   args = {
...
function SC.make_panvimdocs()
  ...
  for _, kv in ipairs(sort_by_key(SC.subcommands)) do
    ...
    table.insert(vimdocs, SC.vimdoc(COMMAND_NAME, name, subcmd, "juu"))
  end

  return string.format(
    [[
<!-- {{{ Generated from juu.commands.lua -->
...
Juu exposes some of its Lua API functions through `:Juu` sub-commands
(e.g., `:Juu clear`), which support shell-like arguments and completion.
