...
M.toggle = function(args, opts)
  local msg = vim.deprecate("require('neominimap').toggle", "require('neominimap.api').toggle", "v4", "neominimap.nvim")
  if msg then
    local logger = require("neominimap.logger")
    logger.notify(msg, vim.log.levels.WARN)
  end
...
M.winEnabled = function(winid)
  local msg =
    vim.deprecate("require('neominimap').winEnabled", "require('neominimap.api').win.enabled", "v4", "neominimap.nvim")
  if msg then
    local logger = require("neominimap.logger")
    logger.notify(msg, vim.log.levels.WARN)
  end
