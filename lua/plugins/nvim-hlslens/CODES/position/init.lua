...
function Position:nearestIndex(curPos, curFoldedLnum, topl, botl)
  ...
  fn.winrestview(wv)
  if hiWinLine > 0 and (loWinLine == 0 or math.ceil((hiWinLine - loWinLine) / 2) - 1 < curWinLine - loWinLine) then
    relIdx = 1
    idx = idx + 1
  end
...
function Position:update(idx, rIdx, searchForward, foldedLine)
  local hit = self.nearestIdx == idx
    and self.nearestRelIdx == rIdx
    and self.searchForward == searchForward
    and self.foldedLine == foldedLine
...
function Position:cursorInRange(curPos)
  return utils.comparePosition(self.sList[self.nearestIdx], curPos) <= 0
    and utils.comparePosition(curPos, self.eList[self.nearestIdx]) <= 0
end
