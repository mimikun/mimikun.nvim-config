...
function def:definition_request(method, handler_T, args)
  if self.pending_request then
    vim.notify("[lspsaga] a peek_definition request has already been sent, please wait.", vim.log.levels.WARN)
    return
  end
  ...
  lsp.buf_request(current_buf, method, params, function(_, result, context)
    ...
    if not result or vim.tbl_count(result) == 0 then
      if #self.list == 0 and count == 0 then
        vim.notify("[lspsaga] response of request method " .. context.method .. " is empty", vim.log.levels.WARN)
      end
      return
    end
...
function def:peek_handler(result, context)
  ...
  api.nvim_win_set_cursor(node.winid, {
    ...
    lsp.util._get_line_byte_from_position(node.bufnr, node.selectionRange.start, client.offset_encoding),
  })
