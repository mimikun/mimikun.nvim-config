...
local function insert_elements(buf, node, elements)
  ...
  if config.color_mode then
    ...
    elements[#elements + 1] = string.format("%s%s#%s%sWord#%s", bar.prefix, type, icon, bar.prefix, node.name)
  end
end
...
local function file_bar(buf)
  ...
  if config.show_file then
    ...
    api.nvim_set_option_value("winbar", bar_prefix().prefix .. " #", { scope = "local", win = winid })
  end
end
...
local function init_winbar(buf)
  ...
  api.nvim_create_autocmd("User", {
    ...
    callback = function(opt)
      ...
      if vim.bo[opt.buf].buftype == "nofile" or curbuf ~= opt.data.bufnr or #opt.data.symbols == 0 then
