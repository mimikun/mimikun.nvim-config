...
function ot:parse(symbols, curline)
  ...
  if config.outline.layout ~= "float" then
    api.nvim_buf_attach(self.main_buf, false, {
      on_detach = function()
        local data = vim.version().minor > 10 and require("lspsaga.symbol.head") or require("lspsaga.symbol")
...
function ot:collapse(node, curlnum)
  ...
  while tmp do
    ...
    buf_set_lines(self.bufnr, row + 1, row + 1, false, { (" "):rep(tmp.value.inlevel) .. tmp.value.name })
    row = row + 1
    ...
    for j = 1, level - 4, 2 do
      ...
      if j + 2 > level - 4 and not has_child then
        virt[#virt + 1] = islast and { config.ui.lines[1], "SagaVirtLine" } or { config.ui.lines[2], "SagaVirtLine" }
        virt[#virt + 1] = { config.ui.lines[4]:rep(2), "SagaVirtLine" }
      else
        virt = { { config.ui.lines[3], "SagaVirtLine" } }
      end
...
function ot:refresh()
  api.nvim_create_autocmd("User", {
    ...
    callback = function(args)
      if not self.bufnr or not api.nvim_buf_is_valid(self.bufnr) or api.nvim_get_current_buf() ~= args.data.bufnr then
        return
      end
      api.nvim_set_option_value("modifiable", true, { buf = self.bufnr })
...
function ot:preview()
  api.nvim_create_autocmd("CursorMoved", {
    ...
    callback = function()
      ...
      local lines = api.nvim_buf_get_lines(self.main_buf, range.start.line, range["end"].line + 1, false)
...
function ot:keymap()
  ...
  util.map_keys(self.bufnr, config.outline.keys.jump, function()
    ...
    local pos = { node.value.selectionRange.start.line + 1, node.value.selectionRange.start.character }
    local callerwinid = self.callerwinid
...
function ot:outline(buf)
  ...
  local curline = api.nvim_win_get_cursor(0)[1]
  local res = not util.nvim_ten() and symbol:get_buf_symbols(buf) or require("lspsaga.symbol.head"):get_buf_symbols(buf)
