  ...
  for index, client_with_actions in ipairs(tuples) do
    ...
    if config.code_action.show_server_name == true then
      if type(client_with_actions[1]) == "string" then
        ...
        action_title = action_title .. "  (" .. lsp.get_client_by_id(client_with_actions[1]).name .. ")"
      end
    end
...
function act:code_action(options)
  if self.pending_request then
    vim.notify("[lspsaga] a code action has already been requested, please wait.", vim.log.levels.WARN)
    return
  end
