...
function M.indent_current(inlevel)
  ...
  for i = 0, api.nvim_buf_line_count(0) - 1 do
    vim.tbl_map(function(item)
      local hi = (item == current and i >= range[1] and i <= range[2]) and { link = "SagaInCurrent" } or to_normal_bg()
  ...
  api.nvim_set_decoration_provider(ns, {
    ...
    on_line = function(_, winid, bufnr, row)
      ...
      for i = 1, total, 2 do
        ...
        local virt = (row == currow and inlevel == 6 and i - 1 == 4) and config.ui.lines[2] or config.ui.lines[3]
