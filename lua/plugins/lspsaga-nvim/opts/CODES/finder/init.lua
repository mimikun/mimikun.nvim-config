...
function fd:event()
  api.nvim_create_autocmd("CursorMoved", {
    ...
    callback = function()
      ...
      local col = lsp.util._get_line_byte_from_position(node.value.bufnr, range.start, client.offset_encoding)
      ...
      if self.layout == "float" and config.ui.title and config.ui.border ~= "none" then
        rwin_conf.title = util.path_sub(api.nvim_buf_get_name(node.value.bufnr), client.config.root_dir)
      ...
      buf_add_highlight(
        ...
        range.start.line,
        col,
        lsp.util._get_line_byte_from_position(node.value.bufnr, range["end"], client.offset_encoding)
      )
...
function fd:toggle_or_open()
  util.map_keys(self.lbufnr, config.finder.keys["toggle_or_open"], function()
    ...
    local tmp = node.next
    while tmp do
      buf_set_lines(self.lbufnr, curlnum, curlnum, false, { (" "):rep(tmp.value.inlevel) .. tmp.value.line })
  ...
  for action, key in pairs(config.finder.keys) do
    util.map_keys(self.lbufnr, key, function()
      if not vim.tbl_contains(black, action) then
        ...
        local range = curnode.value.range or curnode.value.targetSelectionRange or curnode.value.selectionRange

        local pos = {
          range.start.line + 1,
          lsp.util._get_line_byte_from_position(curnode.value.bufnr, range.start, client.offset_encoding),
        }
...
function fd:new(args)
  ...
  if #methods == 0 then
    vim.notify(
      ("[lspsaga] no servers of buffer %s makes these methods available %s"):format(curbuf, table.concat(args, " ")),
      vim.log.levels.WARN
    )
    return
  end
