...
function ch:toggle_or_request()
  ...
  if curnode.value.expand == true then
    ...
    self:set_toggle_icon(config.ui.expand, curlnum - 1, curnode.value.inlevel - 4, curnode.value.virtid)
    vim.bo[self.left_bufnr].modifiable = true
  ...
  if curnode.value.expand == false then
    ...
    self:set_toggle_icon(config.ui.collapse, curlnum - 1, curnode.value.inlevel - 4, curnode.value.virtid)
...
function ch:peek_view()
  api.nvim_create_autocmd("CursorMoved", {
    ...
    callback = function()
      ...
      local col = lsp.util._get_line_byte_from_position(curnode.value.bufnr, range.start, client.offset_encoding)
      ...
      api.nvim_buf_add_highlight(
        ...
        range.start.line,
        col,
        lsp.util._get_line_byte_from_position(curnode.value.bufnr, range["end"], client.offset_encoding)
      )
...
function ch:type_hierarchy(item, client, timer_close, curlnum)
  ...
  util.client_request(client, self.method, { item = item }, function(_, res)
    ...
    for _, data in ipairs(res) do
      ...
      val.inlevel = #indent
      buf_set_lines(self.left_bufnr, curlnum, curlnum == 0 and -1 or curlnum, false, { indent .. data.name })
