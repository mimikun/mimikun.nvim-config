...
M.diff_file_list = async.wrap(function(adapter, left, right, path_args, dv_opt, opt, callback)
  ...
  local errors = {}
  (function()
    local err, tfiles, tconflicts =
      await(adapter:tracked_files(left, right, utils.vec_join(rev_args, "--", path_args), "working", opt))
...
local function is_diff_header(line)
  return line:match(DIFF_HEADER) ~= nil or line:match(DIFF_COMBINED_HEADER) ~= nil or line:match(DIFF_CC_HEADER) ~= nil
end
...
function M.parse_conflicts(lines, winid)
  ...
  local function handle(data)
    local first = math.min(data.ours.first or math.huge, data.base.first or math.huge, data.theirs.first or math.huge)
