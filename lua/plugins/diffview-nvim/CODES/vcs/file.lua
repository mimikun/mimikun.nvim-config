    ...
    if force or new_opt or not cur_state then
      ...
      for _, mapping in ipairs(state.keymaps) do
        ...
        for _, mode in ipairs(modes) do
          save_existing_keymap(self.bufnr, state.saved_keymaps, existing_maps_by_mode, mode, mapping[2])
        end
      ...
      if not vim.b[self.bufnr].diffview_buf_read_emitted and not vim.b[self.bufnr].diffview_disable_ts then
