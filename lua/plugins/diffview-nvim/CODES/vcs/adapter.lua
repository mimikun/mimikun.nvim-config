...
function VCSAdapter.build_top_indicators(path_args, cpath)
  ...
  table.insert(top_indicators, cpath and pl:realpath(cpath) or (vim.bo.buftype == "" and pl:absolute(cfile) or nil))
...
function VCSAdapter:exec_sync(args, cwd_or_opt)
  ...
  if not self.class.bootstrap.ok then
    logger:error(
      ("[VCSAdapter] Can't exec adapter command because bootstrap failed! Cmd: %s"):format(table.concat(cmd, " "))
    )
    return
  end
