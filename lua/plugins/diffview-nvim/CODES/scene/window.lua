...
function Window:is_file_open()
  return self:is_valid() and self.file and self.file:is_valid() and api.nvim_win_get_buf(self.id) == self.file.bufnr
end
...
Window.open_file = async.void(function(self)
  ...
  if recovered and not Window._set_buf_warned then
    ...
    utils.warn(
      "An external autocommand failed while opening a Diffview buffer. " .. "Diffview retried without window events.",
      true
    )
  end
