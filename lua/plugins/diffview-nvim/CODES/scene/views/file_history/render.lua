...
local function render_files(comp, files)
  ...
  for i, file in ipairs(files) do
    ...
    if file:is_null_entry() then
      comp:add_text("No diff", file.active and "DiffviewFilePanelSelected" or "DiffviewFilePanelFileName")
    else
      if file.status then
...
local formatters = {
  ...
  files = function(comp, entry, ctx)
    ...
    if entry.nulled then
      ...
      comp:add_text(
        fmt(" %s file%s", utils.str_left_pad(tostring(#entry.files), #s_num_files), #entry.files > 1 and "s" or " "),
  ...
  subject = function(comp, entry, ctx)
    local subject = utils.str_trunc(entry.commit.subject, ctx.conf.file_history_panel.commit_subject_max_length)
...
return {
  ...
  file_history_panel = function(panel)
    ...
    local root_path = panel.state.form == "column"
        and pl:truncate(pl:vim_fnamemodify(panel.adapter.ctx.toplevel, ":~"), math.max(panel:infer_width() - 6, 1))
