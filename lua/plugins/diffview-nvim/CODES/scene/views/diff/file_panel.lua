...
local function build_file_tree(tree, tree_options)
  ...
  return utils.tbl_merge({ name = "files" }, tree:create_comp_schema({ flatten_dirs = tree_options.flatten_dirs }))
end
...
function FilePanel:highlight_prev_file()
  ...
  pcall(api.nvim_win_set_cursor, self.winid, { self.constrain_cursor(self.winid, -vim.v.count1), 0 })
  utils.update_win(self.winid)
end
