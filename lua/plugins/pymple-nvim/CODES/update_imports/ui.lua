...
function M.get_confirmation_dialog(number_of_imports, number_of_files, on_submit, on_close)
  ...
  local menu = Menu({
    ...
    border = {
      style = "rounded",
      text = {
        top = string.format("[pymple.nvim] Update %s imports in %s files?", number_of_imports, number_of_files),
        top_align = "center",
      },
    },
    win_options = {
...
local function pad_and_suffix_lines(lines, length, suffix)
  local padded_lines = {}
  for _, line in ipairs(lines) do
    table.insert(padded_lines, line .. string.rep(" ", length - #line + 1) .. suffix)
  end
  return padded_lines
end
...
function PreviewWindow:add_result_highlights(icon_hl, line_nr, file_count_len, icon_len, num_lines)
  -- file count
...
function PreviewWindow:render_filepath(file_path, line_count, icon, res_number, res_line_num)
  local res_count = res_number .. "/" .. self.num_results
  local original_file_path_key = make_file_path_key(file_path, res_line_num)
  local maybe_shortened_key = utils.shorten_path(original_file_path_key, self.size.width - 2 - #res_count - #icon - 2)
  ...
  vim.api.nvim_buf_set_lines(self.bufnr, line_count + 1, -1, false, {
    SEPARATORS.above .. string.rep(SEPARATORS.line, self.size.width - 2) .. SEPARATORS.above_suffix,
  })
  return line_count + 2
end
...
function PreviewWindow:render_bottom_separator(line_count)
  vim.api.nvim_buf_set_lines(self.bufnr, line_count, -1, false, {
    SEPARATORS.below .. string.rep(SEPARATORS.line, self.size.width - 2) .. SEPARATORS.below_suffix,
  })
  return line_count + 1
end
...
function PreviewWindow:render_result(result, line_count, res_num)
  ...
  if icons then
    icon, icon_hl = icons.get_icon(result.file_path, vim.fn.fnamemodify(result.file_path, ":e"))
  end
  line_count = self:render_filepath(result.file_path, line_count, icon, res_num, result.line_num)
  ...
  vim.api.nvim_buf_set_lines(self.bufnr, line_count + #result.lines_before + #result.lines_after + 3, -1, false, { "" })
  ...
  self:add_result_highlights(icon_hl, line_start, #(res_num .. self.num_results), #icon, #result.lines_before)
end
    ...
    local result = {
      ...
      lines_before = pad_and_suffix_lines(
        udim_utils.truncate_lines(
          prefix_lines(utils.split_string(rj_result.line_before, "\n"), SEPARATORS.diff_prefix .. " "),
          self.size.width - 2
        ),
        self.size.width,
        SEPARATORS.diff_prefix
      ...
      lines_after = pad_and_suffix_lines(
        udim_utils.truncate_lines(
          prefix_lines(utils.split_string(rj_result.line_after, "\n"), SEPARATORS.diff_prefix .. " "),
          self.size.width - 2
        ),
        self.size.width,
        SEPARATORS.diff_prefix
...
function PreviewWindow:ignore_result()
  ...
  if self.ignored_paths[res_key] then
    ...
    self:add_result_highlights(data.icon_hl, data.line_start, data.file_count_len, data.icon_len, data.num_lines)
  else
    self.ignored_paths[res_key] = true
    vim.api.nvim_buf_clear_highlight(self.bufnr, udim_highlights.preview_window_ns, data.line_start, data.line_end)
    for i = data.line_start, data.line_end - 1 do
      vim.api.nvim_buf_add_highlight(self.bufnr, udim_highlights.preview_window_ns, "PymplePreviewIgnored", i, 0, -1)
    end
  end
end
...
function M.get_preview_window(r_jobs, num_results, num_files)
  ...
  local height = math.min(5 * num_results + 2, editor_size.height - 2 * PREVIEW_WINDOW_H_OFFSET)
  ...
  local preview_window = PreviewWindow({
    ...
    border = {
      style = "rounded",
      text = {
        top = string.format(" [pymple.nvim] Preview changes (%s results in %s files) ", num_results, num_files),
        top_align = "center",
