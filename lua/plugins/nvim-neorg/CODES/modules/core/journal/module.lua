...
module.public = {
  ...
  create_toc = function()
    ...
    local get_fs_handle = function(path)
      path = path or ""
      local handle = vim.loop.fs_scandir(workspace_path .. config.pathsep .. folder_name .. config.pathsep .. path)
              ...
              while true do
                ...
                if dtype == "file" and string.match(dname, "%d%d%.norg") then
                  ...
                  vim.schedule(function()
                    ...
                    local title = get_title(name .. config.pathsep .. mname .. config.pathsep .. dname) or file[1]
