...
module.load = function()
  for _, task in ipairs({
    ...
    vim.keymap.set("", string.format("<Plug>(neorg.qol.todo-items.todo.task-%s)", task), module.public["task-" .. task])
  end
end
...
module.private = {
  ...
  fire_update_event = function(char, line)
    local ev = modules.create_event(module, module.events.defined["todo-changed"].type, { char = char, line = line })
    if ev then
  ...
  make_all = function(buf, node, todo_item_type, char)
    ...
    if not first_status_extension then
      ...
      vim.api.nvim_buf_set_text(buf, range.row_start, range.column_start, range.row_end, range.column_end, { char })
    end
...
module.public = {
  ...
  ["task-cycle"] = function()
    ...
    module.private.task_cycle(buffer, cursor[1], module.config.public.order, module.config.public.order_with_children)
  end,
