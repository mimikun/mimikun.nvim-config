...
module.public = {
    ...
    vim.api.nvim_create_autocmd({ "TextChanged", "TextChangedI" }, {
      ...
      callback = function()
        ...
        if cursor_pos[1] > extmark_begin[1] and cursor_pos[1] <= (extmark_end[1] + 1) then
          ...
          if
            not module.required["core.integrations.treesitter"].find_parent(current_node, "^ranged_verbatim_tag.*")
          then
            vim.api.nvim_buf_delete(target, { force = true })
            vim.api.nvim_buf_clear_namespace(source, namespace, 0, -1)
            return true
          ...
          if cursor_pos[1] - extmark_begin[1] > target_line_count then
            ...
            vim.api.nvim_win_set_cursor(target_window, { cursor_pos[1] - extmark_begin[1] - 1, cursor_pos[2] })
          end
        end
      end,
    })
  ...
  magnify_code_block = function()
    ...
    if not vsplit then
      utils.notify("Unable to magnify current code block because our split didn't want to open :(", vim.log.levels.WARN)
      return
    end
