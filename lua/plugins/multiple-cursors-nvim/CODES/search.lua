...
local function is_past_end(limit, visual_area_end, match)
  ...
  if limit == 1 then
    ...
    if match[1] > visual_area_end[1] or (match[1] == visual_area_end[1] and match[2] > visual_area_end[2] + 1) then
      return true
    end
  end
...
function M.get_matches_and_move_cursor(word, limit_to_visible, limit_to_prev_visual_area)
  ...
  for idx, match in ipairs(matches) do
    ...
    if match[1] == cursor_pos[2] then
      ...
      if cursor_pos[3] >= match[2] and cursor_pos[3] < match[2] + #word or cursor_pos[3] < match[2] then
