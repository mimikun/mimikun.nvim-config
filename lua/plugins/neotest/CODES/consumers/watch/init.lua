...
local init = function(client)
  client.listeners.discover_positions = function(_, tree)
    for _, watcher in pairs(watchers) do
      if watcher.tree:data().path == tree:data().path and not watcher.discover_positions_event.is_set() then
...
local function get_valid_client(bufnr)
  ...
  for _, client in ipairs(clients) do
    ...
    if nio.fn.has("nvim-0.10.1") == 0 then
      ...
      has_definition_support = client.server_capabilities ~= nil and client.server_capabilities.definitionProvider
    else
