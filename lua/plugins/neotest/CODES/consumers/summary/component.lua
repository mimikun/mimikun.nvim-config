...
function SummaryComponent:_render(canvas, tree, expanded, focused, indent)
  ...
  for index, node in ipairs(children) do
    ...
    canvas:add_mapping("next_failed", function()
      ...
      local extmarks = vim.api.nvim_buf_get_extmarks(0, namespace, { row + 1, 0 }, { -1, 0 }, { details = true })
    ...
    canvas:add_mapping("prev_failed", function()
      local row = vim.fn.line(".") - 1
      local extmarks = vim.api.nvim_buf_get_extmarks(0, namespace, { 0, 0 }, { row, 0 }, { details = true })
...
function SummaryComponent:_state_icon(status)
  ...
  return config.icons.running_animated[(self.renders % #config.icons.running_animated) + 1], config.highlights.running
end
