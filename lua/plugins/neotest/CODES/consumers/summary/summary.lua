...
function Summary:_write_header(canvas, adapter_id, tree)
  canvas:write(vim.split(adapter_id, ":", { trimempty = true })[1], { group = config.highlights.adapter_name })
    ...
    while true do
      ...
      if self._starting then
        for _, adapter_id in ipairs(self.client:get_adapters()) do
          ...
          self.components[adapter_id] = self.components[adapter_id] or SummaryComponent(self.client, adapter_id)
