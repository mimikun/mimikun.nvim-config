      ...
      if not adapter_id then
        for a_id, _ in pairs(self._adapters) do
          ...
          if vim.startswith(file_path, root:data().path) then
            logger.info("Not updating positions for", file_path, "as it's in a known directory and no adapter matched")
            return
          end
        end
...
function neotest.Client:_update_adapters(dir)
  local adapters_with_root = lib.files.is_dir(dir) and self._adapter_group:adapters_with_root_dir(dir) or {}

  local adapters_with_bufs = self._adapter_group:adapters_matching_open_bufs(lib.func_util.map(function(i, entry)
    return i, entry.root
