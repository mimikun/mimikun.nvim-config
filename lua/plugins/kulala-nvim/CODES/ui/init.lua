...
local open_kulala_buffer = function(filetype)
  ...
  if not buf then
    ...
    vim.iter(bo):each(function(key, value)
      ...
      if not status then
        Logger.error("Failed to set buffer option `" .. key .. "`: " .. (error or ""))
      end
    end)
...
local function open_kulala_window(buf)
  ...
  vim.iter(wo):each(function(key, value)
    ...
    if not status then
      Logger.error("Failed to set window option `" .. key .. "`: " .. (error or ""))
    end
  end)
...
local function hide_progress()
  ...
  if footer > 0 then
    vim.api.nvim_buf_delete(footer, { force = true })
  end
end

local function show_progress()
  if #CMD.queue.tasks == 0 then
...
local function show(contents, filetype, mode)
  ...
  if MARKDOWN_VIEWS[mode] then
    contents = require("kulala.ui.markdown").normalize_headings(contents)
  end
...
local function get_ft_from_kulala_core(r)
  if not r._kulala_core or not r._kulala_body_type then
    return "text"
  end
  ...
  if type(r._kulala_media_type) == "string" and r._kulala_media_type ~= "" then
    if r._kulala_media_type:find("json") then
      return "json"
    end
    if r._kulala_media_type:find("xml") then
      return "xml"
    end
  end
  return "text"
...
local function ws_live_body_and_ft()
  local WEBSOCKET = require("kulala.cmd.websocket")
  if not WEBSOCKET.response then
    return "", "text"
  end
...
local function ws_filter_response(response)
  if not is_ws_method(response.method) then
    return nil
  end
  local WEBSOCKET = require("kulala.cmd.websocket")
  if WEBSOCKET.response and WEBSOCKET.response.id == response.id then
    return WEBSOCKET.response
  end
  if type(response._ws_messages) == "table" then
    return response
  end
  return nil
end
...
M.toggle_filter = function()
  ...
  vim.ui.input({ prompt = "JQ Filter: ", default = response.filter or "" }, function(filter)
    ...
    if filter == "" then
      ...
      if type(response.body_raw) == "string" and response.body_raw ~= "" then
        ...
        if restored then
          ...
          if restored.body_type then
            response._kulala_body_type = restored.body_type
          end
          if restored.media_type then
            response._kulala_media_type = restored.media_type
          end
        else
    ...
    if Ext_processing.jq(filter, response, cwd) then
      M.show_body()
    end
  end)
end
...
M.jump_to_response = function()
  ...
  elseif win > 0 then
    ...
    if kulala_win and vim.api.nvim_win_is_valid(kulala_win) then
      if WEBSOCKET.is_active() and is_ws_method(response.method) then
        return
      end
      if vim.api.nvim_win_get_config(kulala_win).relative == "editor" then
        vim.api.nvim_win_close(kulala_win, true)
      end
    end
  end
end
...
M.open_default_view = function()
  ...
  local status, errors = xpcall(function()
    if open_view then
      open_view(get_current_response())
    end
  end, debug.traceback)

  if not status then
    Logger.error("Errors displaying response: " .. (errors or ""), 1)
  end
end
...
M.open_all = function(_, line_nr)
  if not is_initialized() then
    return Logger.error("Kulala setup is not initialized. Check the config.")
  end
...
M.jump_next = function()
  ...
  if next then
    vim.api.nvim_win_set_cursor(0, { next.start_line, 0 })
  end
end
...
M.jump_prev = function()
  ...
  if prev then
    vim.api.nvim_win_set_cursor(0, { prev.start_line, 0 })
  end
end
...
M.interrupt_requests = function()
  ...
  if current and current.method == "WS" then
    return require("kulala.cmd.websocket").close()
  end
  ...
  if #CMD.queue.tasks == 0 and CMD.queue.status ~= "running" then
    hide_progress()
  end
end
