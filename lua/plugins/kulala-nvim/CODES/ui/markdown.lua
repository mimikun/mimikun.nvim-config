...
function M.md_table(rows, align)
  ...
  local function fmt_sep(col_idx)
    local w = col_width(col_idx)
    if align == "r" and col_idx > 1 then
      return fmt_cell(string.rep("-", w - 1) .. ":", col_idx)
    end
...
function M.response_headers_source(r)
  if type(r.headers) == "string" and M.trim(r.headers) ~= "" then
    return r.headers
  end
  if type(r.headers_tbl) == "table" and next(r.headers_tbl) then
    return r.headers_tbl
  end
  return nil
end
...
function M.headers_rows(headers)
  ...
  if type(headers) == "string" then
    for line in vim.gsplit(headers, "\n", { plain = true, trimempty = true }) do
      ...
      if k then
        table.insert(rows, { M.md_table_cell(k), M.md_escape_cell(v) })
      end
    end
    return rows
  end
...
function M.pretty_maybe_json(s)
  ...
  if t ~= nil then
    return vim.json.encode(t, { indent = indent_chars, sort_keys = config_sort_keys })
  end
  return s
end
...
---@param lang string
---@param body string
---@param prettify boolean|nil whether to attempt pretty-printing (e.g. for JSON)
---@return string
function M.fenced(lang, body, prettify)
  body = body or ""
  if body == "" then
    return "_Empty_\n"
  end
  ...
  if prettify and lang == "json" then
    body = M.pretty_maybe_json(body)
  end
  return fence .. lang .. "\n" .. body .. "\n" .. fence .. "\n"
end
...
function M.get_body_ft(media_type, kulala_body_type)
  ...
  if type(media_type) == "string" and media_type ~= "" then
    if media_type:find("xml", 1, true) then
      return "xml"
    end
  end
  return "text"
...
---@return string body
---@return string ft
function M.get_body_and_guess_ft(r, body)
  body = body or r.body or ""
  local content_type = M.get_body_ft(r._kulala_media_type, r._kulala_body_type)
...
function M.format_connection_trace(trace)
  ...
  local function trace_header_rows(block)
    ...
    for line in vim.gsplit(block, "\n", { plain = true }) do
      ...
      if text:match("^HTTP/") then
        ...
        if k then
          table.insert(rows, { M.md_table_cell(k), M.md_table_cell(v) })
        end
      end
    end
  ...
  if conn ~= "" then
    ...
    for line in vim.gsplit(conn, "\n", { plain = true }) do
      local text = M.trim(line)
      if text ~= "" then
        table.insert(parts, ("- %s"):format(M.md_escape_cell(text)))
      end
    end
    table.insert(parts, "")
  end
  ...
  if req_hdr ~= "" then
    ...
    if status then
      table.insert(parts, ("**Request line:** %s\n"):format(M.md_escape_cell(status)))
    end
    if #rows > 1 then
      table.insert(parts, M.md_table(rows) .. "\n")
    end
  end
...
function M.format_body_view(body, r)
  ...
  local content, lang
  if r then
    content, lang = M.get_body_and_guess_ft(r, body)
  else
    content = body
  end
...
local function script_console_display_path(origin_file, request_file)
  if type(origin_file) ~= "string" or origin_file == "" then
    return "?"
  end
  if type(request_file) ~= "string" or request_file == "" then
    return vim.fn.fnamemodify(origin_file, ":t")
  end
  if paths_equal(origin_file, request_file) then
    return nil
  end
  ...
  if type(rel) == "string" and rel ~= "" and rel ~= "." then
    return rel
  end
...
local function resolve_request_file(r)
  ...
  if type(r.file) == "string" and vim.trim(r.file) ~= "" then
    return r.file
  end
  if type(r.buf_name) == "string" and vim.trim(r.buf_name) ~= "" then
    return r.buf_name
  end
  return nil
end
...
local function script_console_is_pre(origin)
  ...
  if ph == nil or type(ph) ~= "string" and type(ph) ~= "number" then
    return false
  end
  ph = tostring(ph)
...
local function format_script_console_origin(origin, request_file)
  ...
  if type(origin.line) == "number" then
    ...
    if type(origin.column) == "number" then
      loc = loc .. ":" .. tostring(origin.column)
    end
  else
    loc = "directive L" .. tostring(origin.httpDirectiveLine or origin["httpDirectiveLine"] or "?")
  end
  if fp then
    return ("[%s · %s · %s]"):format(src, fp, loc)
  end
