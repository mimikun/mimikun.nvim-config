...
local function hide_response_summary()
  ...
  if not config.show_request_summary then
    return
  end
...
local function format_assert_output(response, show_asserts)
  ...
  vim.iter(results):each(function(assert)
    ...
    if assert.status and show_asserts == "failed_only" then
      return
    end
...
local function generate_requests_report()
  ...
  if config.show_summary then
    table.insert(parts, format_report_summary(stats or {}))
  end
...
local function print_http_spec(spec, curl)
  ...
  if #spec.cookie > 0 then
    table.insert(lines, "Cookie: " .. spec.cookie)
  end
