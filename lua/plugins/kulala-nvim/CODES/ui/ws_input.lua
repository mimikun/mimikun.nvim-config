...
local function kulala_keymap_modes(name)
  local maps = KEYMAPS.get_kulala_keymaps() or {}
  local map = maps[name]
  if not map or not map.mode then
    return { "n", "i", "v" }
  end
...
function M.send_and_close()
  if opening or not M.is_open() then
    return
  end
...
function M.can_open_from_body()
  if not WEBSOCKET.is_active() then
    return false
  end
...
function M.open()
  ...
  if not M.can_open_from_body() then
    return
  end
