...
M.jq = function(filter, response, cwd, opts)
  opts = opts or {}
  if type(filter) ~= "string" or vim.trim(filter) == "" then
    return false
  end
  ...
  if not filtered then
    if not opts.silent then
      Logger.error(tostring(err and err ~= "" and err or "Failed to filter with jq"))
    end
    return false
  end
  if filtered.text == "" or vim.trim(filtered.text) == "null" then
    return false
  end
  ...
  if filtered.body_type then
    response._kulala_body_type = filtered.body_type
  end
  if filtered.media_type then
    response._kulala_media_type = filtered.media_type
  end
  return true
end
