...
local function code_actions()
  if state.current_ft == "http" or state.current_ft == "rest" then
    return code_actions_http()
  end
  return {}
end
...
local function folding()
  if not vim.api.nvim_buf_is_loaded(state.current_buffer) then
    return {}
  end

  local status, parser = pcall(vim.treesitter.get_parser, state.current_buffer, "kulala_http")
  if not (status and parser) then
    return {}
  end
  ...
  local function server(dispatchers)
    ...
    function srv.request(method, params, handler, notify_reply_callback)
      ...
      handler = function(err, result)
        if responded then
          return
        end -- the error path below may also respond
        responded = true
...
function M.start_lsp(buf, ft)
  ...
  local client_id = vim.lsp.start({
    ...
    on_attach = function(client, bufnr)
      ...
      if Config.options.lsp.on_attach then
        Config.options.lsp.on_attach(client, bufnr)
      end
    end,
