...
local function build_ws_jq_object_source(messages, initial_message)
  ...
  for _, raw in ipairs(messages) do
    local ok, parsed = pcall(vim.json.decode, raw)
    if ok and type(parsed) == "table" and not vim.islist(parsed) then
      table.insert(objects, parsed)
    end
  end
  if #objects == 0 and type(initial_message) == "string" and initial_message ~= "" then
    ...
    if ok and type(parsed) == "table" and not vim.islist(parsed) then
      return vim.json.encode(parsed), true
    end
  end
  if #objects == 0 then
    return "", false
  ...
  if #objects == 1 then
    return vim.json.encode(objects[1]), true
  end
...
local function adapt_ws_jq_filter(filter, body_raw)
  ...
  if type(body_raw) ~= "string" or vim.trim(body_raw):sub(1, 1) ~= "[" then
    return filter
  end
  ...
  if filter:match("^%.%.") or filter:match("^%.%s*%[") then
    return filter
  end
  if filter == "." or filter:match("^%.[%a_]") then
    return string.format("%s | %s)", objects, filter)
  end
  return filter
end
...
function M.connect(request, response, callback, opts)
  ...
  if not status then
    return Logger.error("Failed to connect to websocket:\n" .. result, 2)
  end
  if not result then
    return Logger.error("kulala-core WebSocket failed to start", 2)
  end
