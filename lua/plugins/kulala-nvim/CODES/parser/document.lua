...
local function set_core_parse_diagnostics(bufnr, doc)
  ...
  if not doc or not doc.hasErrors then
    return
  end
...
M.get_document = function(lines, path)
  ...
  if not path then
    set_core_parse_diagnostics(buf, doc)
  end

  local proto_err = DocCore.unsupported_protocol_error(doc)
  if proto_err then
    Logger.warn(proto_err)
  end
...
local function apply_shared_data(shared, request)
  ...
  vim.iter(shared.variables):each(function(k, v)
    if not request.variables[k] then
...
local function get_run_requests(request, line)
  ...
  if not (request_name or file) then
    return {}
  end
...
M.get_request_at = function(requests, linenr)
  local status, result = xpcall(function()
    if not linenr then
      return expand_nested_requests(requests[1])
    end
    if linenr == 0 then
      return expand_nested_requests(requests)
    end
