...
local function hover_plaintext_value(hover)
  if type(hover) ~= "table" or type(hover.contents) ~= "table" then
    return nil
  end
  local contents = hover.contents
  if contents.kind ~= "plaintext" or type(contents.value) ~= "string" then
    return nil
  end
      ...
      callback = function()
        ...
        vim.schedule(function()
          timer = vim.fn.timer_start(1000, function()
            if not cursor_inside_variable_template() then
              return
            end
            KULALA_CORE.lsp_hover_async(0, function(hover, err)
              if err or not hover then
                return
              end
