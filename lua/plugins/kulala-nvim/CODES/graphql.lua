...
local function graphql_host_at_cursor()
  ...
  if not req or (req.method or ""):upper() ~= "GRAPHQL" then
    return nil
  end
...
M.download_schema = function()
  if not KULALA_CORE.enabled() then
    return Logger.error("kulala-core is required for GraphQL schema download")
  end

  local result, err = KULALA_CORE.graphql_introspect()
  if not result then
    return Logger.error(err or "Failed to download GraphQL schema")
  end
  if result.ok ~= true then
    return Logger.error(result.error or err or "Failed to download GraphQL schema")
  end
...
M.clear_schema_cache = function(host)
  if not KULALA_CORE.enabled() then
    return Logger.error("kulala-core is required to clear GraphQL schema cache")
  end

  if not host or host == "" then
    host = graphql_host_at_cursor()
  end

  local ok, err, res = KULALA_CORE.clear_graphql_schema(host)
  if not ok then
    return Logger.error(err or "Failed to clear GraphQL schema cache")
  end
