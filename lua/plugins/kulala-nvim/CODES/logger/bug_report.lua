...
M.create_issue = function(title, body, labels, type)
  if vim.fn.executable("gh") == 0 then
    return Logger.warn("Please install GitHub CLI to create issues.")
  end
  ...
  if not result or result.stderr ~= "" then
    return
  end
  ...
  if link then
    Logger.info("Issue created successfully: " .. link)
  end
...
local function get_current_request()
  ...
  if not request or not buf then
    return
  end
