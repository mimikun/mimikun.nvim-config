...
M.set_autocomands = function()
  vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function(ev)
      if not (Parser.is_up_to_date() and Backend.is_up_to_date() and M.options.lsp.enable) then
        return
      end
      ...
      if script_fts[ft] and not Fs.is_http_script_file(ft, ev.buf) then
        return
      end
...
M.setup = function(config)
  ...
  if M.options.show_icons == "signcolumn" then
    pcall(set_signcolumn_icons)
  end
