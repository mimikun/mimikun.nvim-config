...
M.register_parser = function()
  ...
  if Fs.dir_exists(parser_source_path) then
  ...
  if not Api.has_triggered_ready() and backend.is_up_to_date() then
    Api.trigger("ready")
  end
