...
M.create = function(text, opts)
  ...
  if vim.fn.bufnr(opts.name) == -1 then
  ...
  if opts.hl_group then
    Utils.highlight_range(buf, 0, 0, 0, opts.hl_group)
  end

  if opts.auto_size then
    set_autosize(text, opts)
  end

  local win = vim.api.nvim_open_win(buf, opts.focusable, float_defaults(opts))

  if opts.auto_close then
    set_autoclose(opts.buf, win)
  end
  if opts.close_keymaps then
    set_close_keymaps(buf, win, opts.close_keymaps)
  end
  if opts.bo then
    set_buffer_options(buf, opts.bo)
  end
  if opts.wo then
    set_window_options(win, opts.wo)
  end
...
M.create_progress_float = function(text, opts)
  ...
  timer:start(0, 500, function()
    ...
    vim.schedule(function()
      ...
      if opts.hl_group then
        Utils.highlight_range(float.buf, 0, 0, 0, opts.hl_group)
      end
    end)
  end)
