...
M.toggle_winbar_tab = function(buf, win_id, view)
  ...
  if not (win_id and config.winbar) then
    return UI_utils.set_virtual_text(buf, 0, "? - help", 0, 0)
  end
  ...
  local winbar_title = {}
  if start_idx > 1 then
    table.insert(winbar_title, "%#KulalaTab#...%*")
  end

  for idx = start_idx, end_idx do
    if raw_tabs[idx] then
      table.insert(winbar_title, raw_tabs[idx].display)
    end
  end
