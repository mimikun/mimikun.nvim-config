...
local function edit_env_file(config, picker, name)
  ...
  if not config or not file then
    return
  end
    ...
    .new({}, {
      ...
      attach_mappings = function(prompt_bufnr, map)
        local function run_cmd(cmd)
          ...
          if commands[cmd][1](selection.value, picker) == true then
            return
          end
...
local function open_auth_snacks()
  ...
  local run_cmd = function(key, ctx, item)
    item = item or {}
    if commands[key][1](item.text, ctx) == true then
      return
    end
    ctx:close()
    M.open_auth_config()
  ...
  if not env_file_path then
    return Logger.warn("http-client.env.json not found")
  end
