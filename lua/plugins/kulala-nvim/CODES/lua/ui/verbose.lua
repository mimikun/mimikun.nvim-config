...
local function format_hop_body(body)
  ...
  if body.type == "json" and body.content ~= nil then
    ...
    if ok and s then
      return Markdown.pretty_maybe_json(s), "json"
    end
    return vim.inspect(body.content), "text"
  end
  if body.type == "text" and body.content then
    return Markdown.fenced("text", body.content), "text"
  end
  return "", "text"
end
...
local function format_hop(hop, index, title)
  ...
  if hop.url then
    table.insert(parts, ("**URL:** %s\n"):format(Markdown.md_escape_cell(hop.url)))
  end
...
local function redirect_chain_without_final(chain)
  if not vim.islist(chain) or #chain < 2 then
    return nil
  end
  local out = {}
  for i = 1, #chain - 1 do
...
function M.format(r)
  ...
  if script ~= "_No script output_\n" then
    Markdown.add_section(parts, "## Script output\n", script)
  end
