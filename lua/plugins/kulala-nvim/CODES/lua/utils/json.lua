...
M.parse = function(str, opts, filename)
  ...
  if not status then
    if opts.verbose then
      Logger.error(("Failed to parse %s: %s"):format(filename or "JSON", result))
    end
    return nil, result
  end
...
M.encode = function(obj, opts)
  ...
  if not (opts.sort and type(obj) == "table" and not vim.islist(obj)) then
    return vim.json.encode(obj, opts)
  end
