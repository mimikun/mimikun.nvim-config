...
local function to_query(key, value)
  if type(value) == "table" and not vim.islist(value) then
    return "nested tables not supported"
  end
...
local function add_client_credentials(config_id, body, headers)
  ...
  if not validate_auth_params(config_id, required_params) then
    return body, headers
  end
...
local function add_custom_params(config_id, body, use)
  ...
  vim.iter(custom_params):each(function(key, value)
    ...
    if _use == use or _use == "Everywhere" then
      body = body .. to_query(key, _value)
    end
  end)
...
M.get_device_code = function(config_id)
  local config = get_auth_config(config_id)
  if not validate_auth_params(config_id, { "Client ID", "Device Auth URL" }) then
    return
  end
...
M.verify_device_code = function(config_id)
  local config = get_auth_config(config_id)
  if not validate_auth_params(config_id, { "verification_url", "user_code" }, "auth_data") then
    return
  end

  local browser, status = vim.ui.open(config.auth_data.verification_url)
  if not browser then
    return Logger.error("Failed to open browser: " .. status)
  end
...
local function poll_token_server(config, url, body, headers)
  ...
  for count = 1, tries do
    ...
    if not out and not err:match("authorization_pending") and not err:match("slow_down") then
      break
    end
    out = out or {}
...
M.acquire_device_token = function(config_id)
  ...
  if not device_code or not validate_auth_params(config_id, required_params) then
    return
  end
  ...
  if not out.access_token then
    return Logger.error("Timeout acquiring device token for config: " .. config_id)
  end
...
M.create_JWT = function(config_id)
  local config = get_auth_config(config_id)
  if not validate_auth_params(config_id, { "Grant Type", "JWT" }) then
    return
  end
...
M.acquire_jwt_token = function(config_id)
  ...
  if not assertion or not validate_auth_params(config_id, { "Grant Type", "Token URL" }) then
    return
  end
...
M.acquire_client_credentials = function(config_id)
  ...
  if type == "jwt" then
    return M.acquire_jwt_token(config_id)
  end
...
M.acquire_auth = function(config_id)
  ...
  if not launch_browser(config["Browser CMD"], uri, config["Redirect URL"]) then
    return
  end

  local code = M.receive_code(config_id)
  if not code then
    return Logger.error("Failed to acquire code for config: " .. config_id)
  end
...
M.acquire_token = function(config_id)
  ...
  if config["Grant Type"] == "Client Credentials" then
    return M.acquire_client_credentials(config_id)
  end
  if config["Grant Type"] == "Password" then
    return M.acquire_password_token(config_id)
  end

  local code = M.acquire_auth(config_id)
  if config["Grant Type"] == "Implicit" then
    return code
  end

  local required_params = { "Client ID", "Redirect URL", "Token URL" }
  if not code or not validate_auth_params(config_id, required_params) then
    return
  end
...
local function refresh_token_co(config_id)
  if not validate_auth_params(config_id, { "Grant Type" }) then
    return
  end
  ...
  if not refresh_token then
    return M.acquire_token(config_id)
  end

  if not validate_auth_params(config_id, { "Client ID", "Token URL" }) then
    return
  end
...
M.get_token = function(type, config_id)
  if not config_id then
    return Logger.error("Auth config key not found.")
  end
...
M.revoke_token = function(config_id)
  ...
  if not token then
    return Logger.info("No token to revoke for config: " .. config_id)
  end
...
M.is_token_expired = function(config_id, type)
  ...
  if not acquired_at or not expires_in then
    return true
  end
