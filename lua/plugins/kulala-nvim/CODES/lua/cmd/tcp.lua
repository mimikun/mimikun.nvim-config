...
M.server = {
  run = function(self, host, port, on_request)
    ...
    status, err = server:listen(128, function(listen_err)
      if listen_err then
        return Logger.error("TCP server: failed to accept connection: " .. listen_err)
      end
      ...
      client:read_start(function(read_err, chunk)
        if read_err then
          return Logger.error("TCP server: failed to process request: " .. read_err)
        end
        if not chunk then
          return self:stop(client)
        end
