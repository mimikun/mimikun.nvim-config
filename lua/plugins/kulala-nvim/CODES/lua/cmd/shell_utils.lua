...
M.run = function(cmd, opts, on_exit)
  if vim.fn.executable(cmd[1]) == 0 then
    return Logger.error("Command not found: " .. table.concat(cmd, " "))
  end
    ...
    return vim.system(cmd, opts, function(system)
      if system.code ~= 0 or (opts.abort_on_stderr and system.stderr ~= "") then
        ...
        if opts.verbose then
          Logger.error(opts.err_msg .. "Code: " .. system.code .. ", " .. err_msg, 2)
        end
        if opts.on_error then
          opts.on_error(system)
        end
        return
      end
  ...
  if not status then
    return Logger.error(opts.err_msg .. result, 2)
  end
