...
local queue = {
  ...
  interrupt_progressive = function(self)
    if self:cancel_last_pending() then
      return true
    end
...
function queue.run_next(self)
  if #self.tasks == 0 or self.status == "paused" then
    return
  end
...
local function kulala_core_cursor_line(parsed_request)
  ...
  if type(ln) == "number" and ln > 0 and ln >= lo and ln <= hi then
    return ln
  end
...
local function process_metadata(request, response)
  ...
  for _, metadata in ipairs(request.metadata) do
    ...
    if processor then
      INT_PROCESSING[processor](metadata.value, response)
    end

    processor = ext_meta_processors[metadata.name]
    if processor then
      EXT_PROCESSING[processor](metadata.value, response)
    end
  end
end
...
local function apply_script_console_to_response(request_status, request, response)
  ...
  if request_status then
    request_status._kulala_script_console = nil
  end
  if request then
...
local function inject_payload(errors, request)
  ...
  if #vim.trim(body) > 0 then
    table.insert(lines, lnum + 1, "> Payload:\n\n" .. body .. "\n")
  end
...
local function save_response(request_status, parsed_request)
  ...
  if method_upper == "WS" or method_upper == "WSS" or method_upper == "WEBSOCKET" then
    ...
    if ws_response and ws_response.id == id then
      ...
      if existing == ws_response then
        ...
        if not in_list then
          table.insert(responses, ws_response)
        end
      end
      return existing
    end
  ...
  if #response.body == 0 and response.method ~= "GRPC" then
    response.headers = "Content-Type: text/plain"
  end
  if #response.body == 0 then
    response.body = "No response body (check Verbose output)"
  end
...
local function process_errors(request, request_status, processing_errors)
  ...
  if processing_errors then
    Logger.error(processing_errors, 2, { report = true })
  end
...
local function handle_response_impl(request_status, parsed_request, callback, advance_queue, invoke_ui_callback)
  ...
  if not (code and processing_status) then
...
local function kulala_core_apply_sent_request(item, target)
  ...
  if type(sent) ~= "table" and type(item.url) ~= "string" then
    return
  end

  if type(sent) == "table" then
    if type(sent.method) == "string" and sent.method ~= "" then
      target.method = sent.method
    end
    if type(sent.url) == "string" and vim.trim(sent.url) ~= "" then
      target._kulala_sent_url = sent.url
    end
    if type(sent.headers) == "table" then
      target.headers = sent.headers
    end
    if type(sent.body) == "string" then
      target.body = sent.body
    end
  end
...
local function kulala_core_display_body(item)
  ...
  if type(item.filteredBody) == "table" then
    return kulala_core_body_text(item.filteredBody), item.filteredBody
  end
  if type(item.body) == "table" then
    return kulala_core_body_text(item.body), item.body
  end
...
local function kulala_core_prompt_block_name(item)
  if type(item.blockName) == "string" and vim.trim(item.blockName) ~= "" then
    return vim.trim(item.blockName)
  end
  return nil
end
...
local function kulala_core_batch_limit(requests)
  ...
  for _, req in ipairs(requests) do
    ...
    if type(name) == "string" and name ~= "" then
      table.insert(limit, { filter = "name", name = name })
    end
  end
  if #limit == 0 then
    return nil
...
local function kulala_core_target_for_item(item, targets, index)
  if type(item.blockName) == "string" and item.blockName ~= "" then
    for _, target in ipairs(targets) do
      if target._kulala_block_name == item.blockName then
        return target
      end
    end
  end
...
local function finish_kulala_core_wrapper(
  ...
  if wrapper.type == "error" then
    ...
    if wrapper.data and wrapper.data[1] and wrapper.data[1].error then
      msg = wrapper.data[1].error
    end
    handle_response({
      code = 1,
      errors = msg,
...
process_request_kulala_core = function(parsed_request, callback, retry_depth, run_opts)
  ...
  if run_opts then
    parsed_request._kulala_replay_run_opts = run_opts
  end
...
function process_request(request, callback, run_opts)
  ...
  if parsed_request == "empty" or parsed_request == "skipped" then
    return M.queue:run_next()
  end
...
M.run_parser = function(requests, line_nr, callback, run_opts)
  ...
  if #requests > 1 then
    ...
    if anchor._kulala_core and limit_line then
      anchor._kulala_limit_line = limit_line
    end
    ...
    if batch_limit then
      batch_run_opts.limit = batch_limit
    end
    ...
    for batch_index, request in ipairs(requests) do
      if request._kulala_core and limit_line then
        request._kulala_limit_line = limit_line
      end
