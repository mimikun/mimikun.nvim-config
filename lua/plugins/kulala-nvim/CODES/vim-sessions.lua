...
local function remap_response_bufs(responses)
  for _, resp in ipairs(responses or {}) do
    local buf = -1
    if type(resp.buf_name) == "string" and resp.buf_name ~= "" then
      buf = vim.fn.bufnr(resp.buf_name, true)
    end
    if buf < 1 and type(resp.file) == "string" and resp.file ~= "" then
      buf = vim.fn.bufnr(resp.file, true)
    end
...
M.save_state = function()
  if not CONFIG.get().session.restore then
    return
  end
...
M.restore_state = function()
  ...
  if type(default_view) == "string" and VALID_VIEWS[default_view] then
    CONFIG.options.default_view = default_view
  end
  ...
  if type(selected_env) == "string" and selected_env ~= "" then
    ...
    if DB.data then
      DB.data.selected_env = selected_env
    end
  end
end
...
M.load_session_hook = function()
  ...
  if not (had_kulala_bufs or has_saved_state) then
    return
  end
...
M.setup = function()
  ...
  if vim.v.this_session and vim.v.this_session ~= "" then
    vim.schedule(M.load_session_hook)
  end
end
