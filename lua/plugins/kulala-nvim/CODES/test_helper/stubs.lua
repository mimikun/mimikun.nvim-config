...
local function parse_headers_text(headers_text)
  ...
  for line in vim.gsplit(headers_text or "", "\n", { plain = true, trimempty = true }) do
    ...
    if k and not k:match("^HTTP/") then
      headers[k] = v
    end
  end
  return headers
...
local function url_for_run_payload(payload)
  ...
  if type(line) == "number" and line > 0 then
    ...
    for l in vim.gsplit(content, "\n", { plain = true }) do
      ...
      if u then
        current_url = vim.split(u, "?")[1]
      end
      if lineno >= line and current_url then
        return current_url
      end
    end
  end
...
local function build_success_item(url, mapping)
  if url:match("^wss?://") then
    return build_websocket_item(url, mapping)
  end
  ...
  if mapping.stats then
    ...
    if ok and type(stats) == "table" and stats.response_code then
      status = stats.response_code
    end
  end
...
function KulalaCore.handle(system)
  ...
  if action == "inspect_request" or action == "to_curl" then
    ...
    for i = block_start, #lines_tbl do
      ...
      if i > block_start and l:match("^###") then
        break
      end
      local k, v = l:match("^@([%w_%.%-]+)%s*=%s*(.+)$")
      if k and v then
        vars[k] = v:gsub("^%s+", ""):gsub("%s+$", "")
      end
      if not method then
    ...
    if method_upper == "GRPC" then
      ...
      if body ~= "" then
        cmd = cmd .. (" -d '%s'"):format(body)
      end
      cmd = cmd .. (" '%s'"):format(grpc_address or "")
      if grpc_symbol then
        cmd = cmd .. (" '%s'"):format(grpc_symbol)
      end
    ...
    if method_upper == "WEBSOCKET" then
      ...
      if body ~= "" then
        cmd = ("printf '%%s' '%s' | %s"):format(body, cmd)
      end
      if action == "inspect_request" then
        system.code = 0
        system.stdout = vim.json.encode({ ok = true, lines = { cmd } })
    ...
    if action == "inspect_request" then
      ...
      for k, v in pairs(headers) do
        if k:lower() ~= "cookie" then
          curl = curl .. (" -H '%s:%s'"):format(k, v)
        end
      end
      if body ~= "" then
        curl = curl .. (" --data-binary '%s'"):format(body)
      end
      local cookie = headers.Cookie or headers.cookie
      if cookie then
        curl = curl .. (" --cookie '%s'"):format(cookie)
      end
