...
function M.is_nil(value)
  if value ~= nil then
    fail(string.format("expected nil, got %s", vim.inspect(value)))
  end
end
...
function M.is_true(value)
  if value ~= true then
    fail(string.format("expected true, got %s", vim.inspect(value)))
  end
end

function M.is_false(value)
  if value ~= false then
    fail(string.format("expected false, got %s", vim.inspect(value)))
  end
end

function M.is_truthy(value)
  if not value then
    fail(string.format("expected truthy value, got %s", vim.inspect(value)))
  end
end

function M.is_string(value)
  if type(value) ~= "string" then
    fail(string.format("expected string, got %s", type(value)))
  end
end
...
function M.is_not.same(expected, actual)
  ...
  if ok then
    fail(string.format("expected different values, both are %s", vim.inspect(expected)))
  end
end

function M.matches(pattern, value)
  if type(value) ~= "string" then
    fail(string.format("expected string subject, got %s", type(value)))
  end
