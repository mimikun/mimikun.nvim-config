    ...
    for i = col + 2, #line, 1 do
      ...
      if string.match(char, config.pattern) or (char == " " and string.match(char2, "%w")) then
        ...
        if
          ...
          and (utils.is_quote(char) or (utils.is_close_bracket(char) and utils.is_in_quotes(line, col, prev_char)))
        then
          offset = 0
        end
...
M.highlight_wrap = function(tbl_pos, row, col, end_col, whitespace_line)
  ...
  if config.use_virt_lines then
    ...
    if config.highlight_grey then
      (vim.hl or vim.highlight).range(bufnr, M.ns_fast_wrap, config.highlight_grey, { row, col }, { row, end_col }, {})
    end
    for _, pos in ipairs(tbl_pos) do
