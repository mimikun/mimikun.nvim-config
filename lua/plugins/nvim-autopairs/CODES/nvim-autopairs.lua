...
local autopairs_delete = function(bufnr, key)
  ...
  for _, rule in pairs(rules) do
    if rule.start_pair then
      local prev_char, next_char = utils.text_cusor_line(line, col, #rule.start_pair, #rule.end_pair, rule.is_regex)
      if
...
M.autopairs_map = function(bufnr, char)
  ...
  for _, rule in pairs(rules) do
    if rule.start_pair then
      ...
      --   on a dedicated rule
      if char_matches_rule and utils.compare(rule.end_pair, next_char, rule.is_regex) and rule:can_move(cond_opt) then
...
M.autopairs_cr = function(bufnr)
  ...
  for _, rule in pairs(rules) do
    if rule.start_pair then
      ...
      -- log.debug('prev_char' .. prev_char)
      -- log.debug('next_char' .. next_char)
      if rule.is_endwise and utils.compare(rule.start_pair, prev_char, rule.is_regex) and rule:can_cr(cond_opt) then
        local end_pair = rule:get_end_pair(cond_opt)
        return utils.esc(
          "<CR>"
...
M.autopairs_afterquote = function(line, key_char)
  if M.config.enable_afterquote and not utils.is_block_wise_mode() then
    ...
    if utils.is_bracket(prev_char) and utils.is_quote(next_char) and not utils.is_in_quotes(line, col, next_char) then
      local count = 0
      local index = 0
      ...
      if count == 2 and index >= (#line - 2) then
        ...
        for _, rule in pairs(rules) do
          if rule.start_pair == prev_char and char_end ~= rule.end_pair then
            ...
            return utils.esc("<esc><cmd>lua require'nvim-autopairs'.autopairs_closequote_expr()<cr>" .. append)
          end
        end
      end
    end
