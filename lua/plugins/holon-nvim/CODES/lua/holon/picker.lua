...
local function render_count(state, cfg)
  ...
  local count_str = filtered == total and string.format(" %s (%d) ", cfg.title or "Picker", total)
  ...
  vim.api.nvim_create_autocmd({ "TextChangedI", "TextChanged" }, {
    ...
    callback = function()
      ...
      if cfg.dynamic_source then
        ...
        local get_ordinal = cfg.get_ordinal or function(item)
          return item.ordinal or ""
        end
