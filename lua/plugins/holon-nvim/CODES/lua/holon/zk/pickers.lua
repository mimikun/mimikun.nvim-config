  ...
  for mode, mode_maps in pairs(mappings_config) do
    for key, action_name in pairs(mode_maps) do
      if action_handlers[action_name] then
        ...
        extra_mappings[mode][key] = function()
...
function M.backlinks(opts)
  ...
  if not identifier then
    ...
    identifier = utils.extract_uuid_from_path(current_file) or vim.fn.fnamemodify(current_file, ":t:r")
...
function M.forward_links(opts)
  ...
  if not title then
    ...
    title = require("holon.frontmatter").get_title(fm) or vim.fn.fnamemodify(filepath, ":t:r")
  end
    ...
    mappings = {
      n = {
        ["d"] = function()
          ...
          local msg = string.format("Delete %d note(s)?\n%s\n(y/N): ", #marked, table.concat(names, "\n"))
