...
function M.render_helpline(buf, state)
  ...
  elseif state.view_mode == "horizon" then
    help = " j/k:select  C-j/k:panel  Tab:mark  p:move  b:block  CR:open  c:status  t:target  s:start  a:add  l:link  "
      .. scale_label
      .. "  g:preview  H:back  I:inbox  D:done  q:close"
  else
    help = " j/k:select  C-j/k:panel  Tab:mark  p:move  b:block  CR:open  c:status  t:target  s:start  a:add  l:link  "
      .. scale_label
      .. "  g:preview  H:horizon  I:inbox  D:done  q:close"
  end
