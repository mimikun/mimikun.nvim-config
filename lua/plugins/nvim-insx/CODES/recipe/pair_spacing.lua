...
local function increase(option)
  return {
    ...
    enabled = function(ctx)
      return (
        helper.regex.match(ctx.before(), option.open_pat .. [[\s*$]])
        or helper.regex.match(ctx.after(), [[^\s*]] .. option.close_pat)
      )
    end,
  }
end
  ...
  return {
    ...
    action = function(ctx)
      ...
      if before_match and after_match then
        ...
        -- Logic for separated pair.
        local pair_pos = helper.search.get_pair_close(option.open_pat, option.close_pat)
          or helper.search.get_pair_open(option.open_pat, option.close_pat)
        if pair_pos then
          local row, col = ctx.row(), ctx.col()
          sync_space((get_space({ row, col })), pair_pos)
        end
