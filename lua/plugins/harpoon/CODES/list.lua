...
function HarpoonList:replace_at(idx, item)
  ...
  Extensions.extensions:emit(Extensions.event_names.REPLACE, { list = self, item = item, idx = idx })
end
...
function HarpoonList:add(item)
  ...
  if index == -1 then
    ...
    Extensions.extensions:emit(Extensions.event_names.ADD, { list = self, item = item, idx = idx })
  end
...
function HarpoonList:prepend(item)
  ...
  if index == -1 then
    ...
    Extensions.extensions:emit(Extensions.event_names.ADD, { list = self, item = item, idx = 1 })
  end
...
function HarpoonList:remove(item)
  ...
  for i = 1, self._length do
    ...
    if self.config.equals(v, item) then
      ...
      Extensions.extensions:emit(Extensions.event_names.REMOVE, { list = self, item = item, idx = i })
      break
    end
  end
  return self
...
function HarpoonList:remove_at(index)
  if self.items[index] then
    ...
    Extensions.extensions:emit(Extensions.event_names.REMOVE, { list = self, item = self.items[index], idx = index })
  end
  return self
end
...
function HarpoonList:resolve_displayed(displayed, length)
  ...
  for i = 1, length do
    ...
    elseif index == -1 then
      ...
      local index_in_new_list = index_of(new_list, length, self.items[index], self.config)
...
function HarpoonList:select(index, options)
  ...
  if item or self.config.select_with_nil then
    Extensions.extensions:emit(Extensions.event_names.SELECT, { list = self, item = item, idx = index })
