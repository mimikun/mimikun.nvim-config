...
describe("matcher", function()
  ...
  it("match", function()
    ...
    assert.is.truthy(
      matcher.match("call", "call", config.matching) > matcher.match("call", "condition_all", config.matching)
    )
    assert.is.truthy(
      matcher.match("Buffer", "Buffer", config.matching) > matcher.match("Buffer", "buffer", config.matching)
    )
    assert.is.truthy(
      matcher.match("luacon", "lua_context", config.matching) > matcher.match("luacon", "LuaContext", config.matching)
    )
    ...
    assert.is.truthy(
      matcher.match("conso", "console", config.matching) > matcher.match("conso", "ConstantSourceNode", config.matching)
    )
    assert.is.truthy(
    ...
    assert.is.truthy(
      matcher.match("Unit", "net.UnixListener", { disallow_partial_fuzzy_matching = true }, config.matching) == 0
    )
    assert.is.truthy(
      matcher.match("Unit", "net.UnixListener", { disallow_partial_fuzzy_matching = false }, config.matching) >= 1
    )
