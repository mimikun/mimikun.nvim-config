...
local function construct_config_options(config, prefix, suffix)
  ...
  if prefix then
    table.insert(value, 1, text.highlight(set and "NeojjPopupConfigEnabled" or "NeojjPopupConfigDisabled")(prefix))
  end
...
local Switch = Component.new(function(switch)
  ...
  if switch.options then
    ...
    value = row.id(switch.id).highlight(get_highlight_for_switch(switch))({ text(switch.cli_prefix), text(switch.cli) })
  end
