  ...
  for _, dep in pairs(dependencies) do
    ...
    if not installed then
      ...
      if dep.optional then
        vim.health.warn(("%s %s"):format(err_msg, ("Install %s for extended capabilities"):format(dep.url)))
      else
        vim.health.error(
          ("%s %s"):format(err_msg, ("`GrugFar` will not function without %s installed."):format(dep.url))
        )
      end
    else
