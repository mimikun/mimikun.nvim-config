...
local function get_default_astgrep_language(context)
  ...
  if context.prevBufName ~= nil then
    local byGlob = get_language_by_glob(context.prevBufName, context.options.engines["astgrep-rules"].languageGlobs)
    if byGlob ~= nil then
      lang = byGlob
    end
  end
...
rule:
  ...
  getInputPrefillsForVisualSelection = function(visual_selection_info, initialPrefills, visualSelectionUsage)
