...
local function run_search_with_replace_interpreter(replacementInterpreter, params)
  local eval_fn, interpreterError = replacementInterpreter.get_eval_fn(params.inputs.replacement, { "match" })
  if not eval_fn then
...
function M.search(params)
  ...
  if numSearchChars > 0 and numSearchChars < (params.options.minSearchChars or 1) then
    params.on_finish(
      "success",
      nil,
      "Please enter at least " .. params.options.minSearchChars .. " search chars to trigger search!"
    )
    return
  end
