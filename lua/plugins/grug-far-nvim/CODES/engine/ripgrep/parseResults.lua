      ...
      for _, range in ipairs(ranges) do
        if range.start.line <= current_line_number and range["end"].line >= current_line_number then
          table.insert(highlights, {
            ...
            end_line = current_line,
            end_col = range["end"].line == current_line_number and range["end"].column - 1 or #resultLine,
          })
          local col = range.start.line == current_line_number and range.start.column or 1
          local end_col = range["end"].line == current_line_number and range["end"].column or (#resultLine + 1)
  ...
  for _, match in ipairs(matches) do
    ...
    elseif match.type == "match" then
      ...
      if not isSearchWithReplace or (isSearchWithReplace and showDiff) then
        ...
        local matchHighlightType = (isSearchWithReplace and showDiff) and ResultHighlightType.MatchRemoved
          ...
          :map(function(submatch)
            ...
            return {
              ...
              ["end"] = {
                ...
                column = submatch["end"] - (utils.strFindLast(text_to_submatch .. submatch.match.text, "\n") or 0) + 1,
              },
            }
          end)
          :totable()
        ...
        addResultLines(file_name, match_lines, ranges, lines, highlights, marks, sign, matchHighlightType, bufrange)
      ...
      if isSearchWithReplace then
        ...
        for _, submatch in ipairs(data.submatches) do
          replaced_lines_text = replaced_lines_text .. match_lines_text:sub(last_index + 1, submatch.start)

          local start_line = data.line_number + vim.fn.count(replaced_lines_text, "\n")
          table.insert(ranges, {
            start = {
              line = start_line,
              column = #replaced_lines_text - (utils.strFindLast(replaced_lines_text, "\n") or 0) + 1,
            },
            ["end"] = {
        ...
        local matchHighlightType = showDiff and ResultHighlightType.MatchAdded or ResultHighlightType.Match

        addResultLines(file_name, replaced_lines, ranges, lines, highlights, marks, sign, matchHighlightType, bufrange)
      end
