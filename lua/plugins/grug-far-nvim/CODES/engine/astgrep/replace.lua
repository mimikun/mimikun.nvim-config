    ...
    on_finish = function(status, errorMessage)
      ...
      if status == "success" then
        processingQueue:on_finish(function()
          processingQueue:stop()
          on_finish(processingQueue.status or status, processingQueue:append_error_message(errorMessage))
        end)
      else
        processingQueue:stop()
        on_finish(status or processingQueue.status, processingQueue:append_error_message(errorMessage))
      end
    end,
  })
...
function M.replace(params, isRulesBasedSearch)
  ...
  local args, blacklistedArgs = getArgs(inputs, params.options, extraArgs, blacklistedReplaceFlags, true)
  ...
  if params.replacementInterpreter then
    ...
    eval_fn, interpreterError = params.replacementInterpreter.get_eval_fn(inputs.replacement, { "match", "vars" })
    if not eval_fn then
