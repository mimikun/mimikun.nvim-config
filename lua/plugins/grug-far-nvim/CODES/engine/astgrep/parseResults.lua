...
function M.splitMatchLines(lines, leading, trailing)
  ...
  local trailingLines = first_trailing_newline and trailingStr:sub(first_trailing_newline + 1, -1) or ""

  local matchLines =
    lines:sub(last_leading_newline and #leadingLines + 2 or 1, first_trailing_newline and -#trailingLines - 2 or -1)
...
function M.parseResults(matches, bufrange, isFirst)
  ...
  for i = 1, #matches, 1 do
    ...
    local trailing = match.charCount and match.charCount.trailing or (#match.lines - #match.text - leading)

    local leadingLinesStr, matchLinesStr, trailingLinesStr = M.splitMatchLines(match.lines, leading, trailing)
    ...
    local matchHighlightType = match.replacement and ResultHighlightType.MatchRemoved or ResultHighlightType.Match
...
function M.getReplacedContents(contents, matches)
  ...
  for _, match in ipairs(matches) do
    new_contents = new_contents .. contents:sub(last_index + 1, match.range.byteOffset.start) .. match.replacement
