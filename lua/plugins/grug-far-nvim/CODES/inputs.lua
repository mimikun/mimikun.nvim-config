...
function M.getInputPos(context, buf, name)
  ...
  local endRow = unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, nextExtmarkId, {})) --[[@as integer?]]
...
M.getHeaderRow = function(context, buf)
  ...
  if context.extmarkIds.results_header then
    local row =
      unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, context.extmarkIds.results_header, {})) --[[@as integer]]
    if row then
      headerRow = row
    end
  end
...
function M.getInputAtRow(context, buf, row)
  ...
  for i, input_name in ipairs(names) do
    ...
    if extmarkId and nextExtmarkId then
      local start_row, start_col = unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, extmarkId, {}))
      local end_boundary_row =
        unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, nextExtmarkId, { details = true }))
...
local function extractMapping(buf, mode, lhs)
  ...
  local private_lhs_keycodes = vim.keycode("<Plug>") .. "grug-far-fallback[" .. mapping_data.lhsraw .. "]"
