...
function M.setBufKeymap(buf, desc, keymap, callback)
  local function setMapping(mode, lhs)
    vim.api.nvim_buf_set_keymap(buf, mode, lhs, "", { noremap = true, desc = desc, callback = callback, nowait = true })
  end
  ...
  if #target_windows > 0 then
    if preferredLocation == "prev" then
      ...
      for _, win in ipairs(target_windows) do
        ...
        if preferredLocation == "left" and row == ref_row and col < ref_col and (ref_col - col) < dist then
          dist = ref_col - col
          candidate_win = win
        elseif preferredLocation == "right" and row == ref_row and col > ref_col and (col - ref_col) < dist then
          dist = col - ref_col
          candidate_win = win
        elseif preferredLocation == "above" and col == ref_col and row < ref_row and (ref_row - row) < dist then
          dist = ref_row - row
          candidate_win = win
        elseif preferredLocation == "below" and col == ref_col and row > ref_row and (row - ref_row) < dist then
          dist = row - ref_row
          candidate_win = win
        end
      end
...
function M.get_current_visual_selection_info(strict)
  ...
  local visual_mode = isVisualMode and vim.fn.mode() or vim.fn.visualmode()
