...
local function gotoLocation(params)
  ...
  if bufnr == -1 then
    vim.fn.win_execute(targetWin, "silent! edit " .. utils.escape_path_for_cmd(location.filename), true)
  else
  ...
  vim.api.nvim_win_set_cursor(targetWin, { location.lnum or 1, location.col and location.col - 1 or 0 })
end
