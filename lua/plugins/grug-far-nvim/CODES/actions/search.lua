...
local function search(params)
  ...
  utils.setTimeout(tasks.task_callback_wrap(context, task, vim.schedule_wrap(clearResultsIfNeeded)), 100)
  ...
  local on_finish = function(status, errorMessage, customActionMessage)
    ...
    if status == "error" then
      ...
      if errorMessage and #errorMessage > 0 then
        ...
        state.actionMessage = lastline < winheight and " warnings!" or " warnings, see end of buffer!"
      end
    end
  ...
  abort, effectiveArgs = context.engine.search({
    ...
    on_fetch_chunk = tasks.task_callback_wrap(context, task, function(data)
      ...
      local abortEarly = context.options.maxSearchMatches ~= nil and fetched_matches > context.options.maxSearchMatches
