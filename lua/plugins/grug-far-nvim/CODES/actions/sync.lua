...
local function sync(params)
  ...
  task.abort = context.engine.sync({
    ...
    on_finish = tasks.task_callback_wrap(context, task, function(status, errorMessage, customActionMessage)
      vim.api.nvim_set_option_value("modifiable", true, { buf = buf })
      ...
      elseif status == nil and customActionMessage then
        ...
        state.actionMessage =
          getActionMessage(nil, changesCount, changesCount, context.options.reportDuration and time or nil)
      end
