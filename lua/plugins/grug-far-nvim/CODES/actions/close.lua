...
local function close(params)
  ...
  if #runningNonSearchTasks > 0 then
    local choice = vim.fn.confirm(
      runningNonSearchTasks[1].type .. " task will be aborted. Are you sure you want to close grug-far buffer?",
      "&yes\n&cancel"
    )
    if choice == 2 then
      return
  ...
  local shouldCloseWin = win ~= -1 and context.initialWin ~= context.prevWin and win == context.initialWin
