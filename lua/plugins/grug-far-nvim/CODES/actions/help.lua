  ...
  for i, line in ipairs(lines) do
    ...
    for _, hlText in ipairs(line) do
      ...
      if hlGroup and textLen > 0 then
        vim.hl.range(helpBuf, context.helpHlNamespace, hlGroup, { i - 1, pos }, { i - 1, pos + textLen })
      end
      pos = pos + textLen
    end
  end
...
local function createHelpWindow(context)
  ...
  -- close on <ESC> and q
  vim.api.nvim_buf_set_keymap(helpBuf, "n", "<ESC>", ":q<CR>", { noremap = true, nowait = true, silent = true })
  vim.api.nvim_buf_set_keymap(helpBuf, "n", "q", ":q<CR>", { noremap = true, nowait = true, silent = true })
