...
local function renderHistoryBuffer(historyBuf, context)
  ...
  context.extmarkIds.historyHelp = vim.api.nvim_buf_set_extmark(historyBuf, context.namespace, 0, 0, {
...
local function createHistoryWindow(buf, context)
  ...
  local historyWinConfig = vim.tbl_extend("force", {
    ...
    width = width,
    height = height,
    footer = (opts.getIcon("historyTitle", context) or " ") .. "History (press <:q> or <:bd> to close)",
    footer_pos = "center",
    border = "rounded",
    style = "minimal",
