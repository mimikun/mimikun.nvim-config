...
local function replace(params)
  ...
  local on_finish_all = tasks.task_callback_wrap(context, task, function(status, errorMessage, customActionMessage)
    vim.api.nvim_set_option_value("modifiable", true, { buf = buf })
    ...
    elseif status == nil and customActionMessage then
      ...
      state.actionMessage = getActionMessage(nil, filesCount, filesCount, time, context.options.reportDuration)
    end
