...
function inst:goto_next_match(params)
  self:ensure_open()
  local location =
    require("grug-far.actions.gotoMatch")(vim.tbl_extend("keep", self._params, { increment = 1 }, params or {}))
...
function inst:open_location()
  ...
  require("grug-far.actions.openLocation")(
    vim.tbl_extend("keep", self._params, { useScratchBuffer = self._context.options.openTargetWindow.useScratchBuffer })
  )
end
...
function inst:apply_next_change(params)
  self:ensure_open()
  require("grug-far.actions.applyChange")(vim.tbl_extend("keep", self._params, { increment = 1 }, params or {}))
end
