...
local function renderInput(params, context)
  ...
  if prevExtmarkName then
    local prevInputRow =
      unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, context.extmarkIds[prevExtmarkName], {}))
  ...
  if context.extmarkIds[extmarkName] and prevExtmarkName then
    local row = unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, context.extmarkIds[extmarkName], {})) --[[@as integer?]]
  ...
  if nextExtmarkName and context.extmarkIds[nextExtmarkName] then
    local nextInputRow =
      unpack(vim.api.nvim_buf_get_extmark_by_id(buf, context.namespace, context.extmarkIds[nextExtmarkName], {}))
  ...
  context.extmarkIds[extmarkName] = vim.api.nvim_buf_set_extmark(buf, context.namespace, currentStartRow, 0, {
  ...
  if params.highlightLang then
    local prev_line = unpack(vim.api.nvim_buf_get_lines(buf, currentStartRow - 1, currentStartRow, false))
