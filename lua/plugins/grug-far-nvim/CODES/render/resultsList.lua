...
local function addResultChunkHighlights(buf, context, data, startLine)
  ...
  for _, highlight in ipairs(data.highlights) do
    for j = highlight.start_line, highlight.end_line do
      if maxLineLength > -1 and j == highlight.start_line and highlight.start_col > maxLineLength then
        break
      end
      ...
      vim.hl.range(buf, context.resultListNamespace, highlight.hl_group, { lineNr, start_col }, { lineNr, end_col })
    end
  end
...
function M.appendSearchCommand(buf, context, rgArgs)
  ...
  vim.hl.range(buf, context.helpHlNamespace, "GrugFarResultsCmdHeader", { lastline, 0 }, { lastline, #header })
end
...
function M.rerenderLineNumber(context, buf, loc, mark, is_current_line)
  ...
  pcall(vim.api.nvim_buf_set_extmark, buf, context.locationsNamespace, start_row, start_col, details)
end
