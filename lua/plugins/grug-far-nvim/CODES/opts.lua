...
function grug_far.with_defaults(options, defaults)
  ...
  if not options.normalModeSearch and options.searchOnInsertLeave then
    vim.deprecate("options.searchOnInsertLeave", "options.normalModeSearch", "soon", "grug-far.nvim")
  ...
  if not vim.tbl_contains(newOptions.enabledReplacementInterpreters, newOptions.replacementInterpreter) then
    error("options.enabledReplacementInterpreters must contain options.replacementInterpreter")
  end

  if options.extraRgArgs then
    vim.deprecate("options.extraRgArgs", "options.engines.ripgrep.extraArgs", "soon", "grug-far.nvim")
    newOptions.engines.ripgrep.extraArgs = options.extraRgArgs
  end

  if options.ignoreVisualSelection ~= nil then
    vim.deprecate("options.ignoreVisualSelection", "options.visualSelectionUsage", "soon", "grug-far.nvim")
...
function grug_far.getGlobalOptions()
  return grug_far.with_defaults(_globalOptionsOverride or vim.g.grug_far or {}, grug_far.defaultOptions)
end
