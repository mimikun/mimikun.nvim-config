...
local function render(buf, context)
  ...
  local top_virt_lines = context.options.helpLine.enabled and help.getHelpVirtLines(context, context.actions) or {}
  ...
  for i, input in ipairs(context.engine.inputs) do
    ...
    if input.replacementInterpreterEnabled then
      local interpreterType = context.replacementInterpreter and context.replacementInterpreter.type or nil
