...
function M.validate(opts)
  ...
  if opts.highlights then
    ...
    if opts.highlights.treesitter then
      vim.validate("highlights.treesitter.enabled", opts.highlights.treesitter.enabled, "boolean", true)
      vim.validate("highlights.treesitter.max_lines", opts.highlights.treesitter.max_lines, "number", true)
    end
  ...
  if opts.conflict then
    ...
    vim.validate("conflict.format_virtual_text", opts.conflict.format_virtual_text, "function", true)
    ...
    if type(opts.conflict.keymaps) == "table" then
      ...
      for _, key in ipairs({ "ours", "theirs", "both", "none", "next", "prev" }) do
        vim.validate("conflict.keymaps." .. key, opts.conflict.keymaps[key], keymap_validator, "string or false")
      end
    end
  end
