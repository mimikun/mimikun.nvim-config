...
local function paint_difft_unified(diff_buf, diff_lines, diff_spec, lhs, rhs)
  difftastic.apply_unified(diff_buf, lhs, rhs, diff_lines, diff_spec, rails.width_for_buffer(diff_buf))
...
local function diff_buffer_label(diff_spec)
  ...
  if left.kind == diffspec.endpoint_kind.index and right.kind == diffspec.endpoint_kind.worktree then
    return "unstaged"
  end

  if left.kind == diffspec.endpoint_kind.tree and left.rev == "HEAD" and right.kind == diffspec.endpoint_kind.index then
    return "staged"
  end
  ...
  if left.kind == diffspec.endpoint_kind.stage and right.kind == diffspec.endpoint_kind.worktree then
...
local function setup_review_map_buffer(bufnr, display, layout)
  ...
  if not get_buffer_keymap(bufnr, "n", "gs") then
    vim.keymap.set("n", "gs", "<Plug>(diffs-review-toggle-layout)", { buffer = bufnr, remap = true })
  end
end
...
local function files_source_new_lines(source)
  ...
  if bufnr and vim.api.nvim_buf_is_loaded(bufnr) and vim.api.nvim_buf_get_name(bufnr) == source.right_path then
...
local function render_source(source)
  ...
  if source.kind == "section" then
    return render_section_source(source.repo_root, source.section), nil, source.section .. ":all", nil
  end
  ...
  return render.unified_lines(old_lines, new_lines, source.path, source.path), nil, "unmerged:" .. source.path, nil
end
...
local function warn_vertical_split_ignored()
  notify("++layout=split ignores the :vertical modifier; the split layout manages its own windows", vim.log.levels.WARN)
end
...
function M.diff_command(args, vertical)
  ...
  if args then
    ...
    if sub == "review" then
      return M.review_command(remainder ~= "" and remainder or nil, vertical, { warn_vertical_split = vertical })
    end
    if sub == "files" then
...
function M.diff_files(left, right, opts)
  ...
  local new_lines = right_buf and content.from_buffer(right_buf) or git.get_working_content(right_abs) or {}
...
local function review_split_context(opts)
  ...
  if source_err then
    return nil, "invalid diffs_source metadata: " .. tostring(source_err), vim.log.levels.WARN, review_buf
  end
  ...
  local selected_line = selected.lnum and hunk_model.line_at(lists.generated_hunks(review_buf), selected.lnum)
...
local function first_renderable_review_file(review_spec, repo_root, review_lines, list_opts)
  ...
  local last_level
  local skipped = {}
  for _, selected in ipairs(lists.generated_files(review_lines, review_generated_list_opts(list_opts))) do
    if selected.skipped then
      skipped[#skipped + 1] = selected
    else
      local diff_spec, diff_lines, err, level = render_review_file_selection(review_spec, repo_root, selected)
  ...
  return nil, nil, nil, last_err or "no renderable review file selected", last_level or vim.log.levels.INFO, skipped
end
  ...
  for _, buf in ipairs({ state.left_buf, state.right_buf }) do
    if vim.api.nvim_buf_is_valid(buf) then
      ...
      if not get_buffer_keymap(buf, "n", "gO") then
        vim.keymap.set("n", "gO", "<Plug>(diffs-review-select-file)", { buffer = buf, remap = true })
      end
      if not get_buffer_keymap(buf, "n", "gs") then
        vim.keymap.set("n", "gs", "<Plug>(diffs-review-toggle-layout)", { buffer = buf, remap = true })
      end
    end
  end
end
...
local function switch_review_split_file(state, selected, opts)
  opts = opts or {}
  local diff_spec, diff_lines, err, level = render_review_file_selection(state.review, state.repo_root, selected)
...
local function review_file_message(selection, index, count)
  ...
  local entry = (type(label) == "string" and label ~= "") and ("[%s] %s"):format(label, selection.file)
    or selection.file
...
local function notify_skipped_review_file(skipped, selection, index, count)
  ...
  notify(skipped_message .. "\n[diffs]: " .. review_file_message(selection, index, count), vim.log.levels.INFO)
end
...
local function review_file_formatter(files)
  ...
  return function(file)
    ...
    if max_del > 0 then
      parts[#parts + 1] = file.removed > 0 and string.format("%" .. max_del .. "s", "-" .. file.removed)
...
open_review_split = function(spec, opts)
  ...
  if first then
    ...
    local err, level
    diff_spec, diff_lines, err, level = render_review_file_selection(review_spec, normalized.repo_root, first)
    if not diff_spec or not diff_lines then
...
local function review_split_keep_window(state)
  ...
  local current_buf = vim.api.nvim_get_current_buf()
  if (current_buf == state.left_buf or current_buf == state.right_buf) and vim.api.nvim_win_is_valid(current_win) then
    return current_win
  end
...
local function goto_review_map_selection(bufnr, selected_file, selected_key, position)
  ...
  local matched_line = position
    and hunk_model.line_at_source(lists.generated_hunks(bufnr), { file = selected_file, key = selected_key }, position)
  for _, item in ipairs(vim.fn.getqflist({ items = 0 }).items) do
    ...
    if item.bufnr == bufnr and type(data) == "table" then
      local matches = selected_key and data.key == selected_key or (not selected_key and data.file == selected_file)
      if matches then
...
function M.read_buffer(bufnr)
  ...
  if type(review_marker) == "table" and review_marker.layout ~= "split" then
    setup_review_map_buffer(bufnr, review_marker.display, normalize_review_map_layout(review_marker.layout))
  end
