...
local function apply_highlights(bufnr, regions, config)
  ...
  for _, region in ipairs(regions) do
    ...
    mark_section(bufnr, region.ours_start, region.ours_end, "DiffsConflictOurs", "DiffsConflictOursNr")
    ...
    if region.marker_base then
      ...
      mark_section(bufnr, region.base_start, region.base_end, "DiffsConflictBase", "DiffsConflictBaseNr")
    end

    mark_line(bufnr, region.marker_sep, "DiffsConflictMarker")
    mark_section(bufnr, region.theirs_start, region.theirs_end, "DiffsConflictTheirs", "DiffsConflictTheirsNr")
...
function M.replace_region(bufnr, region, replacement)
  vim.api.nvim_buf_set_lines(bufnr, region_start(region), region.marker_theirs + 1, false, replacement)
end
...
function M.replacement_lines(bufnr, region, side)
  ...
  if side == "both" then
    vim.list_extend(lines, vim.api.nvim_buf_get_lines(bufnr, region.theirs_start, region.theirs_end, false))
  end
  return lines
end
