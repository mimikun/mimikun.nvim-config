...
local function carry_forward_highlighted(old_entry, new_hunks)
  ...
  log.dbg("carry_forward: %d prefix + %d suffix of %d old -> %d new hunks", prefix_len, suffix_len, old_n, new_n)
...
function M.clear_ns_by_start(bufnr, ns_id, start_row, end_row)
  local marks = vim.api.nvim_buf_get_extmarks(bufnr, ns_id, { start_row, 0 }, { end_row - 1, 2147483647 }, {})
...
function Cache:run_deferred_syntax(bufnr, tick, changedtick, job_id, deferred_syntax)
  ...
  if self.syntax_jobs[bufnr] ~= job_id then
    log.dbg("deferred syntax job superseded: cur=%s job=%d", tostring(self.syntax_jobs[bufnr]), job_id)
    return false
  end
  local live_changedtick = vim.api.nvim_buf_get_changedtick(bufnr)
  if live_changedtick ~= changedtick then
    log.dbg("deferred syntax changedtick changed: cur=%d captured=%d", live_changedtick, changedtick)
    return false
  end
  ...
  if cur.tick ~= tick then
    log.dbg("deferred syntax tick changed: cur.tick=%s captured=%d, using current hunks", tostring(cur.tick), tick)
    ...
    if live_changedtick ~= changedtick then
      log.dbg("deferred syntax changedtick changed after refresh: cur=%d captured=%d", live_changedtick, changedtick)
      return false
    end
  end
  local config = self.get_config()
