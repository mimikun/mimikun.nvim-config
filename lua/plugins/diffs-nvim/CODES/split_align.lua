  ...
  local function flush_change(dels, adds, hunk_index)
    ...
    for i = 1, math.max(#dels, #adds) do
      ...
      push(
        ...
        d and { kind = "delete", old_lnum = d, hunk_index = hunk_index } or { kind = "filler", hunk_index = hunk_index },
        a and (new_lines[a] or "") or FILLER,
        a and { kind = "add", new_lnum = a, hunk_index = hunk_index } or { kind = "filler", hunk_index = hunk_index }
      )
    end
  end
