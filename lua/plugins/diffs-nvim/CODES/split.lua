...
local function endpoint_lines(source, opts)
  ...
  local lines, err = render.read_endpoint(side_endpoint(spec, source.side), abs_path(source.repo_root, source.path), {
...
local function rail_segment(info, lnum, number)
  ...
  if row.kind == cfg.kind then
    return ("%%C%%s%%#%s#%s%%#%s#%s%%#%s# "):format(cfg.bar_hl, info.change_bar, cfg.nr_hl, numstr, cfg.line_hl)
  end
  ...
  for _, hunk in ipairs(hunks) do
    ...
    if #texts > 0 and #texts <= intra_cfg.max_lines then
      ...
      for _, span in ipairs(spans) do
        ...
        if buf_row and buf_row >= 1 and buf_row <= line_count and col_start >= 0 and col_end > col_start then
...
function M.open(opts)
  ...
  local left_source = generated.split_endpoint_source(vim.tbl_extend("force", base_source, { side = "left" }))
  local right_source = generated.split_endpoint_source(vim.tbl_extend("force", base_source, { side = "right" }))
  local old_content, left_err = endpoint_lines(left_source, { worktree_lines = opts.worktree_lines })
  if not old_content then
    return nil, left_err
  end
  local new_content, right_err = endpoint_lines(right_source, { worktree_lines = opts.worktree_lines })
  ...
  local reusing = reuse ~= nil and vim.api.nvim_win_is_valid(reuse.left) and vim.api.nvim_win_is_valid(reuse.right)
...
function M.displayed_position(bufnr, row)
  ...
  return position_at_row(bufnr, source.side, pane_row) or position_at_row(bufnr, peer_side, pane_row)
end
...
function M.open_source(bufnr)
  ...
  if endpoint.kind ~= diffspec.endpoint_kind.worktree then
    if endpoint.kind == diffspec.endpoint_kind.index then
      ...
      notify("cannot open read-only tree-backed split endpoint as a worktree file", vim.log.levels.WARN)
    end
    return false
  end
