...
function M.build_alignment(json, old_lines, new_lines)
  ...
  for _, pair in ipairs(json.aligned_lines or {}) do
    ...
    if not both_virtual then
      ...
      local left_kind = left_filler and "filler" or (has_spans(left_changes) and "delete" or "context")
      local right_kind = right_filler and "filler" or (has_spans(right_changes) and "add" or "context")
    ...
    for _, line in ipairs(hunk.lines) do
      ...
      if spans and line.lnum >= 1 and line.lnum <= line_count then
        for _, span in ipairs(spans) do
          local ok = pcall(vim.api.nvim_buf_set_extmark, bufnr, unified_ns, line.lnum - 1, base + span.col_start, {
