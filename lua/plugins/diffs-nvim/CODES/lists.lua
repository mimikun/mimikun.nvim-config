...
local function current_quickfix_item()
  ...
  local items = is_loclist and vim.fn.getloclist(0, { items = 0 }).items or vim.fn.getqflist({ items = 0 }).items
...
function M.ensure_quickfix_keymap()
  ...
  for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_get_option_value("filetype", { buf = bufnr }) == "qf" then
