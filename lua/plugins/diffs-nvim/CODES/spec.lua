...
function M.patch_actions(diff_spec)
  ...
  return {
    can_put = diff_spec.left.kind == M.endpoint_kind.index and diff_spec.right.kind == M.endpoint_kind.worktree,
    can_obtain = diff_spec.left.kind == M.endpoint_kind.tree and diff_spec.right.kind == M.endpoint_kind.index,
  }
end
