...
function M.get_file_content(revision, filepath)
  ...
  local result = vim.fn.systemlist({ "git", "-C", repo_root, "show", revision .. ":" .. rel_path }, nil, true)
  if vim.v.shell_error ~= 0 then
...
function M.get_index_mode(filepath)
  ...
  local result = vim.fn.systemlist({ "git", "-C", repo_root, "ls-files", "--stage", "--", rel_path })
...
function M.is_unmerged(filepath)
  ...
  local rel_path = relative_path(repo_root, filepath)
  local result = vim.fn.systemlist({ "git", "-C", repo_root, "ls-files", "--unmerged", "--", rel_path })
  return vim.v.shell_error == 0 and #result > 0
end
