...
local function apply_vim_syntax_spans(bufnr, ns, hunk, spans, covered_lines, leading_offset, priorities)
...
---@param priorities diffs.HighlightPriorities
---@return integer
local function highlight_vim_syntax(bufnr, ns, hunk, code_lines, covered_lines, leading_offset, priorities)
  local ft = hunk.ft
  if not ft then
    return 0
  end
...
local function highlight_cached_vim_syntax(bufnr, ns, hunk, code_lines, covered_lines, leading_offset, priorities)
  local ft = hunk.ft
  if not ft then
    return 0, false
  end
  ...
  return apply_vim_syntax_spans(bufnr, ns, hunk, spans, covered_lines, leading_offset, priorities), true
end
...
function M.highlight_hunk(bufnr, ns, hunk, opts)
  ...
  if use_ts or use_vim then
    ...
    if hl_count > max_lines then
      dbg("skipping hunk %s:%d (%d highlighted lines > %d max)", hunk.filename, hunk.start_line, hl_count, max_lines)
  ...
  if use_ts then
    ...
    extmark_count =
      highlight_treesitter(bufnr, ns, new_code, hunk.lang, new_map, pw + qw, covered_lines, p, nil, ts_context)
    extmark_count = extmark_count
      + highlight_treesitter(bufnr, ns, old_code, hunk.lang, old_map, pw + qw, covered_lines, p, nil, ts_context)
  ...
  elseif use_vim then
    ...
    if opts.defer_vim_syntax then
      extmark_count, vim_cache_hit = highlight_cached_vim_syntax(bufnr, ns, hunk, code_lines, covered_lines, 0, p)
    else
      extmark_count = highlight_vim_syntax(bufnr, ns, hunk, code_lines, covered_lines, 0, p)
    end
  end

  if hunk.header_start_line and hunk.header_lines and #hunk.header_lines > 0 and opts.highlights.treesitter.enabled then
    ---@type table<integer, integer>
    local header_map = {}
  ...
  if qw > 0 then
    raw_body_lines = vim.api.nvim_buf_get_lines(bufnr, hunk.start_line, hunk.start_line + #hunk.lines, false)
  end
    ...
    if not opts.syntax_only then
      ...
      if qw > 0 or pw > 1 then
        ...
        if hunk.rail_width and hunk.rail_width > 0 then
          ...
          local rail_number_ranges = rails.number_ranges(hunk.rail_width, hunk.rail_separator_width, rail_style)
          ...
          if (has_del or has_add) and rail_number_ranges and not dehl then
            ...
            local last_range = rail_style == "single" and first_range or rail_number_ranges[#rail_number_ranges]
