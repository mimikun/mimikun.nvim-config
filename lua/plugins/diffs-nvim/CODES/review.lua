...
local function default_branch(repo_root)
  local ref = vim.fn.systemlist({ "git", "-C", repo_root, "symbolic-ref", "refs/remotes/origin/HEAD" })
...
local function run_current_state(review, deps)
  ...
  if not unstaged then
    return nil, unstaged_err ~= "" and unstaged_err or "git diff failed for review Unstaged section", nil
  end
  ...
  if review.untracked then
    ...
    if not untracked then
      return nil, untracked_err ~= "" and untracked_err or "git ls-files failed for review Untracked section", nil
    end
