...
function M.parse(diff_lines, diff_spec)
  ...
  local hunks = {}
  local current_old_path = spec and spec.scope.kind == diffspec.scope_kind.file and spec.scope.path or nil
  ...
  for lnum, line in ipairs(diff_lines) do
    if is_review_section_header(line) then
      finish_hunk(lnum - 1)
      current_old_path = spec and spec.scope.kind == diffspec.scope_kind.file and spec.scope.path or nil
    ...
    elseif line:match("^%+%+%+ ") and not current_hunk then
      ...
      if old_range and new_range then
        ...
        current_hunk = {
          ...
          file_header_range = current_file_header_start and range(current_file_header_start, #file_header_lines) or nil,
