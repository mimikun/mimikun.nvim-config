...
MiniTrailspace.setup = function(config)
  ...
  if vim.fn.has("nvim-0.10") == 0 then
    vim.notify(
      "(mini.trailspace) Neovim<0.10 is soft deprecated (module works but is not supported)."
...
MiniTrailspace.highlight = function()
  ...
  if H.get_config().only_in_normal_buffers and not H.is_buffer_normal() then
    return
  end
...
H.track_normal_buffer = function()
  if not H.get_config().only_in_normal_buffers then
    return
  end
...
H.is_buffer_normal = function(buf_id)
  return vim.bo[buf_id or 0].buftype == ""
end
