...
MiniHipatterns.enable = function(buf_id, config)
  ...
  vim.api.nvim_buf_attach(buf_id, false, {
    ...
    on_reload = function()
      pcall(MiniHipatterns.update, buf_id)
    end,
...
MiniHipatterns.update = function(buf_id, from_line, to_line)
  ...
  if type(from_line) ~= "number" then
    H.error("`from_line` should be a number.")
  end
  to_line = to_line or vim.api.nvim_buf_line_count(buf_id)
  if type(to_line) ~= "number" then
    H.error("`to_line` should be a number.")
  end
...
MiniHipatterns.get_matches = function(buf_id, highlighters)
  ...
  if not H.islist(highlighters) then
    H.error("`highlighters` should be an array.")
  end
  highlighters = vim.tbl_filter(function(x)
    return vim.tbl_contains(all_highlighters, x)
  end, highlighters)

  local position_compare = function(a, b)
...
MiniHipatterns.gen_highlighter.hex_color = function(opts)
  ...
  return {
    ...
    group = function(_, _, data)
      return MiniHipatterns.compute_hex_color_group(data.full_match, hl_style, hex_opts)
    end,
...
MiniHipatterns.compute_hex_color_group = function(hex_color, style, opts)
  ...
  if H.hex_color_groups[group_name] then
    return group_name
  end

  -- Limit
  opts = vim.tbl_extend("force", { max_number = 10000 }, opts or {})
  if H.n_hex_color_groups >= opts.max_number then
    return nil
  end
  ...
  if style == "bg" then
    hl_opts = { fg = H.compute_opposite_color(hex), bg = hex_color }
  end
  if style == "fg" then
    hl_opts = { fg = hex_color }
  end
  if style == "line" then
    hl_opts = { sp = hex_color, underline = true }
  end
...
H.auto_enable = vim.schedule_wrap(function(data)
  local buf = data.buf
  if not (vim.api.nvim_buf_is_loaded(buf) and vim.bo[buf].buftype == "") then
    return
  end
...
H.normalize_highlighters = function(highlighters)
  ...
  for hi_name, hi in pairs(highlighters) do
    ...
    local pattern = type(hi.pattern) == "string" and function()
      return hi.pattern
    end or hi.pattern
    ...
    if is_pattern_ok then
      for i, pat in ipairs(pattern) do
        pattern[i] = type(pat) == "string" and function()
          return pat
        end or pat
    ...
    local group = type(hi.group) == "string" and function()
      return hi.group
    end or hi.group
...
H.get_all_highlighters = function()
  local hi_arr = vim.tbl_map(function(x)
    return { x, tostring(x) }
  end, vim.tbl_keys(H.ns_id))
  table.sort(hi_arr, function(a, b)
    return a[2] < b[2]
  end)
...
H.process_buffer_changes = vim.schedule_wrap(function(buf_id, lines_to_process)
  ...
  local buf_cache = H.cache[buf_id]
  if not vim.api.nvim_buf_is_valid(buf_id) or H.is_disabled(buf_id) or buf_cache == nil then
    return
  end
...
H.apply_highlighter_pattern = vim.schedule_wrap(function(pattern, hi, buf_id, ns, lines_to_process)
  ...
  for l_num, _ in pairs(lines_to_process) do
    ...
    while from and (from <= to) do
      ...
      -- `init` is more than 1
      if pattern_has_line_start then
        break
      end
