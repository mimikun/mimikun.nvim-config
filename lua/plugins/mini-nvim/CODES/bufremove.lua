...
MiniBufremove.setup = function(config)
  ...
  if vim.fn.has("nvim-0.10") == 0 then
    vim.notify(
      "(mini.bufremove) Neovim<0.10 is soft deprecated (module works but is not supported)."
...
MiniBufremove.unshow_in_window = function(win_id)
  ...
  vim.api.nvim_win_call(win_id, function()
    ...
    if has_previous and cur_buf ~= vim.api.nvim_win_get_buf(win_id) then
      return
    end
...
H.echo = function(msg, is_important)
  if MiniBufremove.config.silent then
    return
  end
...
H.can_remove = function(buf_id, force, fun_name)
  if force or not vim.bo[buf_id].modified then
    return true
  end
