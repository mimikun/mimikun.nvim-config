...
MiniComment.toggle_lines = function(line_start, line_end, opts)
  ...
  if config.hooks.pre(hook_arg) == false then
    return
  end
  ...
  if config.hooks.post(hook_arg) == false then
    return
  end
end
...
MiniComment.textobject = function()
  ...
  if config.hooks.pre(hook_args) == false then
    return
  end
  ...
  if config.hooks.post(hook_args) == false then
    return
  end
end
...
H.apply_config = function(config)
  ...
  H.map("n", config.mappings.comment_line, function()
...
H.get_comment_parts = function(ref_position, options)
  ...
  if vim.is_callable(options.custom_commentstring) then
    cs = options.custom_commentstring(ref_position)
  end
...
H.make_comment_check = function(parts, options)
  ...
  return function(line)
    return string.find(line, regex) ~= nil
  end
end
