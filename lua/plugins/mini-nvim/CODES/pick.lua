...
MiniPick.start = function(opts)
  if MiniPick.is_picker_active() then
    ...
    return vim.defer_fn(function()
      ...
      if MiniPick.is_picker_active() then
        H.picker_stop(H.pickers.active, true)
      end
      MiniPick.start(opts)
    end, 0.5)
  end
  ...
  if vim.fn.getcmdwintype() ~= "" then
    return H.notify("Can not open a picker with active command window", "WARN")
  end
...
MiniPick.default_match = function(stritems, inds, query, opts)
  ...
  local set_match_inds = is_sync and function(x)
    return x
  end or MiniPick.set_picker_match_inds
  local f = function()
    if #query == 0 then
      return set_match_inds(H.seq_along(stritems))
    end
    local match_data, match_type = H.match_filter(inds, stritems, query)
    if match_data == nil then
      return
    end
    if match_type == "useall" then
      return set_match_inds(H.seq_along(stritems))
    end
    if opts.preserve_order then
      return set_match_inds(H.match_no_sort(match_data))
    end
    local match_inds = H.match_sort(match_data)
    if match_inds == nil then
      return
    end
...
MiniPick.default_preview = function(buf_id, item, opts)
  ...
  if item_data.type == "file" then
    return H.preview_file(buf_id, item_data, opts)
  end
  if item_data.type == "directory" then
    return H.preview_directory(buf_id, item_data)
  end
  if item_data.type == "buffer" then
    return H.preview_buffer(buf_id, item_data, opts)
  end
  if item_data.type == "uri" then
    return H.preview_uri(buf_id, item_data, opts)
  end
...
MiniPick.default_choose = function(item)
  ...
  if not H.is_valid_win(win_target) then
    win_target = H.get_first_valid_normal_window()
  end
  ...
  if item_data.type == "buffer" then
    return H.choose_buffer(win_target, item_data)
  end
  H.choose_print(item)
end
...
MiniPick.ui_select = function(items, opts, on_choice, start_opts)
  ...
  local preview_item = vim.is_callable(opts.preview_item) and opts.preview_item
    or function(x)
      return vim.split(vim.inspect(x), "\n")
    end
  local preview = function(buf_id, item)
    local data = preview_item(item.item)
    if H.islist(data) then
      return H.set_buflines(buf_id, data)
    end
    if not (type(data) == "table" and H.is_valid_buf(data.buf)) then
      return
    end
...
MiniPick.builtin.files = function(local_opts, opts)
  ...
  if tool == "fallback" then
    ...
    opts.source.items = function()
      H.files_fallback_items(cwd)
    end
...
MiniPick.builtin.grep = function(local_opts, opts)
  ...
  if type(local_opts.pattern) ~= "string" then
    pattern = H.user_input("Grep pattern", nil, "file", "editor")
  end
  if pattern == nil then
    return
  end
  if tool == "fallback" then
    local cwd = H.full_path(opts.source.cwd or vim.fn.getcwd())
    opts.source.items = function()
      H.grep_fallback_items(pattern, cwd)
    end
    return MiniPick.start(opts)
  end
...
MiniPick.builtin.grep_live = function(local_opts, opts)
  ...
  local get_name = function()
    return string.format("Grep live (%s %s%s)", tool, method, globs_suffix)
  end
  ...
  local match = function(_, _, query)
    sys:kill()
    if H.querytick == set_items_opts.querytick then
      return
    end
    if #query == 0 then
...
MiniPick.builtin.help = function(local_opts, opts)
  ...
  local choose = function(item, modifier)
    ...
    vim.schedule(function()
      vim.cmd((modifier or default_modifier) .. "help " .. (item.name or ""))
    end)
  end
  local preview = function(buf_id, item)
...
MiniPick.builtin.cli = function(local_opts, opts)
  ...
  opts.source.items = vim.schedule_wrap(function()
    MiniPick.set_picker_items_from_cli(command, set_from_cli_opts)
  end)
  return MiniPick.start(opts)
end
...
MiniPick.builtin.resume = function()
  ...
  if picker == nil then
    H.error("There is no picker to resume.")
  end
...
MiniPick.get_picker_items = function()
  return H.copy_tables((H.pickers.active or {}).items)
end
...
---
---@seealso |MiniPick.set_picker_items()| and |MiniPick.set_picker_items_from_cli()|
MiniPick.get_picker_stritems = function()
...
MiniPick.get_picker_matches = function()
  ...
  res.all = vim.tbl_map(function(ind)
    return items[ind]
  end, match_inds)
  res.current = picker.items[res.current_ind]
  local marked_inds = vim.tbl_keys(picker.marked_inds_map)
  table.sort(marked_inds)
  res.marked_inds, res.marked = marked_inds, vim.tbl_map(function(ind)
    return items[ind]
  end, marked_inds)
  res.shown_inds = vim.tbl_map(function(ind)
    return match_inds[ind]
  end, picker.shown_inds)
  res.shown = vim.tbl_map(function(ind)
    return items[ind]
  end, res.shown_inds)
  return res
end
...
MiniPick.get_picker_opts = function()
  return vim.deepcopy((H.pickers.active or {}).opts)
end
...
MiniPick.get_picker_query = function()
  return vim.deepcopy((H.pickers.active or {}).query)
end
...
MiniPick.set_picker_items_from_cli = function(command, opts)
  ...
  if type(spawn_opts.cwd) == "string" then
    spawn_opts.cwd = H.full_path(spawn_opts.cwd)
  end
...
MiniPick.set_picker_match_inds = function(match_inds, match_type)
  if not H.is_array_of(match_inds, "number") then
    H.error("`match_inds` should be an array of numbers.")
  end
  local set = H.picker_set_inds[match_type or "all"]
  if set == nil then
    H.error('`match_type` should be one of "all", "marked", "current"')
  end
  if not MiniPick.is_picker_active() then
    return
  end
...
MiniPick.set_picker_opts = function(opts)
  ...
  if cur_cwd ~= picker.opts.source.cwd then
    H.win_set_cwd(picker.windows.main, picker.opts.source.cwd)
  end
...
MiniPick.set_picker_query = function(query)
  if not H.is_array_of(query, "string") then
    H.error("`query` should be an array of strings.")
  end
  if not MiniPick.is_picker_active() then
    return
  end
...
MiniPick.get_querytick = function()
  return H.querytick
end
...
H.command_parse_fargs = function(fargs)
  ...
  if lua_load == nil then
    H.error("Could not convert extra command arguments to table: " .. tbl_string)
  end
...
H.command_complete = function(_, line, col)
  ...
  end, vim.tbl_keys(MiniPick.registry))
...
H.validate_picker_opts = function(opts)
  ...
  if type(opts) ~= "table" then
    H.error("Picker options should be table.")
  end
  ...
  if type(options.content_from_bottom) ~= "boolean" then
    H.error("`options.content_from_bottom` should be boolean.")
  end
  if type(options.use_cache) ~= "boolean" then
    H.error("`options.use_cache` should be boolean.")
  end
...
H.picker_advance = function(picker)
  ...
  if not is_aborted then
    item = H.picker_get_current_item(picker)
  end
...
H.picker_compute_win_config = function(win_config, is_for_open)
  ...
  if (config.border or winborder) == "none" then
    config.border = { "", " ", "", "", "", " ", "", "" }
  end
...
H.picker_track_lost_focus = function(picker)
  local track = vim.schedule_wrap(function()
    ...
    if _G.MiniInput ~= nil and _G.MiniInput.get_state() ~= nil then
      return
    end
    if H.cache.is_in_getcharstr then
...
H.picker_set_busy = function(picker, value)
  ...
  if value then
    return H.timers.busy:start(picker.opts.delay.busy, 0, vim.schedule_wrap(update_border_hl))
  end
  update_border_hl()
end
...
H.picker_set_match_inds = function(picker, inds)
  ...
  if picker.opts.options.use_cache then
    picker.cache[cache_prompt] = { inds = inds }
  end
...
H.picker_set_inds = {
  all = function(picker, inds)
    H.picker_set_match_inds(H.pickers.active, inds)
  end,
  current = function(picker, inds)
    if inds[1] == nil or picker.match_inds == nil then
      return
    end
    local current_match_ind, current_abs_ind = nil, inds[1]
    for i, match_abs_ind in ipairs(picker.match_inds) do
      if match_abs_ind == current_abs_ind then
        current_match_ind = i
      end
    end
    if current_match_ind == nil then
...
H.picker_set_lines = function(picker)
  ...
  for i = from, to, (from <= to and 1 or -1) do
    ...
    if marked_inds_map[match_inds[i]] then
      table.insert(marked_lnums, #items_to_show)
    end
  end
  ...
  marked_lnums = vim.tbl_map(function(x)
    return x + n_empty_top_lines
  end, marked_lnums)
...
H.picker_match = function(picker)
  ...
  local prompt_cache
  if picker.opts.options.use_cache then
    prompt_cache = picker.cache[table.concat(picker.query)]
  end
  if prompt_cache ~= nil then
    return H.picker_set_match_inds(picker, prompt_cache.inds)
  end
...
H.normalize_mappings = function(mappings)
  ...
  for name, rhs in pairs(mappings) do
    ...
    if key ~= "" and (res[key] == nil or not res[key].is_custom) then
      if res[key] ~= nil then
        H.notify("Duplicating mapping keys: " .. name .. " and " .. res[key].name, "WARN")
      end
...
H.picker_set_bordertext = function(picker)
  ...
  if view_state == "main" then
    ...
    if after_caret ~= "" then
      table.insert(prompt, 3, { after_caret .. pad_right, "MiniPickPrompt" })
    end
    if before_caret ~= "" then
      table.insert(prompt, 2, { pad_left .. before_caret, "MiniPickPrompt" })
    end
    config = { title = prompt }
  end
...
H.picker_stop = function(picker, abort)
  ...
  if H.cache.getcharstr_errmsg ~= nil then
    error(H.cache.getcharstr_errmsg)
  end
end
...
H.picker_query_add = function(picker, char)
  ...
  if not H.is_query_char(char) then
    return
  end
...
H.is_query_char = function(char)
  return vim.fn.strchars(char) == 1 and vim.fn.char2nr(char) > 31
end
...
H.picker_mark_indexes = function(picker, range_type)
  ...
  if picker.view_state == "info" then
    H.picker_show_info(picker)
  end
end

H.picker_move_caret = function(picker, n)
...
H.picker_scroll = function(picker, direction)
  ...
  if picker.view_state == "main" and (direction == "down" or direction == "up") then
    ...
    vim.api.nvim_win_call(win_id, function()
      vim.cmd("normal! " .. H.replace_termcodes(keys))
    end)
  end
end
...
H.picker_get_register_contents = function(picker)
  ...
  if expand_var then
    return vim.api.nvim_win_call(picker.windows.target, function()
      return vim.fn.expand(expand_var)
    end)
  end
  if register == "\f" then
    return vim.api.nvim_win_call(picker.windows.target, function()
...
H.picker_show_info = function(picker)
  ...
  local append_char_data = function(data, header)
    ...
    table.sort(data, function(a, b)
      return a.desc < b.desc or (a.desc == b.desc and a.char < b.char)
    end)
  ...
  if not H.is_valid_buf(buf_id_info) then
    buf_id_info = H.create_scratch_buf("info")
  end
...
H.get_icon = function(x, icons)
  ...
  if path_type == "none" then
    return { text = icons.none, hl = "MiniPickNormal" }
  end
  ...
  if path_type == "directory" then
    return { text = icons.directory, hl = "MiniPickIconDirectory" }
  end
  local has_devicons, devicons = pcall(require, "nvim-web-devicons")
  if not has_devicons then
    return { text = icons.file, hl = "MiniPickIconFile" }
  end
...
H.preview_file = function(buf_id, item_data, opts)
  ...
  if not is_text then
    return H.set_buflines(buf_id, { is_text == nil and "-No-access-" or "-Non-text-file-" })
  end
...
H.preview_inspect = function(buf_id, obj)
...
H.choose_path = function(win_target, item_data)
  ...
  if item_data.type == "directory" then
    return vim.api.nvim_win_call(win_target, function()
      vim.cmd("edit " .. vim.fn.fnameescape(path))
    end)
  end
  pcall(vim.api.nvim_win_call, win_target, function()
    vim.cmd("normal! m'")
  end)
...
H.set_picker_items_from_feed = function(feed, pattern, opts)
  ...
  vim.schedule(function()
    MiniPick.set_picker_items(opts.postprocess(res), opts.set_items_opts)
  end)
end
...
H.files_get_tool = function()
  if H.is_executable("rg") then
    return "rg"
  end
  if H.is_executable("fd") then
    return "fd"
  end
  if H.is_executable("git") then
    return "git"
  end
  return "fallback"
end
...
H.files_fallback_items = function(cwd)
  ...
  local f = function()
    ...
    for path, path_type in vim.fs.dir(cwd, { depth = math.huge }) do
      ...
      if path_type == "file" and H.is_file_text(string.format("%s/%s", cwd, path)) then
  ...
  local f = function()
    ...
    for i, path in ipairs(files_full) do
      ...
      for lnum, l in ipairs(vim.fn.readfile(path)) do
        ...
        if col ~= nil then
          table.insert(items, string.format("%s\0%d\0%d\0%s", file, lnum, col, l))
        end
      end
    end
...
-- Async ----------------------------------------------------------------------
H.schedule_resume_is_active = vim.schedule_wrap(function(co)
  coroutine.resume(co, MiniPick.is_picker_active())
end)
...
H.poke_picker_throttle = function(querytick_ref)
  ...
  return function()
    local now = hrtime()
    if (now - latest_time) < threshold then
      return true
    end
    latest_time = now
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "minipick://" .. buf_id .. "/" .. name)
end

H.notify = function(msg, level_name)
  vim.notify("(mini.pick) " .. msg, vim.log.levels[level_name])
end
...
H.str_utfindex = function(s, i)
  return vim.str_utfindex(s, "utf-32", i)
end
if vim.fn.has("nvim-0.11") == 0 then
  H.str_utfindex = function(s, i)
    return (vim.str_utfindex(s, i))
  end
end
...
H.parse_uri = function(x)
  ...
  if H.is_windows and x:find("^%a:") ~= nil and path:find("^%a:") ~= nil then
    return nil
  end
  return path
end
...
H.user_input = function(prompt, text, completion, scope)
  ...
  vim.on_key(function(key)
    was_cancelled = was_cancelled or key == "\27"
  end, H.ns_id.input)
