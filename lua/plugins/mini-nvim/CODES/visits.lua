...
MiniVisits.register_visit = function(path, cwd)
  ...
  if path == "" or cwd == "" then
    H.error("Both `path` and `cwd` should not be empty.")
  end
...
MiniVisits.remove_path = function(path, cwd)
  ...
  for _, pair in ipairs(path_cwd_pairs) do
    ...
    if type(cwd_tbl) == "table" then
      cwd_tbl[pair.path] = nil
    end
  end

  for dir, _ in pairs(H.index) do
    if vim.tbl_count(H.index[dir]) == 0 then
      H.index[dir] = nil
    end
  end
end
  ...
  local on_label = function(l)
    ...
    for _, pair in ipairs(path_cwd_pairs) do
      ...
      if type(path_tbl) == "table" and type(path_tbl.labels) == "table" then
        ...
        if vim.tbl_count(path_tbl.labels) == 0 then
          path_tbl.labels = nil
        end
      end
    end
...
MiniVisits.list_labels = function(path, cwd, opts)
  ...
  table.sort(label_arr, function(a, b)
...
MiniVisits.select_path = function(cwd, opts)
  ...
  local items = vim.tbl_map(function(path)
    return { path = path, text = H.short_path(path, cwd_to_short) }
  end, paths)
  local select_opts = {
...
MiniVisits.iterate_paths = function(direction, cwd, opts)
  ...
  if cur_ind == nil then
    cur_ind = is_move_forward and 0 or (n_tot + 1)
  end
...
MiniVisits.reset_index = function()
  ...
  if not ok or stored_index == nil then
    return
  end
...
MiniVisits.normalize_index = function(index)
  ...
  if not vim.is_callable(normalize) then
...
MiniVisits.rename_in_index = function(path_from, path_to, index)
  ...
  local replace = function(x)
    if string.find(x, pattern_from_full) ~= nil then
      return path_to
    end
...
MiniVisits.gen_filter.this_session = function()
  return function(path_data)
    return H.cache.session_start_time <= path_data.latest
  end
end
...
MiniVisits.gen_sort.default = function(opts)
  ...
  if not is_weight then
    H.error("`opts.recency_weight` should be number between 0 and 1.")
  end
  ...
  return function(path_data_arr)
    ...
    table.sort(path_data_arr, function(a, b)
    ...
    table.sort(path_data_arr, function(a, b)
      return a.rank < b.rank or (a.rank == b.rank and a.path < b.path)
    end)
...
MiniVisits.gen_sort.z = function()
  return function(path_data_arr)
    ...
    table.sort(path_data_arr, function(a, b)
      return a.z > b.z or (a.z == b.z and a.path < b.path)
    end)
    return path_data_arr
  end
end
...
H.create_autocommands = function(config)
  ...
  if config.track.event ~= "" then
    au(config.track.event, "*", H.autoregister_visit, "Auto register visit")
  end
  au("VimLeavePre", "*", function()
    if not H.get_config().store.autowrite then
      return
    end
...
H.autoregister_visit = function(data)
  ...
  local f = vim.schedule_wrap(function()
    ...
    if path == nil or path == H.cache.latest_tracked_path then
      return
    end
...
H.ensure_read_index = function()
  if not H.cache.needs_index_read then
    return
  end
...
H.index_prune = function(index, prune_paths, threshold)
  if type(threshold) ~= "number" then
    H.error("Prune threshold should be number.")
  end

  -- Possibly prune non-path cwds
  for cwd, cwd_tbl in pairs(index) do
    if prune_paths and vim.fn.isdirectory(cwd) == 0 then
      index[cwd] = nil
    end
  end
  ...
  for cwd, cwd_tbl in pairs(index) do
    for path, path_tbl in pairs(cwd_tbl) do
      ...
      if path_tbl.labels == nil and should_prune then
        cwd_tbl[path] = nil
      end
    end
  end
  ...
  for cwd, cwd_tbl in pairs(index) do
    if vim.tbl_count(cwd_tbl) == 0 then
      index[cwd] = nil
    end
  end
end
...
H.index_decay_cwd = function(cwd_tbl, threshold, target)
  if type(threshold) ~= "number" then
    H.error("Decay threshold should be number.")
  end
  if type(target) ~= "number" then
    H.error("Decay target should be number.")
  end
  ...
  if total_count == 0 or total_count <= threshold then
    return
  end
...
H.validate_path = function(x)
  ...
  if x == nil then
    H.error("Current buffer is not for a regular file")
  end
...
H.validate_filter = function(x)
  ...
  if type(x) == "string" then
    local label = x
    x = function(path_data)
      return (path_data.labels or {})[label]
    end
  end
  if not vim.is_callable(x) then
    H.error("`filter` should be callable or string label name.")
  end
  return x
end
...
H.validate_sort = function(x)
  ...
  if not vim.is_callable(x) then
    H.error("`sort` should be callable.")
  end
  return x
end
...
H.validate_index = function(x, name)
  ...
  if type(x) ~= "table" then
    H.error(name .. " should be a table.")
  end
  for cwd, cwd_tbl in pairs(x) do
    if type(cwd) ~= "string" then
      H.error("First level keys in " .. name .. " should be strings.")
    end
    if type(cwd_tbl) ~= "table" then
      H.error("First level values in " .. name .. " should be tables.")
    end

    for path, path_tbl in pairs(cwd_tbl) do
      if type(path) ~= "string" then
        H.error("Second level keys in " .. name .. " should be strings.")
      end
      if type(path_tbl) ~= "table" then
        H.error("Second level values in " .. name .. " should be tables.")
      end

      if type(path_tbl.count) ~= "number" then
        H.error("`count` entries in " .. name .. " should be numbers.")
      end
      if type(path_tbl.latest) ~= "number" then
        H.error("`latest` entries in " .. name .. " should be numbers.")
      end
...
H.validate_labels_field = function(x)
  ...
  if type(x) ~= "table" then
    H.error("`labels` should be a table.")
  end

  for key, value in pairs(x) do
    if type(key) ~= "string" then
      H.error("Keys in `labels` table should be strings.")
    end
    if value ~= true then
      H.error("Values in `labels` table should only be `true`.")
    end
  end
end
...
H.buf_get_path = function(buf_id)
  ...
  if not (H.is_valid_buf(buf_id) and vim.bo[buf_id].buftype == "") then
    return nil
  end
...
H.tbl_add_rank = function(arr, key)
  ...
  for i, tbl in ipairs(arr) do
    ...
    if tie_data ~= nil then
      tbl[rank_key] = tie_data.sum / tie_data.n
    end
  end
end
...
H.full_path = function(path)
  return (vim.fn.fnamemodify(path, ":p"):gsub("/+", "/"):gsub("(.)/$", "%1"))
end
