...
MiniSnippets.expand = function(opts)
  ...
  if not vim.is_callable(prepare) then
    H.error("`opts.prepare` should be callable")
  end

  local match = false
  if opts.match ~= false then
    match = opts.match or MiniSnippets.default_match
  end
  if not (match == false or vim.is_callable(match)) then
    H.error("`opts.match` should be `false` or callable")
  end

  local select = false
  if opts.select ~= false then
    select = opts.select or MiniSnippets.default_select
  end
  if not (select == false or vim.is_callable(select)) then
    H.error("`opts.select` should be `false` or callable")
  end

  local insert = false
  if opts.insert ~= false then
    insert = opts.insert or MiniSnippets.default_insert
  end
  if not (insert == false or vim.is_callable(insert)) then
    H.error("`opts.insert` should be `false` or callable")
  end

  -- Match
  local all_snippets, context = prepare(config.snippets)
  if not H.is_array_of(all_snippets, H.is_snippet) then
    H.error("`prepare` should return array of snippets")
  end
  local matches = match == false and all_snippets or match(all_snippets)
  if not H.is_array_of(matches, H.is_snippet) then
    H.error("`match` should return array of snippets")
  end
  ...
  if #all_snippets == 0 then
    return H.notify("No snippets in context:\n" .. vim.inspect(context), "WARN")
  end
  if #matches == 0 then
    return H.notify("No matches in context:\n" .. vim.inspect(context), "WARN")
  end

  local insert_ext = H.make_extended_insert(insert)

  if select == false then
    return insert_ext(matches[1])
  end
...
MiniSnippets.gen_loader.from_lang = function(opts)
  ...
  for lang, tbl in pairs(opts.lang_patterns) do
    if type(lang) ~= "string" then
      H.error("Keys of `opts.lang_patterns` should be string language names")
    end
    if not H.is_array_of(tbl, H.is_string) then
      H.error("Values of `opts.lang_patterns` should be string arrays")
    end
  end
...
MiniSnippets.gen_loader.from_runtime = function(pattern, opts)
  ...
  local read = function(p)
    return MiniSnippets.read_file(p, read_opts)
  end
  return function()
    if cache and H.cache.runtime[pattern] ~= nil then
      return vim.deepcopy(H.cache.runtime[pattern])
    end

    local res = vim.tbl_map(read, vim.api.nvim_get_runtime_file(pattern, opts.all))
    if cache then
      H.cache.runtime[pattern] = vim.deepcopy(res)
    end
    return res
  end
end
...
MiniSnippets.read_file = function(path, opts)
  ...
  if opts.cache and H.cache.file[path] ~= nil then
    return vim.deepcopy(H.cache.file[path])
  end
  ...
  if prob ~= "" then
    H.notify(problem_prefix .. prob, "WARN", opts.silent)
  end

  if res.snippets == nil then
    return nil
  end
  if opts.cache then
    H.cache.file[path] = vim.deepcopy(res.snippets)
  end
  return res.snippets
end
...
MiniSnippets.default_prepare = function(raw_snippets, opts)
  if not H.islist(raw_snippets) then
    H.error("`raw_snippets` should be array")
  end
  opts = vim.tbl_extend("force", { context = nil }, opts or {})
  local context = opts.context
  if context == nil then
    context = H.get_default_context()
  end
...
MiniSnippets.default_match = function(snippets, opts)
  if not H.is_array_of(snippets, H.is_snippet) then
    H.error("`snippets` should be array of snippets")
  end
  opts = vim.tbl_extend("force", { pattern_exact_boundary = "[%s%p]?", pattern_fuzzy = "%S*" }, opts or {})
  if not H.is_string(opts.pattern_exact_boundary) then
    H.error("`opts.pattern_exact_boundary` should be string")
  end
  ...
  -- Fuzzy
  if not H.is_string(opts.pattern_fuzzy) then
    H.error("`opts.pattern_fuzzy` should be string")
  end
  if opts.pattern_fuzzy == "" then
    return {}
  end
  ...
  local snippets_with_prefix = vim.tbl_filter(function(s)
    return s.prefix ~= nil
  end, snippets)
...
MiniSnippets.default_select = function(snippets, insert, opts)
  if not H.is_array_of(snippets, H.is_snippet) then
    H.error("`snippets` should be an array of snippets")
  end
  if #snippets == 0 then
    return H.notify("No snippets to select from", "WARN")
  end
  insert = insert or MiniSnippets.default_insert
  if not vim.is_callable(insert) then
    H.error("`insert` should be callable")
  end
  opts = opts or {}
...
MiniSnippets.default_insert = function(snippet, opts)
  if not H.is_snippet(snippet) then
    H.error("`snippet` should be a snippet table")
  end

  local default_opts = { empty_tabstop = "•", empty_tabstop_final = "∎", lookup = {} }
  opts = vim.tbl_deep_extend("force", default_opts, opts or {})
  if not H.is_string(opts.empty_tabstop) then
    H.error("`empty_tabstop` should be string")
  end
  if not H.is_string(opts.empty_tabstop_final) then
    H.error("`empty_tabstop_final` should be string")
  end
  if type(opts.lookup) ~= "table" then
    H.error("`lookup` should be table")
  end
...
MiniSnippets.session.get = function(all)
  return vim.deepcopy(all and H.sessions or H.get_active_session())
end
...
MiniSnippets.session.jump = function(direction)
  if not (direction == "prev" or direction == "next") then
    H.error('`direction` should be one of "prev", "next"')
  end
  H.call_in_insert_mode(function()
...
MiniSnippets.parse = function(snippet_body, opts)
  if H.is_array_of(snippet_body, H.is_string) then
    snippet_body = table.concat(snippet_body, "\n")
  end
  if type(snippet_body) ~= "string" then
    H.error("Snippet body should be string or array of strings")
  end
...
MiniSnippets.start_lsp_server = function(opts)
  ...
  if not (ok and type(client_id) == "number") then
    H.error("Could not start 'mini.snippets' in-process LSP server")
  end
  if vim.fn.has("nvim-0.11") == 0 then
    pcall(vim.lsp.buf_detach_client, 0, client_id)
  end

  local attach = function(buf_id)
    if not vim.api.nvim_buf_is_valid(buf_id) or opts.before_attach(buf_id) == false then
      return
    end
...
H.is_disabled = function()
  return vim.g.minisnippets_disable == true or vim.b.minisnippets_disable == true
end
...
H.file_readers.lua = function(path, silent)
  ...
  if not ok then
    return { problems = { "Could not execute Lua file" } }
  end
  if type(contents) ~= "table" then
    return { problems = { "Returned object is not a table" } }
  end
...
H.file_readers.json = function(path, silent)
  ...
  if file == nil then
    return { problems = { "Could not open file" } }
  end
...
H.read_snippet_data = function(contents)
  ...
  for name, t in pairs(contents) do
    if H.is_snippet(t) then
      ...
      if type(name) == "string" and (t.desc == nil and t.description == nil) then
        t.desc = name
      end
      table.insert(res, t)
...
H.get_default_context = function()
  ...
  if not has_parser or parser == nil then
    return { buf_id = buf_id, lang = vim.bo[buf_id].filetype }
  end
...
H.traverse_raw_snippets = function(x, target, context)
  if H.is_snippet(x) then
    local body
    if x.body ~= nil then
      body = type(x.body) == "string" and x.body or table.concat(x.body, "\n")
    end
  ...
  if vim.is_callable(x) then
    H.traverse_raw_snippets(x(context), target, context)
  end
end
...
H.parse_verify = function(state)
  if state.name == "dollar_lbrace" then
    H.error('"${" should be closed with "}"')
  end
  if state.name == "choice" then
    H.error('Tabstop with choices should be closed with "|}"')
  end
  if vim.startswith(state.name, "transform_") then
    H.error('Transform should contain 3 "/" outside of `${...}` and be closed with "}"')
  end
  if #state.depth_arrays > 1 then
    H.error('Placeholder should be closed with "}"')
  end
end

H.parse_post_process = function(node_arr, state_name)
  -- Allow "$" at the end of the snippet
  if state_name == "dollar" then
    table.insert(node_arr, { text = { "$" } })
  end
  ...
  traverse = function(arr)
    for _, node in ipairs(arr) do
      -- Clean up trailing `\`
      if node.after_slash and node.text ~= nil then
        table.insert(node.text, "\\")
      end
      ...
      if node.tabstop then
        node.tabstop = table.concat(node.tabstop)
      end
      if node.choices then
        node.choices = vim.tbl_map(table.concat, node.choices)
      end
      if node.var then
        node.var = table.concat(node.var)
      end
      if node.transform then
        node.transform = vim.tbl_map(table.concat, node.transform)
      end

      -- Recursively post-process placeholders
      if node.placeholder ~= nil then
        node.placeholder = traverse(node.placeholder)
      end
    end
    arr = vim.tbl_filter(function(n)
      return n.text == nil or (n.text ~= nil and n.text:len() > 0)
    end, arr)
    if #arr == 0 then
      return { { text = "" } }
    end
...
H.parse_normalize = function(node_arr, opts)
  ...
  local normalize = function(n)
    ...
    local var_value
    if n.var ~= nil then
      var_value = H.parse_eval_var(n.var, lookup)
    end
    ...
    if n.tabstop ~= nil then
      n.text = lookup[n.tabstop]
    end

    -- Ensure text-or-placeholder (use first choice for choice node)
    if n.text == nil and n.placeholder == nil then
      n.placeholder = { { text = (n.choices or {})[1] or "" } }
    end
    if n.text ~= nil and n.placeholder ~= nil then
      n.placeholder = nil
    end
  ...
  if not has_final_tabstop then
    table.insert(node_arr, { tabstop = "0", placeholder = { { text = "" } } })
  end
...
H.parse_nodes_contain_tabstop = function(node_arr, tabstop)
  for _, n in ipairs(node_arr) do
    if n.tabstop == tabstop then
      return true
    end
    if n.placeholder ~= nil and H.parse_nodes_contain_tabstop(n.placeholder, tabstop) then
      return true
    end
  end
  return false
...
H.parse_processors.text = function(c, s, n)
  ...
  if c == "}" and s:is_not_top_level() then
    return H.parse_rise_depth(s)
  end
  if c == "\\" then
    return s:set_in(n, "after_slash", true)
  end
  if c == "$" then
    return s:set_name("dollar")
  end
...
H.parse_processors.dollar = function(c, s, n)
  ...
  if c:find("^[0-9]$") then
    return s:add_node({ tabstop = { c } }):set_name("dollar_tabstop")
  end -- Tabstops
  if c:find("^[_a-zA-Z]$") then
    return s:add_node({ var = { c } }):set_name("dollar_var")
  end -- Variables
  if c == "{" then
    return s:set_name("dollar_lbrace")
  end -- Cases of `${...}`
...
H.parse_processors.dollar_tabstop = function(c, s, n)
  if c:find("^[0-9]$") then
    return table.insert(n.tabstop, c)
  end
  if c == "}" and s:is_not_top_level() then
    return H.parse_rise_depth(s)
  end
  ...
  if c == "$" then
    return s:set_name("dollar")
  end -- Case of `$1$2` and `$1$a`
  s:set_name("text")
  if c == "\\" then
    return s:set_in(new_node, "after_slash", true)
  end -- Case of `${1:{$2\}}`
  table.insert(new_node.text, c) -- Case of `$1a`
end

H.parse_processors.dollar_var = function(c, s, n)
  if c:find("^[_a-zA-Z0-9]$") then
    return table.insert(n.var, c)
  end
  if c == "}" and s:is_not_top_level() then
    return H.parse_rise_depth(s)
  end
...
H.parse_processors.dollar_lbrace = function(c, s, n)
  if n.tabstop == nil and n.var == nil then -- Detect the type of `${...}`
    if c:find("^[0-9]$") then
      return s:add_node({ tabstop = { c } })
    end
    if c:find("^[_a-zA-Z]$") then
      return s:add_node({ var = { c } })
    end
...
H.parse_processors.choice = function(c, s, n)
  ...
  if c == "," then
    return table.insert(n.choices, {})
  end
  if c == "\\" then
    return s:set_in(n, "after_slash", true)
  end
  if c == "|" then
    return s:set_in(n, "after_bar", true)
  end
  table.insert(cur, c)
end
...
H.parse_processors.transform_regex = function(c, s, n)
  ...
  if n.after_slash then
    return s:set_in(n, "after_slash", nil)
  end
  if c == "\\" then
    return s:set_in(n, "after_slash", true)
  end
  if c == "/" then
    return s:set_in(n.transform[1], #n.transform[1], nil):set_name("transform_format")
  end -- Assumes any `/` is escaped in regex
end
...
H.parse_processors.transform_format = function(c, s, n)
  ...
  if n.after_dollar then
    ...
    if c == "{" and not n.inside_braces then
      return s:set_in(n, "inside_braces", true)
    end
  end
  if c == "\\" then
    return s:set_in(n, "after_slash", true)
  ...
  if c == "$" then
    return s:set_in(n, "after_dollar", true)
  end
  if c == "}" and n.inside_braces then
    return s:set_in(n, "inside_braces", nil)
  end
  if c == "/" and not n.inside_braces then
...
H.parse_processors.transform_options = function(c, s, n)
  ...
  if c == "}" then
    return s:set_in(n.transform[3], #n.transform[3], nil):add_node({ text = {} }):set_name("text")
  end
end
...
-- Session --------------------------------------------------------------------
H.get_active_session = function()
  return H.sessions[#H.sessions]
end
...
H.session_init = function(session, full)
  ...
  if full then
    ...
    if session.cur_tabstop == "0" then
      ...
      if is_empty then
        -- Clean up
        H.nodes_traverse(session.nodes, function(n)
...
H.track_sessions = function()
  ...
  local stop_if_final = function(args)
    ...
    if vim.b.changedtick == latest_changedtick and args.event ~= "ModeChanged" then
      return
    end
    ...
    local session, buf_id = H.get_active_session(), args.buf
    if not ((session or {}).buf_id == buf_id and session.cur_tabstop == "0") then
      return
    end
...
H.clean_sessions = function()
  ...
  if #H.sessions > 0 and not H.session_is_valid(H.get_active_session()) then
    MiniSnippets.session.stop()
  end
end
  ...
  ensure_in_nodes = function(nodes)
    for _, n in ipairs(nodes) do
      ...
      if n.placeholder ~= nil then
        if parent_extmarks ~= nil then
          table.insert(parent_extmarks, n.extmark_id)
        end
        ensure_in_nodes(n.placeholder)
        if parent_extmarks ~= nil then
          parent_extmarks[#parent_extmarks] = nil
        end
      end
    end
  end
...
H.session_get_ref_node = function(session)
  ...
  local find = function(n)
    res = res or (n.tabstop == cur_tabstop and n or nil)
  end
...
H.session_sync_current_tabstop = function(session)
  ...
  if ref_node.placeholder ~= nil then
    ...
    if ref_row == phd_row and ref_col == phd_col then
      return
    end
  ...
  local sync = function(n)
    ...
    if not (n.tabstop == cur_tabstop and n.extmark_id ~= ref_extmark_id) then
      return
    end

    -- Ensure no placeholder because reference doesn't have one
    if n.placeholder ~= nil then
      H.nodes_del(buf_id, n.placeholder)
    end
  ...
  if ref_node.text == "" and cur_pos[1] == (row + 1) and cur_pos[2] == col then
    H.show_completion(ref_node.choices)
  end
...
H.session_update_hl = function(session)
  ...
  update_hl_in_nodes = function(nodes, is_in_cur_tabstop)
    for _, n in ipairs(nodes) do
      ...
      if n.placeholder ~= nil then
        update_hl_in_nodes(n.placeholder, is_in_cur_tabstop or n.tabstop == cur_tabstop)
      end
    end
  end
...
H.session_deinit = function(session, full)
  ...
  if not H.is_loaded_buf(session.buf_id) then
    return
  end
  ...
  H.nodes_traverse(session.nodes, function(n)
    extmark_fun(session.buf_id, n.extmark_id)
  end)

  -- Hide completion if stopping was done manually
  if not H.cache.stop_is_auto then
    H.hide_completion()
  end
end
...
H.nodes_set_text = function(buf_id, nodes, tracking_extmark_id, indent, cur_body_line)
  ...
  for _, n in ipairs(nodes) do
    ...
    if n.placeholder ~= nil then
      H.nodes_set_text(buf_id, n.placeholder, tracking_extmark_id, indent, cur_body_line)
    end
...
H.nodes_traverse = function(nodes, f, f_post)
  for i, n in ipairs(nodes) do
    ...
    n = f(n) or n
    if n.placeholder ~= nil then
      n.placeholder = H.nodes_traverse(n.placeholder, f, f_post)
    end
    if f_post then
      n = f_post(n) or n
    end
...
H.compute_tabstop_order = function(nodes)
  local tabstops_map = {}
  H.nodes_traverse(nodes, function(n)
    tabstops_map[n.tabstop or true] = true
  end)
  tabstops_map[true] = nil

  -- Order as numbers while allowing leading zeros. Put special `$0` last.
  local tabstops = vim.tbl_map(function(x)
    return { tonumber(x), x }
  end, vim.tbl_keys(tabstops_map))
...
H.extmark_del = function(buf_id, ext_id)
  vim.api.nvim_buf_del_extmark(buf_id, H.ns_id.nodes, ext_id or -1)
end
...
H.get_indent = function(line)
  ...
  for _, leader in ipairs(H.get_comment_leaders()) do
    ...
    if type(cur_match) == "string" and comment_indent:len() < cur_match:len() then
      comment_indent = cur_match
    end
  end
...
H.dedent = function(lines, row, col)
  ...
  for i = 2, #lines do
    ...
    if cur_indent:len() < indent_width and cur_indent ~= lines[i] then
      indent_width = cur_indent:len()
    end
  end
    ...
    return {
      request = function(method, params, callback, notify_reply_callback)
        ...
        if method == "textDocument/completion" then
          textdocument_completion(params, callback)
        end
        ...
        if notify_reply_callback then
          vim.schedule(function()
            pcall(notify_reply_callback, request_id)
          end)
        end
...
H.is_maybe_string_or_arr = function(x)
  return x == nil or H.is_string(x) or H.is_array_of(x, H.is_string)
end
...
H.is_position = function(x)
  return type(x) == "table" and type(x.line) == "number" and type(x.col) == "number"
end

H.is_region = function(x)
  return type(x) == "table" and H.is_position(x.from) and H.is_position(x.to)
end

-- Utilities ------------------------------------------------------------------
H.error = function(msg)
  error("(mini.snippets) " .. msg, 0)
end
...
H.notify = function(msg, level_name, silent)
  if not silent then
    vim.notify("(mini.snippets) " .. msg, vim.log.levels[level_name])
  end
end
...
H.is_loaded_buf = function(buf_id)
  return type(buf_id) == "number" and vim.api.nvim_buf_is_loaded(buf_id)
end

H.ensure_cur_buf = function(buf_id)
  if buf_id == 0 or buf_id == vim.api.nvim_get_current_buf() or not H.is_loaded_buf(buf_id) then
    return
  end
  local win_id = vim.fn.win_findbuf(buf_id)[1]
  if win_id == nil then
    return vim.api.nvim_win_set_buf(0, buf_id)
  end
...
H.delete_region = function(region)
  if not H.is_region(region) then
    return
  end
...
H.show_completion = function(items, startcol)
  if items == nil or #items == 0 or vim.fn.mode() ~= "i" then
    return
  end
...
H.hide_completion = function()
  ...
  if vim.fn.mode() == "i" then
    vim.cmd('silent noautocmd call complete(col("."), [])')
  end
end
