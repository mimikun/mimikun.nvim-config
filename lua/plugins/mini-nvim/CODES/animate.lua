...
MiniAnimate.is_active = function(animation_type)
  ...
  if res == nil then
    H.error("Wrong `animation_type` for `is_active()`.")
  end
  return res
end
...
MiniAnimate.execute_after = function(animation_type, action)
  ...
  if event_name == nil then
    H.error("Wrong `animation_type` for `execute_after`.")
  end

  local callable = action
  if type(callable) == "string" then
    callable = function()
      vim.cmd(action)
    end
  end
...
MiniAnimate.gen_timing.linear = function(opts)
  return H.timing_arithmetic(0, H.normalize_timing_opts(opts))
end
...
---
---@return __animate_timing_return
MiniAnimate.gen_timing.quadratic = function(opts)
  return H.timing_arithmetic(1, H.normalize_timing_opts(opts))
end
...
---
---@return __animate_timing_return
MiniAnimate.gen_timing.cubic = function(opts)
  return H.timing_arithmetic(2, H.normalize_timing_opts(opts))
end
...
---
---@return __animate_timing_return
MiniAnimate.gen_timing.quartic = function(opts)
  return H.timing_arithmetic(3, H.normalize_timing_opts(opts))
end
...
---
---@return __animate_timing_return
MiniAnimate.gen_timing.exponential = function(opts)
  return H.timing_geometrical(H.normalize_timing_opts(opts))
end
...
MiniAnimate.gen_path.line = function(opts)
  ...
  return function(destination)
    return H.path_line(destination, opts)
  end
end
...
MiniAnimate.gen_path.angle = function(opts)
  ...
  return function(destination)
    ...
    if not opts.predicate(destination) then
      return {}
    end
...
MiniAnimate.gen_path.walls = function(opts)
  ...
  return function(destination)
    -- Don't animate in case of false predicate
    if not predicate(destination) then
      return {}
    end
...
MiniAnimate.gen_subscroll.equal = function(opts)
  ...
  return function(total_scroll)
    return H.subscroll_equal(total_scroll, opts)
  end
end
...
MiniAnimate.gen_subresize.equal = function(opts)
  ...
  return function(sizes_from, sizes_to)
    return H.subresize_equal(sizes_from, sizes_to, opts)
  end
end
...
MiniAnimate.gen_winconfig.static = function(opts)
  ...
  return function(win_id)
    return H.winconfig_static(win_id, opts)
  end
end
...
H.is_disabled = function()
  return vim.g.minianimate_disable == true or vim.b.minianimate_disable == true
end
...
H.auto_cursor = function()
  ...
  if new_state.buf_id ~= prev_state.buf_id then
    return
  end
...
H.auto_resize = function()
  ...
  if not (same_state.layout and not same_state.sizes) then
    return
  end
...
H.auto_scroll = function()
  ...
  if is_same_topline and is_same_bufwin then
    return
  end
...
H.ignore_incsearch_scroll = function()
  ...
  if not (is_insearch or H.cache.scroll_state.aborted_incsearch) then
    return
  end
...
H.auto_openclose = function(action_type)
  ...
  if not config.enable or H.is_disabled() then
    return
  end
  ...
  if action_type == "close" then
    win_id = tonumber(vim.fn.expand("<amatch>"))
  end
  if action_type == "open" then
    win_id = math.max(unpack(vim.api.nvim_list_wins()))
  end
...
-- General animation ----------------------------------------------------------
H.trigger_done_event = function(animation_type)
  vim.cmd("doautocmd User " .. H.animation_done_events[animation_type])
end
...
H.make_cursor_step = function(state_from, state_to, opts)
  ...
  return {
    step_action = function(step)
      ...
      if H.cache.cursor_event_id ~= event_id then
        return false
      end

      -- Don't draw outside of set number of steps or not inside current buffer
      if n_steps <= step or vim.api.nvim_get_current_buf() ~= buf_id then
        return H.stop_cursor()
      end
...
H.undraw_cursor_mark = function(buf_id)
  pcall(vim.api.nvim_buf_del_extmark, buf_id, H.ns_id.cursor, 1)
end
...
H.make_scroll_step = function(state_from, state_to, opts)
  ...
  return {
    step_action = function(step)
      ...
      if H.cache.scroll_event_id ~= event_id then
        return false
      end
      ...
      if not is_same_win_buf then
        return H.stop_scroll()
      end
...
H.scroll_action = function(key, n, cursor_data)
  ...
  if virtcol_past_lineend <= virtcol then
    col = col + virtcol - virtcol_past_lineend + 1
  end
...
H.make_resize_step = function(state_from, state_to, opts)
  ...
  if step_sizes == nil or #step_sizes == 0 then
    return
  end
  ...
  return {
    step_action = function(step)
      ...
      if H.cache.resize_event_id ~= event_id then
        return false
      end
...
H.start_resize = function(start_state)
  ...
  if start_state ~= nil then
    H.apply_resize_state(start_state, false)
  end
  return true
end
...
H.stop_resize = function(end_state)
  if end_state ~= nil then
    H.apply_resize_state(end_state, true)
  end
...
H.get_resize_state = function()
  ...
  for _, win_id in ipairs(windows) do
    ...
    views[win_id] = vim.api.nvim_win_call(win_id, function()
...
H.make_openclose_step = function(action_type, win_id, config)
  ...
  if step_winconfigs == nil or #step_winconfigs == 0 then
    return
  end
...
H.default_path_predicate = function(destination)
  return destination[1] < -1 or 1 < destination[1]
end

-- Animation subscroll --------------------------------------------------------
H.subscroll_equal = function(total_scroll, opts)
  -- Don't animate in case of false predicate
  if not opts.predicate(total_scroll) then
    return {}
  end
...
H.default_subscroll_predicate = function(total_scroll)
...
H.subresize_equal = function(sizes_from, sizes_to, opts)
  -- Don't animate in case of false predicate
  if not opts.predicate(sizes_from, sizes_to) then
    return {}
  end
...
H.default_subresize_predicate = function(sizes_from, sizes_to)
  return true
end
...
H.winconfig_static = function(win_id, opts)
  -- Don't animate in case of false predicate
  if not opts.predicate(win_id) then
    return {}
  end
...
H.default_winconfig_predicate = function(win_id)
  return true
end

-- Utilities ------------------------------------------------------------------
H.error = function(msg)
  error("(mini.animate) " .. msg, 0)
end
...
H.convex_point = function(x, y, coef)
