...
MiniMove.move_selection = function(direction, opts)
  ...
  if vim.api.nvim_buf_line_count(0) == 1 then
    if is_linewise and dir_type == "vert" then
      return
    end
    if not is_linewise and vim.fn.getline(1):len() == 0 then
      return
    end
  end
...
MiniMove.move_line = function(direction, opts)
  ...
  if vim.api.nvim_buf_line_count(0) == 1 and (direction == "down" or direction == "up") then
    return
  end
