...
MiniCompletion.default_process_items = function(items, base, opts)
  ...
  if type(fs) == "string" then
    fs = H.filtersort_methods[fs]
  end
  if not vim.is_callable(fs) then
    H.error('`filtersort` should be callable or one of "prefix", "fuzzy", "none"')
  end
  local res = fs(items, base)

  -- Arrange by kind
  if opts.kind_priority ~= nil then
    res = H.lsp_arrange_by_kind(res, opts.kind_priority)
  end
...
MiniCompletion.get_lsp_capabilities = function(opts)
  ...
  if opts.resolve_additional_text_edits then
    table.insert(resolve_support, 1, "additionalTextEdits")
  end
...
H.setup_config = function(config)
  ...
  local is_string_or_array = function(x)
    return type(x) == "string" or H.islist(x)
  end
...
H.apply_config = function(config)
  ...
  local map_scroll = function(lhs, direction)
    local rhs = function()
      return MiniCompletion.scroll(direction) and "" or lhs
    end
...
H.auto_info = function()
  ...
  H.info.timer:start(delay, 0, function()
    H.show_info_window(cur_info_id)
  end)
end
...
H.auto_signature = function()
  ...
  if not (is_shown or left_char_is_trigger) then
    return
  end
...
H.trigger_lsp = function()
  ...
  if vim.fn.mode() ~= "i" or (H.pumvisible() and not H.completion.force) then
    return
  end
  ...
  if H.completion.lsp.status ~= "received" then
    return H.make_completion_request()
  end
...
H.trigger_fallback = function()
  ...
  if has_popup or vim.fn.mode() ~= "i" then
    return
  end
  ...
  if vim.is_callable(fallback_action) then
    return fallback_action()
  end
  if type(fallback_action) ~= "string" then
    return
  end
...
H.default_fallback_action = function()
...
H.stop_completion = function(keep_source, keep_lsp_is_incomplete, keep_lsp_resolved)
  ...
  if not keep_lsp_is_incomplete then
    H.completion.lsp.is_incomplete = false
  end
  if not keep_lsp_resolved then
    H.completion.lsp.resolved = {}
  end
end
...
H.filtersort_methods = {
  prefix = function(items, base)
    local res = vim.tbl_filter(function(x)
      return vim.startswith(H.lsp_get_filterword(x), base)
    end, items)
  ...
  none = function(items, _)
    return vim.deepcopy(items)
  end,
}

H.lsp_arrange_by_kind = function(items, kind_priority)
  if type(kind_priority) ~= "table" then
    H.error("`kind_priority` should be table")
  end
  ...
  for i, item in ipairs(items) do
    ...
    if priority >= 0 then
      table.insert(res_raw, { priority, i, item })
    end
  end
...
H.lsp_get_filterword = function(x)
  return x.filterText or x.label
end

H.lsp_item_compare = function(a, b)
...
H.make_lsp_extra_actions = function(lsp_data)
  ...
  if item.additionalTextEdits == nil and item.command == nil and not lsp_data.needs_snippet_insert then
    return
  end
...
if vim.fn.has("nvim-0.11") == 0 then
  H.exec_command = function(_, _) end
end
...
H.show_info_window = vim.schedule_wrap(function(info_id)
  ...
  if lines == nil or H.is_whitespace(lines) then
    lines = { "-No-info-" }
  end
...
H.info_window_lines = function(info_id)
  ...
  if resolved_cache[item_id] ~= nil then
    return H.normalize_item_doc(resolved_cache[item_id], info)
  end
...
H.show_signature_window = function()
  ...
  if vim.fn.mode() == "i" then
    H.ensure_action_window("signature", opts)
  end
end
...
H.ensure_buffer = function(cache, name)
  if H.is_loaded_buf(cache.bufnr) then
    return
  end
...
H.ensure_highlight = function(cache, filetype)
  if cache.hl_filetype == filetype then
    return
  end
...
H.close_action_window = function(cache)
  ...
  if H.is_valid_win(cache.win_id) then
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "minicompletion://" .. buf_id .. "/" .. name)
end
...
H.get_completion_range = function(lsp_result)
  ...
  for _, response_data in pairs(lsp_result or {}) do
    ...
    if range ~= nil then
      return { range.start.line + 1, range.start.character }, pos
    end
  end
...
H.get_lsp_edit_range = function(response_data)
  -- TODO: Use only `.err` after compatibility with Neovim=0.10 is dropped
  if response_data.err or response_data.error or type(response_data.result) ~= "table" then
    return
  end
  ...
  if type(edit_range) == "table" then
    return edit_range.insert or edit_range
  end
  ...
  for _, item in ipairs(items) do
    ...
    if type(item.textEdit) == "table" then
      return item.textEdit.range or item.textEdit.insert
    end
  end
end
...
H.del_extmark = function(extmark_id, with_text)
  ...
  if not with_text or data[1] == nil or data[3].end_row == nil then
    return data
  end
...
H.normalize_item_doc = function(lsp_item, fallback_info)
  ...
  if detail == nil and doc == nil then
    return {}
  end
  ...
  if text == "" and fallback_info ~= "" then
    text = H.wrap_in_codeblock(fallback_info)
  end
...
if vim.fn.has("nvim-0.11") == 0 then
  H.ensure_no_concealed_lines = function(buf_id) end
end
...
H.wrap_in_codeblock = function(x)
  return string.format("```%s\n%s\n```", vim.bo.filetype:match("^[^%.]*"), vim.trim(x))
end

-- TODO: Remove after compatibility with Neovim=0.9 is dropped
H.islist = vim.fn.has("nvim-0.10") == 1 and vim.islist or vim.tbl_islist

H.get_buf_lsp_clients = function()
