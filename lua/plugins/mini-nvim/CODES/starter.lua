...
MiniStarter.open = function(buf_id)
  ...
  local trigger_event = function()
    vim.api.nvim_exec_autocmds("User", { pattern = "MiniStarterOpened" })
  end
  if H.is_in_vimenter then
    trigger_event = vim.schedule_wrap(trigger_event)
  end
  trigger_event()
...
MiniStarter.refresh = function(buf_id)
  ...
  if not H.validate_starter_buf_id(buf_id, "refresh()") then
    return
  end
  ...
  if not vim.deep_equal(data.items, old_items) then
    data.current_item_id = 1
  end
...
MiniStarter.close = function(buf_id)
  buf_id = buf_id or vim.api.nvim_get_current_buf()
  if not H.validate_starter_buf_id(buf_id, "close()") then
    return
  end
...
MiniStarter.get_content = function(buf_id)
  buf_id = buf_id or vim.api.nvim_get_current_buf()
  if not H.validate_starter_buf_id(buf_id, "get_content()", "error") then
    return
  end
...
MiniStarter.eval_current_item = function(buf_id)
  buf_id = buf_id or vim.api.nvim_get_current_buf()
  if not H.validate_starter_buf_id(buf_id, "eval_current_item()") then
    return
  end
...
MiniStarter.update_current_item = function(direction, buf_id)
  buf_id = buf_id or vim.api.nvim_get_current_buf()
  if not H.validate_starter_buf_id(buf_id, "update_current_item()") then
    return
  end
  ...
  if data.current_item_id == prev_current then
    return
  end
...
MiniStarter.add_to_query = function(char, buf_id)
  buf_id = buf_id or vim.api.nvim_get_current_buf()
  if not H.validate_starter_buf_id(buf_id, "add_to_query()") then
    return
  end
...
H.normalize_items = function(items)
  ...
  if #res == 0 then
    return { { name = "`config.items` is empty", action = "", section = "" } }
  end
...
H.content_add_line = function(content, content_line)
...
H.make_query = function(buf_id, query, echo_msg)
  ...
  if not data.items[data.current_item_id]._active then
    MiniStarter.update_current_item("next", buf_id)
  end
...
H.make_buffer_autocmd = function(buf_id)
  ...
  au({ "VimResized", "BufEnter" }, function()
...
H.get_buffer_windows = function(buf_id)
  return vim.tbl_filter(function(win_id)
    return vim.api.nvim_win_get_buf(win_id) == buf_id
