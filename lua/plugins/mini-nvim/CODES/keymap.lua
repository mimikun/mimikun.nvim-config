...
MiniKeymap.gen_step.search_pattern = function(pattern, flags, opts)
  if type(pattern) ~= "string" then
    H.error("`pattern` should be string, not " .. vim.inspect(type(pattern)))
  end
  flags = flags or ""
  if type(flags) ~= "string" then
    H.error("`flags` should be string, not " .. vim.inspect(type(flags)))
  end

  opts = vim.tbl_extend("force", { side = "before", stopline = nil, timeout = nil, skip = nil }, opts or {})
  local side = opts.side
  if not (side == "before" or side == "after") then
    H.error('`opts.side` should be one of "before" or "after"')
  end

  local stopline = opts.stopline or function()
    return nil
  end
  ...
  if not vim.is_callable(stopline) then
    H.error("`opts.stopline` should be number or callable")
  end
...
MiniKeymap.map_combo = function(mode, lhs, action, opts)
  ...
  if not H.is_array_of(mode, H.is_string) then
    H.error("`mode` should be string or array of strings")
  end
  ...
  if not (type(delay) == "number" and delay > 0) then
    H.error("`opts.delay` should be a positive number")
  end
  ...
  local watcher = function(key, typed)
    ...
    if key == "" or (i == 0 and not mode_tbl[get_mode()]) or H.combo_ignore then
      return
    end
...
H.apply_config = function(config)
  MiniKeymap.config = config
end

H.is_disabled = function()
  return vim.g.minikeymap_disable == true or vim.b.minikeymap_disable == true
end

-- Combo ----------------------------------------------------------------------
H.combo_lhs_to_seq = function(lhs)
  if H.is_array_of(lhs, H.is_string) then
    return vim.deepcopy(lhs)
  end
  if type(lhs) ~= "string" then
    H.error("`lhs` should be string or array of strings")
  end
...
H.combo_unignore_later = vim.schedule_wrap(function()
  H.combo_ignore = false
end)

H.combo_get_key = function(_, typed)
  return typed
end
if vim.fn.has("nvim-0.11") == 0 then
  H.combo_get_key = function(key)
    return key
  end
end
...
H.normalize_steps = function(steps)
  if not H.islist(steps) then
    H.error("`steps` should be array")
  end
  local res = {}
  for i, step in ipairs(steps) do
    ...
    if not is_step then
      H.error("`steps` should contain valid steps, not " .. vim.inspect(step))
    end
    table.insert(res, s)
  end
...
H.make_cmd_lua_action = function(cmd_string)
  return function()
    return "<Cmd>lua " .. cmd_string .. "<CR>"
  end
end

H.has_module = function(name)
...
H.notify = function(msg, level_name, silent)
  if not silent then
    vim.notify("(mini.keymap) " .. msg, vim.log.levels[level_name])
  end
end

H.normalize_pos = function(row, col)
  -- Input is {0,0} indexed, output is {1,0} indexed
  if row < 0 or (row == 0 and col < 0) then
    return { 1, 0 }
  end
  ...
  if row > last_row or (row == last_row and col > n_col_last_row) then
    return { last_row + 1, n_col_last_row }
  end

  if col < 0 then
    return { row, H.get_row_cols(row - 1) }
  end
  if col > H.get_row_cols(row) then
    return { row + 2, 0 }
  end
  return { row + 1, col }
end

H.get_row_cols = function(row)
  return vim.fn.getline(row + 1):len()
end

H.get_tsnode = function()
