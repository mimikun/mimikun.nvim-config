...
MiniNotify.make_notify = function(opts)
  ...
  for key, val in pairs(opts) do
    if default_opts[key] == nil then
      H.error("Keys should be log level names.")
    end
    if type(val) ~= "table" then
      H.error("Level data should be table.")
    end
    if type(val.duration) ~= "number" then
      H.error("`duration` in level data should be number.")
    end
    if type(val.hl_group) ~= "string" then
      H.error("`hl_group` in level data should be string.")
    end
  end
  ...
  return vim.schedule_wrap(function(msg, level)
    ...
    if level_name == nil then
      H.error("Only valid values of `vim.log.levels` are supported.")
    end
...
MiniNotify.update = function(id, new)
  ...
  if notif == nil then
    H.error("`id` is not an identifier of active notification.")
  end
  H.check_type("new", new, "table")

  if new.msg ~= nil then
    H.validate_msg(new.msg)
  end
  if new.level ~= nil then
    H.validate_level(new.level)
  end
  if new.hl_group ~= nil then
    H.validate_hl_group(new.hl_group)
  end
...
MiniNotify.refresh = function()
  if vim.in_fast_event() then
    return vim.schedule(MiniNotify.refresh)
  end
  ...
  if H.is_disabled() or type(vim.v.exiting) == "number" then
  ...
  if not H.is_notification_array(notif_arr) then
    H.error("Output of `content.sort` should be notification array.")
  end
  if #notif_arr == 0 then
    return H.window_close()
  end
...
MiniNotify.get = function(id)
  return vim.deepcopy(H.history[id])
end
...
H.create_autocommands = function()
  ...
  au({ "TabEnter", "VimResized" }, "*", function()
    MiniNotify.refresh()
...
H.is_disabled = function()
  return vim.g.mininotify_disable == true or vim.b.mininotify_disable == true
end
...
H.lsp_progress_handler = function(err, result, ctx, config)
  ...
  if not lsp_progress_config.enable then
    return
  end
  ...
  if value.kind == "end" then
    ...
    vim.defer_fn(function()
      MiniNotify.remove(progress_info.notif_id)
    end, delay)
  end
end
...
H.window_close = function()
  if H.is_valid_win(H.cache.win_id) then
    vim.api.nvim_win_close(H.cache.win_id, true)
  end
  H.cache.win_id = nil
end
...
H.validate_msg = function(x)
  if type(x) ~= "string" then
    H.error("`msg` should be string.")
  end
end

H.validate_level = function(x)
  if vim.log.levels[x] == nil then
    H.error("`level` should be key of `vim.log.levels`.")
  end
end

H.validate_hl_group = function(x)
  if type(x) ~= "string" then
    H.error("`hl_group` should be string.")
  end
end
...
H.is_notification_array = function(x)
  ...
  for _, y in ipairs(x) do
    if not H.is_notification(y) then
      return false
    end
  end
  return true
...
H.notif_apply_format = function(notif_arr, format)
  for _, notif in ipairs(notif_arr) do
    ...
    if type(res) ~= "string" then
      H.error("Output of `content.format` should be string.")
    end
    notif.msg = res
  end
  return notif_arr
...
H.is_win_in_tabpage = function(win_id)
