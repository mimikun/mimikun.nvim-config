...
MiniOperators.evaluate = function(mode)
  if H.is_disabled() or not vim.bo.modifiable then
    return ""
  end
...
MiniOperators.exchange = function(mode)
  ...
  if not H.exchange_has_step_one() then
    ...
    if H.cache.exchange.step_two == nil then
      return
    end
...
MiniOperators.multiply = function(mode)
  ...
  H.with_temp_context({ registers = { "x", '"' } }, function()
    ...
    local func = H.get_config().multiply.func or function(content)
...
MiniOperators.make_mappings = function(operator_name, lhs_tbl)
  ...
  if not is_keys_tbl then
    H.error("`lhs_tbl` should be a valid table of keys.")
  end
...
MiniOperators.default_sort_func = function(content, opts)
  ...
  local compare_fun = opts.compare_fun or function(a, b)
    return a < b
  end
  if not vim.is_callable(compare_fun) then
    H.error("`opts.compare_fun` should be callable.")
  end

  local split_patterns = opts.split_patterns or { "%s*,%s*", "%s*;%s*", "%s+", "" }
  if not H.islist(split_patterns) then
    H.error("`opts.split_patterns` should be array.")
  end
...
H.is_disabled = function()
  return vim.g.minioperators_disable == true or vim.b.minioperators_disable == true
end
...
H.exchange_do = function()
  ...
  if H.exchange_is_same_steps(step_one, step_two) then
    return
  end
...
H.exchange_set_region_extmark = function(mode, add_highlight)
  ...
  if region_data.mark_to == ">" and vim.o.selection == "exclusive" then
    extmark_to[2] = extmark_to[2] - 1
  end
  ...
  if add_highlight and submode ~= H.submode_keys.block then
    extmark_hl_group = "MiniOperatorsExchangeFrom"
  end
...
H.exchange_stop = function()
  ...
  if cur.step_one ~= nil then
    pcall(vim.api.nvim_buf_clear_namespace, cur.step_one.buf_id, ns_id, 0, -1)
  end
  if cur.step_two ~= nil then
    pcall(vim.api.nvim_buf_clear_namespace, cur.step_two.buf_id, ns_id, 0, -1)
  end
  H.cache.exchange = {}
end
...
H.exchange_is_same_steps = function(step_one, step_two)
  if step_one.buf_id ~= step_two.buf_id or step_one.submode ~= step_two.submode then
    return false
  end
...
H.multiply_get_ref_coords = function(mark_from, mark_to, submode)
  ...
  if mark_to == ">" and vim.o.selection == "exclusive" then
    markcoords_to[2] = markcoords_to[2] - 1
  end

  if submode ~= H.submode_keys.block then
    return markcoords_to
  end
...
H.replace_do = function(data)
  ...
  if reg_is_invalid then
    H.error("Register " .. vim.inspect(register) .. " is invalid.")
  end

  -- Get reginfo and infer missing data (can be empty for special registers)
  local reg_info = vim.fn.getreginfo(register)
  if reg_info.regcontents == nil then
    H.error("Register " .. vim.inspect(register) .. " is empty.")
  end
  ...
  if should_reindent then
    init_indent = H.get_region_indent(mark_from, mark_to)
  end
  ...
  if tmp_reg_info.regtype:sub(1, 1) ~= submode then
    tmp_reg_info.regtype = submode
  end
  if should_reindent then
    tmp_reg_info.regcontents = H.update_indent(tmp_reg_info.regcontents, init_indent)
  end
  ...
  if covers_linewise_all_buffer then
...
H.do_between_marks = function(operator, data)
  ...
  if data.mode == "block" and vim.o.selection == "exclusive" then
  ...
  local context_marks = { "<", ">" }
  if is_yank then
    context_marks = vim.list_extend(context_marks, { "[", "]" })
  end
...
H.is_content = function(x)
  return type(x) == "table" and H.islist(x.lines) and type(x.submode) == "string"
end
...
H.get_region_data = function(mode)
  ...
  if selection_is_visual and H.is_visual_mode() then
    vim.cmd("normal! \27")
  end
  ...
  if pos_to[1] < pos_from[1] or (pos_to[1] == pos_from[1] and pos_to[2] < pos_from[2]) then
    return
  end
...
H.get_mark = function(mark_name)
  return vim.api.nvim_buf_get_mark(0, mark_name)
end

H.set_mark = function(mark_name, mark_data)
  vim.api.nvim_buf_set_mark(0, mark_name, mark_data[1], mark_data[2], {})
end
...
H.get_submode = function(mode)
  if mode == "visual" then
    return H.is_visual_mode() and vim.fn.mode() or vim.fn.visualmode()
  end
