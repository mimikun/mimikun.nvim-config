...
MiniBracketed.buffer = function(direction, opts)
  ...
  if res_buf_id == iterator.state then
    return
  end
...
MiniBracketed.comment = function(direction, opts)
  ...
  local predicate = ({
    near = function(_, cur, _, recent)
      return cur and not recent
    end,
    start = function(above, cur, _, _)
      return cur and not above
    end,
    ["end"] = function(_, cur, below, _)
      return cur and not below
    end,
    both = function(above, cur, below, _)
      return cur and not (above and below)
    end,
  })[opts.block_side]
  ...
  iterator.next = function(line_num)
    ...
    for lnum = line_num + 1, n_lines do
      ...
      if predicate(above, cur, below, above) then
        return lnum
      end
      above, cur = cur, below
    end
  ...
  iterator.prev = function(line_num)
    ...
    for lnum = line_num - 1, 1, -1 do
      ...
      if predicate(above, cur, below, below) then
        return lnum
      end
      below, cur = cur, above
    end
  ...
  if res_line_num == nil or res_line_num == iterator.state or is_outside then
    return
  end
...
MiniBracketed.conflict = function(direction, opts)
  ...
  iterator.next = function(line_num)
    for lnum = line_num + 1, n_lines do
      if H.is_conflict_mark(lnum) then
        return lnum
      end
    end
  end
...
MiniBracketed.diagnostic = function(direction, opts)
  ...
  if vim.is_callable(default_opts.float) then
    default_opts.float = default_opts.float(nil, buf_id)
  end
  ...
  local to_cursor_pos = function(pos)
  ...
  if res_pos == nil or res_pos == iterator.state then
    return
  end
...
MiniBracketed.indent = function(direction, opts)
  ...
  -- Compute loop data to traverse target commented lines in current buffer
  local predicate = ({
    less = function(new, cur)
      return new < cur or cur == 0
    end,
    more = function(new, cur)
      return new > cur
    end,
    diff = function(new, cur)
      return new ~= cur
    end,
  })[opts.change_type]
  if predicate == nil then
  ...
  iterator.next = function(cur_lnum)
    ...
    while new_lnum > 0 do
      ...
      if predicate(new_indent, cur_indent) then
        return new_lnum
      end
  ...
  -- Iterate
  local res_line_num = MiniBracketed.advance(iterator, direction, opts)
  if res_line_num == nil or res_line_num == iterator.state then
    return
  end
...
MiniBracketed.jump = function(direction, opts)
  ...
  iterator.next = function(jump_num)
    for num = jump_num + 1, n_list do
      if is_jump_num_from_current_buffer(num) then
        return num
      end
    end
  end
  ...
  if res_jump_num == nil or jump_list[res_jump_num] == nil then
    return
  end
...
MiniBracketed.oldfile = function(direction, opts)
  ...
  if res_arr_ind == nil or res_arr_ind == iterator.state then
    return
  end
...
MiniBracketed.undo = function(direction, opts)
  ...
  if res_id == nil or res_id == iterator.state then
    return
  end
...
MiniBracketed.window = function(direction, opts)
  ...
  if res_win_nr == iterator.state then
    return
  end
...
MiniBracketed.yank = function(direction, opts)
  ...
  if not vim.deep_equal(cur_state, cache_yank.state) then
  ...
  iterator.next = function(id)
    for i = id + 1, n_history do
      if vim.tbl_contains(opts.operators, history[i].operator) then
        return i
      end
    end
  end
...
H.is_disabled = function()
  return vim.g.minibracketed_disable == true or vim.b.minibracketed_disable == true
end
...
H.make_comment_checker = function()
  ...
  return function(line_num)
    return vim.fn.getline(line_num):find(regex) ~= nil
  end
end
...
H.oldfile_normalize = function()
  ...
  for path, rec in pairs(H.cache.oldfile.recency) do
    if vim.fn.filereadable(path) == 1 then
      table.insert(recency_pairs, { path, rec })
    end
  end
  table.sort(recency_pairs, function(x, y)
    return x[2] < y[2]
  end)
...
H.oldfile_ensure_initialized = function()
  if H.cache.oldfile ~= nil or vim.v.oldfiles == nil then
    return
  end
...
H.qf_loc_implementation = function(list_type, direction, opts)
  ...
  if list_type == "location" then
    get_list, goto_command = function(...)
...
H.undo_sync = function(buf_id, tree, is_advancing)
  ...
  if is_advancing == nil then
    is_advancing = prev_buf_history.is_advancing
  end
  ...
  for i, state_num in ipairs(prev_buf_history) do
    ...
    if allowed_states[state_num] then
      H.undo_append_state(buf_history, state_num)
    end

    -- Correctly track current id when advancing
    if i == prev_buf_history.current_id then
      buf_history.current_id = #buf_history
    end
  end
  ...
  -- (due to `:undo!`).
  if is_advancing and tree.seq_last <= buf_history.seq_last and #buf_history == #prev_buf_history then
    return
  end
  ...
  for new_state = buf_history.seq_last + 1, tree.seq_last do
    if allowed_states[new_state] then
      H.undo_append_state(buf_history, new_state)
    end
  end
...
H.undo_append_state = function(buf_history, state_num)
  ...
  if state_num == nil or buf_history[#buf_history] == state_num then
    return
  end
...
H.get_yank_state = function()
  return { buf_id = vim.api.nvim_get_current_buf(), changedtick = vim.b.changedtick }
end
...
H.set_cursor = function(row, col)
  ...
  local n_lines = vim.api.nvim_buf_line_count(0)
  if n_lines < row then
    return vim.api.nvim_win_set_cursor(0, { n_lines, vim.fn.getline(n_lines):len() - 1 })
  end
