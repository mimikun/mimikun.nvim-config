...
MiniAlign.config = {
  ...
  modifiers = {
    ...
    [vim.api.nvim_replace_termcodes("<BS>", true, true, true)] = function(steps, _)
      ...
      if char == "s" then
        table.remove(steps.pre_split, #steps.pre_split)
      end
      if char == "j" then
        table.remove(steps.pre_justify, #steps.pre_justify)
      end
      if char == "m" then
        table.remove(steps.pre_merge, #steps.pre_merge)
      end
    end,
    --minidoc_replace_end
...
MiniAlign.align_strings = function(strings, opts, steps)
  ...
  if not H.is_array_of(new_strings, H.is_string) then
    H.error("Output of `merge` step should be array of strings.")
  end
  return new_strings
end
...
MiniAlign.align_user = function(mode)
  ...
  if steps_are_from_cache or (not with_preview and opts.split_pattern ~= "") then
    return
  end
  ...
  while true do
    ...
    if id == nil or n_iter > 1000 then
      if lines_were_changed then
        H.undo()
      end
      if n_iter > 1000 then
        H.echo({ { "Too many modifiers typed.", "WarningMsg" } }, true)
      end
      break
    end

    -- Stop preview after `<CR>` (confirmation)
    if with_preview and id == "\r" then
      break
    end
    ...
    if not with_preview and opts.split_pattern ~= "" then
      break
    end
  end
...
MiniAlign.as_parts = function(arr2d)
    ...
    for i, row in ipairs(parts) do
      for j, s in ipairs(row) do
        ...
        if type(new_val) ~= "string" then
          H.error("Input of `apply_inplace()` method should always return string.")
        end
        parts[i][j] = new_val
      end
    end
  ...
  methods.trim = function(direction, indent)
    ...
    local row_indent = vim.tbl_map(function(row)
      return row[1]:match("^(%s*)")
    end, parts)
...
MiniAlign.new_step = function(name, action)
  if type(name) ~= "string" then
    H.error("Step name should be string.")
  end
  if not vim.is_callable(action) then
    H.error("Step action should be callable.")
  end
...
MiniAlign.gen_step.default_split = function()
  return MiniAlign.new_step("split", H.default_action_split)
end
...
MiniAlign.gen_step.default_justify = function()
  return MiniAlign.new_step("justify", H.default_action_justify)
end
...
MiniAlign.gen_step.default_merge = function()
  return MiniAlign.new_step("merge", H.default_action_merge)
end
...
MiniAlign.gen_step.ignore_split = function(patterns, exclude_comment)
  ...
  local action = function(_, opts)
    ...
    for _, patt in ipairs(patterns) do
      if not vim.tbl_contains(excl, patt) then
        table.insert(excl, patt)
      end
    end
    ...
    if exclude_comment then
      ...
      if not vim.tbl_contains(excl, comment_pattern) then
        table.insert(excl, comment_pattern)
      end
    end
...
MiniAlign.gen_step.pair = function(direction)
  return MiniAlign.new_step("pair", function(parts, _)
    parts.pair(direction)
  end)
end
...
MiniAlign.gen_step.trim = function(direction, indent)
  return MiniAlign.new_step("trim", function(parts, _)
    parts.trim(direction, indent)
  end)
end
...
H.pad_functions = {
  left = function(x, n_spaces, no_trailing)
    if no_trailing or H.is_infinite(n_spaces) then
      return x
    end
  ...
  right = function(x, n_spaces, no_trailing)
    if (no_trailing and H.is_whitespace(x)) or H.is_infinite(n_spaces) then
      return x
    end
...
H.trim_functions = {
  both = function(x)
    return H.trim_functions.left(H.trim_functions.right(x))
  end,
  left = function(x)
...
H.setup_config = function(config)
  ...
  for k, v in pairs(config.modifiers) do
    if not vim.is_callable(v) then
      H.error(string.format("`modifiers[%s]` should be callable.", vim.inspect(k)))
    end
  end
...
H.is_disabled = function()
  return vim.g.minialign_disable == true or vim.b.minialign_disable == true
end
...
H.is_valid_steps = function(x, x_name)
  ...
  if type(x) ~= "table" then
    return false, string.format("`%s` should be table.", x_name)
  end

  -- Validators
  local is_steps_array = function(y)
    return H.is_array_of(y, H.is_step)
  end
  local steps_array_msg = "should be array of steps (see `:h MiniAlign.new_step()`)."

  local is_maybe_step = function(y)
    return y == nil or H.is_step(y)
  end
  local step_msg = "should be step (see `:h MiniAlign.new_step()`)."

  -- Actual checks
  if not is_steps_array(x.pre_split) then
    return false, H.msg_bad_steps(x_name, "pre_split", steps_array_msg)
  end

  if not is_maybe_step(x.split) then
    return false, H.msg_bad_steps(x_name, "split", step_msg)
  end

  if not is_steps_array(x.pre_justify) then
    return false, H.msg_bad_steps(x_name, "pre_justify", steps_array_msg)
  end

  if not is_maybe_step(x.justify) then
    return false, H.msg_bad_steps(x_name, "justify", step_msg)
  end

  if not is_steps_array(x.pre_merge) then
    return false, H.msg_bad_steps(x_name, "pre_merge", steps_array_msg)
  end

  if not is_maybe_step(x.merge) then
    return false, H.msg_bad_steps(x_name, "merge", step_msg)
  end
...
H.msg_bad_steps = function(steps_name, key, msg)
  return string.format("`%s.%s` %s", steps_name, key, msg)
end
...
H.apply_step = function(step, arr, opts, step_container_name)
  ...
  if not H.is_parts(arr) then
    ...
    predicate = function(x)
      return H.is_array_of(x, H.is_string)
    end
    suggest = ""
  end
...
H.default_action_split = function(string_array, opts)
  ...
  local add_to_forbidden = function(l, r)
    table.insert(forbidden_spans, { l, r - 1 })
  end
  ...
  local res = vim.tbl_map(function(s)
    return H.default_action_split_string(s, pattern, make_forbidden_spans)
  end, string_array)
...
H.default_action_justify = function(parts, opts)
  ...
  for i, row in ipairs(parts) do
    ...
    for j, s in ipairs(row) do
      ...
      if not (j == #row and side_arr[j] == "left") then
        width_col[j] = math.max(off + w, width_col[j])
      end
    end
  end
...
H.default_action_merge = function(parts, opts)
  ...
  return vim.tbl_map(function(row)
    return H.concat_array(row, delim_arr)
  end, parts)
end
...
H.make_filter_action = function(expr)
  ...
  local is_loaded, f = pcall(function()
    return assert(loadstring("return " .. expr))
  end)
  if not (is_loaded and vim.is_callable(f)) then
    H.error(vim.inspect(expr) .. " is not a valid filter expression.")
  end
...
H.region_get_text = function(region, mode)
  ...
  if mode == "line" then
    return H.get_lines(from.line - 1, to.line)
  end
...
H.region_set_text = function(region, mode, text)
  ...
  if mode == "line" then
    H.set_lines(from.line - 1, to.line, text)
  end
...
H.pos_to_virtcol = function(pos)
  ...
  if eol_col < pos.col then
    return vim.fn.virtcol({ pos.line, "$" }) + pos.col - eol_col
  end
...
H.user_modifier = function(with_preview, msg_chunks)
  ...
  if not ok or char == "" or char == "\3" or char == "\27" then
    return nil
  end
  return char
end
...
H.user_input = function(prompt, text)
  ...
  if _G.MiniInput ~= nil then
    return MiniInput.get({ prompt = prompt, scope = "editor", init_keys = { text } })
  end
...
H.make_status_msg_chunks = function(opts, steps)
  local single_to_string = function(pre_steps, opts_value)
    local steps_str = ""
    if #pre_steps > 0 then
      local pre_names = vim.tbl_map(function(x)
        return x.name
      end, pre_steps)
...
H.is_step = function(x)
  return type(x) == "table" and type(x.name) == "string" and vim.is_callable(x.action)
end

H.is_string = function(v)
  return type(v) == "string"
end

H.is_justify_side = function(x)
  return x == "left" or x == "center" or x == "right" or x == "none"
end

H.is_parts = function(x)
  return H.can_be_parts(x) and (getmetatable(x) or {}).class == "parts"
end

H.can_be_parts = function(x)
  if type(x) ~= "table" then
    return false, "should be table"
  end
  for i = 1, #x do
    if not H.is_array_of(x[i], H.is_string) then
      return false, "values should be an array of strings"
    end
  end
  return true
end

H.is_infinite = function(x)
...
H.get_lines = function(start_row, end_row)
...
H.string_find = function(s, pattern, init)
  ...
  if pattern:sub(1, 1) == "^" and init > 1 then
    return nil
  end
...
H.str_utf_end = function(s, n)
  return n >= s:len() and 0 or vim.str_utf_end(s, n)
end
if vim.fn.has("nvim-0.10") == 0 then
  H.str_utf_end = function(s, n)
    return n >= s:len() and 0 or (vim.str_byteindex(s, H.str_utfindex(s, n)) - n)
  end
end
