...
MiniClue.enable_all_triggers = function()
  ...
  for _, buf_id in ipairs(loaded_bufs) do
    ...
    if vim.fn.buflisted(buf_id) == 1 or is_special then
...
MiniClue.enable_buf_triggers = function(buf_id)
  ...
  if not H.is_valid_buf(buf_id) then
    H.error("`buf_id` should be a valid buffer identifier.")
  end
  H.map_buf_triggers(buf_id)
end
...
MiniClue.set_mapping_desc = function(mode, lhs, desc)
  if type(mode) ~= "string" then
    H.error("`mode` should be string.")
  end
  if type(lhs) ~= "string" then
    H.error("`lhs` should be string.")
  end
  if type(desc) ~= "string" then
    H.error("`desc` should be string.")
  end
...
MiniClue.gen_clues.marks = function()
  local describe_marks = function(mode, prefix)
    local make_clue = function(register, desc)
      return { mode = mode, keys = prefix .. register, desc = desc }
    end
...
MiniClue.gen_clues.windows = function(opts)
  ...
  if opts.submode_move then
    postkeys_move = "<C-w>"
  end
  if opts.submode_navigate then
    postkeys_navigate = "<C-w>"
  end
  if opts.submode_resize then
    postkeys_resize = "<C-w>"
  end
...
H.apply_config = function(config)
  ...
  local exec_register_macro = function()
    exec_macro("@", H.getcharstr())
  end
  if vim.fn.maparg("@", "n") == "" then
    vim.keymap.set("n", "@", exec_register_macro, macro_keymap_opts)
  end

  local exec_latest_macro = function()
    exec_macro("Q", "")
  end
...
H.create_autocommands = function()
  ...
  local ensure_triggers = vim.schedule_wrap(function(ev)
    if not H.is_valid_buf(ev.buf) then
      return
    end
  ...
  au("BufUnload", "*", function(ev)
    did_ensure[ev.buf] = nil
  end, "Track buffer-local trigger keymaps")
...
H.map_buf_triggers = function(buf_id)
  if not H.is_valid_buf(buf_id) or H.is_disabled(buf_id) then
    return
  end
...
H.map_trigger = function(buf_id, trigger)
  ...
  local is_ministarter_map = vim.bo[buf_id].filetype == "ministarter"
    and vim.api.nvim_buf_call(buf_id, function()
      return vim.fn.maparg(lhs, trigger.mode) ~= ""
    end)
  if is_ministarter_map then
    return
  end
...
H.state_advance = function(opts)
  ...
  H.state.timer:start(delay, 0, function()
    H.window_update(opts.same_content)
  end)
  ...
  -- Handle key
  if key == nil then
    return H.state_reset()
  end

  if key == H.keys.cr then
    return H.state_exec()
  end
  ...
  if H.state_is_at_target() then
    return H.state_exec()
  end

  -- - Reset if there are no keys (like after `<BS>`)
  if #H.state.query == 0 then
    return H.state_reset()
  end
...
H.state_exec = function()
  ...
  if has_postkeys then
    H.state_apply_postkeys(clue.postkeys)
  end
end
...
H.compute_exec_keys = function()
  ...
  if vim.startswith(cur_mode, "no") then
    local operator_tweak = H.operator_tweaks[vim.v.operator] or function(x)
      return x
    end
...
H.query_to_keys = function(query)
...
H.window_update = vim.schedule_wrap(function(same_content)
  ...
  if is_different_tabpage then
    H.window_close()
  end

  -- Create-update buffer showing clues
  if not same_content then
...
H.window_scroll = function(is_scroll_down)
  ...
  local f = function()
    ...
    if scroll_count > 0 then
      pcall(vim.cmd, "normal! " .. scroll_count .. scroll_key)
    end
...
H.window_get_config = function()
  ...
  if res.width == "auto" then
    res.width = H.buffer_get_width() + 1
  end
...
H.buffer_get_width = function()
  if not H.is_valid_buf(H.state.buf_id) then
    return
  end
...
H.clues_get_all = function(mode)
  ...
  local mode_filter = function(x)
    return type(x.mode) == "table" and vim.tbl_contains(x.mode, mode) or x.mode == mode
  end
...
H.clues_filter = function(clues, query)
  ...
  for clue_keys, _ in pairs(clues) do
    if not vim.startswith(clue_keys, keys) then
      clues[clue_keys] = nil
    end
  end
  return clues
...
H.clues_to_buffer_content = function(clues, keys)
  ...
  local next_keys_extra = vim.tbl_map(function(x)
    return { key = x, keytype = H.clues_get_next_key_type(x) }
  end, vim.tbl_keys(next_key_data))
  table.sort(next_keys_extra, H.clues_compare_next_key)
  local next_keys = vim.tbl_map(function(x)
    return x.key
  end, next_keys_extra)
...
H.clues_compare_next_key = function(a, b)
  ...
  if a_type == "alphanum" then
    return true
  end
  if b_type == "alphanum" then
    return false
  end
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "miniclue://" .. buf_id .. "/" .. name)
end
...
H.get_default_register = function()
  ...
  if clipboard:find("unnamedplus") ~= nil then
    return "+"
  end
  if clipboard:find("unnamed") ~= nil then
    return "*"
  end
  return '"'
end
...
H.redraw_scheduled = vim.schedule_wrap(function()
  vim.cmd("redraw")
end)
