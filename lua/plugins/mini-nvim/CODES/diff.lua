...
MiniDiff.enable = function(buf_id)
  ...
  if H.is_buf_enabled(buf_id) or H.is_disabled(buf_id) then
    return
  end
...
MiniDiff.toggle_overlay = function(buf_id)
  ...
  local buf_cache = H.cache[buf_id]
  if buf_cache == nil then
    H.error(string.format("Buffer %d is not enabled.", buf_id))
  end
...
MiniDiff.set_ref_text = function(buf_id, text)
  buf_id = H.validate_buf_id(buf_id)
  if not (type(text) == "table" or type(text) == "string") then
    H.error("`text` should be either string or array.")
  end
  ...
  if not H.is_buf_enabled(buf_id) then
    H.error("Can not set reference text for not enabled buffer.")
  end
...
MiniDiff.gen_source.git = function()
  local attach = function(buf_id)
    ...
    if H.git_cache[buf_id] ~= nil then
      return false
    end
  ...
  local clear_buf_cache = function(ev)
    H.git_cache[ev.buf] = nil
  end
...
MiniDiff.gen_source.save = function()
  ...
  local detach = function(buf_id)
    pcall(vim.api.nvim_del_augroup_by_id, augroups[buf_id])
  end
...
MiniDiff.do_hunks = function(buf_id, action, opts)
  ...
  if type(buf_cache.ref_text) ~= "string" then
    H.error(string.format("Buffer %d has no reference text.", buf_id))
  end
  ...
  if type(opts.register) ~= "string" then
    H.error("`opts.register` should be string.")
  end

  local hunks = H.get_hunks_in_range(buf_cache.hunks, line_start, line_end)
  if #hunks == 0 then
    return H.notify("No hunks to " .. action, "INFO")
  end
  if action == "apply" then
    H.get_active_source(buf_cache).apply_hunks(buf_id, hunks)
  end
  if action == "reset" then
    H.reset_hunks(buf_id, hunks)
  end
  if action == "yank" then
    H.yank_hunks_ref(buf_cache.ref_text, hunks, opts.register)
  end
end
...
MiniDiff.goto_hunk = function(direction, opts)
  ...
  if type(opts.line_start) ~= "number" then
    H.error("`opts.line_start` should be number.")
  end
  if type(opts.wrap) ~= "boolean" then
    H.error("`opts.wrap` should be boolean.")
  end

  -- Prepare ranges to iterate.
  local ranges = H.get_contiguous_hunk_ranges(buf_cache.hunks)
  if #ranges == 0 then
    return H.notify("No hunks to go to", "INFO")
  end
  ...
  if did_wrap then
    H.notify("Wrapped around edge in direction " .. vim.inspect(direction), "INFO")
  end
...
MiniDiff.operator = function(mode)
  ...
  if opts.line_end < opts.line_start then
    return H.notify("Not a proper textobject", "INFO")
  end
...
MiniDiff.textobject = function()
  local buf_id = vim.api.nvim_get_current_buf()
  local buf_cache = H.cache[buf_id]
  if buf_cache == nil or H.is_disabled(buf_id) then
    H.error("Current buffer is not enabled.")
  end
  ...
  if cur_region == nil then
    return H.notify("No hunk range under cursor", "INFO")
  end
...
H.auto_enable = vim.schedule_wrap(function(data)
  if H.is_buf_enabled(data.buf) or H.is_disabled(data.buf) then
    return
  end
  local buf = data.buf
  if not (vim.api.nvim_buf_is_loaded(buf) and vim.bo[buf].buftype == "" and vim.bo[buf].buflisted) then
    return
  end
  if not H.is_buf_text(buf) then
    return
  end
  MiniDiff.enable(buf)
end)
...
H.validate_target_lines = function(buf_id, line_start, line_end)
  ...
  if type(line_start) ~= "number" then
    H.error("`line_start` should be number.")
  end
  if type(line_end) ~= "number" then
    H.error("`line_end` should be number.")
  end
  ...
  if not (line_start <= line_end) then
    H.error("`line_start` should be less than or equal to `line_end`.")
  end
...
H.normalize_source = function(source)
  ...
  if type(source) ~= "table" then
    H.error("`source` should be table.")
  end
  ...
  for i, s in ipairs(source) do
    ...
    cur_s.apply_hunks = s.apply_hunks
      or function(_)
        H.error("Current source does not support applying hunks.")
      end

    if type(cur_s.name) ~= "string" then
      H.error("`source.name` should be string.")
    end
...
H.get_active_source = function(buf_cache)
  return buf_cache.source[buf_cache.source_id] or {}
end

H.convert_view_to_extmark_opts = function(view)
  local extmark_data = H.style_extmark_data[view.style]
  if extmark_data == nil then
    H.error("Style " .. vim.inspect(view.style) .. " is not supported.")
  end
...
H.update_buf_diff = vim.schedule_wrap(function(buf_id)
  ...
  if summary.n_ranges > 0 then
    table.insert(summary_string, "#" .. summary.n_ranges)
  end
  if summary.add > 0 then
    table.insert(summary_string, "+" .. summary.add)
  end
  if summary.change > 0 then
    table.insert(summary_string, "~" .. summary.change)
  end
  if summary.delete > 0 then
    table.insert(summary_string, "-" .. summary.delete)
  end
...
H.update_hunk_data = function(diff, buf_cache, buf_lines)
  ...
  for i, d in ipairs(diff) do
    ...
    for l_num = range_from, range_to do
      ...
      if viz_lines[l_num] == nil or hunk_type == "change" then
        viz_lines[l_num] = viz_ext_opts
      end
    end

    if do_overlay then
      if hunk_type == "add" then
        H.append_overlay_add(overlay_lines, hunk, priority)
      end
      if hunk_type == "change" then
        H.append_overlay_change(overlay_lines, hunk, ref_lines, buf_lines, priority)
      end
      if hunk_type == "delete" then
        H.append_overlay_delete(overlay_lines, hunk, ref_lines, priority)
      end
    end
  end
...
H.draw_overlay_line = function(buf_id, ns_id, row, data)
  ...
  if data.type == "change_worddiff" then
    return H.draw_overlay_line_worddiff(buf_id, ns_id, row, data)
  end
...
H.draw_overlay_line_worddiff = function(buf_id, ns_id, row, data)
  ...
  for i = 1, #ref_parts do
    ...
    if index < part[1] then
      table.insert(virt_line, { ref_line:sub(index, part[1] - 1), "MiniDiffOverContext" })
    end
    table.insert(virt_line, { ref_line:sub(part[1], part[2]), "MiniDiffOverChange" })
    index = part[2] + 1
  end
  if index <= ref_line:len() then
    table.insert(virt_line, { ref_line:sub(index), "MiniDiffOverContext" })
  end
...
H.compute_worddiff_changed_parts = function(ref_line, buf_line)
  ...
  for i = 1, #diff do
    local d = diff[i]
    if d[2] > 0 then
      table.insert(ref_ranges, { ref_byte_starts[d[1]], ref_byte_ends[d[1] + d[2] - 1] })
    end
    if d[4] > 0 then
      table.insert(buf_ranges, { buf_byte_starts[d[3]], buf_byte_ends[d[3] + d[4] - 1] })
    end
  end
...
H.iterate_hunk_ranges = function(ranges, direction, opts)
  ...
  if direction == "prev" then
    init_ind = H.get_range_id_prev(ranges, opts.line_start)
  end
  if direction == "next" then
    init_ind = H.get_range_id_next(ranges, opts.line_start)
  end
  ...
  if not opts.wrap and is_on_edge then
    return nil
  end
...
H.get_range_id_next = function(ranges, line_start)
  for i = #ranges, 1, -1 do
    if ranges[i].from <= line_start then
      return i
    end
  end
  return 0
...
H.get_range_id_prev = function(ranges, line_start)
  for i = 1, #ranges do
    if line_start <= ranges[i].to then
      return i
    end
  end
  return #ranges + 1
...
H.git_setup_index_watch = function(buf_id, git_dir_path)
  ...
  local buf_git_set_ref_text = function()
    H.git_set_ref_text(buf_id)
  end
...
H.git_apply_patch = function(path_data, patch)
  ...
  local process
  process = vim.loop.spawn("git", spawn_opts, function()
    process:close()
  end)
...
H.clear_namespace = function(...)
  pcall(vim.api.nvim_buf_clear_namespace, ...)
end

H.is_buf_text = function(buf_id)
  local n = vim.api.nvim_buf_call(buf_id, function()
    return vim.fn.byte2line(1024)
  end)
...
if vim.api.nvim__redraw ~= nil then
  H.redraw_buffer = function(buf_id)
    vim.api.nvim__redraw({ buf = buf_id, valid = true, statusline = true })
  end
end
