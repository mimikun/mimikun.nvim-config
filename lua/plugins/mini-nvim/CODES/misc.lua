...
MiniMisc.log_get = function()
  return H.log_cache.log
end
...
MiniMisc.safely = function(when, f)
  ...
  if when == "later" then
    if #H.safely_cache.later == 0 then
...
MiniMisc.setup_auto_root = function(names, fallback)
  ...
  if not vim.is_callable(fallback) then
    H.error("Argument `fallback` of `setup_auto_root()` should be callable.")
  end
...
MiniMisc.find_root = function(buf_id, names, fallback)
  ...
  if not H.is_valid_buf(buf_id) then
    H.error("Argument `buf_id` of `find_root()` should be valid buffer id.")
  end
  if not (H.is_array_of(names, H.is_string) or vim.is_callable(names)) then
    H.error("Argument `names` of `find_root()` should be array of string file names or a callable.")
  end
  if not vim.is_callable(fallback) then
    H.error("Argument `fallback` of `find_root()` should be callable.")
  end
...
MiniMisc.setup_restore_cursor = function(opts)
  ...
  if type(opts.center) ~= "boolean" then
...
H.setup_config = function(config)
  ...
  for _, v in pairs(config.make_global) do
    if MiniMisc[v] == nil then
      H.error("`make_global` should be a table with exported 'mini.misc' methods")
    end
  end
...
H.notify = function(msg, level)
  vim.notify("(mini.misc) " .. msg, vim.log.levels[level])
end
