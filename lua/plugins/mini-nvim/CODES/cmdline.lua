...
MiniCmdline.default_autocomplete_predicate = function(data, opts)
  return data.line:find("%a") ~= nil
end
...
MiniCmdline.default_autocorrect_func = function(data, opts)
  ...
  if strict_type and not H.autocorrect_strict_types[data.type] then
    return data.word
  end

  -- Get all valid words
  local all = vim.is_callable(opts.get_candidates) and opts.get_candidates(data) or H.get_autocorrect_candidates(data)
  if vim.tbl_contains(all, data.word) or data.word == "" then
    return data.word
  end
...
MiniCmdline.default_autopeek_statuscolumn = function(data, opts)
  ...
  if n == l and n == r then
    return string.format(fmt, signs.same or "🭬", n)
  end
  if n == l then
    return string.format(fmt, signs.left or "┌", n)
  end
  if n == r then
    return string.format(fmt, signs.right or "└", n)
  end
  if (l < n and n < r) or (r < n and n < l) then
    return string.format(fmt, signs.mid or "┊", "")
  end
...
H.apply_config = function(config)
  ...
  if not was_set and config.autocomplete.enable and H.can_autocomplete then
  ...
  if config.autocomplete.map_arrows then
    local map_arrow = function(dir, wildmenu_prefix, desc)
      local rhs = function()
        return (vim.fn.wildmenumode() == 1 and wildmenu_prefix or "") .. dir
      end
...
H.create_autocommands = function()
  ...
  au("VimResized", "*", function()
    H.autopeek(true)
  end, "Adjust peek window")
  au("CmdwinEnter", "*", function()
    H.peek_hide()
  end, "Hide peek window")
...
H.is_disabled = function()
  return vim.g.minicmdline_disable == true or vim.b.minicmdline_disable == true
end

H.get_config = function()
  return vim.tbl_deep_extend("force", MiniCmdline.config, vim.b.minicmdline_config or {})
end

-- Autocommands ---------------------------------------------------------------
H.on_cmdline_enter = function()
  -- Check for Command-line mode to not act on `:...` mappings
  if H.is_disabled() or vim.fn.mode() ~= "c" then
    return
  end
  ...
  if H.cache.config.autopeek.enable then
    H.autopeek()
  end
end

H.on_cmdline_update = function()
  if H.cache.state == nil or H.cache.n_nested ~= nil then
    return
  end
  ...
  if is_same_line and state.pos == H.cache.state.pos then
    return
  end

  -- Update state accounting for some edge cases
  if H.cache.state_prev.compltype == "option" then
    H.adjust_option_cmd_state(state)
  end
  ...
  if config.autocomplete.enable and H.can_autocomplete then
    H.autocomplete()
  end
  if config.autocorrect.enable then
    H.autocorrect(false)
  end
  if config.autopeek.enable then
    H.autopeek()
  end
end
...
H.on_cmdline_leave = function()
  ...
  if H.cache.config.autocorrect.enable and not vim.v.event.abort then
    H.autocorrect(true)
  end
  H.peek_hide()
  H.cache = {}
...
H.get_cmd_state = function(is_init)
  ...
  if is_init then
    return { complpat = "", compltype = compltype, line = "", pos = 1, cmd = {} }
  end
...
H.adjust_option_cmd_state = function(state)
  ...
  if state.compltype == "option" then
    return
  end
...
H.autocomplete = function()
  ...
  if H.block_autocomplete() then
    return
  end

  local delay = H.cache.config.autocomplete.delay
  if delay == 0 then
    return H.trigger_complete()
  end
...
H.autocorrect = function(is_final)
  ...
  if not (is_char_append and (is_word_finished or is_final)) then
    return
  end
  ...
  if word == "" or vim.fn.maparg(word, "c", true) ~= "" then
    return
  end
  ...
  if type(new_word) ~= "string" then
    return H.notify("Can not autocorrect for " .. vim.inspect(word), "WARN")
  end
...
H.get_nearest_command = function(ref, all)
  ...
  local abbr_lens = vim.tbl_map(function(x)
    return cmd_abbr_lens[x]
  end, all)
...
H.autopeek = function(force)
  ...
  if not (H.cache.cmd_type == ":" and not H.cache.buf_is_cmdwin) then
    return
  end
  ...
  if not H.cache.autopeek_predicate(data) then
    return H.peek_hide()
  end
  ...
  if not force and is_data_same then
    return
  end
...
H.peek_show = function(from, to)
  ...
  if config.height < height then
    H.peek_adjust_context(cont_from, cont_to, height, config.height)
  end
  ...
  vim.api.nvim_win_call(win_id, function()
    ...
    if to_before - from_after > 1 then
      vim.cmd(string.format("%s,%sfold", from_after + 1, to_before - 1))
    end
  end)
...
H.peek_get_config = function(height)
  ...
  if vim.is_callable(win_config) then
...
H.peek_adjust_context = function(cont_from, cont_to, height, target_height)
  ...
  while extra > 0 do
    ...
    if n_from_out <= 0 and n_to_out <= 0 then
      break
    end
...
H.make_peek_statuscolumn = function()
  ...
  return function()
    return statuscolumn(H.cache.peek.data) or ""
  end
end

-- Utilities ------------------------------------------------------------------
H.error = function(msg)
  error("(mini.cmdline) " .. msg, 0)
end
...
H.notify = function(msg, level_name, silent)
  if not silent then
    vim.notify("(mini.cmdline) " .. msg, vim.log.levels[level_name])
  end
end
...
H.get_cmd_preview_map = function()
  ...
  for name, data in pairs(vim.api.nvim_get_commands({})) do
    if type(name) == "string" and data.preview then
      res[name] = true
    end
  end
...
H.getcmdcomplpat = function()
  return vim.fn.getcmdcomplpat()
end
if vim.fn.has("nvim-0.11") == 0 then
  -- Match alphanumeric characters to cursor's left, if present
  -- This is not 100% how it works, but good enough
  H.getcmdcomplpat = function()
    return vim.fn.getcmdline():sub(1, vim.fn.getcmdpos() - 1):match("%w+$") or ""
  end
end
