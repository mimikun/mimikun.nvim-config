...
MiniJump.jump = function(target, backward, till, n_times)
  ...
  if MiniJump.state.target == nil then
    return H.message("Can not jump because there is no recent `target`.")
  end
  ...
  local hl = vim.schedule_wrap(function()
  ...
  if expr_mode ~= nil then
    vim.cmd("normal! " .. expr_mode)
  end
  ...
  if not was_jumping then
    H.trigger_event("MiniJumpStart")
  end
...
MiniJump.smart_jump = function(backward, till)
  ...
  if has_changed_mode or has_changed_cursor then
    MiniJump.stop_jumping()
  end
...
H.make_expr_jump = function(backward, till)
  return function()
    ...
    if isnt_repeat_jump and target == nil then
      return "<Esc>"
    end
    H.update_state(target)
...
H.on_cursormoved = function()
  ...
  if MiniJump.state.jumping then
    ...
    if H.cache.n_cursor_moved > 1 then
      MiniJump.stop_jumping()
    end
  end
end
...
H.highlight = function(pattern)
  if H.is_highlighting(pattern) or not MiniJump.state.jumping then
    return
  end
...
H.get_cursor_data = function()
  return { vim.api.nvim_get_current_win(), vim.api.nvim_win_get_cursor(0) }
end
