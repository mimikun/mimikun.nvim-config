...
MiniFiles.synchronize = function()
  ...
  if fs_actions ~= nil then
    ...
    if confirm_res == 1 then
      H.fs_actions_apply(fs_actions, explorer.opts.options.lsp_timeout)
    end
  end
...
H.track_dir_edit = function(data)
  ...
  if vim.b.minifiles_processed_dir then
    ...
    if alt_buf ~= data.buf and vim.fn.buflisted(alt_buf) == 1 then
      return vim.api.nvim_win_set_buf(0, alt_buf)
    end
...
H.explorer_get = function(tabpage_id, ignore_visibility)
  ...
  if ignore_visibility or H.explorer_is_visible(res) then
    return res
  end
...
H.explorer_refresh = function(explorer, opts)
  ...
  for _, win_id in ipairs(explorer.windows) do
    ...
    if opened_windows[win_id] then
      H.trigger_event("MiniFilesWindowOpen", data)
    end
...
H.explorer_track_lost_focus = function()
  local track = vim.schedule_wrap(function()
    if H.vim_ui.select_active or H.vim_ui.input_active then
      return
    end
...
H.explorer_sync_cursor_and_branch = function(explorer, depth)
  ...
  if show_preview and is_cur_buf then
...
H.explorer_show_help = function(explorer, explorer_buf_id, explorer_win_id)
  ...
  if culopt:find("line") == nil then
    vim.wo[win_id].cursorlineopt = culopt .. ",line"
  end
...
H.buffer_create = function(path, mappings)
  ...
  if H.fs_is_imaginary_path(path) then
    return buf_id
  end
...
H.buffer_update = function(buf_id, path, opts, is_preview)
  ...
  if fs_type == "directory" then
    H.buffer_update_directory(buf_id, path, opts, is_preview)
  end
  if fs_type == "file" then
    H.buffer_update_file(buf_id, path, opts, is_preview)
  end
  if fs_type == nil then
    H.set_buflines(buf_id, { "-No-fs-entry-" .. string.rep("-", opts.windows.width_preview) })
  end

  -- Trigger dedicated event
  local data = { buf_id = buf_id, win_id = H.opened_buffers[buf_id].win_id }
  if fs_type ~= nil then
    H.trigger_event("MiniFilesBufferUpdate", data)
  end
...
H.fs_is_imaginary_path = function(path)
  return path:sub(-1) == "\000"
end

H.fs_is_present_path = function(path)
  return vim.loop.fs_stat(path) ~= nil and not H.fs_is_imaginary_path(path)
end
...
H.fs_get_type = function(path)
  if not (not H.fs_is_imaginary_path(path) and H.fs_is_present_path(path)) then
    return nil
  end
...
H.lsp_fs_hook = function(method, diffs, lsp_timeout)
  ...
  for _, d in ipairs(diffs) do
    local file = {}
    if is_create and d.action == "create" then
      file = { uri = to_uri(d.to) }
    end
    if is_delete and d.action == "delete" then
      file = { uri = to_uri(d.from) }
    end
    if is_rename and d.action == "rename" then
      file = { oldUri = to_uri(d.from), newUri = to_uri(d.to) }
    end
    ...
    if (file.uri or file.oldUri) ~= nil and pass_check then
...
H.lsp_fs_hook_client = function(client, full_method, lsp_files, timeout)
  ...
  local is_scheme = function(uri, scheme)
    return scheme == nil or vim.startswith(uri, scheme .. ":")
  end
  local is_fs_type = function(lsp_file, ref_fs_type)
    return ref_fs_type == nil or ref_fs_type == lsp_file.fs_type
  end
  local make_filter = function(scheme, ref_fs_type, glob, ignore_case)
    local adjust_case = ignore_case and vim.fn.tolower or function(x)
      return x
    end
    -- On Windows `uri_to_fname` forces `\`, but forcing / seems more robust
    local to_fname = H.is_windows and function(x)
      return (vim.uri_to_fname(x):gsub("\\", "/"))
    end or vim.uri_to_fname
  ...
  for _, f in ipairs(lsp_files) do
    ...
    f.fs_type = nil
    if ok then
      table.insert(params.files, f)
    end
  end

  -- Perform an action
  if vim.startswith(method, "did") then
    return client:notify(full_method, params)
  end
  ...
  if (response or {}).result ~= nil then
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "minifiles://" .. buf_id .. "/" .. name)
end
