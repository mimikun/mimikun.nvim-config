...
MiniTest.finally = function(f)
  table.insert(H.cache.finally, f)
end
...
MiniTest.execute = function(cases, opts)
  ...
  table.insert(queue, function()
    H.exec_callable(reporter.start, cases)
  end)
...
MiniTest.stop = function(opts)
  ...
  if not opts.close_all_child_neovim then
    return
  end
...
---
---@return boolean
MiniTest.is_executing = function()
...
MiniTest.expect.reference_screenshot = function(screenshot, path, opts)
  ...
  if same_text and same_attr then
    return true
  end
...
H.make_case = function(case, case_num, opts)
  ...
  local exec_hooks = function(name, source)
    ...
    for i, h in ipairs(case.hooks[name]) do
      if source_arr[i] == source then
        exec_step(h, state_prefix .. i)
      end
    end
  end
  ...
  return function()
    ...
    for _ = 1, case.n_retry do
      ...
      local case_f = function()
        case.test(unpack(case.args))
      end
      ...
      if H.cache.skip_message ~= nil then
        case_f = function()
          MiniTest.skip(H.cache.skip_message)
        end
      end
    ...
    if not ok_case and opts.stop_on_error then
      MiniTest.stop()
    end
  end
end
...
H.inject_hooks_once = function(cases, hooks_once)
  ...
  for i = 1, n do
    ...
    case.hooks.pre_source = vim.tbl_map(function()
      return "case"
    end, case.hooks.pre)
  ...
  for i = n, 1, -1 do
    ...
    case.hooks.post_source = vim.tbl_map(function()
      return "case"
    end, case.hooks.post)
...
H.buffer_reporter.update_step_lines = function(case_num, cases, groups)
  ...
  local cur_group_symbols = vim.tbl_map(
    ...
    vim.tbl_filter(function(g)
      return g.name == cur_group
    end, groups)
  )
...
H.normalize_reason = function(reason, fallback, ...)
  if vim.is_callable(reason) then
    reason = reason(...)
  end
...
H.compute_no_equality_cause = function(left, right)
  ...
  if type(left) == "string" then
    if vim.fn.strchars(left) ~= vim.fn.strchars(right) then
      return "different string length"
    end
...
H.screenshot_write = function(screenshot, path)
...
H.screenshot_read = function(path)
  ...
  local f = function(x)
    return H.string_to_screenchars(x:gsub("^%d+|", ""))
  end
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "minitest://" .. buf_id .. "/" .. name)
end
