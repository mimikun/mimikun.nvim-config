...
MiniColors.as_colorscheme = function(x)
  -- Validate input
  if not H.is_table(x) then
    H.error("Input of `as_colorscheme()` should be table.")
  end

  if x.groups ~= nil then
    if not H.is_table(x.groups) then
      H.error("Field `groups` of colorscheme should be table or nil.")
    end
    if not H.all(x.groups, H.is_table) then
      H.error("All elements of `groups` colorscheme field should be tables.")
    end
  end

  if x.terminal ~= nil then
    if not H.is_table(x.terminal) then
      H.error("Field `terminal` of colorscheme should be table or nil.")
    end
...
MiniColors.get_colorscheme = function(name, opts)
  if not (name == nil or type(name) == "string") then
    H.error("Argument `name` should be string or `nil`.")
  end
  ...
  if not ok then
    H.error(string.format('No color scheme named "%s".', name))
  end
...
MiniColors.interactive = function(opts)
  ...
  -- Make local mappings
  local m = function(action, rhs)
    vim.keymap.set("n", maps[action], rhs, { desc = action, buffer = buf_id })
  end
...
MiniColors.animate = function(cs_array, opts)
  ...
  local cs_oklab = vim.tbl_map(function(cs)
    return H.cs_hex_to_oklab(vim.deepcopy(cs))
  end, cs_array)
  ...
  after_action = function(data)
    ...
    -- Advance if possible
    cs_id = cs_id + 1
    if #cs_array < cs_id then
      return
    end

    -- Wait before starting another animation
    local callback = function()
      H.animate_single_transition(cs_oklab[cs_id - 1], cs_oklab[cs_id], after_action, opts)
    end
...
MiniColors.simulate_cvd = function(x, cvd_type, severity)
  ...
  if not H.is_number(severity) then
    H.error("Argument `severity` should be number.")
  end
...
H.cs_add_cterm_attributes = function(self, opts)
  ...
  for _, gr in pairs(res.groups) do
    if gr.fg and (force or not gr.ctermfg) then
      gr.ctermfg = H.get_closest_color_id(gr.fg, term_oklab)
    end
    if gr.bg and (force or not gr.ctermbg) then
      gr.ctermbg = H.get_closest_color_id(gr.bg, term_oklab)
    end
  end
...
H.cs_add_terminal_colors = function(self, opts)
  ...
  local palette_oklch = vim.tbl_map(function(x)
    return MiniColors.convert(x, "oklch")
  end, palette)
  ...
  local dist_color = function(x, y)
    return H.dist(x.l, y.l) / 100 + H.dist_circle(x.h, y.h) / 90
  end
  ...
  for i = 0, 15 do
    if opts.force or not res.terminal[i] then
      res.terminal[i] = MiniColors.convert(terminal[i], "hex")
    end
  end
...
H.cs_add_transparency = function(self, opts)
  ...
  if opts.float then
    update({ "FloatBorder", "FloatTitle", "NormalFloat" })
  end

  if opts.statuscolumn then
    update({ "FoldColumn", "LineNr", "LineNrAbove", "LineNrBelow", "SignColumn" })
    local diag_hl = vim.tbl_map(function(x)
      return "DiagnosticSign" .. x
    end, { "Error", "Warn", "Info", "Hint", "Ok" })
    update(diag_hl)
  ...
  if opts.statusline then
    update({ "StatusLine", "StatusLineNC", "StatusLineTerm", "StatusLineTermNC" })
  end

  if opts.tabline then
    update({ "TabLine", "TabLineFill", "TabLineSel" })
  end

  if opts.winbar then
    update({ "WinBar", "WinBarNC" })
  end
...
H.cs_chan_add = function(self, channel, value, opts)
  ...
  return self:chan_modify(channel, function(x)
    return x + value
  end, opts)
end
...
H.cs_chan_repel = function(self, channel, sources, coef, opts)
  ...
  local f = function(x)
    return H.repel(x, sources, coef, tie_breaker)
  end
...
H.cs_chan_set = function(self, channel, values, opts)
  ...
  if #values == 0 then
    return H.error("Argument `values` should not be empty.")
  end

  local dist_fun = channel == "hue" and H.dist_circle or H.dist
  local f = function(x)
    return H.get_closest(x, values, dist_fun)
  end
...
H.cs_color_modify = function(self, f)
  ...
  for name, spec in pairs(res.groups) do
    if spec.fg ~= nil then
      spec.fg = f(spec.fg, { attr = "fg", name = name })
    end
    if spec.bg ~= nil then
      spec.bg = f(spec.bg, { attr = "bg", name = name })
    end
    if spec.sp ~= nil then
      spec.sp = f(spec.sp, { attr = "sp", name = name })
    end
  end
...
H.cs_compress = function(self, opts)
  ...
  for name, spec in pairs(self.groups) do
    ...
    if not (is_from_clear or is_devicon or is_colorizer) then
      new_groups[name] = spec
    end
  end
...
H.cs_simulate_cvd = function(self, cvd_type, severity, opts)
  local f = function(hex)
    return MiniColors.simulate_cvd(hex, cvd_type, severity, opts)
  end
...
H.is_colorscheme = function(x)
  return type(x) == "table" and type(x.groups) == "table" and type(x.terminal) == "table"
end

H.normalize_f = function(f)
  if not vim.is_callable(f) then
    H.error("Argument `f` should be callable.")
  end
  return f
end
...
H.normalize_gamut_clip = function(x)
  x = x or "chroma"
  if x == "chroma" or x == "lightness" or x == "cusp" then
    return x
  end
...
H.normalize_number = function(x, arg_name)
  if type(x) ~= "number" then
    H.error("Argument `" .. arg_name .. "` should be a number.")
  end
  return x
end
...
H.make_file_basename = function(name)
  ...
  if not vim.tbl_contains(all_colorschemes, name) then
    return name
  end
...
H.get_current_groups = function()
  ...
  local group_names = vim.tbl_map(function(x)
    return x:match("^(%S+)")
  end, group_data)
  local link_data = vim.tbl_map(function(x)
    return x:match("^%S+.* links to (%S+)$")
  end, group_data)
...
H.get_current_terminal = function()
  ...
  for i = 0, 15 do
    ...
    if type(col) == "string" then
      res[i] = H.dec2hex(vim.api.nvim_get_color_by_name(col))
    end
  end
...
H.cs_hex_to_oklab = function(cs)
  local to_oklab = function(hex)
    return MiniColors.convert(hex, "oklab")
  end
...
H.cs_oklab_to_hex = function(cs)
  ...
  local to_hex = function(lab)
    return MiniColors.convert(lab, "hex", { gamut_clip = "chroma" })
  end
  cs.groups = vim.tbl_map(function(gr)
...
H.convex_hl_group = function(from, to, coef)
  if from == nil or to == nil or from.link ~= nil or to.link ~= nil then
    return H.convex_discrete(from, to, coef)
  end
...
H.convex_lab = function(from_lab, to_lab, coef)
  if from_lab == nil or to_lab == nil then
    return H.convex_discrete(from_lab, to_lab, coef)
  end
  return {
...
H.converters["8-bit"] = function(x, _, _)
  return H.get_closest_color_id(x, H.compute_term_oklab())
end

H.converters.hex = function(x, from_space, opts)
  if from_space == "hex" then
    return x
  end
...
H.converters.rgb = function(x, from_space, opts)
  ...
  if from_space == "rgb" then
    return { r = H.clip(x.r, 0, 255), g = H.clip(x.g, 0, 255), b = H.clip(x.b, 0, 255) }
  end

  -- Clip non-gray color to be in gamut
  local lch = MiniColors.convert(x, "oklch", opts)
  if lch.h ~= nil then
    lch = H.clip_to_gamut(lch, opts.gamut_clip)
  end

  return H.oklab2rgb(H.oklch2oklab(lch))
end

H.converters.oklab = function(x, from_space, opts)
  return H.oklch2oklab(MiniColors.convert(x, "oklch", opts))
end

H.converters.oklch = function(x, from_space, opts)
  local res = nil
  if from_space == "8-bit" then
    res = H.oklab2oklch(H.compute_term_oklab()[x])
  end
  if from_space == "hex" then
    res = H.oklab2oklch(H.rgb2oklab(H.hex2rgb(x)))
  end
  if from_space == "rgb" then
    res = H.oklab2oklch(H.rgb2oklab(x))
  end
  if from_space == "oklab" then
    res = H.oklab2oklch(x)
  end
  if from_space == "oklch" then
    res = x
  end
  if from_space == "okhsl" then
    res = H.okhsl2oklch(x)
  end
...
H.converters.okhsl = function(x, from_space, opts)
  return H.oklch2okhsl(MiniColors.convert(x, "oklch", opts))
end

H.infer_color_space = function(x)
  if type(x) == "number" and 16 <= x and x <= 255 then
    return "8-bit"
  end
  ...
  if is_num(x.l) then
    ...
    if is_num(x.a) and is_num(x.b) then
      return "oklab"
    end
    if is_num(x.s) then
      return "okhsl"
  ...
  if is_num(x.r) and is_num(x.g) and is_num(x.b) then
    return "rgb"
  end
...
H.correct_lightness = function(x)
  if not H.adjust_lightness then
    return x
  end
...
H.clip_to_gamut = function(lch, gamut_clip)
  ...
  if gamut_clip == "chroma" then
    res.c = H.clip(res.c, 0, gamut_points.c_upper)
  end
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "minicolors://" .. buf_id .. "/" .. name)
end
...
H.all = function(arr, predicate)
  predicate = predicate or function(x)
    return x
  end
