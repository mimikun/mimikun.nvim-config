...
MiniPairs.cr = function(key)
  ...
  if H.is_disabled() or not H.is_pair_registered(neigh, vim.fn.mode(), "cr") then
    return res
  end
...
H.is_pair_registered = function(pair, mode, key)
  ...
  if mode_pairs["all"][key][pair] then
    return true
  end
...
H.get_open_char = function(x)
  return vim.fn.strcharpart(x, 0, 1)
end
H.get_close_char = function(x)
  return vim.fn.strcharpart(x, 1, 1)
end
...
H.with_temp_option = function(name, value)
  ...
  if H.options_cache[name] == nil then
    H.options_cache[name] = vim.o[name]
  end
  vim.o[name] = value
