...
MiniSplitjoin.gen_hook.pad_brackets = function(opts)
  ...
  return function(join_positions)
    ...
    if first.line == last.line and (last.col - first.col) <= 1 then
      return join_positions
    end
...
MiniSplitjoin.get_indent_part = function(line, respect_comments)
  if respect_comments == nil then
    respect_comments = true
  end
  if not respect_comments then
    return line:match("^%s*")
  end
...
H.is_disabled = function()
  return vim.g.minisplitjoin_disable == true or vim.b.minisplitjoin_disable == true
end
...
H.join_at_extmark = function(extmark_id, pad)
  ...
  if vim.api.nvim_buf_line_count(0) <= line_num then
    return
  end
...
H.find_separator_positions = function(region, separator, exclude_regions)
  ...
  local seps = {}
  region_s:gsub(separator .. "()", function(r)
    table.insert(seps, r - 1)
  end)
  ...
  local add_to_forbidden = function(l, r)
    table.insert(forbidden, { from = l + 1, to = r })
  end
  ...
  local sub_offsets = vim.tbl_filter(function(x)
    return not H.is_offset_inside_spans(x, forbidden)
  end, seps)
  ...
  return vim.tbl_map(function(sub_off)
    return neigh.offset_to_pos(start_offset + sub_off - 1)
  end, sub_offsets)
end

H.is_offset_inside_spans = function(ref_point, spans)
  for _, span in ipairs(spans) do
    if span.from <= ref_point and ref_point <= span.to then
      return true
    end
  end
  return false
...
H.get_neighborhood = function()
  ...
  local pos_to_offset = function(pos)
    return line_offsets[pos.line] + pos.col
  end
  ...
  local region_to_span = function(region)
    return { from = pos_to_offset(region.from), to = pos_to_offset(region.to) }
  end

  -- Convert 1d span to 2d region
  local span_to_region = function(span)
    return { from = offset_to_pos(span.from), to = offset_to_pos(span.to) }
  end
...
H.put_extmark_at_positions = function(positions)
  return vim.tbl_map(function(pos)
    return vim.api.nvim_buf_set_extmark(0, H.ns_id, pos.line - 1, pos.col - 1, {})
  end, positions)
end
...
H.get_comment_indent = function(line, comment_leaders)
  ...
  for _, leader in ipairs(comment_leaders) do
    ...
    if type(cur_match) == "string" and res:len() < cur_match:len() then
      res = cur_match
    end
  end
...
H.is_comment_block = function(lines, comment_leaders)
  for _, l in ipairs(lines) do
    if not H.is_commented(l, comment_leaders) then
      return false
    end
  end
  return true
...
H.is_commented = function(line, comment_leaders)
  for _, leader in ipairs(comment_leaders) do
    if line:find("^%s*" .. vim.pesc(leader) .. "%s*") ~= nil then
      return true
    end
  end
  return false
