...
MiniMap.refresh = function(opts, parts)
  -- Early return
  if H.is_disabled() or not H.is_window_open() then
    return
  end
  ...
  if parts.lines then
    vim.schedule(H.update_map_lines)
  end
  if parts.scrollbar then
    vim.schedule(H.update_map_scrollbar)
  end
  if parts.integrations then
    vim.schedule(H.update_map_integrations)
  end
end
...
MiniMap.close = function()
  ...
  -- next opening
  if vim.tbl_count(MiniMap.current.win_data) == 0 then
    MiniMap.current.opts = {}
  end
end
...
MiniMap.gen_encode_symbols.block = function(id)
  return H.block_symbols[id]
end
...
MiniMap.gen_encode_symbols.dot = function(id)
  return H.dot_symbols[id]
end
...
MiniMap.gen_encode_symbols.shade = function(id)
  return H.shade_symbols[id]
end
...
MiniMap.gen_integration.builtin_search = function(hl_groups)
  ...
  return function()
    ...
    if not H.is_source_buffer() then
      return {}
    end
...
MiniMap.gen_integration.diagnostic = function(hl_groups)
  if hl_groups == nil then
    hl_groups = { error = "DiagnosticFloatingError" }
  end
  ...
  local severity_level_names = vim.tbl_filter(function(x)
    return vim.tbl_contains(vim.tbl_keys(hl_groups), x)
  end, { "error", "warn", "info", "hint" })
  local severity_data = vim.tbl_map(function(x)
    return { severity = vim.diagnostic.severity[x:upper()], hl_group = hl_groups[x] }
  end, severity_level_names)
  ...
  return function()
    ...
    for _, data in ipairs(severity_data) do
      local severity_diagnostic_arr = vim.tbl_filter(function(x)
        return x.severity == data.severity
      end, diagnostic_arr)
...
MiniMap.gen_integration.gitsigns = function(hl_groups)
  if hl_groups == nil then
    hl_groups = { add = "GitSignsAdd", change = "GitSignsChange", delete = "GitSignsDelete" }
  end
...
H.is_disabled = function()
  return vim.g.minimap_disable == true or vim.b.minimap_disable == true
end
...
H.on_content_change = vim.schedule_wrap(function()
  ...
  -- is updated.
  if not H.is_proper_buftype() then
    return
  end
  MiniMap.refresh()
end)
...
H.on_view_change = vim.schedule_wrap(function()
  if not (H.is_proper_buftype() and H.is_source_buffer()) then
    return
  end
...
H.on_winclosed = function(data)
  ...
  if not ok or closed_win_id ~= H.cache.previous_win.id then
    return
  end
...
H.track_map_cursor = function()
  ...
  local cur_win, map_win = vim.api.nvim_get_current_win(), H.get_current_map_win()
  if cur_win ~= map_win or H.cache.n_map_cursor_moves <= 1 then
    return
  end
  ...
  vim.api.nvim_win_call(prev_win_id, function()
    vim.cmd("normal! zvzz")
  end)
end
...
H.on_map_enter = function()
  ...
  if not H.is_window_open() or H.cache.previous_win.cursor == nil then
    return
  end
...
H.mask_from_strings = function(strings, _)
  ...
  for i, s in ipairs(strings) do
    ...
    -- Detect whitespace
    s_ext:gsub("()%s", function(j)
      mask_row[H.str_utfindex(s_ext, j)] = false
    end)
    res[i] = mask_row
  end
...
H.mask_to_symbols = function(mask, opts)
  ...
  for i, row in ipairs(symbol_ind) do
    local syms = vim.tbl_map(function(id)
      return symbols[id + 1]
    end, row)
...
H.is_valid_config_integrations = function(x, x_name)
  ...
  if x ~= nil then
    if not H.is_array_of(x, vim.is_callable) then
      return false, H.msg_config(x_name, "array of callables")
    end
  end
...
H.is_valid_config_symbols = function(x, x_name)
  ...
  if type(x) ~= "table" then
    return false, H.msg_config(x_name, "table")
  end
  ...
  if x.encode ~= nil then
    ...
    if not ok_encode then
      return ok_encode, msg_encode
    end
  end

  -- Current line
  if not H.is_string(x.scroll_line) then
    return false, H.msg_config(x_name .. ".scroll_line", "string")
  end

  -- Current view
  if not H.is_string(x.scroll_view) then
    return false, H.msg_config(x_name .. ".scroll_view", "string")
  end
...
H.is_valid_config_window = function(x, x_name)
  ...
  -- Focusable
  if type(x.focusable) ~= "boolean" then
    return false, H.msg_config(x_name .. ".focusable", "boolean")
  end
...
H.msg_config = function(x_name, msg)
  return string.format("`%s` should be %s.", x_name, msg)
end
...
H.get_current_map_win = function()
  return MiniMap.current.win_data[vim.api.nvim_get_current_tabpage()]
end

H.set_current_map_win = function(win_id)
  MiniMap.current.win_data[vim.api.nvim_get_current_tabpage()] = win_id
end
...
H.update_map_lines = function()
  ...
  if n_cols <= 0 then
    ...
    encoded_lines = vim.tbl_map(function(x)
      return string.format("%s%s", scrollbar_prefix, x)
    end, encoded_lines)
...
H.update_map_integrations = function()
  ...
  if not MiniMap.current.opts.window.show_integration_count then
    return
  end
...
H.is_encode_symbols = function(x, x_name)
  ...
  if type(x.resolution) ~= "table" then
    return false, H.msg_config(x_name .. ".resolution", "table")
  end
  if type(x.resolution.col) ~= "number" then
    return false, H.msg_config(x_name .. ".resolution.col", "number")
  end
  if type(x.resolution.row) ~= "number" then
    return false, H.msg_config(x_name .. ".resolution.row", "number")
  end

  local two_power = x.resolution.col * x.resolution.row
  for i = 1, 2 ^ two_power do
    if not H.is_string(x[i]) then
      return false, H.msg_config(string.format("%s[%d]", x_name, i), "string")
    end
  end
...
H.is_source_buffer = function()
  return vim.api.nvim_get_current_buf() == MiniMap.current.buf_data.source
end
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "minimap://" .. buf_id .. "/" .. name)
end
