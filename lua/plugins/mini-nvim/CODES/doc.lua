...
MiniDoc.config = {
  ...
  hooks = {
    ...
    block_post = function(b)
      ...
      if string.find(last_line, "^<?%s*$") == nil then
...
MiniDoc.generate = function(input, output, config)
  ...
  if not H.is_array_of(help_lines, H.is_string) then
    H.error("Output of `write_pre` should be array of strings.")
  end
  ...
  for _, dir in ipairs({ ".", "lua", "after", "colors" }) do
    ...
    table.sort(files, function(a, b)
      ...
      if a_dir == b_dir then
        ...
        if a_basename == "init.lua" then
          return true
        end
        if b_basename == "init.lua" then
          return false
        end
  ...
  for alias_name, alias_desc in pairs(MiniDoc.current.aliases) do
    ...
    for i, _ in ipairs(s) do
      ...
      s[i] = s[i]:gsub("(.?)(" .. name_escaped .. ")(.?)", function(before, match, after)
        ...
        if not is_valid or is_fixed_name or has_special_type then
...
H.enclose_type = function(s, init)
  ...
  for _ = 1, s[1]:len() do
    if s[1]:sub(to + 1, to + 2) == "[]" then
      to = to + 2
    end
    if s[1]:sub(to + 1, to + 1) == "?" then
      to = to + 1
    end
