  ...
  return function(ai_type)
    ...
    for _, diag in ipairs(cur_diag) do
      ...
      if to.line == nil or to.col == nil then
        to = { line = diag.lnum + 1, col = diag.col + 1 }
      end
...
MiniExtra.gen_ai_spec.indent = function()
  return H.ai_indent_spec
end
...
MiniExtra.gen_highlighter.words = function(words, group, extmark_opts)
  if not H.islist(words) then
    H.error("`words` should be an array.")
  end
  if not (type(group) == "string" or vim.is_callable(group)) then
    H.error("`group` should be string or callable.")
  end
  local pattern = vim.tbl_map(function(x)
    if type(x) ~= "string" then
      H.error("All elements of `words` should be strings.")
    end
...
MiniExtra.pickers.buf_lines = function(local_opts, opts)
  ...
  if is_scope_all then
    for _, buf_id in ipairs(vim.api.nvim_list_bufs()) do
      if vim.bo[buf_id].buflisted and vim.bo[buf_id].buftype == "" then
  ...
  local match = function(stritems, inds, query)
    pick.default_match(stritems, inds, query, match_opts)
  end
...
MiniExtra.pickers.colorschemes = function(local_opts, opts)
  ...
  if not H.islist(items) then
    H.error("`names` should be array of color scheme names")
  end
  for _, item in ipairs(items) do
    if not vim.tbl_contains(all_cs, item) then
      H.error(vim.inspect(item) .. " is not a color scheme name")
    end
  end

  local hl_groups = local_opts.preview_hl_groups
  if hl_groups ~= nil and not H.islist(hl_groups) then
    H.error("`preview_hl_groups` should be array")
  end
  ...
  local restore = function()
    ...
    if type(cs_orig) == "string" then
      return vim.cmd("colorscheme " .. cs_orig)
    end
    cs_orig:apply()
  ...
  local choose_marked = function(items_to_mark)
    choose(items_to_mark[1] or "")
  end
  ...
  if result == nil and needs_restore then
    restore()
  end
  return result
end
...
MiniExtra.pickers.diagnostic = function(local_opts, opts)
  ...
  if vim.is_callable(compare) then
...
MiniExtra.pickers.explorer = function(local_opts, opts)
  ...
  if vim.fn.isdirectory(cwd) == 0 then
    H.error("`local_opts.cwd` should be valid directory path.")
  end
  -- - Call twice "full path" to make sure that possible '..' are collapsed
  cwd = H.full_path(vim.fn.fnamemodify(cwd, ":p"))
  local filter = local_opts.filter or function()
    return true
  end
  if not vim.is_callable(filter) then
    H.error("`local_opts.filter` should be callable.")
  end
  local sort = local_opts.sort or H.explorer_default_sort
  if not vim.is_callable(sort) then
    H.error("`local_opts.sort` should be callable.")
  end
  ...
  local choose = function(item)
    ...
    if vim.fn.filereadable(path) == 1 then
      return pick.default_choose(path)
    end
...
MiniExtra.pickers.git_branches = function(local_opts, opts)
  ...
  local choose = function(item)
    ...
    if win_target == nil or not H.is_valid_win(win_target) then
      return
    end
...
MiniExtra.pickers.git_hunks = function(local_opts, opts)
  ...
  local preview = function(buf_id, item)
    ...
    has_parser = has_parser and parser ~= nil
    if has_parser then
      has_parser = pcall(vim.treesitter.start, buf_id, "diff")
    end
    if not has_parser then
  ...
  if scope == "staged" then
    table.insert(command, 4, "--cached")
  end

  local postprocess = function(lines)
    return H.git_difflines_to_hunkitems(lines, n_context)
  end
...
MiniExtra.pickers.hipatterns = function(local_opts, opts)
  ...
  if not has_hipatterns then
    H.error([[`pickers.hipatterns` requires 'mini.hipatterns' which can not be found.]])
  end
...
MiniExtra.pickers.history = function(local_opts, opts)
  ...
  local choose = function(item)
    if not (type(item) == "string" and vim.fn.mode() == "n") then
      return
    end
    local id, entry = item:match("^(.) (.*)$")
    if id == ":" or id == "/" or id == "?" then
      vim.schedule(function()
        vim.fn.feedkeys(id .. entry .. "\r", "nxt")
      end)
    end
  end
  ...
  local edit_command = function()
    ...
    if not (cur_scope == ":" or cur_scope == "/" or cur_scope == "?") then
      return
    end
    vim.schedule(function()
      vim.api.nvim_input(cur_scope .. cur_item:gsub("<", "<LT>"))
    end)
    return true
  end
...
MiniExtra.pickers.keymaps = function(local_opts, opts)
  ...
  local get_callback_pos = function(maparg)
    if type(maparg.callback) ~= "function" then
      return nil, nil
    end
...
MiniExtra.pickers.list = function(local_opts, opts)
  ...
  if local_opts.scope == nil then
    H.error("`pickers.list` needs an explicit scope.")
  end
...
MiniExtra.pickers.lsp = function(local_opts, opts)
  ...
  if local_opts.scope == nil then
    H.error("`pickers.lsp` needs an explicit scope.")
  end
  --stylua: ignore
...
MiniExtra.pickers.manpages = function(local_opts, opts)
  ...
  if vim.fn.exists(":Man") ~= 2 then
    H.error("`manpages` picker needs `:Man` command")
  end
  ...
  local show_man = function(win_id, item, is_preview)
    ...
    if name == nil or section == nil then
      return
    end
    -- - Use first command
    name = name:gsub(",.*$", "")
  ...
  local preview = function(buf_id, item)
    show_man(vim.fn.win_findbuf(buf_id)[1], item, true)
  end
  local choose = function(item)
    show_man(MiniPick.get_picker_state().windows.target, item)
  end
...
MiniExtra.pickers.marks = function(local_opts, opts)
  ...
  if scope == "all" or scope == "buf" then
    populate_items(vim.fn.getmarklist(vim.api.nvim_get_current_buf()))
  end
  if scope == "all" or scope == "global" then
    populate_items(vim.fn.getmarklist())
  end
...
MiniExtra.pickers.oldfiles = function(local_opts, opts)
  ...
  if not H.islist(oldfiles) then
    H.error("`pickers.oldfiles` picker needs valid `v:oldfiles`.")
  end
...
MiniExtra.pickers.options = function(local_opts, opts)
  ...
  for name, info in pairs(vim.api.nvim_get_all_options_info()) do
    if scope == "all" or scope == info.scope then
      table.insert(items, { text = name, info = info })
    end
  end
  ...
  local show = function(buf_id, items_to_show, query)
    ...
    H.pick_clear_namespace(buf_id, H.ns_id.pickers)
    for i, item in ipairs(items_to_show) do
      if not item.info.was_set then
        H.pick_highlight_line(buf_id, i, "Comment", 199)
      end
    end
  end
  ...
  local preview = function(buf_id, item)
    ...
    if not H.is_valid_win(target_win_id) then
      target_win_id = pick_windows.main
    end
    local value_source = ({ global = "o", win = "wo", buf = "bo" })[item.info.scope]
    local has_value, value = pcall(function()
      return vim.api.nvim_win_call(target_win_id, function()
        return vim[value_source][item.info.name]
      end)
    end)
    -- TODO: consider removing after Neovim<=0.10 compatibility is dropped
    if not has_value then
      value = "<Option is deprecated (will be removed in later Neovim versions)>"
    end
...
MiniExtra.pickers.registers = function(local_opts, opts)
  ...
  local choose = vim.schedule_wrap(function(item)
    ...
    if reg == "=" and regcontents ~= "" then
      reg = reg .. item.regcontents .. "\r"
    end
  ...
  local preview = function(buf_id, item)
    H.set_buflines(buf_id, vim.split(item.regcontents, "\n"))
  end
...
MiniExtra.pickers.treesitter = function(local_opts, opts)
  ...
  if not has_parser or parser == nil then
    H.error("`pickers.treesitter` requires active tree-sitter parser.")
  end
...
MiniExtra.pickers.visit_paths = function(local_opts, opts)
  ...
  if not has_visits then
    H.error([[`pickers.visit_paths` requires 'mini.visits' which can not be found.]])
  end
  ...
  local items = vim.schedule_wrap(function()
    ...
    paths = vim.tbl_map(function(x)
      return H.normalize_path(H.short_path(x, picker_cwd))
    end, paths)
...
MiniExtra.pickers.visit_labels = function(local_opts, opts)
  ...
  local has_visits, visits = pcall(require, "mini.visits")
  if not has_visits then
    H.error([[`pickers.visit_labels` requires 'mini.visits' which can not be found.]])
  end
  ...
  local preview = function(buf_id, label)
    vim.api.nvim_buf_set_lines(buf_id, 0, -1, false, list_label_paths(label))
  end
...
H.apply_config = function(config)
  ...
  if type(_G.MiniPick) == "table" then
    for name, f in pairs(MiniExtra.pickers) do
      _G.MiniPick.registry[name] = _G.MiniPick.registry[name]
...
H.ai_indent_spec = function(ai_type)
  ...
  for i, l in ipairs(lines) do
    ...
    if is_blank then
      H.ai_indent_update_rays(i, indents[i]:gsub("\t", tab_spaces):len(), rays, rays_final)
    end
  end
  ...
  table.sort(rays_final, function(a, b)
...
H.pick_make_no_preview = function(picker_name)
  ...
  return function(buf_id, _)
    H.set_buflines(buf_id, lines)
  end
end

H.pick_validate_one_of = function(target, opts, values, picker_name)
  if vim.tbl_contains(values, opts[target]) then
    return opts[target]
  end
...
H.pick_validate_scope = function(...)
...
H.git_normalize_path = function(path, picker_name)
  ...
  if path == "" then
    H.error(string.format("Path in `%s` is empty.", picker_name))
  end
  path = H.full_path(path)
  local path_is_dir, path_is_file = vim.fn.isdirectory(path) == 1, vim.fn.filereadable(path) == 1
  if not (path_is_dir or path_is_file) then
    H.error("Path " .. path .. " is not a valid path.")
  end
...
H.lsp_make_opts = function(source, opts)
  ...
  local process = function(items)
    if source ~= "document_symbol" then
      items = vim.tbl_map(H.pick_prepend_position, items)
    end
    ...
    if not vim.startswith(source, "workspace_symbol") then
  ...
  picker_opts.source.show = function(buf_id, items_to_show, query)
    if show_explicit ~= nil then
      return show_explicit(buf_id, items_to_show, query)
    end
...
H.list_get = {
  quickfix = function()
    return vim.tbl_map(H.list_enhance_qf_loc, vim.fn.getqflist())
  end,

  location = function()
    return vim.tbl_map(H.list_enhance_qf_loc, vim.fn.getloclist(0))
  end,
...
H.cli_run = function(command, stdout_hook)
  ...
  process = vim.loop.spawn(executable, spawn_opts, function()
    process:close()
  end)
...
H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "miniextra://" .. buf_id .. "/" .. name)
end
