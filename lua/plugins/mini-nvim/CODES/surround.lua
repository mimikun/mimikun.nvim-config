...
MiniSurround.user_input = function(prompt, text)
  ...
  vim.on_key(function(key)
    was_cancelled = was_cancelled or key == "\27" or key == "\3"
  end, H.ns_id.input)
...
H.apply_config = function(config)
  ...
  local m = function(mode, lhs, rhs, desc)
    H.map(mode, lhs, rhs, { expr = true, desc = desc })
  end
...
H.make_surrounding_table = function()
  ...
  return setmetatable(surroundings, {
    __index = function(_, key)
      return H.get_default_surrounding_info(key)
    end,
  })
end
...
H.get_matched_range_pairs_plugin = function(captures)
  ...
  outer_matches = vim.tbl_filter(function(x)
    return x.node.tree ~= nil
  end, outer_matches)
  ...
  return vim.tbl_map(function(m_outer)
    ...
    local inner_ranges = vim.tbl_map(function(m)
...
H.append_lang_ranges = function(outer, inner, missing_query_langs, buf_id, captures, lang_tree)
  ...
  if query ~= nil then
    H.append_ranges(outer, inner, buf_id, query, captures, lang_tree)
  end
  if query == nil then
...
H.map = function(mode, lhs, rhs, opts)
  ...
  if no_global_s_mapping and lhs:find("^s.") ~= nil then
    vim.keymap.set(mode, "s", "<Nop>")
  end
end
