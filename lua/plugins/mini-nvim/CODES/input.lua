...
MiniInput.get = function(opts)
  ...
  for _ = 1, 1000000 do
    ...
    if key == nil and not H.state_is_end() then
...
MiniInput.get_history = function()
  return vim.deepcopy(H.history)
end
...
MiniInput.gen_highlight.treesitter = function(lang)
  local append_ts_hl_range = function(arr, line, tstree, tree)
    ...
    for capture, node in query:iter_captures(tstree:root(), line) do
      ...
      if query.captures[capture]:sub(1, 1) ~= "_" then
        ...
        if from <= to then
          table.insert(arr, { from = from, to = to, hl = hl })
        end
      end
    end
  end
  ...
  return function(state)
    ...
    parser:for_each_tree(function(tstree, tree)
      append_ts_hl_range(highlight, line, tstree, tree)
    end)
...
MiniInput.gen_view.floatwin = function(opts)
  ...
  default_opts.adjust_config = function(_, config)
    return config
  end
...
MiniInput.gen_view.uiline = function(opts)
  ...
  return function(state)
    ...
    local parts = vim.tbl_map(function(c)
      return "%#" .. escape_stl(c[2]) .. "#" .. escape_stl(c[1])
    end, chunks)
    local uiline_value = table.concat(parts) .. "%#MiniInputNormal#"

    local opt = style == "tabline" and vim.o or vim.wo
    opt[style] = uiline_value
    if style == "statusline" and vim.o.laststatus < 2 then
      vim.o.laststatus = 2
    end
    if style == "tabline" and vim.o.showtabline < 2 then
      vim.o.showtabline = 2
    end
  end
end
...
MiniInput.gen_view.virtual = function(opts)
  ...
  return function(state)
    ...
    if is_virtline and (state.status == "start" or style_is_new) then
      ...
      if win_line == 1 and style == "above" then
        vim.cmd("normal! \25")
      end
...
MiniInput.default_key = function(state, key, opts)
  opts = opts or {}
  if key == nil or H.state_is_end(state) then
    return
  end
  if opts.autopair and H.key_methods_autopair[key] ~= nil then
    return H.key_methods_autopair[key](state)
  end
  if H.key_methods[key] ~= nil then
    return H.key_methods[key](state, opts)
  end
  if H.is_special_char(key) then
    return
  end
...
MiniInput.default_highlight = function(state)
  ...
  if state.complete == nil or state.complete.id == 0 then
    return
  end
...
MiniInput.default_complete = function(state, method, opts)
  ...
  if method == "history" then
    return H.complete_history(state, opts.precise_history)
  end
...
MiniInput.state_to_chunks = function(state, max_width, opts)
  ...
  if opts.symbol_caret ~= "" then
    H.append_chunks(chunks, { { opts.symbol_caret, "MiniInputCaret" } }, keytrans)
  end
...
MiniInput.apply_handler = function(state, name, arg)
  ...
  if arg == nil and name == "complete" then
    arg = ""
  end

  if state.opts.hide and (name == "highlight" or name == "complete") then
    return state
  end
  local ok_handler, res = pcall(state.opts.handlers[name], state, arg)
  if not ok_handler then
    H.cache_error(state, "Error applying `" .. name .. "` handler: " .. res)
  end
...
H.apply_config = function(config)
  ...
  if vim.fn.has("nvim-0.12.3") == 0 then
    ...
    vim.schedule(function()
      vim.lsp.buf.rename = H.mock_lsp_buf_rename(vim.lsp.buf.rename)
    end)
  end
end
...
H.adjust_vim_paste = function()
  ...
  vim.paste = function(lines, phase)
    if MiniInput.get_state() == nil then
      return paste_orig(lines, phase)
    end
    if phase ~= -1 then
      return H.notify("There is no streaming paste support. Use `<C-r>+` or `<C-r>*`.", "WARN")
    end
...
H.state_validate = function(x)
  ...
  if x.highlight ~= nil then
    ...
    for i, h in ipairs(x.highlight) do
      ...
      if h.from > h.to then
        H.error(string.format("%s.from is bigger than %s.to", item, item))
      end
    end
  end
...
H.mock_lsp_buf_rename = function(f)
  return function(new_name, opts)
    if new_name == nil then
      (_G.MiniInput or {})._temp_default_scope = "cursor"
    end
...
-- General
H.key_methods[kc("<CR>")] = function(state, _)
  state.status = "accept"
end
H.key_methods[kc("<Esc>")] = function(state, _)
  state.status = "cancel"
end
...
H.key_methods[kc("<C-b>")] = function(state, _)
  state.caret = 1
end
...
H.key_methods[kc("<End>")] = function(state, _)
  state.caret = vim.fn.strchars(state.input) + 1
end
...
H.key_methods[kc("<Del>")] = function(state, _)
  local caret, input = state.caret, state.input
  if caret > vim.fn.strchars(input) then
    return
  end
...
H.key_methods[kc("<C-r>")] = function(state, _)
  ...
  if expand_var then
    reg_content = vim.fn.expand(expand_var)
  end
  if register == "\f" then
    reg_content = vim.fn.getline(".")
  end
...
H.key_methods[kc("<C-q>")] = function(state, _)
  ...
  if not ok or (ok and vim.fn.char2nr(char) <= 31) then
    new_text = char
  end
  H.insert_at_caret(state, new_text)
end
...
H.key_methods[kc("<Up>")] = function(state, _)
  if state.complete == nil then
    state = MiniInput.apply_handler(state, "complete", "history")
  end
...
H.key_methods[kc("<C-y>")] = function(state, _)
  state.complete = nil
end
...
H.key_methods[kc("<Tab>")] = function(state, _)
  if state.complete == nil then
    state = MiniInput.apply_handler(state, "complete", state.opts.completion)
  end
  return H.advance_state_complete(state, 1)
end
...
H.key_methods_autopair["("] = function(state)
  H.autopair_open(state, "()")
end
H.key_methods_autopair["["] = function(state)
  H.autopair_open(state, "[]")
end
...
H.advance_from_choices = function(all, cur, name)
  ...
  if not (new == nil or new == cur) then
    H.notify(string.format("Changed %s to %s", name, vim.inspect(new)))
  end
  return new
end
...
H.autopair_close = function(state, close_char)
  ...
  if is_before_close then
    state.caret = state.caret + 1
  end
  if not is_before_close then
    H.insert_at_caret(state, close_char)
  end
end
...
H.autopair_closeopen = function(state, pair, close_char)
  local is_before_close = vim.fn.strcharpart(state.input, state.caret - 1, 1) == close_char
  if is_before_close then
    H.autopair_close(state, close_char)
  end
  if not is_before_close then
    H.autopair_open(state, pair)
  end
end
...
H.complete_history = function(state, precise_history)
  ...
  local is_precise = function(h)
    return h.cwd == cwd and h.scope == scope and h.prompt == prompt
  end
  if not precise_history then
  ...
  local is_match = function(x)
    return vim.startswith(x, base) and x ~= base
  end

  local raw, matched, seen = MiniInput.get_history(), {}, {}
  for i = #raw, 1, -1 do
    local val = raw[i].input
    if not seen[val] and is_match(val) and is_precise(raw[i]) then
...
H.get_base_from_items = function(text, items)
  ...
  local is_match = function(base_cur)
    for _, item in ipairs(items) do
      if not vim.startswith(item, base_cur) then
        return false
      end
    end
    return true
  ...
  if vim.o.wildoptions:find("fuzzy") ~= nil then
    is_match = function(base_cur)
      return #vim.fn.matchfuzzy(items, base_cur, { matchseq = 1 }) == #items
    end
  end
...
H.handle_view_style = function(state, init_style, all_styles)
  if state.status == "start" then
    state.data.all_styles = all_styles
  end
...
H.default_floatwin_config = function(state, style, target_width)
  ...
  if not (scope == "editor" or scope == "tabpage" or scope == "project") then
    ...
    if scope ~= "cursor" then
      width = H.clamp(width, 1, ref_rect.width - width_offset)
    end
  end
...
H.uiline_handle_option_values = function(state, style_was_changed)
  ...
  if not (H.state_is_end(state) or needs_reset) then
    return
  end
...
H.uiline_unset_option_value = function(state, option_name, win_id)
  ...
  if cur_val == state.data[option_name] then
    return
  end
...
H.append_chunks = function(arr, new, keytrans)
  if not keytrans then
    return vim.list_extend(arr, new)
  end
  ...
  for _, ch in ipairs(new) do
    for s, special in string.gmatch(ch[1], "(%C*)(%c*)") do
      if s ~= "" then
        table.insert(arr, { s, ch[2] })
      end
      if special ~= "" then
        table.insert(arr, { vim.fn.keytrans(special), "MiniInputSpecial" })
      end
    end
  end
end
...
H.fit_chunks_to_displaywidth = function(chunks, width, center_offset)
  ...
  if width == nil or chunks_width < width then
    return chunks
  end
...
H.get_chunks_displaywidth = function(chunks)
  return vim.fn.strdisplaywidth(H.get_chunks_text(chunks))
end
...
H.insert_hl_range = function(arr, max_to, from, to, hl)
  ...
  if from == nil or to == nil or to < 1 or from > max_to then
    return arr
  end
  ...
  for _, cur in ipairs(arr) do
    -- No overlap
    if cur.to < from or to < cur.from then
      table.insert(res, cur)
    end
    -- Overlap with outside left part of current
    if cur.from < from and from <= cur.to then
      table.insert(res, { from = cur.from, to = from - 1, hl = cur.hl })
    end
    -- Overlap
    ...
    if cur.from <= to and to < cur.to then
      table.insert(res, { from = to + 1, to = cur.to, hl = cur.hl })
    end
  end
...
H.make_ui_select_hl_fun = function(hl_fun)
  if not vim.is_callable(hl_fun) then
    return
  end
  ...
  return function(state)
    ...
    if not H.islist(hl_ranges) then
      hl_ranges = {}
    end
...
H.check_array_of = function(name, x, ref_type)
  if not H.islist(x) then
    H.error("`" .. name .. "` should be array")
  end
  for i, k in ipairs(x) do
    if type(k) ~= ref_type then
      H.error("Every `" .. name .. "` item should be " .. ref_type)
    end
  end
end

H.set_buf_name = function(buf_id, name)
  vim.api.nvim_buf_set_name(buf_id, "miniinput://" .. buf_id .. "/" .. name)
end

H.notify = function(msg, level_name)
  vim.notify("(mini.input) " .. msg, vim.log.levels[level_name])
end
...
H.getcharstr = function(lmap, allow_ctrl_c)
  ...
  if not ok and not is_ctrl_c then
    H.cache_error(H.state, char)
  end

  -- Terminate if no input or on hard-coded <C-c>
  if allow_ctrl_c and is_ctrl_c then
    return "\3"
  end
  if not ok or char == "" or char == "\3" then
    return nil
  end
...
H.safe_fn_getcharstr = function()
  return pcall(vim.fn.getcharstr, -1, { cursor = "hide" })
end
...
H.getcharstr_many = function(n, early_stop)
  ...
  for i = 1, n do
    res[i] = H.getcharstr()
    if res[i] == nil or early_stop[res[i]] then
      return nil
    end
  end
...
H.cache_error = function(state, msg)
  if not vim.startswith(msg, "(mini.input) ") then
    msg = "(mini.input) " .. msg
  end
  if H.state == nil then
    error(msg)
...
H.fit_to_displaywidth = function(text, width)
  ...
  for i = 1, vim.fn.strchars(text) do
    ...
    if vim.fn.strdisplaywidth(truncated) <= width then
      return truncated
    end
  end
  return ""
