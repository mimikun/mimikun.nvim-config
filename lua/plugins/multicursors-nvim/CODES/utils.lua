...
local function check_selection_bounds(sel)
  if sel.row > sel.end_row or (sel.row == sel.end_row and sel.col > sel.end_col) then
    return {
      id = sel.id,
      row = sel.end_row,
      col = sel.end_col,
...
M.get_main_selection = function()
  local main = api.nvim_buf_get_extmarks(0, main_ns_id, 0, -1, { details = true })[1]
...
M.get_all_selections = function()
  local extmarks = api.nvim_buf_get_extmarks(0, ns_id, 0, -1, { details = true })
  ---@type Selection[]
  local selections = {}
...
M.call_on_selections = function(callback)
  ...
  for _, selection in pairs(marks) do
    local mark = api.nvim_buf_get_extmark_by_id(0, ns_id, selection.id, { details = true })
