...
local goto_next_selection = function(skip)
  ...
  if goto_first_selection({ main.end_row, main.end_col }, { -1, -1 }, skip) then
    return
  end
...
M.normal_command = function()
  vim.ui.input({ prompt = "Enter normal command: ", completion = "command" }, function(input)
    if not input then
      return
    end
    utils.call_on_selections(function(selection)
...
M.replace = function()
  ...
  if #content ~= selection_count then
    utils.update_selections_with(function(selection)
      api.nvim_buf_set_text(0, selection.row, selection.col, selection.end_row, selection.end_col, content)
  ...
  local index = 1
  utils.update_selections_with(function(selection)
    api.nvim_buf_set_text(0, selection.row, selection.col, selection.end_row, selection.end_col, { content[index] })
...
local align_text = function(line_start)
  ...
  utils.call_on_selections(function(selection)
    ...
    api.nvim_buf_set_text(0, selection.row, col_pos, selection.row, col_pos, { string.rep(" ", length) })
  end)
end
...
M.change = function(config)
  utils.call_on_selections(function(selection)
    api.nvim_buf_set_text(0, selection.row, selection.col, selection.end_row, selection.end_col, {})
  end)
...
M.yank = function()
  ...
  local contents = {}
  utils.call_on_selections(function(selection)
    local text = api.nvim_buf_get_text(0, selection.row, selection.col, selection.end_row, selection.end_col, {})
...
M.yank_line = function()
  ...
  local contents = {}
  utils.call_on_selections(function(selection)
    local text = api.nvim_buf_get_lines(0, selection.row, selection.end_row + 1, true)
    contents[#contents + 1] = text[1]
  end)
  vim.fn.setreg("", contents)
end
...
M.upper_case = function()
  utils.update_selections_with(function(selection)
    ...
    api.nvim_buf_set_text(0, selection.row, selection.col, selection.end_row, selection.end_col, text)
  end)
end
