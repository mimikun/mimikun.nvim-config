...
S._get_new_position = function(selection, motion)
  ...
  if selection.row == selection.end_row and selection.col > selection.end_col then
    selection.col = -1
  end
...
local get_selection_content = function(selection, pos)
  local lines = api.nvim_buf_get_lines(0, selection.row, selection.end_row + 1, false)
...
local function reduce_to_before(selection, text)
  ...
  if selection.col >= selection.end_col and selection.end_col == 0 and selection.row == selection.end_row then
    selection.col = -1
  else
