...
local image = {
  ...
  force = false,
  doc = {
    -- enable image viewer for documents a treesitter parser must be available for the enabled languages.
    enabled = true,
    -- render the image inline in the buffer if your env doesn't support unicode placeholders, this will be disabled takes precedence over `opts.float` on supported terminals
    inline = true,
  ...
  -- window options applied to windows displaying image buffers an image buffer is a buffer with `filetype=image`
