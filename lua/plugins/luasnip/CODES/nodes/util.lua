...
local function find_node_dependents(node)
  ...
  vim.list_extend(nodes, dict:find_all({ node, "dependents" }, "dependent") or {})

  if node.key then
    vim.list_extend(nodes, dict:find_all({ "key", node.key, "dependents" }, "dependent") or {})
  end
...
local function store_cursor_node_relative(node, opts)
  ...
  while snippet_current_node do
    ...
    snip_data.cursor_start_relative = util.pos_offset(snippet_current_node.mark:get_endpoint(-1), cursor_state.pos)

    snip_data.mode = cursor_state.mode

    if cursor_state.pos_v then
      snip_data.selection_end_start_relative =
        util.pos_offset(snippet_current_node.mark:get_endpoint(-1), cursor_state.pos_v)
    end
