...
function Snippet:insert_into_jumplist(current_node, parent_node, sibling_snippets, own_indx)
...
function Snippet:trigger_expand(current_node, pos_id, env, indent_nodes)
  ...
  local pre_expand_res = self:event(events.pre_expand, { expand_pos = pos, expand_pos_mark_id = pos_id }) or {}
  ...
  self:insert_into_jumplist(current_node, parent_node, sibling_snippets, own_indx)
...
function Snippet:get_docstring()
  ...
  self.docstring = self.type == types.snippet and docstring or util.string_wrap(docstring, self.pos)
  return self.docstring
end
