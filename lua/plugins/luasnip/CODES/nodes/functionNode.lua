...
function FunctionNode:update()
  ...
  self.last_args = args
  local text = util.to_string_table(self.fn(args, self.parent, unpack(self.user_args)))
...
function FunctionNode:update_static()
  ...
  self.static_text = util.indent(util.to_string_table(static_text), self.parent.indentstr)
end
