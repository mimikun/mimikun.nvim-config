...
function DynamicNode:update()
  ...
  -- construncting.
  tmp = self.fn(effective_args, self.parent, old_state, unpack(self.user_args))
...
function DynamicNode:update_static()
  ...
  if self.static_snip then
    ...
    ok, tmp = pcall(self.fn, effective_args, self.parent, self.static_snip.old_state, unpack(self.user_args))
  else
    if not args then
      ...
      ok, tmp = pcall(self.fn, effective_args, self.parent, nil, unpack(self.user_args))
    end
  end
  if not ok then
