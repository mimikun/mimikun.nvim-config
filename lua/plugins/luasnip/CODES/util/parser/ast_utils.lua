...
function M.fix_zero(ast)
  ...
  if
    ...
    or (
      zn
      and not is_copied
      and (zn.type == types.TABSTOP or (zn.type == types.PLACEHOLDER and text_only_placeholder(zn)))
    ...
    if node.type == types.TEXT then
      ...
      if capture and #capture > 0 then
        ...
        elseif node.modifier then
          transformed = transformed .. apply_modifier(capture, node.modifier)
        else
...
function M.apply_transform(transform)
  if jsregexp_compile_safe then
    local reg_compiled, err = jsregexp_compile_safe(transform.pattern, transform.option)
