...
local function _complete_ext_opts(ext_opts)
  ...
  ext_opts.passive =
    vim.tbl_extend("keep", ext_opts.passive or shared_empty_table, ext_opts.snippet_passive or shared_empty_table)
  -- both unvisited and visited inherit from passive.
  ext_opts.unvisited =
    vim.tbl_extend("keep", ext_opts.unvisited or shared_empty_table, ext_opts.passive or shared_empty_table)
  ext_opts.visited =
    vim.tbl_extend("keep", ext_opts.visited or shared_empty_table, ext_opts.passive or shared_empty_table)
  -- active inherits from visited.
  ext_opts.active =
    vim.tbl_extend("keep", ext_opts.active or shared_empty_table, ext_opts.visited or shared_empty_table)
...
local function increase_prio(opts, inc)
  ...
  for _, state in ipairs(states) do
    opts[state].priority = opts[state].priority and (opts[state].priority + inc)
  end
end
