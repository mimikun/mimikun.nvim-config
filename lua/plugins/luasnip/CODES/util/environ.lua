...
local function _env_namespace(name, opts)
  ...
  assert(
    ...
    ("Your opts for %s can't set a `eager` field without the `vars` one"):format(name)
  )
  ...
  assert(
    type_of_it == "table" or type_of_it == "boolean" or type_of_it == "function",
    ("Your opts for %s can't have `multiline_vars` of type %s"):format(name, type_of_it)
  )
...
function Environ.env_namespace(name, opts)
  ...
  assert(
    ...
    ("You can't create a namespace with name '%s' because is one one of %s"):format(name, builtin_ns_names)
  )
