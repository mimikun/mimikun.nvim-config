...
function lazy_vars.CURRENT_TIMEZONE_OFFSET()
  return time_util.get_timezone_offset(os.time()):gsub("([+-])(%d%d)(%d%d)$", "%1%2:%3")
end
...
local function eager_vars(info)
  ...
  vars.LS_SELECT_RAW, vars.LS_SELECT_DEDENT, vars.TM_SELECTED_TEXT = select_util.retrieve()
  -- These are for backward compatibility, for now on all builtins that are not part of TM_ go in LS_
  vars.SELECT_RAW, vars.SELECT_DEDENT = vars.LS_SELECT_RAW, vars.LS_SELECT_DEDENT
...
return {
  is_table = function(key)
    ...
    if key:match("LS_TSCAPTURE_.*") or key == "LS_TSPOSTFIX_MATCH" or key == "LS_TSPOSTFIX_DATA" then
      return true
    end
