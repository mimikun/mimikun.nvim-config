  ...
  for ft, ft_snippets in pairs(snippets) do
    for _, addable in ipairs(ft_snippets) do
      for _, snip in ipairs(addable:retrieve_all()) do
        local snip_prio = opts.override_priority or (snip.priority and snip.priority) or opts.default_priority or 1000

        snip.effective_priority = snip_prio

        -- if snippetType undefined by snippet, take default value from opts
        local snip_type = snip.snippetType ~= nil and snip.snippetType or opts.type
        assert(
...
function M.refresh_notify(ft_or_nil)
  ...
  for _, ft in ipairs(fts) do
    ...
    vim.api.nvim_exec_autocmds("User", { pattern = "LuasnipSnippetsAdded", modeline = false })
  end
end
