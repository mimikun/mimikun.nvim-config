...
function TreeWatcher:start()
  ...
  if self.fs_event_providers.libuv then
    ...
    if not success then
      log_tree.error("Could not start libuv-monitor for path %s due to error %s", self.root, err)
    else
...
function TreeWatcher:new_dir(rel, full)
  ...
  self.dir_watchers[rel] =
    M.tree(full, self.depth - 1, self.callbacks, { lazy = false, fs_event_providers = self.fs_event_providers })
end
