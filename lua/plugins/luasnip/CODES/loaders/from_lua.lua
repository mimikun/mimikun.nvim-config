...
local function _luasnip_load_file(file)
  ...
  -- environment.
  _G.__luasnip_get_loaded_file_frame_debuginfo =
    util.ternary(session.config.loaders_store_source, get_loaded_file_debuginfo, nil)
  ...
  for paths_lazy, roots in pairs({
    ...
    for _, collection_root in ipairs(roots) do
      local ok, coll_or_err =
        pcall(Collection.new, collection_root, lazy, o.include, o.exclude, o.add_opts, paths_lazy, o.fs_event_providers)
