...
local function read_json(fname)
  ...
  if json_decoders[fname_extension] == nil then
    log.error("`%s` was expected to have file-extension either `json`, `jsonc` or `code-snippets`, but doesn't.", fname)
    return nil
  end
...
local function get_file_snippets(file, opts)
  ...
  for name, parts in pairs(snippet_set_data) do
    ...
    local filetypes = respect_scope and parts.scope and loader_util.scopestring_to_filetypes(parts.scope) or { false }
...
Data.vscodesnippets_cache = require("luasnip.loaders.snippet_cache").new(function(fname)
...
function Collection:add_file(path, ft)
  ...
  if self.lazy then
    if not session.loaded_fts[ft] then
      ...
      log.info("Filetype `%s` is already active, loading immediately.", ft)
    end
  end
...
function Collection:load_file(path, ft)
  ...
  if not self.path_watchers[path] then
    ...
    local ok, watcher_or_err =
      pcall(SnippetfileWatcher.new, path, ft, self.fs_event_providers, true, self.load_callback)
    if not ok then
...
local function get_manifests(paths)
  ...
  if paths then
    ...
    for _, dir in ipairs(paths) do
      local tentative_manifest_path = Path.expand_keep_symlink(Path.join(dir, "package.json"))
      if Path.exists(tentative_manifest_path) then
        table.insert(manifest_paths, tentative_manifest_path)
      else
        tentative_manifest_path = Path.expand_keep_symlink(Path.join(dir, "package.jsonc"))
        if Path.exists(tentative_manifest_path) then
          table.insert(manifest_paths, tentative_manifest_path)
        else
...
local function _load(lazy, opts)
  ...
  for is_lazy, manifest_paths in pairs({
    ...
    for _, manifest_path in ipairs(manifest_paths) do
      local ok, coll_or_err =
        pcall(Collection.new, manifest_path, lazy, o.include, o.exclude, o.add_opts, is_lazy, o.fs_event_providers)
...
function M.load_standalone(opts)
  ...
  local fs_event_providers = util.if_nil(opts.fs_event_providers, { autocmd = true, libuv = false })
