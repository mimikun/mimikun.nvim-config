...
local function get_parser(bufnr, lang, opts)
  local has_parser, parser_or_err, err = pcall(vim.treesitter.get_parser, bufnr, lang, opts)
...
local function inspect_node(node)
  ...
  return ("%s [%d, %d] [%d, %d]"):format(node:type(), start_row, start_col, end_row, end_col)
end
...
function FixBufferContext:leave()
  ...
  -- in self.
  vim.api.nvim_win_set_cursor(0, { self.region.row + 1, self.region.col_range[2] })
...
local function captures_iter(captures)
  ...
  return function(query, match_iter)
    ...
    iter = function()
      ...
      while true do
        local capture
        current_capture_id, capture = next(current_match, current_capture_id)
    ...
    return {
      record = function(match, node)
        ...
        if
          ...
          or (start_row == best_node_start[1] and start_col > best_node_start[2])
        then
          best_node = node
          best_node_match = match
  ...
  setmetatable(o, {
    ...
    __tostring = function(self)
      return ("trees: %d, source: %s"):format(
        #self.parser:trees(),
        type(self.source) == "number" and tostring(self.source) or "[COPIED]"
      )
    end,
  })
  return o
...
function TSParser:root_at(pos)
  local tree = self.parser:tree_for_range({ pos[1], pos[2], pos[1], pos[2] }, { ignore_injections = false })
  if not tree then
    return nil
  end
