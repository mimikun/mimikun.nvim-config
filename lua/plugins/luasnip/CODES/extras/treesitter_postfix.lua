...
local function generate_match_tsnode_func(opts)
  ...
  if opts.query then
    ...
    match_opts.query = vim.treesitter.query.get(opts.query_lang, opts.query_name or "luasnip")
  end
  ...
  return function(parser, pos)
    return parser:match_at(match_opts, --[[@as LuaSnip.extra.MatchTSNodeOpts]] pos)
  end
end

local function make_reparse_enter_and_leave_func(reparse, bufnr, trigger_region, trigger)
  ...
  elseif reparse == "copy" then
    local parser, source = ts.reparse_buffer_after_removing_match(bufnr, trigger_region)
    return function()
    ...
    return function()
      local parser = require("luasnip.extras._treesitter").get_parser(bufnr)
      if parser then
        parser:parse()
      end
      return parser, bufnr
...
local function wrap_with_reparse_context(reparse, real_resolver)
  return function(snippet, line_to_cursor, matched_trigger, captures)
    ...
    local enter, leave = make_reparse_enter_and_leave_func(reparse, bufnr, trigger_region, matched_trigger)
...
local function generate_resolve_expand_param(match_tsnode, user_resolver)
  ...
  ---@param bufnr number
  ---@param pos { [1]: number, [2]: number }
  return function(snippet, line_to_cursor, matched_trigger, captures, parser, source, bufnr, pos)
    ...
    for capture_name, node in pairs(best_match) do
      env["LS_TSCAPTURE_" .. capture_name:upper()] = vim.split(ts_parser:get_node_text(node), "\n")
    ...
    if user_resolver then
      local user_res = user_resolver(snippet, line_to_cursor, matched_trigger, captures)
      if user_res then
        ret = vim.tbl_deep_extend("force", ret, user_res, { env_override = {} })
      else
        return nil
      end
    end
...
local function treesitter_postfix(context, nodes, opts)
  ...
  local expand_params_resolver = generate_resolve_expand_param(match_tsnode_func, context.resolveExpandParams)

  context.resolveExpandParams = wrap_with_reparse_context(context.reparseBuffer, expand_params_resolver)
...
return {
  ...
  builtin = {
    tsnode_matcher = {
      find_topmost_types = generate_simple_parent_lookup_function(ts.find_topmost_parent),
      find_first_types = generate_simple_parent_lookup_function(ts.find_first_parent),
