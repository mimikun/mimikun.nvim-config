...
local function lua_find_function_call_node_at(bufnr, line)
  local parser, err = require("luasnip.extras._treesitter").get_parser(bufnr, "lua")
  if not parser then
  ...
  error("Query for `(function_call)` starting at line %s did not yield a result.")
end
...
local function json_find_snippet_definition(bufnr, filetype, snippet_name)
  local parser, err = require("luasnip.extras._treesitter").get_parser(bufnr, filetype)
  if not parser then
    error("Error while getting parser: " .. err)
  end
  ...
  error(("Treesitter did not find the definition for snippet `%s`"):format(snippet_name))
end
...
function M.jump_to_snippet(snip, opts)
  ...
  local fcall_range
  local ft =
    util.ternary(vim.bo[0].filetype ~= "", vim.bo[0].filetype, vim.api.nvim_buf_get_name(0):match("%.([^%.]+)$"))
  if ft == "lua" then
    if source.line then
      -- in lua-file, can get region of definition via treesitter.
      -- 0: current buffer.
      local ok
      ok, fcall_range = pcall(lua_find_function_call_node_at, 0, source.line - 1)
      if not ok then
        print("Could not determine range for snippet-definition: " .. fcall_range)
  ...
  elseif ft == "json" or ft == "jsonc" then
    ...
    if not ok then
      print("Could not determine range of snippet-definition: " .. fcall_range)
      return
    end
  else
    print(
  ...
  local new_source = Source.from_location(source.file, { line = fcall_range[1] + 1, line_end = fcall_range[3] + 1 })
