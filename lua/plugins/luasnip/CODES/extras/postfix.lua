...
local function wrap_resolve_expand_params(match_pattern, user_resolve)
  return function(snippet, line_to_cursor, match, captures)
    ...
    local postfix_match = line_to_cursor_except_match:match(match_pattern) or ""
    local res = {
      clear_region = {
    ...
    if user_resolve then
      local user_res = user_resolve(snippet, line_to_cursor, match, captures)
      if user_res then
...
local function postfix(context, nodes, opts)
  ...
  context.resolveExpandParams = wrap_resolve_expand_params(match_pattern, context.resolveExpandParams)

  return snip(context, nodes, opts)
end
extend_decorator.register(postfix, { arg_indx = 1, extend = node_util.snippet_extend_context }, { arg_indx = 3 })
