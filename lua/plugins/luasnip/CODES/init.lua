...
local function match_snippet(line, type)
  return snippet_collection.match_snippet(line, util.get_snippet_filetypes(), type)
end
...
local function node_update_dependents_preserve_position(node, current, opts)
  ...
  local restore_data = opts.cursor_restore_data
    or node_util.store_cursor_node_relative(current, { place_cursor_mark = true })
...
local function safe_jump_current(dir, no_move, dry_run)
  ...
  if not dry_run and node.pos >= 0 then
    local upd_res =
      node_update_dependents_preserve_position(node, node, { no_move = no_move, restore_position = false })
...
function API.jumpable(dir)
  ...
  return API.jump_destination(dir) ~= session.current_nodes[vim.api.nvim_get_current_buf()]
end
...
function API.expandable()
  next_expand, next_expand_params = match_snippet(util.get_current_line_to_cursor(), "snippets")
...
local function _snip_expand(snippet, opts)
  ...
  local info = { trigger = snip.trigger, captures = snip.captures, pos = opts.pos }
  ...
  local snip_parent_node =
    snip:trigger_expand(session.current_nodes[vim.api.nvim_get_current_buf()], pos_id, env, opts.indent)

  -- jump_into-callback returns new active node.
  session.current_nodes[vim.api.nvim_get_current_buf()] = opts.jump_into_func(snip)
...
local function _expand(opts)
  ...
  if next_expand ~= nil then
    ...
    snip, expand_params = match_snippet(util.get_current_line_to_cursor(), "snippets")
  end
  if snip then
...
function API.expand_auto()
  api_do(function()
    local snip, expand_params = match_snippet(util.get_current_line_to_cursor(), "autosnippets")
    if snip then
      local cursor = util.get_cursor_0ind()
      local clear_region = expand_params.clear_region
        or {
...
function API.lsp_expand(body, opts)
  ...
  api_do(_snip_expand, ls.parser.parse_snippet("", body, { trim_empty = false, dedent = false }), opts)
end
...
local function _change_choice(val, opts)
  ...
  if not opts.skip_update then
    ...
    active_choice = session.active_choice_nodes[vim.api.nvim_get_current_buf()]
  ...
  local new_active = safe_choice_action(
    ...
    opts.skip_update and opts.cursor_restore_data
      or node_util.store_cursor_node_relative(current_node, { place_cursor_mark = false })
  )
...
function API.cleanup()
  ...
  vim.api.nvim_exec_autocmds("User", { pattern = "LuasnipCleanup", modeline = false })
  -- clear all snippets.
