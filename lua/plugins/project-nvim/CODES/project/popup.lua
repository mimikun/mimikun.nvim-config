...
M.delete_menu = M.select.new({
  callback = function()
    ...
    vim.ui.select(choices_list, {
      ...
      format_item = function(item) ---@param item string
        return (vim.list_contains(require("project.util.history").session_projects, item) and "* " or "") .. item
      end,
    }, function(item)
      if not item then
        return
  ...
  callback = function()
    ...
    vim.ui.select(choices_list, {
      ...
      format_item = function(item) ---@param item string
        ...
        return item == curr and "* " .. vim.fn.fnamemodify(item, ":~") or vim.fn.fnamemodify(item, ":~")
      end,
    }, function(item)
      if not item then
        return
