...
function M._is_applicable(fn, view)
  ...
  if t == "working_tree_only" then
    ...
    return v.left ~= nil and v.right ~= nil and v.left.type == RevType.STAGE and v.right.type == RevType.LOCAL
  end
...
function M.diff_against_default_branch()
  ...
  if view then
    ...
    local top_indicators = utils.vec_join(vim.bo.buftype == "" and pl:absolute(cfile) or nil, pl:realpath("."))
...
function M.jumpto_conflict(num, use_delta)
  ...
  if view and view:instanceof(StandardView.__get()) then
    ...
    if main then
      ---@cast bufnr integer
      local next_idx
      local conflicts, cur, cur_idx = vcs_utils.parse_conflicts(api.nvim_buf_get_lines(bufnr, 0, -1, false), main.id)
...
function M.scroll_view(distance)
  ...
  if distance % 1 == 0 then
    ...
    scroll_cmd = ([[exe "norm! " . float2nr(winheight(0) * %f) . "%s"]]):format(math.abs(distance), scroll_opr)
  end
    ...
    if next(conflicts) then
      ...
      for _, cur_conflict in ipairs(conflicts) do
        ...
        elseif target == "all" then
          content = utils.vec_join(cur_conflict.ours.content, cur_conflict.base.content, cur_conflict.theirs.content)
        end
...
function M.conflict_choose(target)
  return tag(function()
    ...
    if view and view:instanceof(StandardView.__get()) then
      ...
      if main then
        ---@cast bufnr integer
        local _, cur = vcs_utils.parse_conflicts(api.nvim_buf_get_lines(bufnr, 0, -1, false), main.id)
...
function M.cycle_layout()
  ...
  elseif view:instanceof(DiffView.__get()) then
    ...
    if cur_file then
      layouts = cur_file.kind == "conflicting" and layout_cycles.merge_tool or layout_cycles.standard
