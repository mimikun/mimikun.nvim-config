...
function PerfTimer:__tostring()
  ...
  if #self.laps == 0 then
    return string.format("%s %.3f ms", utils.str_right_pad((self.subject or "TIME") .. ":", 24), self.final_time)
  else
    ...
    for _, lap in ipairs(self.laps) do
      s = s
        .. string.format(">> %s %.3f ms\t(%.3f ms)\n", utils.str_right_pad(tostring(lap[1]), 36), lap[2], lap[2] - last)
      last = lap[2]
    end
