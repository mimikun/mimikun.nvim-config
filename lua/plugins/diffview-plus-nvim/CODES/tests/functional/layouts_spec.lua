...
describe("diffview.layout symbols", function()
  ...
  describe("Diff1Inline:diffget", function()
    ...
    it("applies every hunk inside a multi-line visual range in one pass", function()
      local inst, bufnr = prepare({ "one", "two", "three", "four", "five" }, { "ONE", "two", "THREE", "four", "FIVE" })
  ...
  describe("Diff1Inline:_prerender", function()
    ...
    local scope_supported = (api.nvim_win_add_ns ~= nil and api.nvim_win_remove_ns ~= nil) or api["nvim__ns_set"] ~= nil
  ...
  it("Diff1Inline:_repaint emits no DiffChange/DiffAdd row hl when b.file is nulled", function()
    ...
    for _, m in ipairs(marks) do
      ...
      assert(hl ~= "DiffviewDiffChange" and hl ~= "DiffviewDiffAdd", "unexpected line_hl_group: " .. tostring(hl))
    end
  ...
  it(
    ...
    helpers.async_test(function()
      ...
      assert.is_true(marks_at_open_files > 0, "expected extmarks already on buffer when open_files starts")
