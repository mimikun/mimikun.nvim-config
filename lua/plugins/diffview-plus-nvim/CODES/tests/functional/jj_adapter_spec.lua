...
describe("diffview.vcs.adapters.jj", function()
  ...
  describe("get_show_args()", function()
    ...
    it("escapes fileset metacharacters so jj matches the path literally", function()
      ...
      local args =
        adapter:get_show_args("frontend/src/routes/(test)/View.svelte", adapter.Rev(RevType.COMMIT, "abc123"))
  ...
  describe("get_log_args()", function()
    ...
    it("wraps an `a..b` range revset in `-r`", function()
      ...
      eq({ "log", "--no-graph", "-r", "abc123..def456" }, adapter:get_log_args({ "abc123..def456" }))
    end)
    ...
    it("mixes flags and revsets without re-wrapping flags", function()
      ...
      eq({ "log", "--no-graph", "-n10", "-r", "abc123" }, adapter:get_log_args({ "-n10", "abc123" }))
    end)
    ...
    it("prepends user-configured global flags from `jj_cmd`", function()
      ...
      eq({ "--repository", "/some/path", "log", "--no-graph", "-r", "abc123" }, adapter:get_log_args({ "abc123" }))
    end)
  ...
  describe("integration", function()
    ...
    describe("rev_to_panel_name", function()
      it("includes the description for a single-change range", function()
        ...
        local parent_hash = run({ "jj", "show", "-T", "parents.first().commit_id()", "@", "--no-patch" }, repo.dir)
        ...
        eq("update file", adapter:rev_to_panel_name(left:abbrev() .. ".." .. right:abbrev(), left, right))
      end)
      ...
      it("falls back to `rev_arg` when left is not a parent of right", function()
        ...
        local adapter = repo.adapter()
        local grandparent_hash = run({ "jj", "show", "-T", "commit_id", "@--", "--no-patch" }, repo.dir)
        local commit_hash = run({ "jj", "show", "-T", "commit_id", "@", "--no-patch" }, repo.dir)
    ...
    describe("tracked_files", function()
      it(
        ...
        helpers.async_test(function()
          ...
          local adapter = repo.adapter()
          local left =
            adapter.Rev(RevType.COMMIT, run({ "jj", "show", "-T", "commit_id", "@-", "--no-patch" }, repo.dir))
          local right = adapter.Rev(RevType.LOCAL)
    ...
    describe("merge conflict detection", function()
      ...
      it(
        ...
        helpers.async_test(function()
          ...
          local err, files, conflicts = await(
            adapter:tracked_files(left, right, args, "working", { default_layout = Diff2, merge_layout = Diff3Hor })
          )
      ...
      it(
        ...
        helpers.async_test(function()
          ...
          local err, _, conflicts = await(
            adapter:tracked_files(left, right, {}, "working", { default_layout = Diff2, merge_layout = Diff3Hor })
          )
      ...
      it(
        ...
        helpers.async_test(function()
          ...
          local err, _, conflicts = await(
            adapter:tracked_files(left, right, args, "working", { default_layout = Diff2, merge_layout = Diff4Mixed })
          )
    ...
    describe("file_history_dry_run", function()
      it(
        ...
        helpers.async_test(function()
          ...
          local adapter = repo.adapter()
          local ok, _, err = adapter:file_history_dry_run({ path_args = { "src/routes/(app)/[id]/page.svelte" } })
  ...
  describe("structure_fh_data", function()
    ...
    it("tolerates empty optional fields without shifting downstream slots", function()
      ...
      eq({ { status = "M", path = "f.txt", stats = { additions = 2, deletions = 4 } } }, data.namestat)
    end)
  ...
  describe("parse_tracked_files_line", function()
    local parse_tracked_files_line = require("diffview.vcs.adapters.jj")._test.parse_tracked_files_line
    ...
    it("parses a renamed line with the old path in the 3rd field", function()
      local status, path, oldpath = parse_tracked_files_line("R" .. US .. "common/new.txt" .. US .. "common/old.txt")
  ...
  describe("history_scope", function()
    ...
    it("returns single_file for a real tracked file in a jj repo", function()
      ...
      vim.system({ "jj", "config", "set", "--repo", "user.email", "t@t" }, { cwd = repo_dir }):wait()
  ...
  describe("is_non_literal_pathspec", function()
    local is_non_literal_pathspec = require("diffview.vcs.adapters.jj")._test.is_non_literal_pathspec
  ...
  describe("quote_path_args", function()
    ...
    it("matches a glob-character path literally when it exists on disk", function()
      ...
      eq({ 'cwd:"src/routes/(app)/[id]/page.svelte"' }, quote_path_args({ "src/routes/(app)/[id]/page.svelte" }, top))
  ...
  describe("is_ambiguous_literal_path", function()
    local is_ambiguous_literal_path = require("diffview.vcs.adapters.jj")._test.is_ambiguous_literal_path
