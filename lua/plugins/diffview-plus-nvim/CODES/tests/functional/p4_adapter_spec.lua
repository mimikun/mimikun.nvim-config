    ...
    it(
      ...
      test_utils.async_test(function()
        ...
        for _, file in ipairs(files) do
          assert.is_nil(file.path:match("#%d+"), ("path %q should not contain a revision specifier"):format(file.path))
          assert.is_nil(file.path:match("^//"), ("path %q should not be a depot path"):format(file.path))
        end
      end)
    )
