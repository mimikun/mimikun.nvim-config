...
describe("diffview.scene.inline_diff", function()
  ...
  describe("DiffviewDiffAddInline highlight", function()
    ...
    it("refreshes on re-setup instead of pinning the first colourscheme's value", function()
      ...
      assert.are.equal(0x004400, api.nvim_get_hl(0, { name = "DiffviewDiffAddInline", link = false }).bg)

      api.nvim_set_hl(0, "DiffAdd", { bg = "#88ccff" })
      api.nvim_set_hl(0, "DiffviewDiffAdd", { link = "DiffAdd" })
      hl.setup()

      assert.are.equal(0x88ccff, api.nvim_get_hl(0, { name = "DiffviewDiffAddInline", link = false }).bg)
    end)
  end)
  ...
  describe("DiffviewDiffTextInline highlight", function()
    ...
    it("refreshes on re-setup instead of pinning the first colourscheme's value", function()
      ...
      -- `hl.setup()`, WITHOUT clearing the group first -- must update the
      -- derived background rather than keep the stale one.
      assert.are.equal(0x394b70, api.nvim_get_hl(0, { name = "DiffviewDiffTextInline", link = false }).bg)

      api.nvim_set_hl(0, "DiffText", { bg = "#88ccff" })
      api.nvim_set_hl(0, "DiffviewDiffText", { link = "DiffText" })
      hl.setup()

      assert.are.equal(0x88ccff, api.nvim_get_hl(0, { name = "DiffviewDiffTextInline", link = false }).bg)
    end)
    ...
    it("paints the row backdrop in gaps around the char overlay range", function()
      ...
      for _, b in ipairs(backdrops) do
        if b.row == overlay.row and b.hl_eol then
          ...
          assert.is_true(b.start >= overlay.finish, "the `hl_eol` segment must start at or after the overlay end")
        end
      end
    ...
    it("preserves a user-set `bg` on DiffviewDiffTextInline at the overlay cells", function()
      ...
      assert.are.equal(0xffffff, api.nvim_get_hl(0, { name = "DiffviewDiffTextInline", link = false }).bg)
  ...
  describe("overleaf style", function()
    ...
    it("still emits DiffviewDiffAddInline hl on added chars in overleaf style", function()
      ...
      inline_diff.render(bufnr, { "hello world" }, { "hello brave new world" }, { style = "overleaf" })
    ...
    it("does not echo old paired lines as virt_lines (no double-rendering)", function()
      local bufnr = fresh_buf({ "first changed", "second" })
      inline_diff.render(bufnr, { "first", "second" }, { "first changed", "second" }, { style = "overleaf" })
  ...
  describe("deletion_highlight", function()
    ...
    it("'full_width' appends a padding chunk sized to the displayed window", function()
      ...
      inline_diff.render(target, { "\thi", "tail" }, { "tail" }, { deletion_highlight = "full_width" })
    ...
    it("'full_width' pads to the displayed window's width, not vim.o.columns", function()
      ...
      inline_diff.render(bufnr, { "deleted", "context" }, { "context" }, { deletion_highlight = "full_width" })
    ...
    -- chunk is emitted.
    it("'full_width' emits no padding when the buffer is undisplayed and no winid hint is given", function()
      local target = fresh_buf({ "tail" })
      assert.are.same({}, vim.fn.win_findbuf(target))

      inline_diff.render(target, { "deleted", "tail" }, { "tail" }, { deletion_highlight = "full_width" })
    ...
    it("'hanging' splits leading whitespace off the highlight", function()
      local bufnr = fresh_buf({ "tail" })
      inline_diff.render(bufnr, { "  indented", "tail" }, { "tail" }, { deletion_highlight = "hanging" })
  ...
  describe("helpers", function()
    ...
    it("tokenize handles non-Latin scripts: joins lowercase, splits at uppercase", function()
      ...
      assert.are.same({ "type\xe6\x97\xa5\xe6\x9c\xac" }, (h.tokenize("type\xe6\x97\xa5\xe6\x9c\xac")))
      -- A following uppercase letter still triggers the lower→upper split.
      assert.are.same({ "\xe6\x97\xa5\xe6\x9c\xac", "Type" }, (h.tokenize("\xe6\x97\xa5\xe6\x9c\xacType")))
    end)
    ...
    it("refinement_safe admits multi-hunk sub-diffs with a shared prefix", function()
      ...
      assert.is_true(h.refinement_safe({ "r", "e", "c", "i", "e", "v", "e" }, { "r", "e", "c", "e", "i", "v", "e" }, 2))
    end)
    ...
    it("refinement_safe rejects multi-hunk sub-diffs with no substantial overlap", function()
      ...
      assert.is_false(h.refinement_safe({ "p", "a", "r", "a", "m" }, { "r", "e", "t", "u", "r", "n" }, 2))
    end)
  ...
  describe("similarity gate", function()
    ...
    it("skips char-level highlights when lines are too dissimilar (unified)", function()
      ...
      inline_diff.render(bufnr, { old }, { new })
      local t = text_extmarks(bufnr)
      assert.are.equal(0, t.hl, "expected no DiffviewDiffTextInline fragments on dissimilar pairing")
    end)
  ...
  describe("intraline tokenization", function()
    ...
    it("refines 1:1 word replacements with char-level precision", function()
      ...
      for _, r in ipairs(ranges) do
        assert.is_true(r.finish - r.start < #"receive", "each highlight should be narrower than the full word")
      end
    end)
    ...
    it("emits char-level inline deletions inside a 1:1 word replacement (overleaf)", function()
      ...
      for _, v in ipairs(inlines) do
        assert.is_true(#v.text < #"recieve", "refined inline deletions must be sub-word, not the whole old word")
      end
    end)
    ...
    it("renders PascalCase identifier replacements as whole-subword swaps (overleaf)", function()
      ...
      local ranges = char_ranges(extmarks(bufnr))
      assert.are.equal(1, #ranges, "expected one DiffviewDiffAddInline span over the inserted subword")
    ...
    it("ensure_eof_virt_lines_visible is a no-op when the cursor is not on the last line", function()
      local bufnr = fresh_buf({ "a", "b", "c" })
    ...
    it("does not spuriously mark the trailing token as deleted when paired line grows", function()
      ...
      local inlines = inline_virt_texts(extmarks(bufnr), "DiffviewDiffDeleteInline")
      assert.are.equal(0, #inlines, "pure addition after a shared prefix should emit no inline deletions")
    ...
    it("falls back to atomic word delete for 1:1 pairs with only a coincidental letter match", function()
  ...
  describe("treesitter captures", function()
    ...
    it("captured_chunks falls back to a single chunk when no captures supplied", function()
      assert.are.same({ { "hello", "DiffviewDiffDelete" } }, captured_chunks("hello", nil, "DiffviewDiffDelete"))
      assert.are.same({ { "hello", "DiffviewDiffDelete" } }, captured_chunks("hello", {}, "DiffviewDiffDelete"))
    end)
    ...
    it("captured_chunks stacks overlapping captures so each contributes attrs", function()
      ...
      -- (`@comment`) does.
      local chunks = captured_chunks("hello", { { 0, 5, "@variable" }, { 1, 3, "@string" } }, "DiffviewDiffDelete")
    ...
    it("captured_chunks preserves an earlier capture's fg under a later attr-less capture", function()
      ...
      -- redefine it.
      local chunks = captured_chunks("-- comment", { { 0, 10, "@comment" }, { 0, 10, "@spell" } }, "DiffviewDiffDelete")
      assert.are.equal(1, #chunks)
    ...
    if lua_parser_available then
      ...
      it("deletion_treesitter=false skips the TS layer", function()
        local bufnr = fresh_buf({ "tail" })
        vim.api.nvim_set_option_value("filetype", "lua", { buf = bufnr })
        inline_diff.render(bufnr, { "local x = 1", "tail" }, { "tail" }, { deletion_treesitter = false })
      ...
      it("'hanging' offsets TS captures past the leading indent", function()
        local bufnr = fresh_buf({ "tail" })
        vim.api.nvim_set_option_value("filetype", "lua", { buf = bufnr })
        inline_diff.render(bufnr, { "  local x = 1", "tail" }, { "tail" }, { deletion_highlight = "hanging" })
  ...
  describe("window scoping", function()
    ...
    local supported = (api.nvim_win_add_ns ~= nil and api.nvim_win_remove_ns ~= nil) or api["nvim__ns_set"] ~= nil
    ...
    it("emits a one-shot warning when no scope API is available and the buffer is shared", function()
