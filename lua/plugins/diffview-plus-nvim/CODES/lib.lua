...
local FileDirDiffView = lazy.access("diffview.scene.views.diff.file_dir_diff_view", "FileDirDiffView") ---@type FileDirDiffView|LazyModule
...
function M.file_history(range, args)
  ...
  if pin_local and not (adapter:instanceof(GitAdapter.__get()) or adapter:instanceof(HgAdapter.__get())) then
...
function M.diffview_merge_files(args)
  ...
  if #argo.args ~= 3 and #argo.args ~= 4 then
    utils.err("DiffviewMergeFiles requires three or four file paths: <output> [<base>] <left> <right>.")
    return
  end
