...
local Diff1InlinePinned = lazy.access("diffview.scene.layouts.diff_1_inline_pinned", "Diff1InlinePinned") ---@type Diff1InlinePinned|LazyModule
...
function validate.string_list(t, key, fallback, opts)
  ...
  if type(v) ~= "table" or not utils.islist(v) then
    warn_invalid(v, opts.path or tostring(key), "a list of strings" .. (opts.nilable and " or nil" or ""))
    t[key] = fallback_value(fallback)
    return
  end
  local filtered = {}
...
function M.setup(user_config)
  ...
  if
    type(user_config.file_history_panel) == "table" and type(user_config.file_history_panel.log_options) == "table"
  then
  ...
  local merge_concrete = { "diff1_plain", "diff3_horizontal", "diff3_vertical", "diff3_mixed", "diff4_mixed" }
