...
function Panel:open()
  ...
  if config.type == "split" then
    ...
    local rel_winid = config.relative == "win" and api.nvim_win_is_valid(config.win or -1) and config.win or 0
...
function Panel:on_autocmd(event, opts)
  ...
  local callback = function(_, state)
    ...
    if state.event:match("^Win") then
      if vim.tbl_contains({ "WinLeave", "WinEnter" }, state.event) and api.nvim_get_current_win() == self.winid then
        buf_match = state.buf
      else
