...
function DiffView:post_open()
  ...
  if config.get_config().watch_index and self.adapter:instanceof(GitAdapter.__get()) then
    ...
    self._gitsigns_augroup = api.nvim_create_augroup("diffview_gitsigns_" .. self.tabpage, { clear = true })
...
function DiffView:_modified_stage_paths()
  ...
  for _, file in self.files:iter() do
    for _, f in ipairs(file.layout:files()) do
      if f.rev.type == RevType.STAGE and f.bufnr and api.nvim_buf_is_loaded(f.bufnr) and vim.bo[f.bufnr].modified then
...
function DiffView:_should_focus_diff(next_file)
  ...
  local conf = config.get_config()
  local view_conf = next_file and next_file.kind == "conflicting" and conf.view.merge_tool or conf.view.default
