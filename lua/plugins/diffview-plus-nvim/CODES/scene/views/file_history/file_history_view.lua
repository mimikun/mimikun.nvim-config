...
function FileHistoryView:_resolve_pinned_target(entry)
  ...
  if rev_a and rev_a.commit then
    local ok, exists = pcall(self.adapter.file_exists_at_rev, self.adapter, pinned_path, rev_a.commit)
...
function FileHistoryView:resolve_pinned_layout(layout_class)
  ...
  return config.name_to_layout(pinned_variant[FileHistoryView.get_default_diff2().name] --[[@as string ]])
end
