...
Diff1Inline._prerender = async.void(function(self)
  ...
  if self._cached_old_lines == nil then
    ...
    if not (self.b and self.b.file and self.b.file.bufnr == bufnr and api.nvim_buf_is_valid(bufnr)) then
      return
    end
...
local function register_repaint_autocmds(self, bufnr)
  ...
  if not self._resize_autocmd then
    self._resize_debounced = debounce.debounce_trailing(RESIZE_REPAINT_DEBOUNCE_MS, false, function()
      self:_repaint()
    end)
...
Diff1Inline._render_inline = async.void(function(self)
  ...
  if old_lines == nil then
    ...
    -- buffer that no longer matches `self.b.file`.
    if
      not (self.b and self.b:is_valid() and self.b.file and self.b.file.bufnr == bufnr and api.nvim_buf_is_valid(bufnr))
    then
      return
    end
    self._cached_old_lines = old_lines
