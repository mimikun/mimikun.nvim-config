...
local function is_word_byte(b)
  return b >= 0x80 or (b >= 0x30 and b <= 0x39) or (b >= 0x41 and b <= 0x5A) or (b >= 0x61 and b <= 0x7A) or b == 0x5F
end
...
local function render_char_highlights(bufnr, new_row, old_line, new_line, inline_del, add_hl)
  ...
  for _, h in ipairs(hunks) do
    ...
    if not refined then
      ...
      render_hunk(bufnr, new_row, 0, new_line_len, new_map, new_start, new_count, del_text, inline_del, add_hl, ranges)
    end
  end
...
local cache_cleanup_augroup = api.nvim_create_augroup("diffview_inline_diff_hunk_cache", { clear = true })

-- Track buffers whose CursorMoved scroll-adjuster is already installed.
---@type table<integer, true>
local scroll_adjuster_registered = {}

local scroll_adjuster_augroup = api.nvim_create_augroup("diffview_inline_diff_scroll", { clear = true })
