...
local function select_layout_for_status(default_class, opt)
  ...
  if
    class_descends_from(default_class, Diff1Inline.__get()) or class_descends_from(default_class, Diff1Raw.__get())
  then
    return default_class
  end
  if class_descends_from(default_class, Diff1.__get()) or class_descends_from(default_class, Diff2.__get()) then
...
function FileEntry.with_layout(layout_class, opt)
  ...
  local b_substituted = using_raw and opt.revs.b and try_should_null(Diff2.__get(), opt.revs.b, opt.status, "b")
    or false
