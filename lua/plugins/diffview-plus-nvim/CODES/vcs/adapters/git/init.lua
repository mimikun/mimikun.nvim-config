...
function GitAdapter:get_merge_context()
  ...
  local ret = {}
  local out, code =
    self:exec_sync({ "show", "-s", "--no-show-signature", "--pretty=format:%H%n%D", "HEAD", "--" }, self.ctx.toplevel)
...
function GitAdapter:history_scope(path_args, log_options)
  ...
  local out =
    self:exec_sync(utils.vec_join("-c", "core.quotePath=false", "ls-files", "--", path_args), self.ctx.toplevel)
  if #out == 1 then
...
function GitAdapter:file_history_options(range, paths, argo)
  ...
  if not self.ctx.dir then
    utils.err(fmt("Failed to find the git dir for the repository: %s", utils.str_quote(self.ctx.toplevel)))
    return
  end
...
GitAdapter.file_history_worker = async.void(function(self, out_stream, opt)
  local single_file = self:is_single_file(opt.log_opt.single_file.path_args, opt.log_opt.single_file.L)

  local log_options =
    config.get_log_options(single_file, single_file and opt.log_opt.single_file or opt.log_opt.multi_file, "git") --[[@as GitLogOptions ]]
...
function GitAdapter:build_local_log_entry(opt)
  ...
  local args = utils.vec_join({ "-c", "core.quotePath=false", "diff", "--raw", "--numstat", "HEAD", "--" }, path_args)
  ...
  local user_out = self:exec_sync({ "config", "user.name" }, { cwd = self.ctx.toplevel, silent = true })
...
function GitAdapter:get_branch_name()
  local out, code = self:exec_sync({ "symbolic-ref", "--short", "HEAD" }, { cwd = self.ctx.toplevel, silent = true })
...
function GitAdapter:get_remote_url(remote)
  ...
  local out, code = self:exec_sync({ "remote", "get-url", remote }, { cwd = self.ctx.toplevel, silent = true })
...
function GitAdapter:parse_revs(rev_arg, opt)
  ...
  elseif rev_arg:match("%.%.%.") then
    ...
    if code ~= 0 then
      utils.err(utils.vec_join(fmt("Failed to parse rev %s!", utils.str_quote(rev_arg)), "Git output: ", stderr))
      return
...
function GitAdapter:stage_index_file(file)
  ...
  local ok, ret = pcall(function()
    ...
    out, code =
      self:exec_sync({ "--literal-pathspecs", "hash-object", "-w", "--", pl:convert(temp) }, self.ctx.toplevel)
...
function GitAdapter:show_untracked(opt)
  ...
  local out = self:exec_sync({ "config", "status.showUntrackedFiles" }, { cwd = self.ctx.toplevel, silent = true })
...
GitAdapter.tracked_files = async.wrap(function(self, left, right, args, kind, opt, callback)
  ...
  local namestat_job = Job({
    command = self:bin(),
    args = utils.vec_join(self:args(), "diff", "-z", rename_flag, "--ignore-submodules", "--name-status", args),
    cwd = self.ctx.toplevel,
    log_opt = log_opt,
  })
  local numstat_job = Job({
    command = self:bin(),
    args = utils.vec_join(self:args(), "diff", "-z", rename_flag, "--ignore-submodules", "--numstat", args),
    cwd = self.ctx.toplevel,
    log_opt = log_opt,
  })
...
GitAdapter.untracked_files = async.wrap(function(self, left, right, opt, callback)
  local job = Job({
    command = self:bin(),
    args = utils.vec_join(self:args(), "-c", "core.quotePath=false", "ls-files", "--others", "--exclude-standard"),
    cwd = self.ctx.toplevel,
...
function GitAdapter:rev_candidates(arg_lead, opt)
  ...
  local stashes = self:exec_sync({ "stash", "list", "--pretty=format:%gd" }, { cwd = self.ctx.toplevel, silent = true })
