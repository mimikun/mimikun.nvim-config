...
function P4Adapter:get_log_args(args, paths)
  if paths and #paths > 0 then
    utils.warn("Path-filtered commit log is not yet supported for Perforce; showing the range's full log.")
  end
...
P4Adapter.file_history_worker = async.void(function(self, out_stream, opt)
  ...
  ---@type table
  local log_options =
    config.get_log_options(single_file, single_file and single_file_opt or opt.log_opt.multi_file, self.config_key)
