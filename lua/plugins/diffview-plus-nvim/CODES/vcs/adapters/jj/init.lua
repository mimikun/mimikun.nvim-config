...
function JjAdapter:get_show_args(path, rev)
  return utils.vec_join(self:args(), "file", "show", "-r", rev and rev:object_name() or "@", "--", fileset_exact(path))
end
...
function JjAdapter:resolve_rev_arg(rev_arg)
  ...
  if code ~= 0 or not out[1] then
    utils.err(utils.vec_join(fmt("Failed to parse rev %s!", utils.str_quote(rev_arg)), "Jujutsu output: ", stderr))
    return
  end
...
function JjAdapter:symmetric_diff_revs(rev_arg)
  ...
  local out, code, stderr = self:exec_sync({ "log", "-r", revset, "-T", [[commit_id ++ "\n"]], "--no-graph" }, {
    cwd = self.ctx.toplevel,
    retry = 2,
    fail_on_empty = true,
...
function JjAdapter:is_single_file(path_args, lflags)
  if path_args and self.ctx.toplevel then
    return #path_args == 1 and not pl:is_dir(path_args[1]) and #self:list_files_at_head(path_args) < 2
  end
  return true
end
...
function JjAdapter:fh_compute_pushed_set(path_args)
  return fh_collect_revset(self, "::remote_bookmarks()", path_args, "JjAdapter:fh_compute_pushed_set()")
end
...
function JjAdapter:prepare_fh_options(log_options, single_file) ---@diagnostic disable-line: unused-local
  ...
  return {
    ...
    flags = utils.vec_join(o.limit and { "--limit=" .. o.limit } or nil, o.reversed and { "--reversed" } or nil),
  }
end
...
function JjAdapter:file_history_dry_run(log_opt)
  ...
  if code ~= 0 and stderr and table.concat(stderr, "\n"):find("Failed to parse fileset", 1, true) then
...
JjAdapter.file_history_worker = async.void(function(self, out_stream, opt)
  ...
  local log_options =
    config.get_log_options(single_file, single_file and opt.log_opt.single_file or opt.log_opt.multi_file, "jj") --[[@as JjLogOptions ]]
...
function JjAdapter:parse_fh_data(data, commit, state)
  ...
  for _, file in ipairs(data.namestat) do
    if in_scope(file.path) then
      ...
      local rev_a = data.left_hash and JjRev(RevType.COMMIT, data.left_hash) or JjRev.new_null_tree()
...
JjAdapter._query_merge_context = async.wrap(function(self, callback)
  ...
  local base_job =
    log_job(self, fmt("latest(fork_point(%s-), 1)", merge_id), "commit_id", "JjAdapter:_query_merge_context() base")
...
JjAdapter.tracked_files = async.wrap(function(self, left, right, args, kind, opt, callback)
  ...
  if merge_ctx then
    ...
    local base_rev = merge_ctx.base and JjRev(RevType.COMMIT, merge_ctx.base) or JjRev.new_null_tree()
...
JjAdapter.file_restore = async.wrap(function(self, path, kind, commit, callback)
  if kind == "staged" then
    self:_warn_once("no_staging", "Jujutsu has no staging index; staging-related operations are not supported.")
    callback(false)
    return
  end
...
function JjAdapter:rev_candidates(arg_lead, opt)
  ...
  local tags = self:exec_sync({ "tag", "list", "-T", [[name ++ "\n"]] }, { cwd = self.ctx.toplevel, silent = true })
