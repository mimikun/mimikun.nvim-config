...
function HgAdapter:get_log_args(args, paths)
  ...
  -- the same concern.
  local literal_paths = paths and #paths > 0 and vim.tbl_map(function(p)
    return "path:" .. p
  end, paths) or nil
  return utils.vec_join(self:args(), "log", "--stat", "--rev", args, literal_paths and "--" or nil, literal_paths)
end
...
function HgAdapter:file_history_dry_run(log_opt)
  ...
  local cmd =
    utils.vec_join("log", log_options.rev and "--rev=" .. log_options.rev or nil, options, log_options.path_args)
...
function HgAdapter:stream_fh_data(state)
  ...
  mjob = MultiJob({ namestat_job, numstat_job }, { on_exit = utils.hard_bind(stream.close, stream) })
  mjob:start()
...
HgAdapter.file_history_worker = async.void(function(self, out_stream, opt)
  ...
  local log_options =
    config.get_log_options(single_file, single_file and opt.log_opt.single_file or opt.log_opt.multi_file, "hg") --[[@as HgLogOptions ]]
  ...
  for _, item in in_stream:iter() do
    ...
    -- have changes.
    if new_data.merge_hash and new_data.numstat[1] or (#new_data.numstat - 1) ~= #new_data.namestat then
      local job = Job({
        command = self:bin(),
        args = utils.vec_join(
          self:args(),
...
function HgAdapter:build_local_log_entry(opt)
  ...
  local user_out = self:exec_sync({ "config", "ui.username" }, { cwd = self.ctx.toplevel, silent = true })
...
function HgAdapter:file_exists_at_rev(path, rev_arg)
  ...
  local out, code = self:exec_sync({ "files", "-r", rev_arg, "--", path }, { cwd = self.ctx.toplevel, silent = true })
...
function HgAdapter:rev_to_args(left, right)
  assert(not (left.type == RevType.LOCAL and right.type == RevType.LOCAL), "Can't diff LOCAL against LOCAL!")
...
function HgAdapter:parse_revs(rev_arg, opt)
  ...
  if not rev_arg then
    ...
    elseif to and to ~= "" then
      ...
      local node, code, stderr =
        self:exec_sync({ "log", "--limit=1", "--template={node}", "--rev=" .. rev_arg }, self.ctx.toplevel)
...
HgAdapter.untracked_files = async.wrap(function(self, left, right, opt, callback)
  local job = Job({
    command = self:bin(),
    args = utils.vec_join(self:args(), "status", "--print0", "--unknown", "--no-status", "--template={path}\\n"),
    cwd = self.ctx.toplevel,
...
function HgAdapter:rev_candidates(arg_lead, opt)
  ...
  local branches = self:exec_sync({ "branches", "--template={branch}\n" }, { cwd = self.ctx.toplevel, silent = true })

  local heads = self:exec_sync({ "heads", "--template={node|short}\n" }, { cwd = self.ctx.toplevel, silent = true })
