...
function Hydra:initialize(input)
  do -- validate parameters
    ...
    if input.config then
      ...
      vim.validate({
        foreign_keys = {
          input.config.foreign_keys,
          function(foreign_keys)
            if type(foreign_keys) == "nil" or foreign_keys == "warn" or foreign_keys == "run" then
              return true
            else
              return false
            end
          end,
          'Hydra: config.foreign_keys value could be either "warn" or "run"',
        },
      })
      vim.validate({
        color = {
          ...
          "Hydra: color value could be one of: red, blue, amaranth, teal, pink",
        },
      })
    end
...
function Hydra:_setup_hydra_keymaps()
  self:_set_keymap(self.plug_wait, function()
    self:_leave()
  end)
...
function Hydra:_setup_pink_hydra()
  ...
  for head, map in pairs(self.heads) do
    ...
    if self.body and not self.config.invoke_on_body and not opts.exit and not opts.private then
...
function Hydra:_enter()
  ...
  o.ttimeout = not self.options.original.o.timeout or self.options.original.o.ttimeout
