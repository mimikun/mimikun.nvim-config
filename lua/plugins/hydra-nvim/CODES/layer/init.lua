...
function Layer:initialize(input)
  ...
  if self.config.on_enter then
    getmetatable(self.options.bo).__index = util.add_hook_before(getmetatable(self.options.bo).__index, function(_, opt)
      assert(
        type(opt) ~= "number",
        '[Hydra] vim.bo[bufnr] meta-aссessor in config.on_enter() function is forbiden, use "vim.bo" instead'
      )
    end)
    getmetatable(self.options.wo).__index = util.add_hook_before(getmetatable(self.options.wo).__index, function(_, opt)
      assert(
        type(opt) ~= "number",
  ...
  if self.config.on_exit then
    ...
    local function show_disable_message(name)
      util.warn(string.format('[Hydra] "vim.%s" meta-accessor is disabled inside config.on_exit() function', name))
    end

    local env = util.deepcopy(getfenv())
    env.vim.o = self.options:make_meta_accessor(function(opt)
      return api.nvim_get_option_value(opt, {})
    end, function()
      show_disable_message("o")
    end)
    env.vim.go = self.options:make_meta_accessor(function(opt)
      return api.nvim_get_option_value(opt, { scope = "global" })
    end, function()
      show_disable_message("go")
    end)
    env.vim.bo = self.options:make_meta_accessor(function(opt)
      return api.nvim_buf_get_option(0, opt)
    end, function()
      show_disable_message("bo")
    end)
    env.vim.wo = self.options:make_meta_accessor(function(opt)
      return api.nvim_win_get_option(0, opt)
    end, function()
      show_disable_message("wo")
    end)
  ...
  local exit_keymaps
  self.enter_keymaps, self.layer_keymaps, exit_keymaps = self:_normalize_input(input.enter, input.layer, input.exit)
  ...
  if exit_keymaps then
    for mode, keymaps in pairs(exit_keymaps) do
      for lhs, map in pairs(keymaps) do
        ...
        local keymap = self:_make_keymap_function(mode, rhs, opts)
        local rhs_fun = opts.exit_before and function()
          self:exit()
          keymap()
        end or function()
          keymap()
...
function Layer:_setup_keymaps(bufnr)
  ...
  if util.tbl_rawget(self.saved_keymaps, bufnr) then
    return
  end
...
function Layer:_save_keymaps(bufnr)
  ...
  for mode, esc_termcodes_keymaps in pairs(self.esc_termcodes_layer_keymaps) do
...
function Layer:_timer()
  if not self.config.timeout then
    return
  end
